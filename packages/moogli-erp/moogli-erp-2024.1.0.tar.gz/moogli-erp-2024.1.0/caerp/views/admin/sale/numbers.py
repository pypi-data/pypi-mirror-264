...
SALE_NUMBERING_CONFIG_URL = os.path.join(SALE_URL, "numbering")
...
<li>\
<code>{SEQMONTH}</code> : numéro de séquence mensuel (ràz chaque mois)\
</li>\
<li>\
<code>{SEQMONTHANA}</code>: numéro de séquence par enseigne et par mois \
(ràz chaque mois)</li>\
</ul>\
<p>Dans le cas d'une migration depuis un autre outil de gestion, \
il est possible d'initialiser les séquences à une valeur différente de zéro.\
</p>\
    """
...
def includeme(config):
    add_routes(config)
    config.add_admin_view(SaleNumberingIndex, parent=SaleIndexView)
    config.add_admin_view(SalesNumberingConfigView, parent=SaleNumberingIndex)
