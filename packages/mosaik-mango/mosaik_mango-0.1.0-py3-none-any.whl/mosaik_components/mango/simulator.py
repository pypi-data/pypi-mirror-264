...
class Simulator(mosaik_api_v3.Simulator):
    ...
    @override
    def init(self, sid: str, time_resolution: float, step_size: int):
        self._step_size = step_size
        self._mango_container = yield create_container(
            codec=self._mango_codec, clock=ExternalClock(start_time=-step_size)
        )
        return self.meta
    ...
    def create(self, num: int, model: str, **params: Any) -> list[CreateResult]:
        ...
        for name, value in params.items():
            param_model = agent_spec.agent_params.get(name)
            if not param_model:
                continue
        ...
        for _ in range(num):
            eid = params.get("id", f"{model}-{len(self._agents[model])}")
