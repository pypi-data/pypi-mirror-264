...
impl QNameExt for QName<'_> {
    fn ln(&self) -> String {
        ...
        std::str::from_utf8(local_name.as_ref()).unwrap().to_string()
    }
}
            ...
            Ok(Event::Empty(e)) => {
                ...
                if e.attributes().count() == 0 {
                    ...
                    for attr in e.attributes() {
                        ...
                        attrs
                            .set_item(format!("@{}", attr.key.ln()), attr.unescape_value().unwrap())
                            .unwrap();
                    }
            ...
            Ok(Event::Start(e)) => {
                ...
                if e.attributes().count() > 0 {
                    for attr in e.attributes() {
                        ...
                        match value {
                            Value::Dict(d) => {
                                d.set_item(format!("@{}", attr.key.ln()), attr.unescape_value().unwrap())
                                    .unwrap();
                            }
                            _ => unreachable!(),
                        }
