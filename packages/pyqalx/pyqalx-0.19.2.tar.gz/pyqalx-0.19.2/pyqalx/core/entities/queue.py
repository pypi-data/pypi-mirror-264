...
from pyqalx.core.entities import Group, Item, QalxEntity, Set
...
class Queue(QalxEntity):
    ...
    def _credentials_expired_or_expiring_soon(self):
        """
        Returns True if the credentials have expired or are expiring soon.
        The expiration datetime will be a UTC datetime object.
        """
        ...
        return (
            expiration <= utcnow
            or (expiration - utcnow).seconds <= REFRESH_LIMIT_HOURS
        )
    ...
    def delete_message(self, message):
        ...
        if message is not None:
            ...
            if message._deleted:
                # Noop if the message was deleted earlier by the user in a bot function.
                # Avoids potential `ReceiptHandleIsInvalid` errors in jobs > 12 hours.
                return
