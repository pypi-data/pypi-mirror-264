...
from pyqalx.core.log import configure_logging, exception_hook
...
class Worker(Process):
    ...
    def _check_queue_credentials(self):
        ...
        """
        if self.queue._credentials_expired_or_expiring_soon():
            self.log.debug(
    ...
    def _process_functions(self):
        def process_as_far_as_possible(functions):
            ...
            # Processing complete
            # Check credentials and refresh if necessary
            # (e.g processing time > 12 hours) and delete message
            self._check_queue_credentials()
            self._delete_message()
            return self.job
