...
def load_lpt_emu(verbose=True):
    ...
    for n in range(15):
        ...
        emulator['model'].append(load_model(
            i_emulator_name, custom_objects=customs))
...
def load_nonlinear_lbias_emu(emu_type='nn', fold_name=None, detail_name=None,
                             field_name=None, read_rotation=True, verbose=True):
    ...
    if fold_name is None:
        ...
        assert os.path.exists(os.path.join(emulator_name, detail_name)), f'{os.path.join(emulator_name, detail_name)} does not exist'
