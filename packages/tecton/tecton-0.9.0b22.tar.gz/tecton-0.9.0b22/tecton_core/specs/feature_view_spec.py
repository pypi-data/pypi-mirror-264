...
class MaterializedFeatureViewSpec(FeatureViewSpec):
    ...
    def from_args_proto(
        ...
        if is_aggregate:
            ...
            slide_interval_proto = _get_aggregation_interval_from_feature_view_args(proto, is_continuous)
...
def _get_aggregation_interval_from_feature_view_args(
    proto: feature_view__args_pb2.FeatureViewArgs, is_continuous: bool
) -> Optional[duration_pb2.Duration]:
    batch_schedule = utils.get_field_or_none(proto.materialized_feature_view_args, "batch_schedule")
    # Default set in Kotlin for compaction fvs is the batch schedule. Batch schedule is required for fvs with batch_compaction_enabled=True
    if proto.materialized_feature_view_args.batch_compaction_enabled and batch_schedule and not is_continuous:
