...
class TupleDuckDBTerm(TupleTerm):
    def get_sql(self, **kwargs):
...
class AsofJoinDuckDBNode(nodes.AsofJoinNode):
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        left_join_struct = TupleDuckDBTerm(*[left_df.field(col) for col in self.join_cols])
        right_join_struct = TupleDuckDBTerm(*[AliasedQuery(right_name).field(col) for col in self.join_cols])
