...
class AsofSecondaryKeyExplodeNode(QueryNode):
    ...
    def _to_query(self) -> pypika.queries.QueryBuilder:
        ...
        left_struct = TupleTerm(*[spine_q.field(col) for col in self.fdw.join_keys])
        right_struct = TupleTerm(*[projected_features_q.field(col) for col in self.fdw.join_keys])
        ...
        if not (self.fdw.is_temporal_aggregate and self.fdw.has_lifetime_aggregate):
            ...
            if self.fdw.is_temporal_aggregate:
                earliest_anchor_time = self.fdw.earliest_window_start.total_seconds()
            else:
                earliest_anchor_time = -int(self.fdw.serving_ttl.total_seconds())

            earliest_timestamp = self.func.date_add("second", earliest_anchor_time, spine_timestamp_field)
            join_condition &= earliest_timestamp < features_timestamp_field
...
class AggregationSecondaryKeyRollupNode(QueryNode):
    ...
    def columns(self) -> Sequence[str]:
        ...
        return tuple(agg_columns + [o.name for o in self.fdw.materialized_fv_spec.secondary_key_rollup_outputs])
