...
try:
    from nakivo_prometheus_exporter.nakivo_api_handler.prom_parser import load_config_file
except ImportError:
    from nakivo_api_handler.prom_parser import load_config_file
...
This is free software, and you are welcome to redistribute it under certain conditions; Please type --license for more info.""",
    ...
    try:
        if _DEV or os.name == 'nt':
            uvicorn.run("nakivo_api_handler.metrics:app", **server_args)
        else:
            StandaloneApplication(nakivo_api_handler.metrics.app, server_args).run()
