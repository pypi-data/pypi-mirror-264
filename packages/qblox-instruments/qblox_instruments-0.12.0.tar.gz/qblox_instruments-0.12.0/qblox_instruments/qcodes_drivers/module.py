...
class Module(InstrumentChannel):
    ...
    def __init__(
        ...
        for attr_name in Module._get_required_parent_attr_names():
...
def add_qcodes_params(parent: Union[Instrument, Module], num_seq: int) -> None:
    """
    ...
    if parent.is_qrm_type:
        for x in range(0, 2):
            ...
            parent.add_parameter(
                ...
                label="Scope acquisition averaging mode enable for input path {}".format(
                    x
                ),
