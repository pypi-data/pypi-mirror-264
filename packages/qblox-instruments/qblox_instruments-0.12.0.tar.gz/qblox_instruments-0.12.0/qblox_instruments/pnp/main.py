...
class PlugAndPlay:
        ...
        for serial, data in sorted(
            ...
            if remote_ip is None:
                ...
                remote_net = ipaddress.IPv4Network(
                    "{}/32".format(remote_ip), strict=False
                )
            ...
            print(
                ' - {}: {} {} with name "{}" and serial number {}'.format(
                    ip_info,
    ...
    def set_name(
        ...
        response, _ = self._query(
            serial_or_name, "SET_NAME {}".format(new_name), retries, timeout
        )
        if response != "OK":
            raise RuntimeError("Unexpected response: {}".format(response))
    ...
    def set_ip(
        ...
        if not serial_or_name:
            raise ValueError("Serial number or name must not be blank")
        response, _ = self._query(
            serial_or_name, "SET_IP {}".format(ip_address), retries, timeout
        )
        if response != "OK":
            raise RuntimeError("Unexpected response: {}".format(response))
...
def _main(args: Union[None, Iterable[str]] = None) -> Any:
    ...
    with PlugAndPlay() as p:
        ...
        def print_help_and_exit(arg_stack, fn):
            ...
            print(
                "Syntax: {} {}\n\n{}".format(
                    sys.argv[0], " ".join(arg_stack), inspect.cleandoc(fn.__doc__)
                ),
                file=sys.stderr,
            )
            sys.exit(2)
