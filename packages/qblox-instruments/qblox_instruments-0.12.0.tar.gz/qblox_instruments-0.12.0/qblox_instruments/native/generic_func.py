...
class StatusTuple:
    ...
    def __str__(self) -> str:
        ...
        pretty_str = "Status: {}, State: {}, Info Flags: {}, Warning Flags: {}, Error Flags: {}, Log: {}".format(
            status, state, flags[0], flags[1], flags[2], log
        )
...
class FuncRefs:
        ...
        def create_unique_func(name: str) -> Callable[..., None]:
            def raise_not_implemented_error(*args, **kwargs) -> None:
                raise NotImplementedError(
                    '"{}" has not yet been registered to this function reference container.'.format(
                        name
                    )
                )
    ...
    def register(
        ...
        if attr_name in self.funcs:
            ...
            raise KeyError(
                "Attribute name that is being registered ({}) is not in the instrument function list".format(
                    attr_name
                )
            )
...
def check_sequencer_index(sequencer: int) -> None:
    ...
    if sequencer < 0:
        raise ValueError("Sequencer index is out-of-range ({})".format(sequencer))
...
def _check_program_length(program: str) -> None:
    ...
    if len(program.encode("utf-8")) > MAX_PROGRAM_LENGTH:
        ...
        if len(checked_program.encode("utf-8")) > MAX_PROGRAM_LENGTH:
            raise RuntimeError(
                "Program length too large, expected something below {} bytes but got {} bytes.".format(
                    MAX_PROGRAM_LENGTH, len(checked_program.encode("utf-8"))
                )
            )
        else:
...
def set_acq_scope_config_val(funcs: FuncRefs, keys: Any, val: Any) -> None:
    ...
    for key in keys[:-1]:
        if (
            isinstance(d, list)
            and (not isinstance(key, int) or key < 0 or key >= len(d))
        ) or (isinstance(d, dict) and (not isinstance(key, str) or key not in d)):
            raise KeyError(
                f'acq_scope_dict[{"][".join(str(e) for e in keys)}] is not a valid scope path, failed at {key}'
            )
        d = d[key]

    key = keys[-1]
    if (
        isinstance(d, list) and (not isinstance(key, int) or key < 0 or key >= len(d))
    ) or (isinstance(d, dict) and (not isinstance(key, str) or key not in d)):
        raise KeyError(
            f'acq_scope_dict[{"][".join(str(e) for e in keys)}] is not a valid scope path, failed at {key}'
        )
...
def clear_sequencer_flags(funcs: FuncRefs, scpi_cmd_prefix: str) -> None:
    ...
    def clear_sequencer_flags(instrument: Any):
        funcs._write("{}:CLR:FLAGS".format(scpi_cmd_prefix))
...
def get_sequencer_state(
    ...
    while (
        ...
        if elapsed_time >= timeout:
            raise TimeoutError(
                "Sequencer {} did not stop in timeout period of {} minutes.".format(
                    sequencer, int(timeout / 60)
                )
            )
...
def get_acquisition_state(
    ...
    while acq_state is False and elapsed_time < timeout:
        ...
        if elapsed_time >= timeout:
            raise TimeoutError(
                "Acquisitions on sequencer {} did not complete in timeout period of {} minutes.".format(
                    sequencer, int(timeout / 60)
                )
            )
