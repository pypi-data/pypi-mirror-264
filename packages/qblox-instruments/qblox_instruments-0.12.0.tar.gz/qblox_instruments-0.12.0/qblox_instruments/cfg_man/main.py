...
class ConfigurationManager:
    ...
    def _check_capability(self, cmd: str) -> None:
        ...
        if not self.has_capability(cmd):
            raise NotImplementedError(
                "{}() is not implemented for {} connections".format(
                    cmd, self._ci.protocol
                )
            )
    ...
    def set_name(self, name: str) -> None:
        ...
        if not hasattr(self._conn, "set_name"):
            raise NotImplementedError(
                "set_name() is not implemented for {} connections".format(
                    self._ci.protocol
                )
            )
...
def _main(args: Union[None, Iterable[str]] = None) -> None:
    ...
    if args is not None:
        ...
        usage = "{} [options] <device> [commands...]".format(
        ...
        try:
            ...
            while True:
                ...
                if cmd == "describe":
                    ...
                    if source is None:
                        raise ValueError('Unknown log type "{}"'.format(log_type))
                ...
                if reuse_key is not None:
                    if reuse_key in commands_used:
                        raise ValueError(
                            "{} command conflicts with earlier {} command".format(
                                cmd, commands_used[reuse_key]
                            )
                        )
