...
def parse_arguments_list():
    ...
    parser.add_argument('-dl', '--input-lf-data-type', choices=('xyz', 'ani', 'aev', 'pca', 'sqlite'), default='sqlite',
    ...
    #parser.add_argument('--trtsid-name', nargs='*', default=['lesgo.txt'],
...
def set_up_task_weights(model, args, optimizer):
    if args.optimize_force_weight:
        ...
        mtl = prep.task_weights(num_tasks=2, traditional=False)#.to(self.gpu_id)
...
def prep_model(args, load_opt=True,
    ...
    print('model random seed:', args.randomseed[0])
...
def sae_calculator(energies, atom_count, sae_guess=np.array([-1035.30436565, -16.8356588])): #sae_guess=np.array([-1028.6, -13.8])):
    ...
    for i in range(len(sae_guess)):
        low_bounds.append(sae_guess[i]-3)
        high_bounds.append(sae_guess[i]+3)
...
def load_data(args, get_aevs=True, fid='', mf=None):
    ...
    if args.pre_saved:
        ...
        if get_aevs:
            train_vld_aevs = np.array(torch.load(args.trpath+'train_aevs'))
        train_vld_engs = np.array(torch.load(args.trpath+'train_engs'))
        ...
        train_engs = train_vld_engs[train_indx]
        valid_engs = train_vld_engs[valid_indx]
        del train_vld_engs
        
        if get_aevs:
            sae_energies = np.load(args.trpath + 'sae_energies.npy')
            dimdat = torch.load(args.trpath+'aev_length')
            train_aevs = train_vld_aevs[train_indx]
            valid_aevs = train_vld_aevs[valid_indx]
            del train_vld_aevs
        ...
        if args.floss:
            train_vld_forces = np.array(torch.load(args.trpath+'train_forces'))
            train_vld_fdims = np.array(torch.load(args.trpath+'train_fdims'))
...
def prep_data(args, mf=None, fid='', get_aevs=True):
    ...
    if args.gen_byid:
        print('prep_data: calling read_xidtxt with args.trtsid_name[0]:',args.trtsid_name[0])
        xid = read_xidtxt(args.trtsid_name[0])
        print('prep_data: calling get_data with xid length:',len(xid))
    ...
    if args.sae_fit:
        ...
        #sae_guess=np.array([-1035.30436565, -16.8356588, -2000.000])
        ...
        for tr in range(len(dpes.train_engs)):
            dpes.train_engs[tr] -= energies_to_subtract[tr]#subtracted_energies[tr]

        for ts in range(len(dpes.test_engs)):
            for ty in range(len(dpes.test_aevs[ts])):
                dpes.test_engs[ts] = dpes.test_engs[ts] - sae_energies[ty] * len(dpes.test_aevs[ts][ty])
