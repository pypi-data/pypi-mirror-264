...
class DockerRunner:
    ...
    class DockerLogHandler(Thread):
        ...
        def run(self):
            ...
            for log in self.log_generator:
                DockerRunner.logger.info(
                    "DockerMsg: {}".format(log.decode("utf-8")).rstrip()
                )
    ...
    def prepare_docker(
        ...
        docker_registry_address: str = None,
    ):
        """
        ...
        if image_version is None:
            ...
            self.image_full_name = (
                self.image_name + self.DOCKER_TAG_DELIMITER + self.image_tag
            )
        else:
    ...
    def login(self):
        ...
        """
        self.logger.info(
            f"You haven't logged in to {self.docker_registry_address} yet, log in now"
        )
        ...
        os.system(
            f"docker login {self.docker_registry_address} --username {username} --password {password}"
        )
    ...
    def get_latest_tag(image: Image) -> str:
        ...
        image_name_delimiter = (
            DockerRunner.DOCKER_TAG_DELIMITER + DockerRunner.DOCKER_IMAGE_TAG_PREFIX
        )
        tags = image.tags
        splited_tags = {tuple(tag.split(image_name_delimiter)) for tag in tags}
        versions = {
            tag_value[1] if len(tag_value) == 2 else None for tag_value in splited_tags
        }
    ...
    def get_fresh_container(self, container_name: str) -> Container:
        ...
        if container is None:
            self.logger.info(
                "Container {} not found, creating it".format(container_name)
            )
        else:
            ...
            if self.DockerStatus.CREATED == status:
                ...
                self.logger.debug(
                    "Container {} stopped and removed".format(container_name)
                )
            elif self.DockerStatus.RESTARTING == status:
                self.logger.debug(
                    "Restarting docker container found, stopping it and removing it"
                )
                ...
                self.logger.debug(
                    "Running docker container found, stopping and removing it"
                )
                self.stop_container(container)
                self.remove_container(container)
                self.logger.debug(
                ...
                self.logger.debug(
                    "Paused docker container found, stopping and removing it"
                )
                self.stop_container(container)
                self.remove_container(container)
                self.logger.debug(
    ...
    def start(self):
        ...
        self.container.reload()
        self.docker_logger = DockerRunner.DockerLogHandler(
            self.container.logs(stream=True, timestamps=True)
        )
