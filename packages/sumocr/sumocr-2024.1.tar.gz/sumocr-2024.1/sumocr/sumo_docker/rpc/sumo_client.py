...
from commonroad.scenario.scenario import LaneletNetwork, Scenario
from commonroad.scenario.trajectory import State
from grpc import ChannelCredentials

from sumocr.interface.ego_vehicle import EgoVehicle
from sumocr.interface.sumo_simulation import SumoSimulation
from sumocr.scenario import AbstractScenarioWrapper
from sumocr.sumo_config import DefaultConfig
from sumocr.sumo_docker.rpc.messaging import (
    get_data_chunks,
    save_chunks_to_file,
    unzip_file,
)
...
class SumoRPCClient:
    ...
    def __init__(self, address: str):
        ...
        self._request_provider = SumoRPCClient.RequestProvider(
            self.channel, self.__rpc_module
        )
    ...
    def _create_request(self, method_name: str, request_type: RequestTypes):
        CREATE_REQUEST_MAPPING = {
            SumoRPCClient.RequestTypes.DIRECT: self._request_provider.create_request_method,
    ...
    def _create_helper_requests(self):
        ...
        self._create_request(
            "generate_maps_from_cr_file", SumoRPCClient.RequestTypes.DIRECT
        )

        self._create_request(
            "upload_scenario", SumoRPCClient.RequestTypes.CLIENT_STREAMING
        )
        self._create_request(
            "download_scenario", SumoRPCClient.RequestTypes.SERVER_STREAMING
        )
    ...
    def _create_property_requests(self):
        ...
        sumo_interface_properties = inspect.getmembers(
            SumoSimulation, lambda a: not (inspect.isroutine(a))
        )
        sumo_interface_public_properties = [
            tup for tup in sumo_interface_properties if not tup[0].startswith("_")
        ]
        for sumo_interface_public_variable in sumo_interface_public_properties:
            self._create_request(
                f"{sumo_interface_public_variable[0]}_get",
                SumoRPCClient.RequestTypes.DIRECT,
            )
            self._create_request(
                f"{sumo_interface_public_variable[0]}_set",
                SumoRPCClient.RequestTypes.DIRECT,
            )
    ...
    def _create_variable_requests(self):
        ...
        sumo_interface_public_variables = [
            attr for attr in vars(SumoSimulation()) if not attr.startswith("_")
        ]
        for sumo_interface_public_variable in sumo_interface_public_variables:
            self._create_request(
                f"{sumo_interface_public_variable}_get",
                SumoRPCClient.RequestTypes.DIRECT,
            )
            self._create_request(
                f"{sumo_interface_public_variable}_set",
                SumoRPCClient.RequestTypes.DIRECT,
            )
    ...
    def _create_method_requests(self):
        ...
        sumo_interface_public_methods = [
            tup for tup in sumo_interface_methods if not tup[0].startswith("_")
        ]
        for sumo_interface_public_method in sumo_interface_public_methods:
            self._create_request(
                sumo_interface_public_method[0], SumoRPCClient.RequestTypes.DIRECT
            )
    ...
    class RequestProvider:
        ...
        def _grpc_unary_unary_call(self, func, obj_to_bind=None):
            ...
            def func_called_with_arguments(*args):
                ...
                if (
                    isinstance(ret_val, tuple)
                    and len(ret_val) == 1
                    and ret_val[0] is None
                ):
                    ret_val = None
                return ret_val
        ...
        def _pack(obj) -> bytes:
            ...
            try:
                ...
                SumoRPCClient.logger.critical(
                    "Exception occurred during packing the object: {}".format(e)
                )
        ...
        def _unpack(msg: bytes) -> object:
            ...
            try:
                ...
                SumoRPCClient.logger.critical(
                    "Exception occurred during unpacking the messsage: {}".format(e)
                )
        ...
        def create_request_method(self, method_name):
            ...
            """
            grpc_request = self._channel.unary_unary(
                "/" + self._module + "/" + method_name,
        ...
        def create_client_streaming_method(self, method_name):
            ...
            :param method_name: The name of the method which call is requested by the gRPC server
            """
            grpc_request = self._channel.stream_unary(
                "/" + self._module + "/" + method_name,
                request_serializer=self._pack,
                response_deserializer=self._unpack,
            )
        ...
        def create_server_streaming_method(self, method_name):
            ...
            :param method_name: The name of the method which call is requested by the gRPC server
            """
            grpc_request = self._channel.unary_stream(
                "/" + self._module + "/" + method_name,
                request_serializer=self._pack,
                response_deserializer=self._unpack,
            )
    ...
    def stop_server(self) -> bool:
        ...
        :return: True if the server has been successfully stopped
        """
    ...
    def generate_sumo_scenario(
        self, scenario_name: str, scenario_path: str
    ...
    def initialize(
        ...
        planning_problem_set: Union[None, PlanningProblemSet] = None,
    ) -> None:
        """
        ...
        scenario_wrapper.sumo_cfg_file = os.path.join(
            base_dir, os.path.split(scenario_wrapper.sumo_cfg_file)[1]
        )
    ...
    @Decorators.log_call(level=logging.DEBUG)
    def print_lanelet_net(
        self, with_lane_id=True, with_succ_pred=False, with_adj=False, with_speed=False
    ):
        """
    ...
    def commonroad_scenario_at_time_step(
        ...
        """
        return self._request["commonroad_scenario_at_time_step"](
            time_step, add_ego, start_0
        )

    @Decorators.log_call(level=logging.DEBUG)
    def commonroad_scenarios_all_time_steps(
    ...
    def check_lanelets_future_change(
        ...
        """
        return self._request["check_lanelets_future_change"](
            current_state, planned_traj
        )
