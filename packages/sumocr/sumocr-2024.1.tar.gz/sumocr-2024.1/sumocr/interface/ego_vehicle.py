...
class EgoVehicle:
    ...
    def __init__(
        ...
        self._state_dict: Dict[int, State] = dict()  # collects driven states
        self.shape = Rectangle(
            self.length, self.width, center=np.array([0, 0]), orientation=0.0
        )
        self._driven_trajectory = None  # returns trajectory object of driven
        self._planned_trajectories: Dict[
            int, List[State]
        ] = {}  # collects trajectories from planner for every time step
        self._current_time_step: int = initial_state.time_step
        self.delta_steps = delta_steps
        self.planning_problem = planning_problem

    @property
    def pp_id(self) -> Optional[int]:
        """
    ...
    def set_planned_trajectory(self, planned_state_list: List[State]) -> None:
        ...
        assert (
            len(planned_state_list) >= self.delta_steps
        ), "planned_trajectory must contain at least {} states, but contains {}. (See delta_steps in sumo_config file)".format(
            self.delta_steps, len(planned_state_list)
        )
        assert (
            1 == planned_state_list[0].time_step
        ), "planned_trajectory must always start at time_step ({}) but starts at time_step {}".format(
            1, planned_state_list[0].time_step
        )
    ...
    def get_dynamic_obstacle(
        ...
        if time_step is None:
            return DynamicObstacle(
                ...
                prediction=self.driven_trajectory,
            )
            ...
            if time_step in self._state_dict:
                if time_step in self._planned_trajectories:
                    prediction = TrajectoryPrediction(
                        Trajectory(
                            self._planned_trajectories[time_step][0].time_step,
                ...
                return DynamicObstacle(
                    ...
                    initial_state=self.get_state_at_timestep(time_step),
    ...
    def get_planned_state(self, delta_step: int = 0):
        ...
        planned_state: State = copy.deepcopy(
            self._planned_trajectories[self.current_time_step][0]
        )
        if self.delta_steps > 1:
            ...
            for state in planned_state.attributes:
                ...
                setattr(
                    ...
                    state,
                    curr_state
                    + (delta_step + 1) / self.delta_steps * (next_state - curr_state),
                )
    ...
    def current_state(self, current_state):
        raise PermissionError(
            "current_state cannot be set manually, use set_planned_trajectory()"
        )
    ...
    def driven_trajectory(self) -> TrajectoryPrediction:
        ...
        return TrajectoryPrediction(
            Trajectory(self.initial_state.time_step + 1, state_list), self.shape
        )
