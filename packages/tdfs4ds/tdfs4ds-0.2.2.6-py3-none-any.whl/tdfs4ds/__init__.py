...
def run(process_id, return_dataset = False):
    ...
    query = f"""
    {validtime_statement}
    SEL A.*, B.FOR_PRIMARY_INDEX, B.FOR_DATA_PARTITIONING
    FROM {tdfs4ds.SCHEMA}.{tdfs4ds.PROCESS_CATALOG_NAME} A
    INNER JOIN {tdfs4ds.SCHEMA}.{tdfs4ds.DATA_DISTRIBUTION_NAME} B
    ON A.PROCESS_ID = B.PROCESS_ID
    WHERE A.PROCESS_ID = '{process_id}' AND B.PROCESS_ID = '{process_id}'
    """
    ...
    primary_index = df['FOR_PRIMARY_INDEX'].values[0].split(',')
