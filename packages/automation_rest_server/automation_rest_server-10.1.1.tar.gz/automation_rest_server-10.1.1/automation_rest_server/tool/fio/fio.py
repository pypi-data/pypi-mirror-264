...
class Fio(LinuxFio):
    ...
    def set_fio_parameters(self, fio_parameters):
        ...
        # self.set_parm("write_iops_log", bw_log)
        numjobs = fio_parameters["numjobs"]
        bw_logs = list()
        # iops_logs = []
        for index in range(int(numjobs)):
            name = "{}_bw.{}.log".format(bw_log, index+1)
            # name_iops = "{}_iops.{}.log".format(bw_log, index+1)
            bw_logs.append(name)
            # iops_logs.append(name_iops)
