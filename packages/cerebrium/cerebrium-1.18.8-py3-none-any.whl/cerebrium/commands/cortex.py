...
def do_partial_upload(
    ...
    temp_file_list.extend(
        [os.path.join(os.getcwd(), f) for f in constants.INTERNAL_FILES]
    )
...
def _setup_request(
    ...
    if not cerebrium_config.build.disable_animation:
        setup_spinner = yaspin(text="Building App...", color="light_grey")
        setup_spinner.start()
    else:
        setup_spinner = None
    ...
    if setup_response.status_code != 200:
        if setup_spinner:
            ...
            print("❌ There was an error deploying your app")
        cerebrium_log(
