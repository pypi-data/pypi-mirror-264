...
class ModelClient(BceInternalClient):
    ...
    def model_dump(self, artifact_name: str, location_style: str, rename: str = None, output_uri: str = "."):
        """
        ...
        # b. 遍历manifest_list artifacts，下载文件
        artifacts = {}
        m_name = model_name.get_name()
        for item in manifest_list.artifacts:
            m = parse_model_name(item["objectName"])
            new_local_name = None
            if rename and item["objectName"] == m_name:
                new_local_name = rename

            artifact = self._dump_artifact(filesystem, item, location_style, new_local_name, output_uri)
            artifacts[m.local_name] = artifact

        # c. 获取模型列表，生成apply.yaml文件
        apply_model_list = []
        ensemble = None
        model_list = self.list_model(model_name.workspace_id,
                                     model_name.model_store_name,
                                     names=list(artifacts.keys()))
        for model in model_list.result:
            model["category"] = model["category"]["category"]

            if match(model["category"], Category.CategoryImageEnsemble.value):
                update_model_metadata(artifacts[model["localName"]]["metadata"])
                model["artifact"] = artifacts[model["localName"]]
                ensemble = {"kind": "Model", "metadata": model}
                continue

            apply_model_list.append({"kind": "Model", "metadata": model})
        apply_model_list.append(ensemble)

        write_apply_yaml(apply_model_list, "---\n", output_uri + "apply.yaml")

    def _dump_artifact(self, filesystem, artifact, location_style: str, rename: str = None, output_uri: str = "."):
        """
        dump artifact
        Args:
            filesystem: file system instance
            artifact(dict): artifact info
            location_style: location style, default is "Triton"
            output_uri: model output file path
        ...
        """
        # 1. 获取model dump列表文件
        model_name = parse_model_name(artifact["objectName"])
        if rename:
            model_name.local_name = rename

        output_path = output_uri.rstrip("/") + "/" + model_name.local_name
        output_path = get_location_path(output_path, location_style, str(artifact["version"]))
        if not os.path.exists(output_path):
            os.makedirs(output_path)

        # 2. 下载模型文件
        download_by_filesystem(filesystem, artifact["uri"], output_path)

        # 3. 调整模型文件
        for file_name in os.listdir(output_path):
            src_path = output_path + "/" + file_name
            if os.path.isdir(src_path):
                continue

            if file_name.endswith(".yaml") or file_name.endswith(".yml") or file_name.endswith(".pbtxt"):
                dest_path = os.path.abspath(output_path + "/../" + file_name)
                shutil.copyfile(src_path, dest_path)

        # 4. 读取 meta.yaml 文件内容 生成创建模型输入
        with open(output_path + "/artifact.yaml", 'r') as file:
            data = yaml.safe_load(file)
        data["uri"] = output_path
        return data
