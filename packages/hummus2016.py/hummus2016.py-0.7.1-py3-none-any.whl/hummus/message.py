...
class Message:
    ...
    async def _makeEmbed(self,data):
      for embed in data:
        e = Embed(embed['title'],embed['description'],embed['color'],embed['url'],embed['timestamp'])
        for field in embed['fields']:
          await e.addField(field['name'],field['value'],field['inline'])
        if embed.get('image'):
          await e.addImage(embed['image']['url'])
        if embed.get('thumbnail'):
          await e.addThumbnail(embed['thumbnail']['url'])
        if embed.get('footer'):
          await e.addFooter(embed['footer']['text'],embed['footer']['icon_url'])
        if embed.get('author'):
          await e.addAuthor(embed['author']['name'],embed['author']['url'],embed['author']['icon_url'])
        #There is an extra parameter known as "type", seen data as "rich", but I see no purpose in it, so I have not added it. Correct me if I am wrong.
