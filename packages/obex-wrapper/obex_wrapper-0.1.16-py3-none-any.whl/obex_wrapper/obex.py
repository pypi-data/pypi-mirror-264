...
otlp_exporter = OTLPMetricExporter(
    endpoint=OTEL_COLLECTOR_ENDPOINT+"/v1/metrics")
...
class ObexAbusiveLanguageDetector:
    def __init__(self, config_provider):
    ...
    def check(self, text):
        ...
        output = []
        for k in preds:
            if k in abuse_config and preds[k] > (.01 * abuse_config[k]):
                output.append(k)
        return output
...
class Obex:
    def __init__(self, *, aws_session=None):
        ...
        self.abuse_checker = ObexAbusiveLanguageDetector(self.config)
    ...
    def increase_counter(self, provider, dlp=False):
        if provider == "openai":
    ...
    def protect(self, func):
        ...
        def new_send(*args, **kwargs):
            ...
            provider = self.get_provider(request.url)
            self.increase_counter(provider)
            if provider == "openai":
                openai_counter.add(1)
            ...
            if dlp_check:
                ...
                return self.generate_error(request, "Request blocked by Obex for violating DLP rules: " + str(dlp_check))
            ...
            if abuse_check_request:
                ...
                self.logger.log(audit_data)
                return self.generate_error(request, "Request blocked by Obex for violating abusive content rules: " + str(abuse_check_request))
            ...
            if abuse_check_response:
                ...
                self.logger.log(audit_data)
                return self.generate_error(request, "Request blocked by Obex for violating abusive content rules: " + str(abuse_check_response))
