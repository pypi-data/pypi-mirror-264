...
class DuckDBQueryWidget:
    ...
    def run_query(self, sql_query):
        ...
        try:
            result = self.con.execute(modified_query).fetchdf()
...
def grid_to_updated_dataframe(grid):
    updated_data = []
    
    column_names = [extract_text_from_html(str(grid[0, j].value)) for j in range(grid.n_columns - 1)]
    
    for i in range(1, grid.n_rows):
        row_data = []
        for j in range(grid.n_columns - 1):
            widget = grid[i, j]
            
            if isinstance(widget, widgets.FloatText) or isinstance(widget, widgets.Text):
                value = widget.value
...
HAVING COUNT(*) > 1"""
    ...
    duplicate_count = con.execute(duplicate_count_sql).df().iloc[0, 0]

    sample_sql = sql_query
    if sample_size > 0:
        sample_sql += f" LIMIT {sample_size}"

    sample_duplicate_rows = con.execute(sample_sql).df()
...
def create_sample_distinct(con, table_name, column_name, sample_size):
    ...
    sample_values = con.execute(query).df()

    return sample_values

def count_total_distinct(con, table_name, column_name):
    total_distinct_count = con.execute(f"SELECT COUNT(DISTINCT {column_name}) FROM {table_name} WHERE {column_name} IS NOT NULL").df().iloc[0, 0]
