...
class IssueWokflow(Base):
    ...
    #     jira_issues_list = IssueS2.all_key()  # 7 sec
    #     for issue_key in jira_issues_list:  # 11sec
    #         IssueWokflow.save_issue_workflow(issue_key) #18min

    @staticmethod
    def fill_issue_workflow_bdd():
        IssueWokflow.create_all_tables()
        IssueWokflow.delete_all()
        IssueWokflow.get_workflow(IssueS2.all_key(), config.jiraStatusName)
    ...
    def get_issue_changelog(issue_key):
        try:
            ...
            response = IssueS2.jira_resquest('get', history_url)
            time.sleep(config.time_to_sleep)
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
            print(f"Error fetching changelog for issue {issue_key}: {e}")
            return None

    @staticmethod
    def get_workflow(issue_keys, status):
        from_time = []
        to_time = []
        workflows = []
        for issue_key in issue_keys:
            history_data = IssueWokflow.get_issue_changelog(issue_key)
            histories = history_data.get('values', [])
