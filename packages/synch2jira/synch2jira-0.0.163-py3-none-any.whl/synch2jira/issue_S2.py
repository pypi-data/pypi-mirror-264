...
class IssueS2(Issue):
    ...
    def save(self):
        ...
        print(self.format_issue_into_json())
    ...
    def save_data_json_in_file():
        ...
        while True:
            params = {'jql': '', "startAt": start_at, "maxResults": batch_size, "fields": '',
                      "expand": 'changelog'}
            response = IssueS2.jira_request(method='get', url=config.jira_url_all, params=params)
            if response.status_code == 200:
                batch_tickets = response.json()["issues"]
                ...
                start_at += batch_size
                if len(batch_tickets) < batch_size:
                    break  # Arrêter la boucle si moins de tickets sont retournés que le batch_size

        with open(config.json_issues_file, 'w') as json_file:
            json.dump(all_tickets, json_file, indent=4)

        print(f"Données JSON enregistrées dans le fichier : {config.json_issues_file}")
