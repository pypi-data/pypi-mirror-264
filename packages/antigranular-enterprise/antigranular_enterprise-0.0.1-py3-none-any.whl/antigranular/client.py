...
class AGClient:
    ...
    def read_config(self, profile) -> None:
        self.config.read_config(profile=profile)

    def write_config(self, yaml_config, profile) -> None:
        self.config.write_config(yaml_config, profile)
    ...
    def __get_refresh_token(self) -> None:
        try:
            if not self.__is_token_expired():
                return
            res = requests.get(
                self.config.CONSOLE_URL + "/user/token/refresh",
    ...
    def connect(self) -> None:
        try:
            params = {self.config.CONSOLE_API_KEY_PARAM: self.config.CONSOLE_API_KEY}
            response = requests.get(self.config.CONSOLE_URL + "/user/token/request", params=params, stream=True)
    ...
    def __get_output(self, message_id) -> None:
        ...
        return_value = None
        while True:
            if count > int(self.config.AG_EXEC_TIMEOUT):
