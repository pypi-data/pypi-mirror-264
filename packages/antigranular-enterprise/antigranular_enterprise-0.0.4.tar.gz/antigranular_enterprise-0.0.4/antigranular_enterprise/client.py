...
class AGClient:
    ...
    def __get_refresh_token(self) -> None:
        try:
            if not self.__is_token_expired():
                return
            res = requests.get(
                config.CONSOLE_URL + "/user/token/refresh",
    ...
    def connect(self, api_key) -> None:
        try:
            params = {config.CONSOLE_API_KEY_PARAM: api_key}
            response = requests.get(config.CONSOLE_URL + "/user/token/request", params=params, stream=True)
