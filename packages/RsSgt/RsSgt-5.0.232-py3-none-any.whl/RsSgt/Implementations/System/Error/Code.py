...
class CodeCls:
	...
	def get_all(self) -> str:
		...
		Queries the error numbers of all entries in the error queue and then deletes them. \n
			:return: all_py: string Returns the error numbers. To retrieve the entire error text, send the command method RsSgt.System.Error.all. 0 'No error', i.e. the error queue is empty Positive value Positive error numbers denote device-specific errors Negative value Negative error numbers denote error messages defined by SCPI.
		"""
	...
	def get_next(self) -> str:
		...
		Queries the error number of the oldest entry in the error queue and then deletes it. \n
			:return: next_py: string Returns the error number. To retrieve the entire error text, send the command method RsSgt.System.Error.all. 0 'No error', i.e. the error queue is empty Positive value Positive error numbers denote device-specific errors Negative value Negative error numbers denote error messages defined by SCPI.
		"""
