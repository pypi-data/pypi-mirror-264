...
class GUI:
    ...
    def add_chekbox(self, tab, name: str, label_to_input_ratio=0.5, label: str = None, empty_return: str = '', margin: float = 0):
        ...
        a.place(relx=indent, rely=rely, relwidth=item_w)
        relwidth = (1-label_to_input_ratio) * (1-2*margin)
        b = Checkbutton(root, command=partial(self.callback, self.vars[name]), background=self.bg, activebackground=self.bg,
                        highlightcolor=self.bg, highlightbackground=self.bg, borderwidth=None, variable=self.vars[name], onvalue=1, offvalue=0)
        indent += item_w
        b.place(relx=indent, rely=rely, relwidth=relwidth)
