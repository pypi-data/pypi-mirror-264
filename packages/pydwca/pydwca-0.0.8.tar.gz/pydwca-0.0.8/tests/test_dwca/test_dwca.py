...
class TestDWCA(unittest.TestCase):
    ...
    def test_as_pandas(self):
        ...
        self.assertEqual(len(df), 163460, "Wrong number of rows")
        self.assertEqual(len(df.columns), len(self.object.core.__fields__), "Wrong number of fields")
        for extension, rows in zip(self.object.extensions, [153621, 98518, 7617]):
            df = extension.as_pandas()
            self.assertEqual(len(df), rows, f"Wrong number of rows at {extension.__class__.__name__}")
            self.assertEqual(len(df.columns), len(extension.__fields__), "Wrong number of fields")
