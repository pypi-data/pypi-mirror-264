...
class TestDriverPysigma(unittest.TestCase):
    ...
    def test_01_run_PySIGMA(self):
        ...
        for magnet_name in self.magnet_names:
            input_folder_path = os.path.join(Path(self.test_folder).parent, 'builders', 'model_library', 'magnets', magnet_name, 'output')
    ...
    def test_02_export_B_txt_to_map2d(self):
        ...
        for magnet_name in self.magnet_names:
            ...
            output_folder_path = os.path.join(Path(self.test_folder).parent, 'builders', 'model_library', 'magnets', magnet_name, 'output')
            input_file_path = os.path.join(output_folder_path, f'{magnet_name}.yaml')
            ...
            path_result_txt_Bx = os.path.join(output_folder_path, 'output_folder', 'mf.Bx.txt')
            path_result_txt_By = os.path.join(output_folder_path, 'output_folder', 'mf.By.txt')
