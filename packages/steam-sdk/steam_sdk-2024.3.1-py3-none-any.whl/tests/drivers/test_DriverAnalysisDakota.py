...
class TestDriverAnalysisDakota(unittest.TestCase):
    ...
    def test_runAnalysisDakota_FiQuS(self):
        ...
        """
        input_folder_path = os.path.join(self.test_folder, 'input', 'AnalysisFiQuS')
        self.analysis_yaml_path = os.path.join(input_folder_path, "TestFile_AnalysisSTEAM_FiQuS_MQXA.yaml")
        ...
        initial_analysis_file_path = f'{self.analysis_yaml_path[:-4]}_0.yaml'
        ...
        parameters_file = os.path.join(input_folder_path, "dakota_parameters_file")

        output_folder_path = os.path.join(self.test_folder, 'output', 'AnalysisFiQuS')
        make_folder_if_not_existing(output_folder_path)
        results_file = os.path.join(output_folder_path, "dakota_results_file.txt")

        da = DriverAnalysis(analysis_yaml_path=self.analysis_yaml_path, iterable_steps=iterable_steps, sim_number_offset=0)
    ...
    def test_runAnalysisDakota_LEDET(self):
        ...
        input_folder_path = os.path.join(self.test_folder, 'input', 'AnalysisLEDET')

        self.analysis_yaml_path = os.path.join(input_folder_path, "TestFile_AnalysisSTEAM_LEDET_SMC.yaml")
        iterable_steps = ['setup_folder', 'makeModel_BM', 'modifyModel_EE', 'RunSim_BM']
        parameters_file = os.path.join(input_folder_path, "dakota_parameters_file")

        output_folder_path = os.path.join(self.test_folder, 'output', 'AnalysisLEDET')
        make_folder_if_not_existing(output_folder_path)
        results_file = os.path.join(output_folder_path, "dakota_results_file.txt")
