...
class TestParserXYCE(unittest.TestCase):
    ...
    def test_translate_create_run_full_model_multipleCircuits(self):
        ...
        for name in list_circuits:
            ...
            output_path_cir = os.path.join(output_path,f'{name}_XYCE.cir')
            pXYCE.write2XYCE(full_path_file_name=output_path_cir, verbose=verbose)
            pXYCE.copy_additional_files(flag_translate=True, verbose=verbose)
            pXYCE.resolve_paths_netlist(full_path_file_name=output_path_cir)

            dXYCE = DriverXYCE(self.path_XYCE, output_path, verbose=verbose)
            dXYCE.run_XYCE(nameCircuit=name, suffix='_XYCE')
            self.assertTrue(os.path.exists(output_csd))
