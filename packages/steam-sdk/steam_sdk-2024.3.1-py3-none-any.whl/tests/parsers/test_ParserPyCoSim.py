...
class TestParserPyCoSim(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        path_settings, self.data_settings = read_settings_file(relative_path_settings='../', verbose=True)

        self.local_PyCosim_folder = os.path.join('output', 'PyCoSim')  # Note: This should match the folder deleted by the setUpClass() method
    ...
    def test_write_model_PyCoSim(self):
        ...
        sim_number = 125
        verbose = True
        path_output = os.path.join(self.local_PyCosim_folder, 'RQX')
        ...
        BC = BuilderCosim(file_model_data=path_model_data, data_settings=self.data_settings, verbose=verbose)
