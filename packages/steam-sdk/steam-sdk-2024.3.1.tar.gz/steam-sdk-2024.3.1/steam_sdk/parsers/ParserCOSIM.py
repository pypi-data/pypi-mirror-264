...
class ParserCOSIM:
    ...
    def write_cosim_model(self, sim_name: str, sim_number: int, output_path_COSIM_folder: str, verbose: bool = None):
        ...
        for model_name, model in self.cosim_data.Simulations.items():
            ...
            if model.type == 'FiQuS':
                ...
                self.write_model_LEDET(model_name=model_name, model=model, output_path=output_path_COSIM_folder, verbose=verbose)
            elif model.type == 'PSPICE':
                self.write_model_PSPICE(model_name=model_name, model=model, output_path=output_path_COSIM_folder, verbose=verbose)
    ...
    def write_model_LEDET(self, model_name: str, model, output_path: str, verbose: bool = False):
        '''
        ** Write selected LEDET model **
        ...
        # Make subfolders
        path_submodel_folder = os.path.join(self.local_COSIM_folder, self.cosim_name, str(self.sim_number), 'Input', model_name)
        ...
        self.write_config_file_ledet(output_file=path_config_file,
                                     LEDET_path=self.reformat_path(
                                         self.cosim_data.Options_COSIM.solverPaths[model_name]),
        ...
        write_model_input_files(cosim_data=self.cosim_data, model_name=model_name, model=model, cosim_software='COSIM',
                                local_cosim_folder=output_path, data_settings=self.data_settings,
                                nsti=NSTI(self.sim_number, None, 0, None), verbose=verbose)

    def write_model_PSPICE(self, model_name: str, model, output_path: str, verbose: bool = False):
        '''
        ** Write selected PSPICE model **
        ...
        # Make configuration file
        path_config_file = os.path.join(path_submodel_folder, f'{model_name}_config.json')
        self.write_config_file_pspice(output_file=path_config_file, model=model, solver_path=self.cosim_data.Options_COSIM.solverPaths[model_name])
        # Make input/output port definition file
        path_ports_file = os.path.join(path_submodel_folder, f'{model_name}_InputOutputPortDefinition.json')
        self.write_ports_file(output_file=path_ports_file, model_name=model_name)
