...
class TestParserCOSIM(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        path_settings, self.local_settings_dict = read_settings_file(relative_path_settings='../', verbose=True)

        self.local_COSIM_folder = os.path.join('output', 'COSIM')  # Note: This should match the folder deleted by the setUpClass() method
    ...
    def test_write_config_file_COSIM(self):
        ...
        sim_number = 123
        model_output_path = os.path.join(self.local_COSIM_folder, cosim_name, str(sim_number))
        ...
        path_name_GENERATED = os.path.join(model_output_path, 'Input', file_name_GENERATED)
        verbose = True

        # act - initialize BuilderCosim and ParserCosim objects
        BC = BuilderCosim(file_model_data=path_model_data, data_settings=self.local_settings_dict, verbose=verbose)
        pCOSIM = ParserCOSIM(BC.cosim_data, data_settings=self.local_settings_dict)
        ...
        pCOSIM.local_COSIM_folder = self.local_COSIM_folder
    ...
    def test_write_model_COSIM_LEDET_PSPICE(self):
        ...
        BC = BuilderCosim(file_model_data=path_model_data, data_settings=self.local_settings_dict, verbose=verbose)
        pCOSIM = ParserCOSIM(BC.cosim_data, data_settings=self.local_settings_dict)
        pCOSIM.write_cosim_model(sim_name=cosim_name, sim_number=sim_number, output_path_COSIM_folder=self.local_COSIM_folder, verbose=verbose)
