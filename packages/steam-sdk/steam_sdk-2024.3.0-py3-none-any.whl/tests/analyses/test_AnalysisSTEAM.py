...
class TestAnalysisSTEAM(unittest.TestCase):
    ...
    def test_AnalysisSTEAM_init_LocalSettings(self):
        ...
        """
        # arrange
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_REFERENCE.yaml')
        ...
        self.assertEqual(self.settings.comsolexe_path, aSTEAM.settings.comsolexe_path)
        self.assertEqual(self.settings.JAVA_jdk_path, aSTEAM.settings.JAVA_jdk_path)
        self.assertEqual(self.settings.CFunLibPath, aSTEAM.settings.CFunLibPath)
        self.assertEqual(self.settings.ProteCCT_path, aSTEAM.settings.ProteCCT_path)
        self.assertEqual(self.settings.LEDET_path, aSTEAM.settings.LEDET_path)
        self.assertEqual(self.settings.PSPICE_path, aSTEAM.settings.PSPICE_path)
        self.assertEqual(self.settings.COSIM_path, aSTEAM.settings.COSIM_path)
        self.assertEqual(self.settings.FiQuS_path, aSTEAM.settings.FiQuS_path)
        self.assertEqual(self.settings.PSPICE_library_path, aSTEAM.settings.PSPICE_library_path)
    ...
    def test_AnalysisSTEAM_exceptionSettingsFileNotFound(self):
        ...
        self.assertTrue('Settings file not found at this path' in str(context.exception))
    ...
    def test_AnalysisSTEAM_exceptionLibraryFolderNotExisting(self):
        ...
        """
        # arrange
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_exceptionLibraryFolderNotExisting.yaml')
    ...
    def test_AnalysisSTEAM_FiQuS_CWS(self):
        ...
        """
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_FiQuS_CWS.yaml')
        aSTEAM = AnalysisSTEAM(file_name_analysis=file_name_analysis, verbose=True)
        aSTEAM.run_analysis()
        output_file = Path(os.path.join(self.test_folder, self.settings.local_FiQuS_folder, 'CWS_1', 'Geometry_1', 'Mesh_1', 'Solution_1', 'selfMutualInductanceMatrix_transient.csv')).resolve()
    ...
    def test_AnalysisSTEAM_LEDET_magnet_conductor(self):
        ...
        # arrange - set inputs
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_REFERENCE.yaml')
        expected_local_LEDET_folder = Path(os.getcwd() / Path(self.settings.local_LEDET_folder)).resolve()
        expected_default_file_LEDET = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_1' + '.xlsx')
        expected_mod1_file_LEDET = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_10' + '.xlsx')
        expected_mod1bis_file_LEDET = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_11' + '.xlsx')
        expected_mod2_file_LEDET = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_12' + '.xlsx')
        expected_default_file_LEDET_json = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_1' + '.json')
        expected_mod1_file_LEDET_json = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_10' + '.json')
        expected_mod1bis_file_LEDET_json = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_11' + '.json')
        expected_mod2_file_LEDET_json = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Input', 'MBRB' + '_12' + '.json')
        ...
        expected_output_file_LEDET_mat1 = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Output', 'Mat Files', 'SimulationResults_LEDET_10.mat')
        expected_output_file_LEDET_mat2 = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Output', 'Mat Files', 'SimulationResults_LEDET_1.mat')
        expected_output_file_LEDET_mat3 = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Output', 'Mat Files', 'SimulationResults_LEDET_12.mat')
        expected_output_file_LEDET_txt1 = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Output', 'Txt Files', 'MBRB_VariableHistory_12.txt')
        expected_output_file_LEDET_txt2 = os.path.join(expected_local_LEDET_folder, 'MBRB', 'Output', 'Txt Files', 'MBRB_VariableStatus_12.txt')
    ...
    def test_AnalysisSTEAM_PSPICE_RB(self):
        ...
        sim_number = 1
        circuit_name = 'RB'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_PSPICE_RB.yaml')
        # expected_default_file_PSPICE = os.path.join('output', f'analysis_{circuit_name}', str(sim_number), f'{circuit_name}.cir')
        expected_model_name = 'BM'  # manually written, but it corresponds to the entry in the input analysis file
        expected_local_PSPICE_folder = Path(os.getcwd() / Path(
            self.settings.local_PSPICE_folder)).resolve()
        expected_output_file_PSPICE_out1 = os.path.join(expected_local_PSPICE_folder, circuit_name, str(sim_number), f'{circuit_name}.out')
    ...
    def test_AnalysisSTEAM_XYCE_RU(self):
        ...
        # arrange - got local settings (done in the setUp() function)
        # arrange - set inputs
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_XYCE_RU.yaml')
        # expected_model_name = 'BM'  # manually written, but it corresponds to the entry in the input analysis file
        expected_output_file_XYCE_out1 = os.path.join(
            self.settings.local_XYCE_folder, 'RU', '1', 'RU.csd')
    ...
    # def test_AnalysisSTEAM_runAnalysis_FiQuS_CCT_magnet(self):
    #     """
    ...
    def test_AnalysisSTEAM_runAnalysis_FiQuS_Multipole_magnet(self):
        """
            Check that AnalysisSTEAM analysis can be run with a FiQuS inputs
        """
        # arrange - get local settings
        ...
        # arrange - set inputs
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_FiQuS_Multipole.yaml')
    ...
    def test_AnalysisSTEAM_load_circuit_parameters(self):
        # arrange
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_PSPICE_load_circuit_parameters_IPQ.yaml')
    ...
    def test_AnalysisSTEAM_LEDET_parsim_event_magnet(self, max_relative_error=1E-5, verbose=True):
        # arrange - got local settings (done in the setUp() function)
        magnet_name = 'MBRB'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_parsim_event_magnet.yaml')
        expected_local_LEDET_folder = Path(os.getcwd() / Path(self.settings.local_LEDET_folder)).resolve()
        outputfile = os.path.join(os.getcwd(), 'output', 'run_parsim_event', 'TestFile_AnalysisSTEAM_run_parsim_event_output.yaml')
        list_output_file = [
            os.path.join(expected_local_LEDET_folder, magnet_name, 'Input', f'{magnet_name}_{i + 1}.xlsx') for i in
            range(3)]
        if os.path.exists(outputfile): os.remove(outputfile)
        for file in list_output_file:
            if os.path.exists(file): os.remove(file)
        list_reference_file = [
            os.path.join(os.getcwd(), 'references', 'run_parsim_event', f'{magnet_name}_reference_{i + 1}.xlsx') for i
            in range(3)]
    ...
    def test_AnalysisSTEAM_LEDET_parsim_conductor_opt_fCu(self, max_relative_error=1E-5, verbose=True):
        ...
        sim_number = 21
        magnet_name = 'MBRB'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_parsim_conductor.yaml')
        expected_local_LEDET_folder = Path(os.getcwd() / Path(self.settings.local_LEDET_folder)).resolve()
    ...
    def test_AnalysisSTEAM_LEDET_parsim_conductor_opt_length(self, max_relative_error=1E-5, verbose=True):
        ...
        sim_number = 22
        magnet_name = 'MBRB'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_parsim_conductor_variation1.yaml')
        expected_local_LEDET_folder = Path(os.getcwd() / Path(self.settings.local_LEDET_folder)).resolve()
    ...
    def test_AnalysisSTEAM_LEDET_parsim_sweep_magnet(self, max_relative_error=1E-5, verbose=True):
        # arrange
        magnet_name = 'MBRB'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_parsim_sweep_magnet.yaml')
        expected_local_LEDET_folder = Path(os.getcwd() / Path(self.settings.local_LEDET_folder)).resolve()
    ...
    def test_AnalysisSTEAM_LEDET_parsim_sweep_conductor(self):
        ...
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_LEDET_parsim_sweep_conductor.yaml')
        expected_local_LEDET_folder = Path(os.getcwd() / Path(self.settings.local_LEDET_folder)).resolve()
        list_output_file = [
    ...
    def test_AnalysisSTEAM_PSPICE_parsim_sweep_circuit(self):
        # arrange
        circuit_name = 'RQX'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_PSPICE_parsim_sweep_circuit.yaml')
        expected_local_PSPICE_folder = Path(os.getcwd() / Path(
            self.settings.local_PSPICE_folder)).resolve()
    ...
    def test_AnalysisSTEAM_PSPICE_parsim_event_circuit_from_eos_RB(self, verbose=True):
        ...
        sim_number = 2
        circuit_name = 'RB'
        file_name_analysis = os.path.join('input', 'TestFile_AnalysisSTEAM_PSPICE_parsim_event_circuit_from_eos_RB.yaml')
        expected_local_PSPICE_folder = Path(os.getcwd() / Path(
            self.settings.local_PSPICE_folder)).resolve()
        outputfile = os.path.join(os.getcwd(), 'output', 'run_parsim_event_circuit', 'TestFile_AnalysisSTEAM_PSPICE_parsim_event_circuit_from_eos_output.yaml')
        list_output_file = [
            os.path.join(expected_local_PSPICE_folder, circuit_name, str(sim_number), f'{circuit_name}.cir')]
        list_output_file.append(os.path.join(expected_local_PSPICE_folder, circuit_name, str(sim_number), 'coil_resistances.stl'))
        delete_if_existing(outputfile, verbose=verbose)
        for file in list_output_file:
            delete_if_existing(file, verbose=verbose)
        list_reference_file = [
            os.path.join(os.getcwd(), 'references', 'run_parsim_event_circuit', f'{circuit_name}_REFERENCE_from_eos_2.cir')]
