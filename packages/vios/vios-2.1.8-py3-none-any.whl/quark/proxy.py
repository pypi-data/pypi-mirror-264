...
class QuarkProxy(object):
    ...
    def submit(self, task: dict, block: bool = False):
        ...
        try:
            ...
            try:
                sql = f'INSERT INTO proxy (TID, EID, NAME, SYSTEM, CREATED) VALUES ("{t.tid}","{eid}","{name}","{chip}","{ctime}")'
                self.dbcon.execute(sql)
                self.dbcon.commit()
            except Exception as e:
                logger.critical(f'Failed to add record[{sql}] to proxy table: {e}!')
                import traceback
                traceback.print_exc()
