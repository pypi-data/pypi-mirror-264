...
from detensor_cli import _, cache
from detensor_cli.consts import WINDOWS, REQUIRES_PYTHON
from detensor_cli.config import GLOBAL_CONFIG, ConfigManager, detensorConfig
from detensor_cli.exceptions import (
    PipNotInstalledError,
    PythonInterpreterError,
    detensorNotInstalledError,
)
...
def draw_logo() -> str:
    return figlet_format("DeTensor", font="basic").strip()


def get_detensor_config() -> detensorConfig:
    return GLOBAL_CONFIG.get_detensor_config()
...
if TYPE_CHECKING:
    ...
    async def get_detensor_version(
        python_path: Optional[str] = None, cwd: Optional[Path] = None
    ) -> str:
        ...
    ...
    async def get_detensor_version(
        ...
        t = templates.get_template("meta/detensor_version.py.jinja")
        proc = await create_process(
            python_path,
            "-W",
            "ignore",
...
def requires_detensor(
    func: Callable[[P], Coroutine[Any, Any, R]]
) -> Callable[[P], Coroutine[Any, Any, R]]:
    @wraps(func)
    @requires_python
    async def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        if await get_detensor_version(
            cast(Optional[str], kwargs.get("python_path")),
            cast(Optional[Path], kwargs.get("cwd")),
        ):
            return await func(*args, **kwargs)

        raise detensorNotInstalledError(_("detensor is not installed."))
