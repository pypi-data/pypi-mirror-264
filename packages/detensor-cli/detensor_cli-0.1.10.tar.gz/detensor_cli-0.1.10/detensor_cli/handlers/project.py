...
from detensor_cli.config import SimpleInfo
...
TEMPLATE_PRO_ROOT = Path(__file__).parent.parent / "template" / "pro_project"/"PRO"
...
def list_pro_project_templates() -> List[str]:
    return sorted(t.name for t in (TEMPLATE_PRO_ROOT).iterdir())
...
def create_pro_project(
        project_template: str,
        context: Optional[Dict[str, Any]] = None,
        output_dir: Optional[str] = None,
        no_input: bool = True,
) -> None:
    path = TEMPLATE_PRO_ROOT / project_template
    path = str(path.resolve()) if path.exists() else project_template
...
async def generate_run_script(
    ...
    if adapters is None or builtin_plugins is None:
        bot_config = get_detensor_config()
        if adapters is None:
