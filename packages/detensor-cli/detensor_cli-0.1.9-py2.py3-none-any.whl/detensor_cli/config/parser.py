...
from detensor_cli.log import SUCCESS
from detensor_cli.consts import WINDOWS
from detensor_cli.exceptions import ProjectNotFoundError

from .model import SimpleInfo, detensorConfig
...
class ConfigManager:
    ...
    def _get_detensor_config(self, data: TOMLDocument) -> Dict[str, Any]:
        return data.get("tool", {}).get("detensor", {})

    def get_detensor_config(self) -> detensorConfig:
        return detensorConfig.parse_obj(self._get_detensor_config(self._get_data()))

    def add_adapter(self, adapter: SimpleInfo) -> None:
        data = self._get_data()
        table: Dict[str, Any] = data.setdefault("tool", {}).setdefault("detensor", {})
