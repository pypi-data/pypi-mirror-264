...
async def create_pro(
    ...
    if not template:
        ...
        try:
            ...
            # click.echo(f"Obtained template！：{template}")
    ...
    try:
        install_deploy_env = await ConfirmPrompt(
            _("Automatic download dependence ?"), default_choice=True
        ).prompt_async(style=CLI_DEFAULT_STYLE)
    except CancelledError:
        ctx.exit()
    ...
    try:
        ...
        print("Project pull failed. Error message:", str(e))
    except Exception as e:
        print("Other unknown errors have occurred. Error message:", str(e))


    if install_deploy_env:
        print("Installing the virtual environment and downloading dependency packages to the virtual environment, please wait...")
        # 指定脚本位置和参数
