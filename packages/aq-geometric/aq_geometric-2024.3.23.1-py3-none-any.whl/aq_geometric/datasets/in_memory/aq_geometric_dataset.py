...
class AqGeometricInMemoryDataset(InMemoryDataset):
    ...
    def _load_data_from_memory(
        ...
        node_features = self._graph_data.x[:, start_idx:start_idx +
                                           self.num_samples_in_node_features]
        node_targets = self._graph_data.y[:, start_idx + self.
        ...
        x_mask = self._graph_data.x_mask[:, start_idx:
                                                                 start_idx +
                                                                 self.
                                                                 num_samples_in_node_features]
        y_mask = self._graph_data.y_mask[:, start_idx +
                                                               self.
                                                               num_samples_in_node_features:
                                                               end_idx]
