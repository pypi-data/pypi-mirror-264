...
class AqGeometricDataset(Dataset):
    def __init__(
        ...
        q_start_time, q_end_time = self.timestamps[0], self.timestamps[-1] + pd.Timedelta(
        ...
        if self.max_samples_in_graph_on_disk < len(self.timestamps):
            query_start_times = self.timestamps[::self.max_samples_in_graph_on_disk].tolist()
            query_end_times = self.timestamps[self.max_samples_in_graph_on_disk::self.max_samples_in_graph_on_disk].tolist()
            query_start_times.append(query_end_times[-1])
            query_end_times.append(
                pd.to_datetime(self.timestamps[-1]) + pd.Timedelta(
                    self.freq))
    ...
    def raw_file_names(self) -> List[str]:
        ...
        raw_fps.extend(
            [osp.basename(f) for f in list(glob(osp.join(self.raw_dir, f"*_{f}_data.csv")) for f in self.features) for f in f]
        )
        raw_fps.extend(
            [osp.basename(f) for f in list(glob(osp.join(self.raw_dir, f"*_{f}_data.csv")) for f in self.targets if f not in self.features) for f in f]
        )
        return raw_fps
    ...
    def download(self):
        ...
        for f in features_and_targets:
            for i, (q_start_time, q_end_time) in enumerate(zip(self.query_start_times,
                                               self.query_end_times)):
                ...
                feature_df.to_csv(osp.join(self.raw_dir, f"{f}_data_{i}.csv"), index=False)
        return None
    ...
    def process(self):
        ...
        if self.verbose:
            print(f"[{datetime.now()}] start and end times: {[(start_time, end_time) for start_time, end_time in zip(q_start_times, q_end_times)]}")
        ...
        for i, (start_time, end_time) in enumerate(zip(q_start_times,
                                                       q_end_times)):
            ...
            graph_timestamps = pd.date_range(
                start=start_time, end=end_time, freq=self.freq, inclusive="left"
            )
            ...
            node_targets = stack_node_features(
                self.nodes,
                node_targets
            )
            # make the graph
            graph = make_graph(
                ...
                edge_attr=self.edge_attr.drop(columns=["from", "to"]).to_numpy() if self.edge_attr is not None else None,
            ...
            if self.pre_transform is not None:
                if self.verbose:
                    print(f"Pre-transforming graph {i} of {num_graphs_on_disk}")
    ...
    def _load_graph_from_disk(self, i):
        ...
        if i not in self._loaded_graph_indices:
            ...
            if i < self.num_graphs_on_disk - 1:
                ...
                assert all(
                    h1 == h2
                    for h1, h2 in zip(graph.h3_index.tolist(), next_graph.h3_index.tolist(
                    ))), "The h3_index must be the same for both graphs."
                assert all(h1 == h2
                        for h1, h2 in zip(graph.aqsid.tolist(), next_graph.aqsid.tolist())
                ...
                # combine the masks
                x_mask = torch.cat(
                    (graph.x_mask, next_graph.x_mask), dim=1)
                y_mask = torch.cat(
                    (graph.y_mask, next_graph.y_mask), dim=1)
    ...
    def _load_data_from_memory(
        ...
        node_targets = self._graph.y[:, start_idx + self.
        ...
        x_mask = self._graph.x_mask[:, start_idx:
                                                                 start_idx +
                                                                 self.
                                                                 num_samples_in_node_features]
        ...
        data = Data(
            ...
            timestamps=np.hstack([feature_timestamps,
                                  target_timestamps]),
