...
def make_graph(
    ...
    if isinstance(nodes, pd.DataFrame):
        nodes = nodes[["h3_index", "aqsid", "node_id"]].to_numpy()  # this should already be a numpy array
    ...
    if edges is not None:
        edge_index = np.array([(h3_index_to_id_map[e[0]], h3_index_to_id_map[e[1]])
                            for e in edges.T if e[0] in h3_index_to_id_map and e[1] in h3_index_to_id_map]).T
    else:
        edge_index = None
    ...
    if verbose:
        print(
            f"[{datetime.now()}] x_mask has shape {x_mask.shape}"
        )
    if verbose:
        print(
            f"[{datetime.now()}] y_mask has shape {y_mask.shape}"
        )
    if verbose:
        print(
