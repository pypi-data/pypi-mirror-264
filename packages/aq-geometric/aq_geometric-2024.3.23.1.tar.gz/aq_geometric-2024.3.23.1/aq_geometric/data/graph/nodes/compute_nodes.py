...
def _get_nodes_for_h3_resolutions(
    ...
    if verbose:
        print(f"[{datetime.now()}] computed nodes with initial dataframe shape: {df_lvl.shape}")
...
def _get_nodes_filtered_df(
    ...
    node_df = df[["aqsid", "h3_index"]].copy(deep=True).groupby("h3_index").first().reset_index()
