...
def ensure_feature_axis(df: pd.DataFrame,
                        h3_indices: List[str],
    ...
    if verbose:
        print(f"[{datetime.now()}] dropping any columns that are not h3 indices (current shape: {df.shape})")
    ...
    if verbose:
        print(f"[{datetime.now()}] dropped any columns that are not h3 indices (current shape: {df.shape})")
    # add new columns ful of nans for any missing h3 indices
    missing_h3_indices = list(set(h3_indices) - set(h3_indices_in_df))
    if verbose:
        print(f"[{datetime.now()}] adding {len(missing_h3_indices)} columns for missing h3 indices (current shape: {df.shape})")
