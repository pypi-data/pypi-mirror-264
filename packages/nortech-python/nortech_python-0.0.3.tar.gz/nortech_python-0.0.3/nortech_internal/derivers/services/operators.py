...
from bytewax.operators.window import EventClockConfig, TumblingWindow, collect_window
from pandas import DataFrame, DateOffset, DatetimeIndex

from nortech_internal.derivers.values.schema import DeriverInputSchema, InputType
...
def ffill(step_id: str, up: Stream[InputType]) -> Stream[InputType]:
    ...
    def ffill_mapper(state: Dict[str, Any], item: InputType):
        if state is None:
            state = {}
...
def resample(
    ...
    clock_config = EventClockConfig(
        dt_getter=lambda item: item.timestamp,
    ...
    collected_windows_stream = unkey_all(
        step_id="unkey_all", up=keyed_collected_windows_stream
    )
