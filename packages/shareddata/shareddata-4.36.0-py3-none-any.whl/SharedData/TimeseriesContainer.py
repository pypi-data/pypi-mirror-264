...
class TimeseriesContainer:
    ...
    def load(self):
        ...
        if self.type == 'DISK':
            ...
            tagslist = path.glob('*.bin')
            for tag in tagslist:
                if tag.is_file():
                    tagname = tag.name.replace('.bin', '')
    ...
    def read(self):
        ...
        if (head_io is None) & (self.shareddata.save_local) & (self.type == 'MEMORY'):
            # read local
            if os.path.isfile(str(headpath)):
                head_io = open(str(headpath), 'rb')

        if (tail_io is None) & (self.shareddata.save_local) & (self.type == 'MEMORY'):
