...
class validation:
    ...
    def validate_antibiotic_name(antibiotic):
        # Get the path to the antibiotics.txt file
        antibiotics_file_path = pkg_resources.resource_filename('enaCLI.packages', 'antibiotics.txt')
        # Read the contents of the antibiotics.txt file
        with open(antibiotics_file_path, 'r') as file:
            antibiotic_name = file.read()
