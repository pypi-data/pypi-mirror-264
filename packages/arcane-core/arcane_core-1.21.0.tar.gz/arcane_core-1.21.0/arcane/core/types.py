from typing import Dict, List, Union, Tuple, Any, Optional, cast
...
BASIC_LICENSE: LicenseAccess = LicenseAccess(
    imports_access=cast(ImportAccessLiteral, [import_type for import_type in vars(BasicLicenseImportType) if '__' not in import_type]),
    channels_access=cast(ChannelAccessLiteral,[channel.value for channel in BaseChannelType])
)

FULL_LICENSE: LicenseAccess = LicenseAccess(
    imports_access=cast(ImportAccessLiteral, [import_type for import_type in vars(FullLicenseImportType) if '__' not in import_type]),
    channels_access=cast(ChannelAccessLiteral,[channel.value for channel in BaseChannelType])
)
