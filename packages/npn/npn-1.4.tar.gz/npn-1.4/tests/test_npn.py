...
def test_npn():
    ...
    for tt in itertools.product([False, True], repeat=2 ** 8):
        if count != 0:
            c_tt = npn.npn_canonical_representative(tt, refinement=True)
        count += 1
        if count > num_test_tts:
            break
    print("average time cost with refinement: %.2f" % (time.time() - start_time))
    start_time = time.time()
    count = 0
    for tt in itertools.product([False, True], repeat=2 ** 8):
        if count != 0:
            c_tt = npn.npn_canonical_representative(tt, refinement=False)
        count += 1
        if count > num_test_tts:
            break
    print("average time cost without refinement: %.2f" % (time.time() - start_time))
