...
@click.option(
    ...
    type=str,
    required=False,
    help="Local dir where data_files are stored. Used to resolve existing data_files in Highlighter DataSources",
    default="")
@click.pass_context
def create_from_dataset(
    ...
    --data-source-ids. We use the local data_file location to lookup the Highlighter
    data_file_id. So we expect the data_file filename defined in the dataset to exist
    in Highlighter, if not you'll get an error.
    """
    client = ctx.obj["client"]
    ...
    if data_source_ids:
        ...
        def get_new_image_id(filename, local_image_dir=local_image_dir) -> int:
            _filename = str(Path(local_image_dir) / filename)
            new_id = original_source_url_to_id.get(_filename, None)
            if new_id is not None:
                return new_id

            candidate_matches = [
                abs_path
                for abs_path in original_source_url_to_id
                if abs_path.endswith(filename)
            ]
            if candidate_matches:
                candidate_key = candidate_matches[0]
                proposed_local_image_dir = candidate_key[: -len(filename)]
                extra_help = (
                    f"A similar filename '{candidate_key}' is in "
                    f"{data_source_ids}. Try using "
                    f"--local-image-dir {proposed_local_image_dir} to "
                    "fix this mismatch."
                )
            else:
                extra_help = "Upload missing images first before retrying."

            raise KeyError(
                f"filename '{filename}' not in the DataSources provided. "
                f"{extra_help}"
            )
    ...
    writer = HighlighterSubmissionsWriter(client, workflow_id, user_id=user_id)
    writer.write(ds)
