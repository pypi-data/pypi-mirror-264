...
def update_run_metadata(run: Union[str, Run],
    ...
    try:
        ...
        if "expired transaction" in str(e):
            # This is a temporary solution to handle MAPI timeouts from frequent update run metadata requests.
            # Typically, we see this error with end of batch updates from Composer which are not critical.
            logger.error((f"Timeout while updating run metadata. Ignoring this update. "
                          f"Please retry your update run metadata request. metadata={valid_metadata}."),
                         exc_info=True)
