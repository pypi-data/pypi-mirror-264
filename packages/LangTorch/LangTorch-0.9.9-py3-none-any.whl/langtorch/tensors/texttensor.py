...
class TextTensor(torch.Tensor, metaclass=_ParameterMeta):
    ...
    Keyword Arguments:
        ...
        embedding (Optional[Union[torch.Tensor, List[float], np.ndarray, bool]]): Optional embedding for the TextTensor. If True, computes embeddings for text entries automatically.
    ...
    def __new__(cls, content="", embedding: Optional[Union[torch.Tensor, List[float], np.ndarray, bool]] = None,
                metadata=None, requires_grad: bool = False,
                is_gradient: bool = False, is_param: bool = False, parse=True, **kwargs):
        if metadata is None:
            metadata = dict()
        ...
        if embedding == True:
            ...
            metadata["embedded_content"] = metadata["content"]
        else:
            metadata["embedded_content"] = None
    ...
    def embedded_content(self):
        return self._embedded_content

    @embedded_content.setter
    def embedded_content(self, content):
        self._embedded_content = TextTensor.content_to_object_array(content, parse=False)
    ...
    functions_on_texts = [torch.cat, torch.vstack, torch.stack, torch.squeeze, torch.unsqueeze, torch.reshape, torch.swapaxes, torch.sum]
    ...
    def metadata(self, metadata):
        ...
        # set core attributes
        attrs = ["content", "embedding", "embedded_content"]
        for attr in attrs:
            if attr in metadata:
    ...
    def sum(self, dim: Union[int, List[int]]=None, keepdim: bool=False) -> 'TextTensor':
        ...
        for d in dim:
            assert dim is None or d < len(self.shape), f"Dimension {d} is out of range for input of shape {self.content.shape}"
            ...
            try:
                ...
                try:
                    ...
                    if callable(attr.flat[0]):
                        ...
                        return lambda: np.array([getattr(obj, name)() for obj in self.content.flat], dtype=object).reshape(self.content.shape)
                    else:
                        return attr
                except:
        ...
        try:
            if is_torch_return_type(index):
                if "indices" in dir(index):
                    ...
                    raise ValueError("Trying to index with an unsupported torch.return_types object. Please use an index compatible with numpy indexing.")
    ...
    def embed(self, verbose=False):
        self.embedded_content = self.content.copy().reshape(-1)
        assert self._embedding_model is not None, ""
        self.embedding = get_embedding(self, model = self._embedding_model, verbose=verbose)
