...
def conv1d_texttensor(x: TextTensor, weights, bias=None, stride=1, padding=0, dilation=1, groups=1):
...
x = TextTensor(example_content).reshape(3,1,10)
...
weights = TextTensor(np.arange(3).reshape(1, 1, 3)).content  # Simplified weights for demonstration
bias = TextTensor(['bias','bias2']).content  # Simplified bias
