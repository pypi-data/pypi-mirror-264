...
def jsonlines_to_html(jsonlines_input_name, output):
    ...
    subprocess.call(
        "python3 "
        + cwd
        + "/src/utils/corefconversion/corefconversion/jsonlines2text.py "
        + cwd
        + "/"
...
def evaluate(conf: omegaconf.DictConfig):
    ...
    jsonlines_to_html(pl_data_module.val_dataloader().dataset.path, "test")
    ...
    if "gap" not in pl_data_module.val_dataloader().dataset.path:
        gold = []
        info = []
        with open(hydra.utils.get_original_cwd() + "/" + pl_data_module.val_dataloader().dataset.path, "r") as f:
        ...
        predictions = model_predictions_with_dataloader(model, pl_data_module.val_dataloader(), device)
