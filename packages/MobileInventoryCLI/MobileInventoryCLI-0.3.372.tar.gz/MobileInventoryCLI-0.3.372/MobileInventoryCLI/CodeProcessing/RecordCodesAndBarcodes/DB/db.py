...
class Shift(BASE):
    ...
    def duration_completed(self):
        ...
        if self.end:
            try:
                print(f"{Style.bold+Style.underline}{Fore.green_yellow}Shift{Style.reset} Has {Fore.light_red}Ended{Style.reset}: {(self.end-self.start)-(self.break_end-self.break_start)}")
            except Exception as e:
