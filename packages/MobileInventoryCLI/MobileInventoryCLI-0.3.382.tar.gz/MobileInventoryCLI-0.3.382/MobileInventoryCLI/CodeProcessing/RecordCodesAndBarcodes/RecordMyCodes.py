...
class Main:
	...
	def __init__(self,engine,tables,error_log):
		...
		while True:
			self.currentMode=input(f"which mode do you want to use[?]:{Fore.green_yellow} ").lower()
			if self.currentMode == '?':
				print(self.modeString)
				continue
			elif self.currentMode in ['quick','order','mini-help','mh','m']:
				print(f"""
...
{Fore.cyan}1{Style.reset} ->{Fore.light_yellow} #42{Style.reset} [{Fore.light_green}Starbux water Display Barge and Display at end of Self-Checkout{Style.reset}]
{Fore.cyan}2{Style.reset} ->{Fore.light_yellow} #40{Style.reset} [{Fore.light_green}Starbux Water Kooler{Style.reset}]
...
{Fore.cyan}11{Style.reset} -{Fore.light_yellow} ts{Style.reset}  [{Fore.light_green}TouchStampt - create a note related to barcode(+) or synthetically-generated barcode(s){Style.reset}]
