...
def create_ad_hoc_regex_recognizer(
    regex_pattern: str, entity_type: str, score: float, context: Optional[List[str]] = None
) -> Optional[PatternRecognizer]:
    """
    ...
    if not regex_pattern:
        return None
    pattern = Pattern(name=entity_type, regex=regex_pattern, score=score)
...
def scan(text, **kwargs):
    ...
    kwargs.setdefault("regex_params", [])
    kwargs.setdefault("ad_hoc_recognizers", [])
