    ...
    with ZipFile(output_path, "w", ZIP_DEFLATED) as zipf:
        ...
        if source_root_path.is_file():
            ...
            for root, dirs, files in os.walk(source_root):
                root_path = Path(root)
                # Filter directories and files if gitignore is not None
                if gitignore:
                    dirs[:] = [
                        d for d in dirs if not gitignore.match_file(str(root_path / d))
                    ]
                    files = [
                        f for f in files if not gitignore.match_file(str(root_path / f))
                    ]
...
def run_command(popen_args, cwd=None):
    """Run a command with the given arguments and print its output."""
    with subprocess.Popen(
        popen_args, cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True
    ) as process:
