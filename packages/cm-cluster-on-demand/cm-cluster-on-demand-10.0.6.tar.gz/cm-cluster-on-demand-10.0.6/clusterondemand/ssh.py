...
class SSHExecutor:
    ...
    @staticmethod
    def _get_key(key_path: str | None) -> paramiko.PKey | None:
        # This function is needed because of a bug described here: https://github.com/paramiko/paramiko/issues/1839
        # Supplying rsa key to paramiko's connect(key_filename=rsa_key_path) sometimes causes a nasty
    ...
    def _connect(self) -> None:
        ...
        def connect_wait() -> None:
            self._ssh_client.connect(  # type: ignore
                self._host,
                username=self._user,
                pkey=self._get_key(self._identity_file),
