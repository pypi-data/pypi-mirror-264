...
class PSC:
    ...
    def __init__(
        ...
        self.model = model
        f.write("PSC init. The parameters are:\n")
    ...
    def __train_model(self, X, x):
        ...
        f.write(str(connectivity.nonzero()) + "\n")
        affinity_matrix_ = 0.5 * (connectivity + connectivity.T)
        f.write("affinity_matrix_\n")
        f.write(str(affinity_matrix_.nonzero()) + "\n")
    ...
    def fit(self, X):
        ...
        f.write("The model has been trained. The parameters are:\n")
        for name, param in self.model.named_parameters():
            f.write(f"Parameter name: {name}, Shape: {param.shape}")
            f.write(f"Parameter values:\n{param.data}\n")
        f.close()
