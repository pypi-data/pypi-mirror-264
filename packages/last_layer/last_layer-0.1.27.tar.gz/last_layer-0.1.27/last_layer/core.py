...
try:
    ...
    warn.warn(f"Failed to load shared library {so_path}: {e} {platform.libc_ver()}")
    # Fallback to trying to load directly in case of failure
    for path in [
        f"lib/{lib_file}",
        pkg_resources.resource_filename("last_layer", "lib/amazonlinux.so"),
    ]:
        try:
        ...
        raise RuntimeError(f"Failed to load shared library {so_path}") from e
