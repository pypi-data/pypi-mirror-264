...
class AgentRuntime(WithDB):
    ...
    async def stream_kubernetes_pod_logs(self, name: str, websocket: WebSocket):
        """
        Stream logs from a Kubernetes pod to a WebSocket connection.

        Parameters:
            name (str): The name of the pod whose logs are to be streamed.
            websocket (WebSocket): The WebSocket connection to stream logs to.
        """
        ...
        try:
            for log_entry in w.stream(
                v1.read_namespaced_pod_log, name=name, namespace=namespace, follow=True
            ):
                try:
                    await websocket.send_text(log_entry)
                except WebSocketDisconnect:
                    print(
                        f"WebSocket disconnected while streaming logs for pod {name}."
                    )
                    break  # Exit the loop if the WebSocket connection is closed
        except ApiException as e:
            print(f"Exception when streaming logs for pod {name}: {e}")
            # Attempt to close the WebSocket connection with an error code if an ApiException occurs
            try:
                await websocket.close(
                    code=1011
                )  # Indicates an unexpected condition that prevented the request from being fulfilled
            except Exception:
                pass  # If there's an error closing the WebSocket, pass since the connection might already be closed
        except Exception as e:
            print(f"An error occurred while streaming logs: {e}")
            # Similar handling for general exceptions as above
            try:
