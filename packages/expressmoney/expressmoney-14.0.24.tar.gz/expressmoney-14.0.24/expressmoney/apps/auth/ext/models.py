...
class Ext(models.Model):
    created = models.DateTimeField('Создано', auto_now_add=True)
    updated = models.DateTimeField('Изменено', auto_now=True)
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='Пользователь')
    phonenumber = PhoneNumberField(unique=True, verbose_name='Номер телефона')
    department = models.ForeignKey(Department, on_delete=models.PROTECT, related_name='Подразделение')
