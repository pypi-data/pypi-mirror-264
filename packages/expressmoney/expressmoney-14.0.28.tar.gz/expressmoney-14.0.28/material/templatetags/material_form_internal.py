...
@register.filter
def modelchoice_value(bound_field):
    try:
        value = bound_field.field.to_python(bound_field.value())
        if value is None:
            return ''
        return value
...
@register.filter
def multimodelchoice_value(bound_field):
    try:
        values = bound_field.field.to_python(bound_field.value())
        if values:
            return [
                (item.pk, bound_field.field.label_from_instance(item))
                for item in values
            ]
    except ValidationError:
        pass
