...
class DataTableMixin(ContextMixin):
    ...
    def get_data_attr(self, attr_name):
        ...
        """
        try:
            return ModelField(self.opts.get_field(attr_name))
        except FieldDoesNotExist:
            if attr_name == "__str__":
                return ModelAttr(self.object_list.model, attr_name, self.opts.verbose_name)
            else:
                ...
                try:
                    if bool(self.list_actions):
                        order = self.get_data_attr(
                            self.get_list_display()[int(column_num)-1]
                        ).orderable
                    else:
                        order = self.get_data_attr(
...
class FilterMixin(BaseFilterMixin):
    ...
    def get_filterset_class(self):
        ...
        if self.filterset_class:
            ...
            meta = type(
                str('Meta'), (object,),
                {'model': model, 'fields': self.filterset_fields}
            )
            filterset = type(
                str('{}FilterSet'.format(model._meta.object_name)),
                (FilterSet,), {'Meta': meta})
            return filterset
    ...
    def get_object_list(self):
        ...
        if filterset_class is not None:
            ...
            return super(FilterMixin, self).get_object_list()


class ListModelView(TemplateResponseMixin, FilterMixin, DataTableMixin, View):
    ...
    :keyword filterset_fields: List or dictinary of fields spec for django-filters.
                               See details at https://github.com/carltongibson/django-filter#usage

    :keyword filterset_class: Subclass of `django_filters.FilterSet`,
                              if specified `filterset_fields` option ignored.
