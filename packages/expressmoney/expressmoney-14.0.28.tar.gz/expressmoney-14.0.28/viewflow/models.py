...
class Task(AbstractTask):
    ...
    owner_permission_obj_pk = models.CharField(
        max_length=255, blank=True, null=True)
    owner_permission_obj = GenericForeignKey(
        'owner_permission_content_type', 'owner_permission_obj_pk',
...
class Subprocess(Process):
    """
    Base model class for subprocess data storage.

    If you have custom process/task models you should define
    `parent_task` foreign key field pointed to `task_cls`
    """
    parent_task = models.ForeignKey(Task, blank=True, null=True, on_delete=models.CASCADE)
