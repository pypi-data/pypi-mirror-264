...
def topsort(flow_class):
    ...
    nodes = list(flow_class._meta.nodes())
    incoming_edges = {
    ...
    while nodes:
        start_nodes, cirle_broken = [], False
        for node in nodes:
...
def calc_layout_data(flow_class, edge_start_gap_size=EDGE_START_GAP_SIZE, edge_end_gap_size=EDGE_END_GAP_SIZE):
    ...
    # Topologically sorted incoding edges
    incoming_edges = {
        node: sorted(
...
def calc_text(grid):
    for cell in grid.cells:
        ...
        if text_align == 'middle':
            if cell.node.task_title is not None:
...
def grid_to_svg(grid):
    svg_template = get_template('viewflow/graph.svg')
...
def grid_to_bpmn(grid):
    svg_template = get_template('viewflow/graph.bpmn')
    return svg_template.render({
        'grid': grid,
        'cells': grid.cells,
        'edges': grid.edges})
