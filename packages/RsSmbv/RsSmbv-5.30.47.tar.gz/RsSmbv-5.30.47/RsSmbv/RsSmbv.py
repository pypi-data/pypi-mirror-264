...
class RsSmbv:
	"""9679 total commands, 27 Subgroups, 0 group commands"""
	_driver_options = "SupportedInstrModels = SMW/SMBV/SMCV, SupportedIdnPatterns = SMW/SMBV/SMCV, SimulationIdnString = 'Rohde&Schwarz,SMBV100B,100001,5.30.47.0028'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '5.30.47.0028'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '5.30.47.0028'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsSmbv version failed. Current version: '5.30.47.0028', minimum required version: '{min_version}'")
