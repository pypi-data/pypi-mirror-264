...
def spatial_bigcell_plot(adata, ncol=1, s=15, figsize=(7, 7), line_color='red', linewidth=1, xlabel='X', ylabel='Y',
    ...
    plt.figure(figsize=figsize)
    plt.scatter(x=coords[:, 0], y=coords[:, 1], c=adata.obs[color_key], s=s)
    ...
    legend_patches = [Line2D([0], [0], marker='o', color='w', label=label, markerfacecolor=color, markersize=10) for
                      label, color in adata.obs[[color_key_obs, color_key]].drop_duplicates().values]
...
def plot_spatial(adata, by=None, s=5, cmap=None, savefig=None, dpi=360, figsize=(6, 6),
                 tiff=None, coord='spatial', title=None, legend=False, categorical=False):
    """
