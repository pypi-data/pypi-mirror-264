...
class SftpClient(CloudStorage):
    ...
    def get_sftp_client(self) -> paramiko.SFTPClient:
        ...
        try:
            if self.key:
                logger.debug(f"Connecting to {self.host} using key-based authentication")
                ssh = paramiko.SSHClient()
                ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                key = paramiko.RSAKey.from_private_key(open(self.key))
                ssh.connect(hostname=self.host, port=self.port,
                            username=self.user, pkey=key)
                return ssh.open_sftp()

            else:
                logger.debug(f"Connecting to {self.host} using password-based authentication")
