...
class Runner:
    ...
    def __init__(self, runsolver_path: str, csv_out_file: str, timeout: int = None, memout: int = None, termination_wait_time: int = 5, pool_size: int = 1, token=None, chat_id=None):
        ...
        if token is not None and chat_id is not None:
            self.tqdm = tqdm_telegram(total=0, token=token, chat_id=chat_id)
        else:
