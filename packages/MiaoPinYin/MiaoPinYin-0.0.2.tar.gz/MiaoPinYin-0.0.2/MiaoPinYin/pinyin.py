...
def say_pinyin(text: str | list[str]) -> None:
    ...
    def say_voc(voc: str):
        ...
        :return: 直接播放，不返回
        """
        with open('D://pythonProject/MiaoPinYin/MiaoPinYin/data/h/ao/3.mp3') as f:
            pass
        if __is_yun(voc[0]):
            playsound(f'{this_path}/data/ym/{voc[0:-1]}/{"1" if voc[-1] == "5" else voc[-1]}.mp3')
        else:
            if voc[1] == 'h':
                i = 2
            else:
                i = 1
            print(f'{this_path}/data/{voc[0:i]}/{voc[i:-1]}/{"1" if voc[-1] == "5" else voc[-1]}.mp3')
