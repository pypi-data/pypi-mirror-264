...
class time(py_datetime.time):  # noqa
    def __repr__(self):
        return f'shdatetime.time({self.hour}, {self.minute}, {self.second})'


_thread_local_locales = dict()


def set_locale(locale_: str | None, /) -> str | None:
    ...
    _thread_local_locales[thread_identity] = locale_
    return prev_locale
...
class date:  # noqa
    ...
    sh_ampm_fa = {'PM': 'بعد از ظهر', 'AM': 'قبل از ظهر'}
    ...
    def __init__(self, year, month, day, **kwargs):
        ...
        if self.__month == 12 and day == 30 and self.isleap():
            ...
            raise ValueError('day is out of range for month')
        elif day > sh_month_days[self.__month - 1]:
            raise ValueError('day is out of range for month')
        self.__day = day
        self.__locale = (
            kwargs['locale']
        ...
        if self._is_fa_locale():
            self.sh_months = self.sh_months_fa
            self.sh_months_short = self.sh_months_fa
            self.sh_weekdays = self.sh_weekdays_fa
            self.sh_weekdays_short = self.sh_weekdays_fa
            self.sh_ampm = self.sh_ampm_fa
        else:
            self.sh_months = self.sh_months_en
            self.sh_months_short = self.sh_months_short_en
            self.sh_weekdays = self.sh_weekdays_en
            self.sh_weekdays_short = self.sh_weekdays_short_en
            self.sh_ampm = self.sh_ampm_en
    ...
    def togregorian(self):
        ...
        (y, m, d) = solar_hijri_to_gregorian(self.year, self.month, self.day)
        return py_datetime.date(y, m, d)

    @staticmethod
    def fromgregorian(**kw):
        """Convert gregorian to solar hijri and return shdatetime.date

        shdatetime.date.fromgregorian(day=X,month=X,year=X)
        shdatetime.date.fromgregorian(date=datetime.date)
        shdatetime.date.fromgregorian(date=datetime.date, locale='fa_IR')
        """
        locale = kw.get('locale')
        if 'date' in kw:
            d = kw['date']
            try:
                (y, m, d) = gregorian_to_solar_hijri(d.year, d.month, d.day)
        ...
        if 'day' in kw and 'month' in kw and 'year' in kw:
            ...
            (y, m, d) = gregorian_to_solar_hijri(year, month, day)
            return date(y, m, d, locale=locale)
    ...
    def today():
        ...
        (y, m, d) = gregorian_to_solar_hijri(to.year, to.month, to.day)
        return date(y, m, d)
    ...
    def fromisoformat(date_string: str):
        """
        Convert an ISO 8601 formatted string to a shdatetime.date
        """
    ...
    @staticmethod
    def sh_month_to_num(month_name):
        return date.sh_months_en.index(month_name.lower().capitalize()) + 1

    @staticmethod
    def sh_month_short_to_num(month_name):
        return (
            date.sh_months_short_en.index(month_name.lower().capitalize()) + 1
        )

    @staticmethod
    def sh_month_fa_to_num(month_name):
        return date.sh_months_fa.index(month_name) + 1

    def __repr__(self):
        return f'shdatetime.date({self.year}, {self.month}, {self.day})'
    ...
    def jweekday_short(self):
        return self.sh_weekdays_short[self.weekday()]

    def jweekday(self):
        return self.sh_weekdays[self.weekday()]
    ...
    def jmonth_short(self):
        return self.sh_months_short[self.month - 1]

    def jmonth(self):
        return self.sh_months[self.month - 1]
    ...
    def _strftime_p(self):
        return self.sh_ampm['AM']

    def _strftime_z(self):
        return ''  # date does not have utcoffset
    ...
    def strftime(self, format: str) -> str:  # noqa
        def repl(match):
            symbol = match[0]
...
class datetime(date):  # noqa
    ...
    def __repr__(self):
        if self.__time.tzinfo is not None:
            return f'shdatetime.datetime({self.year}, {self.month}, {self.day}, {self.hour}, {self.minute}, {self.second}, {self.microsecond}, tzinfo={self.tzinfo})'

        if self.__time.microsecond != 0:
            return f'shdatetime.datetime({self.year}, {self.month}, {self.day}, {self.hour}, {self.minute}, {self.second}, {self.microsecond})'

        if self.__time.second != 0:
            return f'shdatetime.datetime({self.year}, {self.month}, {self.day}, {self.hour}, {self.minute}, {self.second})'

        return f'shdatetime.datetime({self.year}, {self.month}, {self.day}, {self.hour}, {self.minute})'
    ...
    def fromisoformat(cls, date_string: str):
        """
        Convert an ISO 8601 formatted string to a shdatetime.datetime
        """
    ...
    def combine(d=None, t=None, **kw):
        ...
        if not isinstance(c_date, date):
            raise TypeError(
                'combine() argument 1 must be shdatetime.date, not %s'
                % (type(c_date))
            )
        if not isinstance(c_time, time):
            raise TypeError(
                'combine() argument 2 must be shdatetime.time, not %s'
                % (type(c_time))
            )
    ...
    def fromordinal(ordinal):
        ...
        if ordinal < 1:
            raise ValueError('ordinal must be >= 1')
        d = py_datetime.date.fromordinal(226894 + ordinal)
        sh_date = date.fromgregorian(date=d)
        return datetime(sh_date.year, sh_date.month, sh_date.day, 0, 0)
    ...
    @staticmethod
    def strptime(date_string, format):  # noqa
        ...
        if isinstance(month, str):
            try:
                if get('b'):
                    month = date.sh_month_short_to_num(month_name=month)
                elif month.isascii():
                    month = date.sh_month_to_num(month_name=month)
                else:
                    month = date.sh_month_fa_to_num(month_name=month)
            except ValueError:
                raise ValueError(
                    f"time data '{date_string}' does not match format '{format}'"
                )
    ...
    def __eq__(self, other_datetime):
        ...
        if not isinstance(other_datetime, datetime):
            return NotImplemented
        if (
            self.year == other_datetime.year
            and self.month == other_datetime.month
            and self.day == other_datetime.day
            and self.locale == other_datetime.locale
        ):
            return (
                self.timetz() == other_datetime.timetz()
    ...
    def fromgregorian(**kw):
        ...
        if date_param:
            try:
                (y, m, d) = gregorian_to_solar_hijri(
    ...
    def isoformat(self, sep='T', timespec='auto'):
        ...
        assert (
            isinstance(sep, str) and len(sep) == 1
