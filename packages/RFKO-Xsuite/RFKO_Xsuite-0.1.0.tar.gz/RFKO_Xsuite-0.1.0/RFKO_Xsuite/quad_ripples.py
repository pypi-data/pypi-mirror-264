...
from . import utility
from .machine_class import Rfko
...
def binning(outdict, turn_binning=5, current=True, plot_stairs=False, duty_factor=True,time_binning=None,
    ...
    for x in range(len(bincount) // turn_binning + 1):
        ...
        #### TO CHECK
        if duty_factor:
            mean = np.mean(bincount[x * turn_binning : x * turn_binning + turn_binning])
            mean_sq = mean ** 2
            sq_mean = np.mean( np.array(bincount[x * turn_binning : x * turn_binning + turn_binning]) ** 2)
            duty.append(mean_sq / sq_mean)
...
def binning_data(folder_name,turn_binning=5,time_binning=None,duty_factor=False):
    ...
    time_step = []
    duty = []
    ## The last turn will in general consider a differe number of turns
...
def dependencies_quad(outdict, names_dict):
        ...
        if len(names_dict[key]) != 0:
            ...
            for name in names:
                if key in ['multipole', 'combf_knl', 'quad_mult']:
                    ...
                    print('something is strange man')
...
def tracking(outdict, f=100, A=0.001,sextupole_turns = None,twiss=False,return_time=False):
    ...
    # FIND THE QUADRUPOLE VARS
    quad_names = xh.find_multipole_component(line_df, 'quadrupole') # find quad elems
    # find the variables underlying
    dep_vars = dependencies_quad(outdict, quad_names)
    amp_val = dict()
    for x in dep_vars:
        # initial values
    ...
    for _ in tqdm(np.arange(turns)):
        ...
        ### (A * amp_val[x]) is consideres the amplitude of the sinusoid
        if sextupole_turns is not None:
