...
class SDistBuilder:
    ...
    def build(self):
        ...
        try:
            ...
            if self.source_wheel.endswith(".whl"):
                opener = partial(zipfile.ZipFile, mode="r")
                metadata_suffix = ".dist-info/METADATA"
                stub_only = False
            else:
                opener = partial(tarfile.open, mode="r:gz")
                metadata_suffix = "/PKG-INFO"
                stub_only = True
            # Then we read the PKG-INFO, which is just the METADATA file from the wheel
            with opener(self.source_wheel) as z:
                with z.open(
                    f"{replace_underscore(self.distribution)}-{self.version}{metadata_suffix}",
                    mode="r",
                ) as metadata_file:
