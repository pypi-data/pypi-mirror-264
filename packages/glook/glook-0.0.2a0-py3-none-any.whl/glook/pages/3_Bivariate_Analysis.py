...
# plot_type = st.selectbox("Select plot type", ["Scatter Plot", "Line Plot", "Bar Plot", "Histogram", "Box Plot", "Violin Plot", "ECDF", "Strip Chart", "Density Contour", "Density Heatmap"])
    ...
    plot_type = st.selectbox("Select plot type", ["Scatter Plot", "Line Plot", "Bar Plot", "Histogram", "Box Plot", "Violin Plot", "Strip Chart", "Density Contour", "Density Heatmap"])
    # Streamlit UI for selecting x-axis and y-axis columns
    ...
    if plot_type == "Scatter Plot":
        fig = px.scatter(dff, x=x_axis_column, y=y_axis_column, log_x=True, size_max=60)
    elif plot_type == "Line Plot":
        fig = px.line(dff, x=x_axis_column, y=y_axis_column)
    elif plot_type == "Bar Plot":
        fig = px.bar(dff, x=x_axis_column, y=y_axis_column)
    elif plot_type == "Histogram":
        fig = px.histogram(dff, x=x_axis_column, y=y_axis_column, hover_data=dff.columns)
    elif plot_type == "Box Plot":
        fig = px.box(dff, x=x_axis_column, y=y_axis_column, notched=True)
    elif plot_type == "Violin Plot":
        fig = px.violin(dff, y=y_axis_column, x=x_axis_column, box=True, points="all", hover_data=dff.columns)
    # elif plot_type == "ECDF":
    #     fig = px.ecdf(dff, x=x_axis_column)
    elif plot_type == "Strip Chart":
        fig = px.strip(dff, x=x_axis_column, y=y_axis_column, orientation="h")
    elif plot_type == "Density Contour":
        fig = px.density_contour(dff, x=x_axis_column, y=y_axis_column)
    elif plot_type == "Density Heatmap":
        fig = px.density_heatmap(dff, x=x_axis_column, y=y_axis_column, marginal_x="rug", marginal_y="histogram")
