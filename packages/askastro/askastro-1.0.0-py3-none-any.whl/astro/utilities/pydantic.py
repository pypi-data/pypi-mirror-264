...
def cast_to_model(
    ...
    Example:
        ...
        ```python
        from astro.utilities.pydantic import cast_to_model
...
def parse_as(
    ...
    Example:
        ...
        ```python
        from astro.utilities.pydantic import parse_as
        from pydantic import BaseModel
        ...
        parsed = parse_as(ExampleModel, {"name": "Astro"})
        assert isinstance(parsed, ExampleModel)
        assert parsed.name == "Astro"

        # parsing json strings
        parsed = parse_as(
            list[ExampleModel],
            '[{"name": "Astro"}, {"name": "Arthur"}]',
            mode="json"
        )
        assert all(isinstance(item, ExampleModel) for item in parsed)
        assert parsed[0].name == "Astro"
