...
from astro.tools.assistants import CodeInterpreter
from astro.tools.filesystem import getcwd, ls, read, read_lines

from . import threads as threads_cli

assistants_app = typer.Typer(no_args_is_help=True)

ASSISTANTS_DIR = Path.home() / ".astro/cli/assistants"
...
default_assistant = Assistant(
    ...
    instructions=f"""
        ...
        personality is helpful and friendly, but humorously based on Astro the
...
def load_assistant_from_path(path: Union[str, Path]) -> Assistant:
    ...
    if not assistant:
        ...
        raise TypeError(
            "Assistant must be an instance of astro.beta.assistants.Assistant"
        )
    return assistant
...
def say(
    ...
    thread: str = typer.Option(
        ...
        "--thread",
        "-t",
        help="The thread name to send the message to. Set ASTRO_CLI_THREAD to provide a default.",
        envvar="ASTRO_CLI_THREAD",
    ),
    assistant_name: str = typer.Option(
        None,
        "--assistant",
        "-a",
        help="The name of the assistant to use. Set ASTRO_CLI_ASSISTANT to provide a default.",
        envvar="ASTRO_CLI_ASSISTANT",
    ),
