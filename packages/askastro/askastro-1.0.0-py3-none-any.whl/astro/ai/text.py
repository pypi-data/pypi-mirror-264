...
from astro._mappings.types import (
    cast_labels_to_grammar,
    cast_type_to_labels,
)
from astro.ai.prompts.text_prompts import (
    CAST_PROMPT,
    CLASSIFY_PROMPT,
    EXTRACT_PROMPT,
    FUNCTION_PROMPT,
...
from astro.client.openai import AsyncAstroClient, ChatCompletion, AstroClient
from astro.types import ChatRequest, ChatResponse
from astro.utilities.asyncio import run_sync
from astro.utilities.context import ctx
from astro.utilities.jinja import Transcript
from astro.utilities.logging import get_logger
from astro.utilities.mapping import map_async
from astro.utilities.python import PythonFunction
...
async def generate_llm_response(
    ...
    client: Optional[AsyncAstroClient] = None,
) -> ChatResponse:
    """
    ...
    """
    client = client or AsyncAstroClient()
...
def _get_tool_outputs(request: ChatRequest, response: ChatCompletion) -> list[Any]:
    ...
    for tool_call in tool_calls:
        tool_output = astro.utilities.tools.call_function_tool(
            tools=request.tools,
...
async def _generate_typed_llm_response_with_tool(
    ...
    Args:
        ...
        client (AstroClient, optional): The client to use for the AI function.
    ...
    model_kwargs = model_kwargs or {}
    prompt_kwargs = prompt_kwargs or {}
    tool = astro.utilities.tools.tool_from_type(type_, tool_name=tool_name)
...
async def cast_async(
    ...
    Args:
        ...
        model_kwargs (dict, optional): Additional keyword arguments for the
            language model. Defaults to None.
        client (AsyncAstroClient, optional): The client to use for the AI
            function.
...
async def classify_async(
    ...
    Args:
        ...
        model_kwargs (dict, optional): Additional keyword arguments for the
            language model. Defaults to None.
        client (AsyncAstroClient, optional): The client to use for the AI function.
...
async def generate_async(
    ...
    for r in list(cached_responses) if use_cache else []:
        if tokens > astro.settings.ai.text.generate_cache_token_cap:
            continue
...
def fn(
    ...
    model_kwargs: Optional[dict] = None,
    client: Optional[AstroClient] = None,
) -> Callable:
    """
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        post_processor = astro.settings.post_processor_fn
...
class Model(BaseModel):
    ...
    async def from_text_async(
        ...
        Example:
            ```python
            from astro.ai.text import Model
...
def classifier(cls=None, *, instructions=None, model_kwargs=None):
    ...
    `astro.classify` function instead of the default Enum behavior. This allows
