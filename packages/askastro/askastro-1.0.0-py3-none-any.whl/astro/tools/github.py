...
from astro.utilities.strings import slice_tokens
...
async def get_token() -> str:
    try:
        ...
        getattr(get_logger("astro"), "debug_kv")(
            (
                "Nebula Secret for GitHub token not retrieved. "
    ...
    try:
        return getattr(astro.settings, "github_token")
    except AttributeError:
        pass

    if token := os.environ.get("ASTRO_GITHUB_TOKEN", ""):
        return token
...
async def search_github_issues(
    query: str, repo: str = "kozmoai/nebula", n: int = 3
) -> str:
    """
