...
try:
    ...
    raise ImportError(
        ...
        " it with `pip install chromadb` or `pip install askastro[chroma]`."
    )
...
try:
    HOST, PORT = (
        getattr(astro.settings, "chroma_server_host"),
        getattr(astro.settings, "chroma_server_http_port"),
    )
    DEFAULT_COLLECTION_NAME = getattr(
        astro.settings, "chroma_default_collection_name", "astro"
    )
except AttributeError:
    HOST = os.environ.get("ASTRO_CHROMA_SERVER_HOST", "localhost")  # type: ignore
    PORT = os.environ.get("ASTRO_CHROMA_SERVER_HTTP_PORT", 8000)  # type: ignore
    DEFAULT_COLLECTION_NAME = os.environ.get(
        "ASTRO_CHROMA_DEFAULT_COLLECTION_NAME", "astro"
    )
...
def create_openai_embeddings(texts: list[str]) -> list[float]:
    ...
    from astro.client.openai import AstroClient

    embedding: "CreateEmbeddingResponse" = AstroClient().client.embeddings.create(
        input=[text.replace("\n", " ") for text in texts],
...
async def query_chroma(
    ...
    Example:
        User: "What are nebula blocks?"
        Assistant: >>> query_chroma("What are nebula blocks?")
    """
...
async def multi_query_chroma(
    ...
    Example:
        User: "What are nebula blocks and tasks?"
        Assistant: >>> multi_query_chroma(
            ["What are nebula blocks?", "What are nebula tasks?"]
        )
