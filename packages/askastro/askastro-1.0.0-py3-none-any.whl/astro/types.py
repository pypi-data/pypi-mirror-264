...
class Function(AstroType, Generic[T]):
    name: str
...
class ToolSet(AstroType, Generic[T]):
...
class FunctionCall(AstroType):
    name: str


class ImageUrl(AstroType):
    url: str = Field(
...
class ImageFileContentBlock(AstroType):
...
class TextContentBlock(AstroType):
...
class TranscriptRequest(AstroType):
...
class StreamingChatResponse(AstroType):
...
class Audio(AstroType):
    ...
    def play(self):
        ...
        astro.audio.play_audio(self.data)
