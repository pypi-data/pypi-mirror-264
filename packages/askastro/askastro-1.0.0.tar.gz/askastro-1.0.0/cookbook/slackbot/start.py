...
from astro.tools.chroma import store_document
from astro.tools.github import search_github_issues
from astro.utilities.logging import get_logger
from astro.utilities.slack import (
    SlackPayload,
    get_channel_name,
    get_workspace_info,
    post_slack_message,
)
from astro.utilities.strings import count_tokens, slice_tokens
...
from tools import get_info, get_nebula_code_example, search_nebula_docs

BOT_MENTION = r"<@(\w+)>"
CACHE = JSONBlockState(block_name="astro-thread-cache")
    ...
    if (user := re.search(BOT_MENTION, user_message)) and user.group(
        ...
        with Assistant(
            ...
            tools=[
                ...
                get_info,
                get_nebula_code_example,
            ],
            instructions=(
                "You are Astro, the paranoid android from Hitchhiker's Guide to the"
                ...
                " You must use your tools, as Nebula 2.x is new and you"
...
def get_parent_app() -> Application:
    astro = getattr(app.state, "astro", None)
    if not astro:
        logger.warning("Astro instance not available")
    return astro
