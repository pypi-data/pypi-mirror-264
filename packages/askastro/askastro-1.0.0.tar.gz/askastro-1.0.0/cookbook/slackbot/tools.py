...
import httpx
import astro
from astro.tools.chroma import multi_query_chroma

Topic = Literal["latest_nebula_version"]


async def search_nebula_docs(queries: list[str]) -> str:
    """Searches the Nebula documentation for the given queries.
...
async def get_latest_release_notes() -> str:
    """Gets the first whole h2 section from the Nebula RELEASE_NOTES.md file."""
    async with httpx.AsyncClient() as client:
        response = await client.get(
            "https://raw.githubusercontent.com/kozmoai/nebula/main/RELEASE-NOTES.md"
        )
        return response.text.split("\n## ")[1]


tool_map = {"latest_nebula_version": get_latest_release_notes}
...
async def get_info(topic: Topic) -> str:
    ...
    As of now, the only topic is "latest_nebula_version".
    """
...
async def get_nebula_code_example(related_to: str) -> str:
    """Gets a Nebula code example"""

    base_url = "https://raw.githubusercontent.com/zzstoatzz/nebula-code-examples/main"
    ...
    async with httpx.AsyncClient() as client:
        ...
        key = await astro.classify_async(
