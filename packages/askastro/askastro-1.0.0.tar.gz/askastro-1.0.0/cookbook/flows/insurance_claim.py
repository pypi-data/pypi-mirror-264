...
hypothetical car insurance claim using Astro. We use nebula to define
...
import astro
from nebula import flow, pause_flow_run, task
from nebula.artifacts import create_markdown_artifact
from nebula.input import RunInput
...
def astro_extract_damages_from_url(image_url: str) -> list[DamagedPart]:
    return astro.beta.extract(
        data=astro.beta.Image.from_url(image_url),
        target=DamagedPart,
        instructions=(
...
def submit_damage_report(report: M, car: Car):
    ...
    print(
        ...
        f" {NEBULA_UI_URL.value()}/artifacts/artifact/{uuid}"
    )
...
def process_damage_report(car: Car):
    damaged_parts = sorted(
        astro_extract_damages_from_url(car.image_url), key=lambda x: x.part
    )
