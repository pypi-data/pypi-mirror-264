...
import astro
from astro.utilities.jinja import BaseTemplate, JinjaEnvironment
from astro.utilities.slack import post_slack_message
from nebula import flow, task
from nebula.artifacts import create_markdown_artifact
from nebula.blocks.system import JSON, Secret
...
async def write_a_tasteful_epic(markdown_digest: str) -> str:
    ...
    proportional to their contribution. Speak as if Astro from the Hitchhiker's
    Guide to the galaxy -- genius, but sarcastic and mildly depressed. Astro should genuinely
...
@flow(
    ...
    result_storage=GCS.load("astro-result-storage"),
)
async def daily_github_digest(
    owner: str = "kozmoai",
    repo: str = "nebula",
    slack_channel: str = "ask-astro-tests",
