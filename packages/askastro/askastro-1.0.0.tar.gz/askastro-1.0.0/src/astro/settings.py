"""Settings for configuring `astro`."""
...
class AstroSettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_file="" if os.getenv("ASTRO_TEST_MODE") else ("~/.astro/.env", ".env"),
        extra="allow",
...
class ChatCompletionSettings(AstroSettings):
    model_config = SettingsConfigDict(env_prefix="astro_chat_completions_")
    model: str = Field(
...
class ChatVisionSettings(AstroSettings):
    model_config = SettingsConfigDict(env_prefix="astro_chat_vision_")
    model: str = Field(
...
class ChatSettings(AstroSettings):
...
class ImageSettings(AstroSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="astro_image_")
...
class SpeechSettings(AstroSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="astro_speech_")
...
class AssistantSettings(AstroSettings):
    ...
    model_config = SettingsConfigDict(env_prefix="astro_assistant_")
...
class AudioSettings(AstroSettings):
    speech: SpeechSettings = Field(default_factory=SpeechSettings)


class OpenAISettings(AstroSettings):
    ...
    Example:
        ...
        astro.settings.openai.api_key = "sk-..."

        assert astro.settings.openai.api_key.get_secret_value() == "sk-..."
        ```
    """

    model_config = SettingsConfigDict(
        env_prefix="astro_openai_",
    )
...
class TextAISettings(AstroSettings):
    model_config = SettingsConfigDict(env_prefix="astro_ai_text_")
    generate_cache_token_cap: int = Field(600)


class AISettings(AstroSettings):
...
class Settings(AstroSettings):
    """Settings for `astro`.

    This is the main settings object for `astro`.
    ...
    Example:
        ...
        astro.settings.log_level = "INFO"

        assert astro.settings.log_level == "INFO"
        ```
    """
    ...
    auto_import_beta_modules: bool = Field(
        True,
        description="If True, the astro.beta module will be automatically imported when astro is imported.",
    )
    ...
    def set_log_level(cls, v):
        from astro.utilities.logging import setup_logging
...
def temporary_settings(**kwargs: Any):
    """
    Temporarily override Astro setting values, including nested settings objects.
    ...
    Example:
        ...
        with temporary_settings(log_level="INFO"):
            assert astro.settings.log_level == "INFO"
        assert astro.settings.log_level == "DEBUG"

        # Override nested settings
        with temporary_settings(openai__api_key="new-api-key"):
            assert astro.settings.openai.api_key.get_secret_value() == "new-api-key"
        assert astro.settings.openai.api_key.get_secret_value().startswith("sk-")
        ```
    """
