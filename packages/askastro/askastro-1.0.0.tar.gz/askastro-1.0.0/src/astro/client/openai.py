...
async def should_fallback(e: NotFoundError, request: ChatRequest) -> bool:
    if (
        ...
        and request.model == astro.settings.openai.chat.completions.model
    ):
        get_logger().warning(
            "Astro's default chat model is"
            f" {astro.settings.openai.chat.completions.model!r}, which your"
            ...
            " `ASTRO_CHAT_COMPLETIONS_MODEL=<accessible model>` in"
            f" `~/.astro/.env` - for example, `gpt-3.5-turbo`.\n\n {e}"
        )
        return True
    else:
        return False
...
class AstroClient(pydantic.BaseModel):
    ...
    def generate_image(
        ...
        **kwargs: Any,
        ...
        # validate request
        request = ImageRequest(**astro.settings.openai.images.model_dump() | kwargs)
...
class AsyncAstroClient(pydantic.BaseModel):
    model_config = pydantic.ConfigDict(
        arbitrary_types_allowed=True, protected_namespaces=()
    )
