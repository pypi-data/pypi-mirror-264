...
async def get_token() -> str:
    ...
    try:
        token = (
            astro.settings.slack_api_token
        )  # set `ASTRO_SLACK_API_TOKEN` in `~/.astro/.env
    except AttributeError:
        if token := os.getenv("ASTRO_SLACK_API_TOKEN"):
            return token
        try:  # TODO: clean this up
            ...
            pass
        raise ValueError(
            "`ASTRO_SLACK_API_TOKEN` not found in environment."
            " Please set it in `~/.astro/.env` or as an environment variable."
        )
    return token
