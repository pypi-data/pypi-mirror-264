...
from astro.ai.prompts.vision_prompts import CAPTION_PROMPT
from astro.ai.text import EjectRequest
from astro.client.openai import AsyncAstroClient
from astro.types import (
    BaseMessage,
    ChatResponse,
    Image,
    VisionRequest,
...
async def generate_vision_response(
    ...
    response = await AsyncAstroClient().generate_vision(
...
async def _two_step_vision_response(
    ...
    images: list[Image],
    astro_call: Union[Callable, Coroutine],
    vision_model_kwargs: dict = None,
):
    """
    Helper function to process images and data for various Astro operations.

    Args:
        data (Union[str, None]): Additional data for processing.
        images (list[Image]): Images to be processed.
        astro_call (Union[Callable, Coroutine]): A function that takes a single
            argument, data, encapsulating the specific Astro function call.
    ...
    if not images and not isinstance(data, Image):
        if inspect.iscoroutinefunction(astro_call):
            return await astro_call(data)
        else:
            return astro_call(data)
    ...
    # is used in the astro_call function
    with ctx(eject_request=True):
        try:
            if inspect.iscoroutinefunction(astro_call):
                await astro_call(data)
            else:
                astro_call(data)
    ...
    if inspect.iscoroutinefunction(astro_call):
        return await astro_call(msg)
    else:
        return astro_call(msg)
...
async def cast_async(
    ...
    async def astro_call(x):
        return await astro.cast_async(
            data=x,
            target=target,
...
async def extract_async(
    ...
    async def astro_call(x):
        return await astro.extract_async(
            data=x,
            target=target,
            instructions=instructions,
            model_kwargs=model_kwargs,
...
async def classify_async(
    ...
    async def astro_call(x):
        return await astro.classify_async(
            data=x,
            labels=labels,
            instructions=instructions,
            model_kwargs=model_kwargs,
