...
Astro can generate synthetic data according to a schema and instructions. Generating synthetic data with an LLM can yield extremely rich and realistic samples, making this an especially useful tool for testing code, training or evaluating models, or populating databases. 
...
!!! example
    ...
    === "Names (`str`)"
        ...
        french_names = astro.generate(
            n=4, instructions="first names from France"
        )
        
        star_wars_names = astro.generate(
...
<div class="admonition info">
  ...
  <p>
    Astro instructs the LLM to generate a list of JSON objects that satisfy the provided schema and instructions. Care is taken to introduce variation in the output, so that the samples are not all identical.
  </p>
</div>
...
Data generation relies even more on instructions than other Astro tools, as the potential for variation is much greater. Therefore, you should provide as much detail as possible in your instructions, in addition to any implicit documentation in your requested type. 
...
To mediate this issue, Astro maintains an in-memory cache of the last 100 results produced by each `generate` prompt. These responses are shown to the LLM during generation to encourage variation. Note that the cache is not persisted across Python sessions. Cached results are also subject to a token cap to avoid flooding the LLM's context window. The token cap can be set with `ASTRO_AI_TEXT_GENERATE_CACHE_TOKEN_CAP` and defaults to 600.
...
=== "Single call"
    ...
    ```python
    cities = astro.generate(n=10, instructions='major US cities')
    ```
...
=== "Five calls, with caching"
    ...
    cities = []
    for _ in range(5):
        cities.extend(astro.generate(n=2, instructions='major US cities'))
    ```
    !!! success "Result"
        ```python
        cities == [
...
## Async support
If you are using Astro in an async environment, you can use `generate_async`:
  
```python
cat_names = await astro.generate_async(
    n=4, 
