...
!!! example "Example: categorize user feedback"
    ...
    !!! success "Result"
        Astro correctly identifies the statement as a bug report.
        ```python
        assert category == "bug"
        ```
...
<div class="admonition info">
  ...
  <p>
    Astro enumerates your options, and uses a <a href="https://twitter.com/AAAzzam/status/1669753721574633473">clever logit bias trick</a> to force the LLM to deductively choose the index of the best option given your provided input. It then returns the choice associated with that index.
  </p>
</div>
...
Astro's classification tool is designed to accommodate a variety of label formats, each suited to different use cases.
...
request = astro.classify("Reset my password", RequestType)
...
response = astro.classify('no way', bool)
...
4. **Prefer `classify()` over `@classifier`**: `classify()` is more versatile and adaptable for a wide range of scenarios. It should be the primary tool for classification tasks in Astro.
...
```python
result = await astro.classify_async(
    "The app crashes when I try to upload a file.", 
    labels=["bug", "feature request", "inquiry"]
) 
...
result = astro.classify.map(inputs, ["bug", "feature request", "inquiry"])
assert result == ["bug", "inquiry"]
```

(`astro.classify_async.map` is also available for async environments.)
