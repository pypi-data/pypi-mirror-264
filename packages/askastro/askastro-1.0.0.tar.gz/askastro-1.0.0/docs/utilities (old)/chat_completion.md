...
In *Astro*, each supported Large Language Model can be accessed with one common API. This means that 
...
just to use a new model. With *Astro* you can simply import ChatCompletion and 
...
!!! example "Example: Specifying a Model"
    ...
    ```python
    from astro import ChatCompletion
    import os
...
!!! tip "Replacing OpenAI's ChatCompletion."
    ...
    Just import openai from astro or, equivalently, ChatCompletion from astro.openai. 

    ```python
    from astro import openai
...
With *Astro*, you can get structured outputs from model providers by passing a response type. This lets developers
write prompts with Python objects, which are easier to develop, version, and test than language.

!!! note "In plain English."

    You can specify a type, struct, or data model to ChatCompletion, and *Astro*
...
*Astro* lets you pass your choice of JSON Schema or Python functions directly to ChatCompletion. It does the right thing.

!!! note "In plain English."

    You can pass regular Python functions to ChatCompletion, and *Astro* will take care of serialization of that function using `Pydantic` in a way you can customize.
...
or summarize the results of what we've computed for it. This *agentic* pattern is easily enabled with *Astro*. 
...
!!! note "In plain English."
    ...
    *Astro* maintains state to make it easier to maintain and observe this conversation.
