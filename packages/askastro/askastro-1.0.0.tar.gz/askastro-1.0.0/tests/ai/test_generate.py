...
class TestGenerate:
    class TestBuiltins:
        ...
        def test_type_is_string_if_only_instructions_given(self):
            result = astro.generate(instructions="major cities in California", n=2)
            assert_equal(
                result,
    ...
    class TestAsync:
        async def test_generate_locations(self):
            result = await astro.generate_async(Location, n=3)
            assert_equal(result, "a list of three locations")
