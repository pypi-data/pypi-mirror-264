...
class TestModels:
    ...
    def test_literal(self):
        ...
        class LLMConference(BaseModel):
            speakers: list[
                Literal["Adam", "Nate", "Jeremiah", "Astro", "Billy Bob Thornton"]
            ]

        x = LLMConference(
            """
            The conference for best LLM framework will feature talks by
            Adam, Nate, Jeremiah, Astro, and Billy Bob Thornton.
        """
        )
        assert len(set(x.speakers)) == 5
        assert set(x.speakers) == set(
            ["Adam", "Nate", "Jeremiah", "Astro", "Billy Bob Thornton"]
        )
...
class TestInstructions:
    def test_instructions(self):
        ...
        @astro.model(instructions="first translate the text to French")
        class Text(BaseModel):
            text: str
