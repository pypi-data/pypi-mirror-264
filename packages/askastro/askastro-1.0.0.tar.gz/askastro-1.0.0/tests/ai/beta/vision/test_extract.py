...
import astro
import pytest
from astro.utilities.testing import assert_equal
...
class TestVisionExtract:
    def test_ny(self):
        ...
        result = astro.beta.extract(img, target=Location)
        assert result in (
    ...
    def test_ny_images_input(self):
        ...
        result = astro.beta.extract(data=None, images=[img], target=Location)
        assert result in (
            [Location(city="New York", state="NY")],
            [Location(city="New York City", state="NY")],
        )
    ...
    def test_ny_image_input(self):
        ...
        result = astro.beta.extract(data=img, target=Location)
        assert result in (
            [Location(city="New York", state="NY")],
            [Location(city="New York City", state="NY")],
        )
    ...
    def test_dog(self):
        ...
        result = astro.beta.extract(img, target=Animal)
        assert set(result) == {
...
class TestAsync:
    async def test_ny(self):
        ...
        result = await astro.beta.extract_async(img, target=Location)
        assert result in (
            [Location(city="New York", state="NY")],
            [Location(city="New York City", state="NY")],
        )
...
class TestMapping:
    def test_map(self):
        ...
        result = astro.beta.extract.map([ny, dc], target=Location)
    ...
    async def test_async_map(self):
        ...
        result = await astro.beta.extract_async.map([ny, dc], target=Location)
        assert isinstance(result, list)
        assert result[0][0] in (
            Location(city="New York", state="NY"),
            Location(city="New York City", state="NY"),
