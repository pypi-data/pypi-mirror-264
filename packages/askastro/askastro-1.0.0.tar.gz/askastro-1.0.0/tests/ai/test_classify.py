...
class TestClassify:
    class TestLiteral:
        def test_classify_sentiment(self):
            result = astro.classify("This is a great feature!", Sentiment)
    ...
    class TestEnum:
        def test_classify_bug_tag(self):
            result = astro.classify("This is a bug", GitHubIssueTag)
            assert result == GitHubIssueTag.BUG

        def test_classify_feature_tag(self):
            result = astro.classify("This is a great feature!", GitHubIssueTag)
            assert result == GitHubIssueTag.FEATURE

        def test_classify_enhancement_tag(self):
            result = astro.classify("This is an enhancement", GitHubIssueTag)
            assert result == GitHubIssueTag.ENHANCEMENT

        def test_classify_docs_tag(self):
            result = astro.classify("This is a documentation update", GitHubIssueTag)
    ...
    class TestList:
        ...
        def test_classify_number(self):
            ...
            result = astro.classify(0, ["letter", "number"])
            assert result == "number"

    class TestBool:
        def test_classify_positive_sentiment(self):
            result = astro.classify("This is a great feature!", bool)
            assert result is True

        def test_classify_negative_sentiment(self):
            result = astro.classify("This feature is terrible!", bool)
            assert result is False

        def test_classify_falseish(self):
            result = astro.classify("nope", bool)
            assert result is False
    ...
    class TestAsync:
        async def test_classify_positive_sentiment(self):
            result = await astro.classify_async("This is a great feature!", bool)
            assert result is True
    ...
    class TestExamples:
        ...
        @pytest.mark.parametrize(
            ...
            [
                ("I want to do an event with astro!", "events and relations"),
...
class TestMapping:
    def test_classify_map(self):
        result = astro.classify.map(["This is great!", "This is terrible!"], Sentiment)
        assert isinstance(result, list)
        assert result == ["Positive", "Negative"]

    def test_classify_map_with_instructions(self, gpt_4):
        result = astro.classify.map(
            ["o", "0"],
            ["letter", "number"],
    ...
    async def test_async_classify_map(self):
        result = await astro.classify_async.map(
