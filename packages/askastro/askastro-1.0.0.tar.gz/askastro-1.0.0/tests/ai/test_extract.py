...
class TestExtract:
    class TestBuiltins:
        ...
        def test_float_to_int(self):
            ...
            result = astro.extract("the numbers are 1, 2, and 3.2", int)
            assert result == [1, 2, 3]
    ...
    class TestPydantic:
        def test_extract_location(self):
            result = astro.extract("I live in New York, NY", Location)
        ...
        def test_extract_multiple_locations_by_nickname(self):
            result = astro.extract("I live in the big apple and work in SF", Location)
            assert result == [
                Location(city="New York", state="NY"),
                Location(city="San Francisco", state="CA"),
            ]
    ...
    class TestAsync:
        async def test_extract_numbers(self):
            result = await astro.extract_async("one, two, three", int)
            assert result == [1, 2, 3]


class TestMapping:
    def test_map(self):
        result = astro.extract.map(
    ...
    async def test_async_map(self):
        result = await astro.extract_async.map(
            ["I have one donut", "I bought two donuts and ate one"], int
        )
        assert isinstance(result, list)
        assert result == [[1], [2, 1]]
