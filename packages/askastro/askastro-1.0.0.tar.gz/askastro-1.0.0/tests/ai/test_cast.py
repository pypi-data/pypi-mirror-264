...
class TestCast:
    ...
    class TestPydantic:
        ...
        def test_pay_attention_to_field_descriptions(self, gpt_4):
            ...
            result = astro.cast("I bought a Chevrolet", Car)
            assert result == Car(make="Ford")

    class TestInstructions:
        def test_cast_text_with_significant_instructions(self):
            result = astro.cast("one", int, instructions="return the number 4")
            assert result == 4
        ...
        def test_cast_text_with_subtle_instructions(self, gpt_4):
            ...
            assert result == "My name is ASTRO"
    ...
    class TestCastCallsClassify:
        @patch("astro.ai.text.classify_async")
...
class TestMapping:
    def test_map(self):
        result = astro.cast.map(["one", "two"], int)
        assert isinstance(result, list)
        assert result == [1, 2]

    def test_map_with_instructions(self):
        result = astro.cast.map(
            ["one", "two"],
            int,
    ...
    async def test_async_map(self):
        result = await astro.cast_async.map(["one", "two"], int)
        assert isinstance(result, list)
        assert result == [1, 2]
