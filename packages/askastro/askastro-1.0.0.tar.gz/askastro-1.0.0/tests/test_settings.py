...
class TestApiKeySetting:
    def test_api_key_initialization_from_env(self, env):
        ...
        env.set("ASTRO_OPENAI_API_KEY", test_api_key)

        temp_model_config = SettingsConfigDict(env_prefix="astro_")
    ...
    def test_runtime_api_key_override(self, env):
        ...
        env.set("ASTRO_OPENAI_API_KEY", override_api_key)
...
class TestTemporarySettings:
    def test_temporary_settings_override_and_restore(self, env):
        default_log_level = astro.settings.log_level
        ...
        assert astro.settings.log_level == default_log_level

    def test_temporary_settings_override_and_restore_nested(self, env):
        default_log_level = astro.settings.log_level
        initial_api_key = astro.settings.openai.api_key.get_secret_value()
        default_openai_speech_model = astro.settings.openai.audio.speech.model
        ...
        with temporary_settings(
            ...
            assert astro.settings.log_level == "INFO"
            assert astro.settings.openai.api_key.get_secret_value() == "test_api_key"
            assert astro.settings.openai.audio.speech.model == "test_model"

        assert astro.settings.log_level == default_log_level
        assert astro.settings.openai.api_key.get_secret_value() == initial_api_key
        assert astro.settings.openai.audio.speech.model == default_openai_speech_model
