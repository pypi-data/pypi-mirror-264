...
class TestSimilarityMethods(unittest.TestCase):
    ...
    def test_cosine_similarity(self):
        cos_sim = cosine_similarity(self.prob_distributions[0], self.prob_distributions[1])
        self.assertAlmostEqual(cos_sim, 0.8366600265340755)

    def test_jaccard_similarity(self):
        jacc_sim = jaccard_similarity(self.prob_distributions[0], self.prob_distributions[1])
        self.assertAlmostEqual(jacc_sim, 0.6)

    def test_levenshtein_distance(self):
        lev_dist = levenshtein_distance(self.prob_distributions[0], self.prob_distributions[1])
        self.assertAlmostEqual(lev_dist, 2)

    def test_sorensen_dice_coefficient(self):
        dice_coeff = sorensen_dice_coefficient(self.prob_distributions[0], self.prob_distributions[1])
