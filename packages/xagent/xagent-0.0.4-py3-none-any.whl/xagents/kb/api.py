...
def reindex_knowledge_base(name: str, reindex=False) -> str:
    """重新构建知识库索引

    Args:
        name (str): 知识库名称
        reindex(bool): 是否重新embedding
    """
    kb = get_knowledge_base(name=name)
    kb.rebuild_index(reindex=reindex)
...
def create_kb_file(kb_name: str, file: UploadFile | str, do_cut=True, do_index=True,
    ...
    if do_index:
        ...
        kb.add_kb_file2index(index, kb_file, reindex=True, do_save=True)
    return kb_file.get_info()
...
def index_kb_file(kb_name: str, file_name: str, reindex=False) -> str:
    """添加知识库文件到索引中
    ...
    kb = get_knowledge_base(kb_name)
    kb_file = get_kb_file(kb_name=kb_name, file_name=file_name)
    kb.add_kb_file2index(index=kb.get_index(), kb_file=kb_file, reindex=reindex, do_save=True)
