...
# Создаем DataFrame data для примера
data = pd.DataFrame({
    'value': [10, 12, 11, 9, 10, 100]
})
...
# Вычисляем tau (критерий Стьюдента) для каждого значения в DataFrame
data['tau'] = np.abs((data['value'] - mean) / std_dev)

# Вычисляем критическое значение tau для уровня значимости 0.05 (95% доверительный интервал)
n = len(data)
critical_tau = t.ppf(0.975, n-1)

# Находим аномалии, у которых tau больше критического значения
anomalies = data[data['tau'] > critical_tau]
...
print('t_max(n) =',max_row, '; v(n) =', n_series)

#Проверка гипотезы: при условии случайности ряда (при отсутствии систематической составляющей) протяженность самой длинной серии не должна быть слишком большой, а общее число серий - слишком маленьким.
#Чтобы не была отвергнута гипотеза о случайности исходного ряда (об отсутствии систематической составляющей), должны выполняться следующие неравенства

n = len(data)
(max_row < 3.3 * (np.log(n) + 1)) and (n_series > (n+1-1.96*(n-1)**(0.5))/2)
#вывело FALSE

#условие не выполняется, это может означать, что в исходном ряде присутствует систематическая составляющая, то есть есть тренд.
    '''
    return pc.copy(s)

def irvin_pasha():
    s = '''result = pd.DataFrame()  y = data2['UNEMPL_Q']
n = len(y)result['y'] = y
# Вычисляем среднеквадратическое отклонение
S_y = np.sqrt(((y - y.mean()) ** 2).sum() / (n - 1))
# Вычисляем величину l_t для наблюденийl_t = [0] + [abs(y.diff().iloc[i]) / S_y for i in range(1, n)]
result['lambda'] = l_t
# Если величина l_t превышает табличный уровень, то значение y_t считается аномальнымdifference = data2['UNEMPL_Q'].diff()
lamb = np.abs(difference)/S_yl_kririch = 1.478*(n/10)**(-0.1767)
result_normal = result[result['lambda'] <= l_kririch]  # Убираем аномалии из таблицы
# Проводим анализ по методу Ирвина
anomalies = result[result['lambda'] > l_kririch]num_anomalies = len(anomalies)

if num_anomalies == 0:    print("Аномалий не обнаружено.")
else:    print(f"Обнаружено {num_anomalies} аномалий:")
    print(anomalies)
# Отобразим таблицу без аномалийprint("Таблица без аномалий:")
print(result_normal)
#Используя метод Ирвина проанализировани датасет на аномальные значения. Таких не оказалось => Датасет оставляем без изменений.
    '''
    return pc.copy(s)