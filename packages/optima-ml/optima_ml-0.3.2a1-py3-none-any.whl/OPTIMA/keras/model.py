...
def build_model(
    ...
    targets_train : np.ndarray
        The target labels for the training dataset. While not used for this model architecture, other models may need
        this to build certain layers, thus a ``build_model``-function in the run-config needs to be provided with the
        target labels.
