...
while True:
    cmla = input("spm >> ")
    if cmla.startswith("spm install"):
        args = cmla.split()[2:]
        if len(args) != 2:
            print("Invalid number of arguments. Usage: spm install PACKAGE_NAME VERSION")
        else:
            package = args[0]
            version = args[1]
        ...
        print("Invalid command. Type 'spm install' to install a package or 'exit' to quit.")
