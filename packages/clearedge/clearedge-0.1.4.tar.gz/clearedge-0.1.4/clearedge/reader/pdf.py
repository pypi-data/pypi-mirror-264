...
def process_pdf(
  ...
  if use_ocr or should_perform_ocr(doc):
    return _process_file_with_ocr(doc, ocr_model)
  else:
...
def _process_file_with_ocr(doc, ocr_model):
  ...
  results = docseg_model(source=images, save=True, show_labels=True, show_conf=True, show_boxes=True)
  ...
  rapid_ocr = RapidOCR(config_path="clearedge/ocr_config/config.yaml", rec_model_path="clearedge/reader/en_PP-OCRv4_rec_infer.onnx")
