...
```bash
nebula worker start --pool 'my-work-pool' --type kubernetes
```

Replace `my-work-pool` with the name of the work pool you want the worker
to poll for flow runs.

### Securing your Nebula Cloud API key
If you are using Nebula Cloud and would like to pass your Nebula Cloud API key to
created jobs via a Kubernetes secret, set the
`NEBULA_KUBERNETES_WORKER_STORE_NEBULA_API_IN_SECRET` environment variable before
starting your worker:

```bash
export NEBULA_KUBERNETES_WORKER_STORE_NEBULA_API_IN_SECRET="true"
nebula worker start --pool 'my-work-pool' --type kubernetes
```
...
by work pool basis. These customizations can be made via the Nebula UI when
...
checkout out the [Nebula docs](https://docs.nebula.io/concepts/work-pools/).
"""
...
from nebula.server.schemas.core import Flow
from nebula.server.schemas.responses import DeploymentResponse
...
from nebula_kubernetes.events import KubernetesEventsReplicator
from nebula_kubernetes.utilities import (
    _slugify_label_key,
...
class KubernetesWorkerJobConfiguration(BaseJobConfiguration):
    ...
    def prepare_for_flow_run(
        ...
        self._update_nebula_api_url_if_local_server()
    ...
    def _populate_env_in_manifest(self):
        ...
        our environment variables to the list to ensure Nebula setting propagation.
        ...
        # we need to prepend our environment variables to the list to ensure Nebula
        # setting propagation.
    ...
    def _update_nebula_api_url_if_local_server(self):
        ...
        """
        if self.env.get("NEBULA_API_URL") and self._api_dns_name:
            self.env["NEBULA_API_URL"] = (
                self.env["NEBULA_API_URL"]
    ...
    def _populate_image_if_not_present(self):
        ...
        with the default Nebula image if it is not present."""
        try:
            if (
                "image"
    ...
    def _populate_command_if_not_present(self):
        ...
        with the `nebula -m nebula.engine` if a command is not present.
        """
        try:
...
class KubernetesWorkerVariables(BaseVariables):
    ...
    image: Optional[str] = Field(
        ...
        "If not set, the latest Nebula image will be used.",
        example="docker.io/kozmoai/nebula:1-latest",
    )
...
class KubernetesWorker(BaseWorker):
    """Nebula worker that executes flow runs within Kubernetes Jobs."""
    ...
    def _get_configured_kubernetes_client(
        ...
        try:
            ...
            if os.environ.get(
                "NEBULA_KUBERNETES_WORKER_ADD_TCP_KEEPALIVE", "TRUE"
    ...
    def _replace_api_key_with_secret(
        ...
        """Replaces the NEBULA_API_KEY environment variable with a Kubernetes secret"""
        ...
        manifest_api_key_env = next(
            (
                ...
                if env_entry.get("name") == "NEBULA_API_KEY"
            ),
            {},
        )
        api_key = manifest_api_key_env.get("value")
        if api_key:
            secret_name = f"nebula-{_slugify_name(self.name)}-api-key"
            ...
            manifest_env = [
                entry if entry.get("name") != "NEBULA_API_KEY" else new_api_env_entry
    ...
    def _create_job(
        ...
        """
        if os.environ.get(
            "NEBULA_KUBERNETES_WORKER_STORE_NEBULA_API_IN_SECRET", ""
        ).strip().lower() in ("true", "1"):
    ...
    def _get_cluster_uid(self, client: "ApiClient") -> str:
        ...
        NEBULA_KUBERNETES_CLUSTER_UID can be set in cases where the `kube-system`
        ...
        env_cluster_uid = os.environ.get("NEBULA_KUBERNETES_CLUSTER_UID")
        if env_cluster_uid:
