...
from nebula_kubernetes.events import EVICTED_REASONS, KubernetesEventsReplicator
...
@pytest.fixture
def worker_resource():
    return {"nebula.resource.id": "nebula.worker.my-k8s-worker"}
...
def related_resources():
    return [
        RelatedResource(
            __root__={
                "nebula.resource.id": "nebula.flow-run.1234",
                "nebula.resource.role": "flow-run",
            }
        )
    ]
...
def test_replicate_successful_pod_events(replicator, successful_pod_stream):
    ...
    with patch("nebula_kubernetes.events.emit_event", side_effect=event) as mock_emit:
        with patch.object(replicator, "_watch", mock_watch):
            with replicator:
                time.sleep(0.3)
    ...
    mock_emit.assert_has_calls(
        [
            call(
                event="nebula.kubernetes.pod.pending",
                resource={
                    "nebula.resource.id": "nebula.kubernetes.pod.1234",
                    "nebula.resource.name": "test-pod",
                ...
                related=[
                    ...
                    RelatedResource(
                        __root__={
                            "nebula.resource.id": "nebula.worker.my-k8s-worker",
                            "nebula.resource.role": "worker",
                        }
                    ),
                ],
                follows=None,
            ...
            call(
                event="nebula.kubernetes.pod.running",
                resource={
                    "nebula.resource.id": "nebula.kubernetes.pod.1234",
                    "nebula.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
            ...
            call(
                event="nebula.kubernetes.pod.succeeded",
                resource={
                    "nebula.resource.id": "nebula.kubernetes.pod.1234",
                    "nebula.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
...
def test_replicate_failed_pod_events(replicator, failed_pod_stream):
    ...
    mock_emit.assert_has_calls(
        [
            ...
            call(
                event="nebula.kubernetes.pod.failed",
                resource={
                    "nebula.resource.id": "nebula.kubernetes.pod.1234",
                    "nebula.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
...
def test_replicate_evicted_pod_events(replicator, evicted_pod_stream):
    ...
    mock_emit.assert_has_calls(
        [
            ...
            call(
                event="nebula.kubernetes.pod.evicted",
                resource={
                    "nebula.resource.id": "nebula.kubernetes.pod.1234",
                    "nebula.resource.name": "test-pod",
                    "kubernetes.namespace": "test-namespace",
