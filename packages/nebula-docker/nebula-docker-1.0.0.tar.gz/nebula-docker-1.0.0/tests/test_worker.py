...
from nebula.docker import get_nebula_image_name
from nebula.events import RelatedResource
from nebula.exceptions import InfrastructureNotAvailable, InfrastructureNotFound
from nebula.settings import (
    NEBULA_EXPERIMENTAL_ENABLE_WORKERS,
    NEBULA_EXPERIMENTAL_WARN_WORKERS,
    get_current_settings,
    temporary_settings,
)
from nebula.testing.utilities import assert_does_not_warn
from packaging import version

from nebula_docker.credentials import DockerRegistryCredentials
from nebula_docker.worker import (
    CONTAINER_LABELS,
    DockerWorker,
...
def enable_workers():
    with temporary_settings(
        {NEBULA_EXPERIMENTAL_ENABLE_WORKERS: 1, NEBULA_EXPERIMENTAL_WARN_WORKERS: 0}
    ):
        yield
...
def bypass_api_check(monkeypatch):
    monkeypatch.setenv("NEBULA_DOCKER_TEST_MODE", True)
...
async def test_uses_label_setting(
    ...
    assert call_labels == {
        **CONTAINER_LABELS,
        "io.nebula.flow-run-id": str(flow_run.id),
        "io.nebula.flow-run-name": flow_run.name,
        "foo": "FOO",
        "bar": "BAR",
    }
...
async def test_uses_env_setting(
    ...
    flow_run_id = (
        str(flow_run.id)
        if version.parse(nebula.__version__) >= version.parse("2.13.5")
        else flow_run.id.hex
    )
    assert call_env == {
        **get_current_settings().to_environment_variables(exclude_unset=True),
        "NEBULA__FLOW_RUN_ID": flow_run_id,
        "foo": "FOO",
        "bar": "BAR",
    }
...
async def test_allows_unsetting_environment_variables(
    ...
    default_docker_worker_job_configuration.env = {"NEBULA_TEST_MODE": None}
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
    ...
    mock_docker_client.containers.create.assert_called_once()
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert "NEBULA_TEST_MODE" not in call_env
...
async def test_network_mode_defaults_to_host_if_using_localhost_api_on_linux(
    ...
    default_docker_worker_job_configuration.env = dict(
        NEBULA_API_URL=f"http://{localhost}/test"
    )
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
...
async def test_network_mode_defaults_to_none_if_using_networks(
    ...
    default_docker_worker_job_configuration.env = dict(
        NEBULA_API_URL="http://localhost/test"
    )
...
async def test_network_mode_defaults_to_none_if_using_nonlocal_api(
    ...
    default_docker_worker_job_configuration.env = dict(NEBULA_API_URL="http://foo/test")
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
...
async def test_replaces_localhost_api_with_dockerhost_when_not_using_host_network(
    ...
    mock_docker_client.containers.create.assert_called_once()
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert "NEBULA_API_URL" in call_env
    assert call_env["NEBULA_API_URL"] == hosted_api_server.replace(
...
async def test_does_not_replace_localhost_api_when_using_host_network(
    ...
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert "NEBULA_API_URL" in call_env
    assert call_env["NEBULA_API_URL"] == hosted_api_server
...
async def test_does_not_override_user_provided_api_host(
    ...
    default_docker_worker_job_configuration.env = dict(
        NEBULA_API_URL="http://localhost/api"
    )
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
    ...
    mock_docker_client.containers.create.assert_called_once()
    call_env = mock_docker_client.containers.create.call_args[1].get("environment")
    assert call_env.get("NEBULA_API_URL") == "http://localhost/api"
...
async def test_default_image_pull_policy_pulls_image_with_latest_tag(
    ...
    default_docker_worker_job_configuration.image = "nebula:latest"
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
    ...
    mock_docker_client.images.pull.assert_called_with("nebula", "latest")


async def test_default_image_pull_policy_pulls_image_with_no_tag(
    mock_docker_client, flow_run, default_docker_worker_job_configuration
):
    default_docker_worker_job_configuration.image = "nebula"
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
    ...
    mock_docker_client.images.pull.assert_called_once()
    mock_docker_client.images.pull.assert_called_with("nebula", None)
...
async def test_default_image_pull_policy_pulls_image_with_tag_other_than_latest_if_not_present(  # noqa
    ...
    default_docker_worker_job_configuration.image = "nebula:omega"
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    async with DockerWorker(work_pool_name="test") as worker:
        await worker.run(
            flow_run=flow_run, configuration=default_docker_worker_job_configuration
    ...
    mock_docker_client.images.pull.assert_called_once()
    mock_docker_client.images.pull.assert_called_with("nebula", "omega")
...
async def test_warns_if_docker_version_does_not_support_host_gateway_on_linux(
    ...
    default_docker_worker_job_configuration.env = (
        {"NEBULA_API_URL": explicit_api_url} if explicit_api_url else {}
    )
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
    with pytest.warns(
        UserWarning,
...
async def test_does_not_warn_about_gateway_if_user_has_provided_nonlocal_api_url(
    ...
    default_docker_worker_job_configuration.env = {
        "NEBULA_API_URL": "http://my-domain.test/api"
    }
    default_docker_worker_job_configuration.prepare_for_flow_run(flow_run=flow_run)
...
async def test_container_metadata(
    ...
    assert container.image.tags[0] == get_nebula_image_name()
...
async def test_emits_events(
    ...
    with patch("nebula_docker.worker.emit_event", side_effect=event) as mock_emit:
        async with DockerWorker(work_pool_name="test") as worker:
            await worker.run(
                flow_run=flow_run, configuration=default_docker_worker_job_configuration
            )
    ...
    worker_resource["nebula.resource.role"] = "worker"
    worker_related_resource = RelatedResource(__root__=worker_resource)

    mock_emit.assert_has_calls(
        [
            call(
                event="nebula.docker.container.created",
                resource={
                    "nebula.resource.id": "nebula.docker.container.fake-id",
                    "nebula.resource.name": "fake-name",
                },
                related=[worker_related_resource],
                follows=None,
            ),
            call(
                event="nebula.docker.container.running",
                resource={
                    "nebula.resource.id": "nebula.docker.container.fake-id",
                    "nebula.resource.name": "fake-name",
                },
            ...
            call(
                event="nebula.docker.container.exited",
                resource={
                    "nebula.resource.id": "nebula.docker.container.fake-id",
                    "nebula.resource.name": "fake-name",
                },
...
async def test_emits_event_container_creation_failure(
    ...
    with patch("nebula_docker.worker.emit_event") as mock_emit:
        with pytest.raises(docker.errors.APIError, match="test error"):
            async with DockerWorker(work_pool_name="test") as worker:
                worker_resource = worker._event_resource()
                await worker.run(
        ...
        mock_emit.assert_called_once_with(
            event="nebula.docker.container.creation-failed",
