"""
Nebula deployment steps for building and pushing Docker images.


These steps can be used in a `nebula.yaml` file to define the default
...
def build_docker_image(
    ...
    **build_kwargs,
    ...
    """
    Builds a Docker image for a Nebula deployment.

    Can be used within a `nebula.yaml` file to build a Docker
    ...
    if auto_build:
        ...
        if Path("requirements.txt").exists():
            lines.append(
                f"COPY requirements.txt /opt/nebula/{dir_name}/requirements.txt"
            )
            lines.append(
    ...
    Examples:
        ...
        push:
            - nebula_docker.deployments.steps.push_docker_image:
                ...
                image_name: "{{ build-image.image_name }}"
                tag: "{{ build-image.tag }}"
                credentials: "{{ nebula.blocks.docker-registry-credentials.dev-registry }}"
        ```
