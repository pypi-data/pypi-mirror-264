...
```bash
nebula worker start --pool 'my-work-pool' --type docker
```
...
from nebula.workers.base import BaseJobConfiguration, BaseWorker, BaseWorkerResult
...
CONTAINER_LABELS = {
    "io.nebula.version": nebula.__version__,
}
...
class DockerWorkerJobConfiguration(BaseJobConfiguration):
    ...
    image: str = Field(
        default_factory=get_nebula_image_name,
        description="The image reference of a container image to use for created jobs. "
        "If not set, the latest Nebula image will be used.",
        example="docker.io/kozmoai/nebula:2-latest",
    )
    ...
    def _base_environment(self):
        ...
        if (
            "NEBULA_API_URL" in base_env
            and base_env["NEBULA_API_URL"] is not None
            and network_mode != "host"
        ):
            base_env["NEBULA_API_URL"] = (
                base_env["NEBULA_API_URL"]
    ...
    def prepare_for_flow_run(
        ...
        self.image = self.image or get_nebula_image_name()
    ...
    def get_network_mode(self) -> Optional[str]:
        ...
        api_url = self.env.get("NEBULA_API_URL", NEBULA_API_URL.value())
    ...
    def get_extra_hosts(self, docker_client) -> Optional[Dict[str, str]]:
        ...
        if sys.platform == "linux" and (
            ...
            # a local address
            "NEBULA_API_URL" not in self.env
            or re.search(
...
class DockerWorker(BaseWorker):
    """Nebula worker that executes flow runs within Docker containers."""
    ...
    _documentation_url = "https://kozmoai.github.io/nebula-docker/worker/"
    ...
    def __init__(self, *args: Any, test_mode: bool = None, **kwargs: Any) -> None:
        if test_mode is None:
            self.test_mode = bool(os.getenv("NEBULA_DOCKER_TEST_MODE", False))
        else:
        ...
        if not self.test_mode:
            ...
            if self._client.server_type == ServerType.EPHEMERAL:
                raise RuntimeError(
                    ...
                    " NEBULA_API_URL to the URL for your Nebula API instance. You"
                    " can use a local Nebula API instance by running `nebula server"
                    " start`."
                )
    ...
    def _container_as_resource(self, container: "Container") -> Dict[str, str]:
        ...
        return {
            "nebula.resource.id": f"nebula.docker.container.{container.id}",
            "nebula.resource.name": container.name,
        }

    def _emit_container_creation_failed_event(
        self, configuration: DockerWorkerJobConfiguration
    ) -> Event:
        """Emit a Nebula event when a docker container fails to be created."""
        return emit_event(
    ...
    def _emit_container_status_change_event(
        ...
        """Emit a Nebula event for a Docker container event."""
        related = self._event_related_resources(configuration=configuration)
        ...
        return emit_event(
            event=f"nebula.docker.container.{container.status.lower()}",
