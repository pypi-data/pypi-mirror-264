...
logger = util.create_logger('base')
...
class IBM3270(tricycle.ScopedObject):
    ...
    async def _connect_windows(self, hostname: str) -> trio.Process:
        ...
        if not os.path.exists(cmd[0]):
            raise FileNotFoundError(f'{cmd[0]} precisa existir')
        if self._window:
            # try:
            ...
            self._p2 = await trio.lowlevel.open_process(
                cmd,
                stdin=subprocess.PIPE,
                stdout=subprocess.PIPE,
                close_fds=True,
