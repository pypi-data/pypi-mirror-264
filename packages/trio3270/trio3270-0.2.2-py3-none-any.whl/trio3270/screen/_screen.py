...
logger = _util.create_logger('screen')
        ...
        with trio.move_on_after(_timeout):
            while True:
                for check in checks:
                    ...
                    # All checks passed; verify fields
                    if num_fields > len(self.entry_fields):
                        ...
                        try:
                            ...
                            if (not self.buffer) or self.buffer.is_empty():
                                error = RuntimeError('Empty screen')
                            else:
                                error = None
                if error is None:
                    if self.command_sent_no_change is None:
                        logger.debug(f'error none, command sent no change none, returning {kwarg_fields}')
                        return kwarg_fields
                    ...
                    if minimum_wait_needed > 0:
                        ...
                        logger.debug(f'error none, we wait {wait_time}')
                    else:  # no more wait needed so just accept this
                        logger.debug(f'error none, no more wait needed, returning {kwarg_fields}')
                        return kwarg_fields
                else:
                    wait_time = min(0.1 + 2 ** wait_attempt * 0.05, 0.5)
                    logger.debug(f'Error found: {error}; waiting {wait_time}')
                await trio.sleep(wait_time)
                logger.debug(f"{wait_time} passed, refreshing buffer")
                await self.refresh_buffer()
                wait_attempt += 1
