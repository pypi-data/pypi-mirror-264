...
def test_default(app: SphinxTestApp, status: StringIO,
  ...
  figcaption = figures[0].select_one('figcaption span')
  assert figcaption and (figcaption.get_text().strip()
                         == 'This is a test screenshot')

  # The caption should render other directives.
  code_element = figures[1].select_one('code')
  assert code_element and (code_element.get_text().strip()
