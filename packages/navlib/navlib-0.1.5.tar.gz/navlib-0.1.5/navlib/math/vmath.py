...
def norm(mat: Union[np.ndarray, List[float]]) -> Union[float, np.ndarray]:
    """
...
def normalize(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    """
    Returns the normalized vector or matrix. If the input is a matrix,
    normalization is performed on each row.
...
def mean(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    """
...
def median(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    """
    ...
    return np.median(mat, axis=0, keepdims=True)


def std(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    """
    ...
    return np.std(mat, axis=0, keepdims=True)


def min(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    """
    ...
    return np.min(mat, axis=0, keepdims=True)


def max(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    """
    ...
    return np.max(mat, axis=0, keepdims=True)
...
def print_stats(mat: Union[np.ndarray, List[float]]) -> None:
    ...
    if mat.ndim >= 2:
        print("Mean:   " + ", ".join("%.2f" % f for f in mean(mat).flatten()))
        print("Median: " + ", ".join("%.2f" % f for f in median(mat).flatten()))
        print(
            "Standard deviation: " + ", ".join("%.2f" % f for f in std(mat).flatten())
        )
        print("Min: " + ", ".join("%.2f" % f for f in min(mat.flatten())))
        print("Max: " + ", ".join("%.2f" % f for f in max(mat).flatten()))
        print("Norm: " + ", ".join("%.2f" % f for f in norm(mat).flatten()))
        print(
            "MinMax: " + ", ".join("%.2f" % f for f in (max(mat) - min(mat)).flatten())
        )
    else:
...
def remove_mean(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    ...
    return mat - np.mean(mat, axis=0, keepdims=True)
...
def remove_median(mat: Union[np.ndarray, List[float]]) -> np.ndarray:
    ...
    return mat - np.median(mat, axis=0, keepdims=True)
...
def distance_traveled(
    ...
    if linear:
        if full:
            return norm(remove_offset(mat, mat[0, :])).flatten()
        else:
            return norm(remove_offset(mat, mat[0, :])).flatten()[-1]
    ...
    # cumulative sum.
        ...
        if full:
            return np.r_[0.0, np.cumsum(norm(difference(mat)).flatten())]
        else:
