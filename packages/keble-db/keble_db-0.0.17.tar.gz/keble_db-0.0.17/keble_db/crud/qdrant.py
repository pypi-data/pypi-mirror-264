...
class QdrantCRUDBase(Generic[ModelType, VectorModelType]):
    ...
    def scroll_records(self, client: QdrantClient, *, query: QueryBase) -> Tuple[List[Record], Optional[qdrant_types.PointId]]:
        filter_ = cleanse_query_for_search(query)
        ...
        return [Record(**record.model_dump(exclude={"payload"}), payload=record.payload) for record in records], next_point_id

    def scroll(self, client: QdrantClient, *, query: QueryBase) -> Tuple[List[ModelType], Optional[qdrant_types.PointId]]:
        records, next_point_id = self.scroll_records(client, query=query)
