...
def etl_main(etl_workflow_prefix):
    ...
    if etl_workflow_prefix == "kb":
        target_module_to_run = getattr(importlib.import_module(f"etld_knowledgebase.knowledgebase_02_workflow_manager"), f"knowledgebase_etl_workflow")
        target_method_in_module_to_run = f"knowledgebase_etl_workflow"
    else:
        target_module_to_run = getattr(importlib.import_module(f"etld_{etl_workflow_prefix}.{etl_workflow_prefix}_02_workflow_manager"), f"{etl_workflow_prefix}_etl_workflow")
        target_method_in_module_to_run = f"{etl_workflow_prefix}_etl_workflow"
