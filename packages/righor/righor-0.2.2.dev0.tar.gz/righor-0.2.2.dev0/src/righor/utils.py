...
def genes_matching(x: str, model, exact=False):
    """ Map relatively standard gene name to
        ...
        * exact: if true will only return only the gene with that *exact name* in the model (or None)
        @ Return:
        * list of righor Gene object (x.name to get their names), or Gene object if exact is true
        @ Example:
    ...
    if exact:
        result =  [a[-1] for a in possible_genes if a[-1].name == x]
        if len(result) > 1:
            return result
        elif len(result) == 1:
            return result[0]
