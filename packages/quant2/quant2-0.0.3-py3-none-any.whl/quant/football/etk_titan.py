...
def wt20240320_event_data_parse(event_data: list, points: list, ehead: str):
    ...
    for idx, point in zip(indexes[:-1], points[1:]):
        ...
        if offset > 60 or offset < -60:
            ...
            assert -300 < offset < 300, f"offset({offset}) more than 5 minutes"
...
def wt20240320_odds_data_parse(odds_data: list, points: list, year: str, ehead: str):
    last_data = sorted(odds_data, key=lambda x: x["crawler_end_time"])[-1]

    segment_1x2, first_1x2 = wt20240320_odds_data_parse_x(
        last_data["胜平负"], points, year, f"{ehead},胜平负"
    )
    segment_overunder, first_overunder = wt20240320_odds_data_parse_x(
        last_data["进球数"], points, year, f"{ehead},进球数"
    )

    return segment_1x2, segment_overunder, first_1x2 + first_overunder


def wt20240320_odds_data_parse_x(data: list, points: list, year: str, ehead: str):
...
def feature_selection_wt20240320(base_data: list, event_data: list, odds_data: list, match_id: str):
    ...
    out_event = wt20240320_event_data_parse(
    ...
    out_odds = wt20240320_odds_data_parse(
        odds_data, points, year, f"{match_id=},赔率数据"
    )
...
def extract_match_feature(match_section: list, match_info: list, match_id: str):
    ...
    vals = feature_selection_wt20240320(
