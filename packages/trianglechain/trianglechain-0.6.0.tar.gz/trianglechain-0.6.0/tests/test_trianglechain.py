...
def test_credible_intervals():
    ...
    tri = TriangleChain(
        params=["omega_m", "S8", "A_IA"], labels=[r"$\Omega_m$", r"$S_8$", r"$A_{IA}$"]
    )
    tri.contour_cl(samples, show_values=True, credible_interval=0.5)

    tri = TriangleChain(
        params=["omega_m", "S8", "h"], labels=[r"$\Omega_m$", r"$S_8$", r"$h$"]
    )
    ...
    tri.contour_cl(samples[samples["omega_m"] < 0.3], show_values=True)
