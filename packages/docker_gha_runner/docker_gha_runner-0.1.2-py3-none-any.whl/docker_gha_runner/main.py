...
class DockerGhaRunner:
    ...
    def _delete_all_containers_on_exit(self, signal, frame):
    ...
    def run(self, process_signal: bool = True) -> None:
        if process_signal:
            signal.signal(signal.SIGINT, self._delete_all_containers_on_exit)
            signal.signal(signal.SIGTERM, self._delete_all_containers_on_exit)
        self._config_check()
