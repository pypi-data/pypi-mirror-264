...
class trafficControl():
    def __init__(self,test_device_ip,caseName):
        ...
        # 初始化网络
        self.upWorkCondition = deepcopy(constNetWorkCondition)
        self.downWorkCondition = deepcopy(constNetWorkCondition)
        # 网络赋值
        self.upWorkCondition = [caseInfo['up']['band'],caseInfo['up']['delay'],caseInfo['up']['jiter'],caseInfo['up']['loss'],caseInfo['up']['packet']]
        self.downWorkCondition = [caseInfo['down']['band'], caseInfo['down']['delay'], caseInfo['down']['jiter'], caseInfo['down']['loss'],caseInfo['down']['packet']]
        print(self.upWorkCondition)
        print(self.downWorkCondition)
        # upd 开关
        try:
        ...
        # 背景流量类型  tcp udp
        try:
            self.upbackGroundType = caseInfo['bg_type']
        except:
            pass
        try:
            self.downbackGroundType = caseInfo['bg_type']
        except:
            pass
        # 周期网络赋值
        try:
            self.upinterDuration = caseInfo['occupy']
            self.upinterInterval = caseInfo['idle']
        except:
            self.upinterDuration = 5
            self.upinterInterval = 30

        try:
            self.downinterDuration = caseInfo['occupy']
            self.downinterInterval = caseInfo['idle']
        except:
            self.downinterDuration = 5
            self.downinterInterval = 30
        # 网络变换 赋值
        try:
            self.upinterMertic = caseInfo['change']
        except:
            self.upinterMertic = 'None'

        try:
            self.downinterMertic = caseInfo['change']
        except:
