...
class Reliability:
    ...
    def __init__(self, demand: Union[pd.Series,list,float,int], supply: pd.Series, demand_timescale: Literal["daily","monthly","yearly"]="daily", demand_type: Literal["total","daily_constant"]="total", ignore_leap_years=False) -> None:
        ...
        if type(demand) == pd.Series:
            print(f"Comparing provided demand timeseries with supply timeseries.")
            state = "ts"
        ...
        if type(demand) is list:
            ...
            print(f"Comparing list of monthly demands with supply timeseries. demand_timescale parameter is unused.")

            if demand_type == "daily_constant":
                print(f"Each monthly value in demand list will be applied as the daily demand for the respective month.")
                print(f"ignore_leap_years parameter is unused.")
                state = "monthly_constant_list"
            else:
                print(f"Each monthly total in demand list will be disaggregated to a daily demand for the respective month.")
                print(f"Leap years will be disaggregated assuming {lookup_leap[ignore_leap_years]} days in February.")
                state = "monthly_total_list"

        if type(demand) in [float, int]:
            print(f"Comparing provided demand with supply timeseries.")
            if (demand_timescale == "daily") or (demand_type == "daily_constant"):
                print(f"Assumed that daily demand is a constant {demand} ML/d.")
                if (demand_timescale == "daily") and (demand_type == "daily_constant"):
                    print(f"Only one of demand_timescale = {demand_timescale} and demand_type = {demand_type} were required.")
                    print(f"ignore_leap_years parameter is unused.")
                if (demand_timescale == "daily") and (demand_type != "daily_constant"):
                    print(f"demand_type and ignore_leap_years parameters are unused.")
                if (demand_type == "daily_constant") and (demand_timescale != "daily"):
                    print(f"demand_timescale and ignore_leap_years parameters are unused.")
                state="daily_constant"
            else:
                print(f"Assumed that {demand} ML is the total demand over the {demand_timescale} timescale. {demand} ML {demand_timescale} will be disaggregated to ML/d.")
                print(f"Leap years will be disaggregated assuming {lookup_leap[ignore_leap_years]} days in February.")
