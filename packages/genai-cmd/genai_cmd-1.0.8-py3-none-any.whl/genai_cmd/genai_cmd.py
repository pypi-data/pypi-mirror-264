...
def start():
	...
	parser.add_argument('-c', '--context', dest='context', help="The context in which the Mixtral-8x7B-Instruct-v0 model to operate under. (As of 3/24/24)")
	...
	try:
		if args.textdata:
			print(f"\n   genai:")
			data = generate(args.textdata, get_data('context'))
		elif args.rawdata:
			data = generate(args.rawdata, get_data('context'), raw=True)
		else:
			shell_interface()
			while True:
				text = input("genai> ")
				if text == "exit":
					break;
				data = generate(text, get_data('context'))
				if data == "exit":
					break;
