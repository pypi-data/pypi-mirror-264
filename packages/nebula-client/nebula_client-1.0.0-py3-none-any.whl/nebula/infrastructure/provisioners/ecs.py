...
from nebula.cli._prompts import prompt
from nebula.client.orchestration import NebulaClient
from nebula.client.schemas.actions import BlockDocumentCreate
...
class IamPolicyResource:
    ...
    Args:
        policy_name: The name of the IAM policy. Defaults to "nebula-ecs-policy".
    """
...
class CredentialsBlockResource:
    ...
    async def provision(
        ...
        Args:
            ...
            advance: A callback function to indicate progress.
            client: A Nebula client to use for interacting with the Nebula API.
        """
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    To register the `aws-credentials` block type, run:
                            ...
                            nebula blocks register -m nebula_aws
...
class AuthenticationResource:
    def __init__(
        self,
        work_pool_name: str,
        user_name: str = "nebula-ecs-user",
        policy_name: str = "nebula-ecs-policy",
...
class ClusterResource:
    def __init__(self, cluster_name: str = "nebula-ecs-cluster"):
    ...
    async def get_planned_actions(self) -> List[str]:
        ...
        if await self.requires_provisioning():
            return [
                "Creating an ECS cluster for running Nebula flows:"
...
class VpcResource:
    def __init__(
        self,
        vpc_name: str = "nebula-ecs-vpc",
        ecs_security_group_name: str = "nebula-ecs-security-group",
    ):
    ...
    async def _get_nebula_created_vpc(self):
    ...
    async def requires_provisioning(self) -> bool:
        ...
        if await self._get_nebula_created_vpc() is not None:
            self._requires_provisioning = False
            return False
    ...
    async def provision(
        ...
        if await self.requires_provisioning():
            ...
            vpc = await self._get_nebula_created_vpc()
...
class ContainerRepositoryResource:
    def __init__(self, work_pool_name: str, repository_name: str = "nebula-flows"):
    ...
    async def _get_nebula_created_registry(self):
        try:
    ...
    async def requires_provisioning(self) -> bool:
        ...
        if await self._get_nebula_created_registry() is not None:
            self._requires_provisioning = False
            return False
    ...
    async def get_planned_actions(self) -> List[str]:
        ...
        if await self.requires_provisioning():
            return [
                "Creating an ECR repository for storing Nebula images:"
    ...
    async def provision(
        ...
        if await self.requires_provisioning():
            ...
            update_current_profile({NEBULA_DEFAULT_DOCKER_BUILD_NAMESPACE: namespace})
                    ...
                    Syntax(
                        dedent(
                            f"""\
                                ...
                                from nebula.deployments import DeploymentImage
...
class ExecutionRoleResource:
    def __init__(self, execution_role_name: str = "NebulaEcsTaskExecutionRole"):
        self._iam_client = boto3.client("iam")
...
class ElasticContainerServicePushProvisioner:
    ...
    def _generate_resources(
        ...
        policy_name: str = "nebula-ecs-policy",
        credentials_block_name: str = None,
        cluster_name: str = "nebula-ecs-cluster",
        vpc_name: str = "nebula-ecs-vpc",
        ecs_security_group_name: str = "nebula-ecs-security-group",
        repository_name: str = "nebula-flows",
    ):
        return [
        ...
        try:
            if self.console.is_interactive and Confirm.ask(
                ...
                policy_name = prompt(
                    ...
                    default="nebula-ecs-policy",
                )
                cluster_name = prompt(
                    "Enter a name for the ECS cluster (hosts ECS tasks)",
                    default="nebula-ecs-cluster",
                )
                ...
                ecs_security_group_name = prompt(
                    ...
                    default="nebula-ecs-security-group",
                )
