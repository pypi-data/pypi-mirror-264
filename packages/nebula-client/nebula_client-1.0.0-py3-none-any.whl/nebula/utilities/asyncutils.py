...
NEBULA_THREAD_LIMITER: Optional[anyio.CapacityLimiter] = None

logger = get_logger()


def get_thread_limiter():
    global NEBULA_THREAD_LIMITER

    if NEBULA_THREAD_LIMITER is None:
        NEBULA_THREAD_LIMITER = anyio.CapacityLimiter(250)

    return NEBULA_THREAD_LIMITER
...
def sync_compatible(async_fn: T) -> T:
    ...
    def coroutine_wrapper(*args, **kwargs):
        ...
        from nebula._internal.concurrency.calls import get_current_call, logger
        from nebula._internal.concurrency.event_loop import get_running_loop
        from nebula._internal.concurrency.threads import get_global_loop
        ...
        if current_thread.ident == global_thread_portal.thread.ident:
            ...
            # In the nebula async context; return the coro for us to await
            ...
            # call in Nebula's event loop thread
...
async def add_event_loop_shutdown_callback(coroutine_fn: Callable[[], Awaitable]):
    ...
    async def on_shutdown(key):
        ...
        # https://github.com/kozmoai/nebula/issues/7709#issuecomment-1560021109
