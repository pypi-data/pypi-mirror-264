"""
Utilities for loading plugins that extend Nebula's functionality.

Plugins are detected by entry point definitions in package setup files.

Currently supported entrypoints:
    - nebula.collections: Identifies this package as a Nebula collection that
        should be imported when Nebula is imported.
"""
import sys
...
from nebula.utilities.compat import EntryPoint, EntryPoints, entry_points
...
def load_extra_entrypoints() -> Dict[str, Union[Exception, Any]]:
    ...
    if not nebula.settings.NEBULA_EXTRA_ENTRYPOINTS.value():
        return results

    values = {
        value.strip()
        for value in nebula.settings.NEBULA_EXTRA_ENTRYPOINTS.value().split(",")
    }

    entrypoints = []
    for value in values:
        try:
            entrypoint = EntryPoint(name=None, value=value, group="nebula-extra")
        except Exception as exc:
            print(
                (
    ...
    for value, result in zip(
        ...
        if isinstance(result, Exception):
            ...
            if nebula.settings.NEBULA_DEBUG_MODE:
                print(
...
def load_nebula_collections() -> Dict[str, ModuleType]:
    """
    Load all Nebula collections that define an entrypoint in the group
    `nebula.collections`.
    """
    collection_entrypoints: EntryPoints = entry_points(group="nebula.collections")
