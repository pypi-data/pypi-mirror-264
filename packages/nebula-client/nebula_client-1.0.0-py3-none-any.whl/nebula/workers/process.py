...
```bash
nebula worker start --pool 'my-work-pool' --type process
```
...
checkout out the [Nebula docs](/concepts/work-pools/).
"""
import asyncio
import contextlib
import os
...
from nebula.utilities.filesystem import relative_path_to_current_platform
...
if TYPE_CHECKING:
    from nebula.client.schemas.objects import Flow
...
class ProcessJobConfiguration(BaseJobConfiguration):
    ...
    def prepare_for_flow_run(
        ...
        self.command = (
            f"{get_sys_executable()} -m nebula.engine"
...
class ProcessWorker(BaseWorker):
    ...
    _documentation_url = (
        "https://docs.nebula.io/latest/api-ref/nebula/workers/process/"
    )
    ...
    async def run(
        ...
        if not command:
            command = f"{get_sys_executable()} -m nebula.engine"
        ...
        working_dir_ctx = (
            tempfile.TemporaryDirectory(suffix="nebula")
