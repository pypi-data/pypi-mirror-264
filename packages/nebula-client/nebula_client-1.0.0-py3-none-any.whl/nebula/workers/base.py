...
from nebula.client.schemas.objects import StateType, WorkPool
...
from nebula.utilities.dispatch import get_registry_for_type, register_base_type
...
class BaseJobConfiguration(BaseModel):
    ...
    def is_using_a_runner(self):
        return self.command is not None and "nebula flow-run execute" in self.command
    ...
    def _base_flow_run_environment(flow_run: "FlowRun") -> Dict[str, str]:
        ...
        """
        return {"NEBULA__FLOW_RUN_ID": str(flow_run.id)}
...
class BaseWorker(abc.ABC):
    ...
    def __init__(
        ...
        *,
        ...
        """
        Base class for all Nebula workers.
        ...
        self._prefetch_seconds: float = (
            prefetch_seconds or NEBULA_WORKER_PREFETCH_SECONDS.value()
        )
        self.heartbeat_interval_seconds = (
            heartbeat_interval_seconds or NEBULA_WORKER_HEARTBEAT_SECONDS.value()
        )
    ...
    def get_flow_run_logger(self, flow_run: "FlowRun") -> NebulaLogAdapter:
    ...
    async def sync_with_backend(self):
        ...
        self._logger.debug("Worker synchronized with the Nebula API server.")
    ...
    def _event_resource(self):
        return {
            "nebula.resource.id": f"nebula.worker.{self.type}.{self.get_name_slug()}",
            "nebula.resource.name": self.name,
            "nebula.version": nebula.__version__,
            "nebula.worker-type": self.type,
        }
    ...
    def _emit_flow_run_submitted_event(
        ...
        return emit_event(
            event="nebula.worker.submitted-flow-run",
    ...
    def _emit_flow_run_executed_event(
        ...
        for resource in related:
            if resource.role == "flow-run":
                resource["nebula.infrastructure.identifier"] = str(result.identifier)
                resource["nebula.infrastructure.status-code"] = str(result.status_code)

        emit_event(
            event="nebula.worker.executed-flow-run",
            resource=self._event_resource(),
            related=related,
    ...
    def _emit_flow_run_cancelled_event(
        ...
        for resource in related:
            if resource.role == "flow-run":
                resource["nebula.infrastructure.identifier"] = str(
                    flow_run.infrastructure_pid
                )

        emit_event(
            event="nebula.worker.cancelled-flow-run",
            resource=self._event_resource(),
            related=related,
        )
