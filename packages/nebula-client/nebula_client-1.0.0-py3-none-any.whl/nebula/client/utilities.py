...
# This module must not import from `nebula.client` when it is imported to avoid
...
def get_or_create_client(
    ...
    if (
        ...
        from nebula.client.orchestration import get_client as get_httpx_client
...
def inject_client(
    ...
    async def with_injected_client(*args: P.args, **kwargs: P.kwargs) -> Any:
        client = cast(Optional["NebulaClient"], kwargs.pop("client", None))
        client, inferred = get_or_create_client(client)
        if not inferred:
            context = client
        else:
            from nebula.utilities.asyncutils import asyncnullcontext
