...
from nebula.client.schemas.objects import CsrfToken
from nebula.exceptions import NebulaHTTPStatusError
from nebula.logging import get_logger
from nebula.settings import (
    NEBULA_CLIENT_MAX_RETRIES,
    NEBULA_CLIENT_RETRY_EXTRA_CODES,
    NEBULA_CLIENT_RETRY_JITTER_FACTOR,
)
from nebula.utilities.math import bounded_poisson_interval, clamped_poisson_interval
...
async def app_lifespan_context(app: ASGIApp) -> AsyncGenerator[None, None]:
    ...
    #       See https://github.com/kozmoai/orion/pull/1696
...
class NebulaResponse(httpx.Response):
    """
    A Nebula wrapper for the `httpx.Response` class.
    ...
    def raise_for_status(self) -> None:
        ...
        The `NebulaHTTPStatusError` contains useful additional information that
        is not contained in the `HTTPStatusError`.
        """
        try:
            return super().raise_for_status()
        except HTTPStatusError as exc:
            raise NebulaHTTPStatusError.from_httpx_error(exc) from exc.__cause__

    @classmethod
    def from_httpx_response(cls: Type[Self], response: httpx.Response) -> Self:
        """
        Create a `NebulaReponse` from an `httpx.Response`.

        By changing the `__class__` attribute of the Response, we change the method
        resolution order to look for methods defined in NebulaResponse, while leaving
...
class NebulaHttpxClient(httpx.AsyncClient):
    """
    A Nebula wrapper for the async httpx client with support for retry-after headers
    ...
    async def _send_with_retry(
        ...
        Sends the provided request and retries it up to NEBULA_CLIENT_MAX_RETRIES times
        ...
        while try_count <= NEBULA_CLIENT_MAX_RETRIES.value():
            try_count += 1
            retry_seconds = None
            exc_info = None
            ...
            try:
                ...
                if try_count > NEBULA_CLIENT_MAX_RETRIES.value():
                    raise
            ...
            # Add jitter
            jitter_factor = NEBULA_CLIENT_RETRY_JITTER_FACTOR.value()
            ...
            logger.debug(
                ...
                "This is attempt"
                f" {try_count}/{NEBULA_CLIENT_MAX_RETRIES.value() + 1}.",
                exc_info=exc_info,
            )
    ...
    async def send(self, request: Request, *args, **kwargs) -> Response:
        ...
        - Any additional status codes provided in `NEBULA_CLIENT_RETRY_EXTRA_CODES`
        """
        ...
        response = await self._send_with_retry(
            ...
            retry_exceptions=(
                ...
                httpx.WriteError,
                # Uvicorn bug, see https://github.com/kozmoai/nebula/issues/7512
                httpx.RemoteProtocolError,
                # HTTP2 bug, see https://github.com/kozmoai/nebula/issues/7442
                httpx.LocalProtocolError,
            ),
        )

        # Convert to a Nebula response to add nicer errors messages
        response = NebulaResponse.from_httpx_response(response)
    ...
    async def _add_csrf_headers(self, request: Request):
        ...
        request.headers["Nebula-Csrf-Token"] = self.csrf_token
        request.headers["Nebula-Csrf-Client"] = str(self.csrf_client_id)
