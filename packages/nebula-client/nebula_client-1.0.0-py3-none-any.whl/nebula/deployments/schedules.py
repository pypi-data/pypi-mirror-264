...
try:
    from nebula.server.schemas.schedules import SCHEDULE_TYPES as SERVER_SCHEDULE_TYPES

    SERVER_SCHEDULE_TYPES = get_args(SERVER_SCHEDULE_TYPES)
except ImportError:
    # `nebula-client` does not have access to the server schemas.
        ...
        for obj in schedules:
            if isinstance(obj, get_args(SCHEDULE_TYPES)):
                ...
                raise ValueError(
                    ...
                    "the schedule objects from `nebula.client.schemas.schedules`"
                )
            else:
                raise ValueError(
