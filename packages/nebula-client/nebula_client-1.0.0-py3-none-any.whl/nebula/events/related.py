...
if TYPE_CHECKING:
    from nebula._internal.schemas.bases import ObjectBaseModel
...
def tags_as_related_resources(tags: Iterable[str]) -> List[RelatedResource]:
    return [
        RelatedResource(
            __root__={
                "nebula.resource.id": f"nebula.tag.{tag}",
                "nebula.resource.role": "tag",
            }
        )
...
def object_as_related_resource(kind: str, role: str, object: Any) -> RelatedResource:
    resource_id = f"nebula.{kind}.{object.id}"

    return RelatedResource(
        __root__={
            "nebula.resource.id": resource_id,
            "nebula.resource.role": role,
            "nebula.resource.name": object.name,
        }
    )
