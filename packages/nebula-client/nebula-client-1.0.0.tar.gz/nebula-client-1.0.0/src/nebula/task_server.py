...
from nebula import Task, get_client
from nebula._internal.concurrency.api import create_call, from_sync
from nebula.client.schemas.objects import TaskRun
from nebula.client.subscriptions import Subscription
from nebula.engine import emit_task_run_state_change_event, propose_state
...
from nebula.settings import (
    ...
    NEBULA_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS,
)
...
from nebula.utilities.asyncutils import asyncnullcontext, sync_compatible
from nebula.utilities.processutils import _register_signal
...
class TaskServer:
            ...
            try:
                ...
                if exc.status_code == 403:
                    logger.error(
                        ...
                        " `NEBULA_API_URL` and `NEBULA_API_KEY` environment variables."
                    )
                else:
                    raise
    ...
    async def _submit_scheduled_task_run(self, task_run: TaskRun):
        ...
        if not task:
            if NEBULA_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS.value():
                logger.warning(
...
async def serve(*tasks: Task, task_runner: Optional[Type[BaseTaskRunner]] = None):
    ...
    Example:
        ...
        ```
    """
    if not NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value():
        raise RuntimeError(
            "To enable task scheduling, set NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING"
            " to True."
        )
