...
For more user-accessible information about the current run, see [`nebula.runtime`](../runtime/flow_run).
"""
import os
import sys
import warnings
...
from nebula.client.schemas import FlowRun, TaskRun
from nebula.events.worker import EventsWorker
...
from nebula.settings import NEBULA_HOME, Profile, Settings
from nebula.states import State
from nebula.task_runners import BaseTaskRunner
from nebula.utilities.importtools import load_script_as_module
...
class NebulaObjectRegistry(ContextModel):
    """
    A context that acts as a registry for all Nebula objects that are
    ...
    def register_instances(cls, type_: Type[T]) -> Type[T]:
        """
        Decorator for a class that adds registration to the `NebulaObjectRegistry`
...
class RunContext(ContextModel):
    ...
    Attributes:
        ...
        client: The Nebula client instance being used for API communication
    """
...
class EngineContext(RunContext):
    ...
    task_run_futures: List[NebulaFuture] = Field(default_factory=list)
    ...
    # Events worker to emit events to Nebula Cloud
...
class TagsContext(ContextModel):
    """
    The context for `nebula.tags` management.
...
class SettingsContext(ContextModel):
    """
    The context for a Nebula settings.
    ...
    def __enter__(self):
        ...
        try:
            nebula_home = Path(self.settings.value_of(NEBULA_HOME))
            nebula_home.mkdir(mode=0o0700, exist_ok=True)
        except OSError:
            warnings.warn(
                (
                    "Failed to create the Nebula home directory at "
                    f"{self.settings.value_of(NEBULA_HOME)}"
                ),
                stacklevel=2,
            )
...
def get_settings_context() -> SettingsContext:
    ...
    profile and environment. See `nebula.context.use_profile` for more details.
    """
...
def tags(*new_tags: str) -> Set[str]:
    ...
    Examples:
        >>> from nebula import tags, task, flow
        >>> @task
        >>> def my_task():
        >>>     pass
...
def use_profile(
    ...
    if isinstance(profile, str):
        profiles = nebula.settings.load_profiles()
    ...
    if not override_environment_variables:
        for key in os.environ:
            if key in nebula.settings.SETTING_VARIABLES:
                profile_settings.pop(nebula.settings.SETTING_VARIABLES[key], None)
...
def root_settings_context():
    ...
    - Command line via 'nebula --profile <name>'
    - Environment variable via 'NEBULA_PROFILE'
    ...
    if "NEBULA_PROFILE" in os.environ:
        active_name = os.environ["NEBULA_PROFILE"]
        profile_source = "by environment variable"

    if (
        sys.argv[0].endswith("/nebula")
        and len(sys.argv) >= 3
...
GLOBAL_OBJECT_REGISTRY: ContextManager[NebulaObjectRegistry] = None
...
def initialize_object_registry():
    ...
    GLOBAL_OBJECT_REGISTRY = NebulaObjectRegistry()
