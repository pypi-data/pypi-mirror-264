...
The `SettingsContext` is set when the `nebula` module is imported. This context is
...
management should be left to Nebula application internals.
...
from nebula._internal.compatibility.deprecated import generate_deprecation_message
from nebula._internal.pydantic import HAS_PYDANTIC_V2
from nebula.exceptions import MissingProfileError
from nebula.utilities.names import OBFUSCATED_PREFIX, obfuscate
from nebula.utilities.pydantic import add_cloudpickle_reduction
...
REMOVED_EXPERIMENTAL_FLAGS = {
    ...
    "NEBULA_EXPERIMENTAL_ENABLE_ENHANCED_DEPLOYMENT_PARAMETERS",
}
...
class Setting(Generic[T]):
    ...
    def value_from(self, settings: "Settings", bypass_callback: bool = False) -> T:
        ...
        Example:
        ```python
        from nebula.settings import get_default_settings
        NEBULA_API_URL.value_from(get_default_settings())
        ```
        """
    ...
    def name(self):
        ...
        raise ValueError("Setting not found in `nebula.settings` module.")
...
def get_extra_loggers(_: "Settings", value: str) -> List[str]:
    """
    `value_callback` for `NEBULA_LOGGING_EXTRA_LOGGERS`that parses the CSV string into a
...
def debug_mode_log_level(settings, value):
    """
    `value_callback` for `NEBULA_LOGGING_LEVEL` that overrides the log level to DEBUG
    when debug mode is enabled.
    """
    if NEBULA_DEBUG_MODE.value_from(settings):
        return "DEBUG"
    else:
        return value
...
def only_return_value_in_test_mode(settings, value):
    """
    `value_callback` for `NEBULA_TEST_SETTING` that only allows access during test mode
    """
    if NEBULA_TEST_MODE.value_from(settings):
        return value
    else:
        return None
...
def default_ui_api_url(settings, value):
    """
    `value_callback` for `NEBULA_UI_API_URL` that sets the default value to
    ...
    return template_with_settings(
        NEBULA_SERVER_API_HOST, NEBULA_SERVER_API_PORT, NEBULA_API_URL
    )(settings, value)


def status_codes_as_integers_in_range(_, value):
    """
    `value_callback` for `NEBULA_CLIENT_RETRY_EXTRA_CODES` that ensures status codes
    ...
    if any(not v.isdigit() or int(v) < 100 or int(v) > 599 for v in values):
        raise ValueError(
            "NEBULA_CLIENT_RETRY_EXTRA_CODES must be a comma separated list of "
...
def max_log_size_smaller_than_batch_size(values):
    ...
    """
    if (
        values["NEBULA_LOGGING_TO_API_BATCH_SIZE"]
        < values["NEBULA_LOGGING_TO_API_MAX_LOG_SIZE"]
    ):
        raise ValueError(
            "`NEBULA_LOGGING_TO_API_MAX_LOG_SIZE` cannot be larger than"
...
def warn_on_database_password_value_without_usage(values):
    ...
    """
    value = values["NEBULA_API_DATABASE_PASSWORD"]
    if (
        value
        and not value.startswith(OBFUSCATED_PREFIX)
        and (
            "NEBULA_API_DATABASE_PASSWORD"
            not in values["NEBULA_API_DATABASE_CONNECTION_URL"]
        )
    ):
        warnings.warn(
...
def check_for_deprecated_cloud_url(settings, value):
    deprecated_value = NEBULA_CLOUD_URL.value_from(settings, bypass_callback=True)
    if deprecated_value is not None:
        warnings.warn(
            (
                "`NEBULA_CLOUD_URL` is set and will be used instead of"
                " `NEBULA_CLOUD_API_URL` for backwards compatibility."
                " `NEBULA_CLOUD_URL` is deprecated, set `NEBULA_CLOUD_API_URL`"
                " instead."
            ),
            DeprecationWarning,
        )
...
def warn_on_misconfigured_api_url(values):
    ...
    """
    api_url = values["NEBULA_API_URL"]
    if api_url is not None:
        misconfigured_mappings = {
            "app.nebula.cloud": (
                "`NEBULA_API_URL` points to `app.nebula.cloud`. Did you"
                " mean `api.nebula.cloud`?"
            ),
            "account/": (
                "`NEBULA_API_URL` uses `/account/` but should use `/accounts/`."
            ),
            "workspace/": (
                "`NEBULA_API_URL` uses `/workspace/` but should use `/workspaces/`."
            ),
        }
        warnings_list = []
        ...
        if parsed_url.path and not parsed_url.path.startswith("/api"):
            warnings_list.append(
                "`NEBULA_API_URL` should have `/api` after the base URL."
            )

        if warnings_list:
            example = 'e.g. NEBULA_API_URL="https://api.nebula.cloud/api/accounts/[ACCOUNT-ID]/workspaces/[WORKSPACE-ID]"'
...
def default_database_connection_url(settings, value):
    templater = template_with_settings(NEBULA_HOME, NEBULA_API_DATABASE_PASSWORD)
    ...
    home = NEBULA_HOME.value_from(settings)

    old_default = home / "orion.db"
    new_default = home / "nebula.db"
...
def default_ui_url(settings, value):
    ...
    ui_url = api_url = NEBULA_API_URL.value_from(settings)

    if not api_url:
        return None

    cloud_url = NEBULA_CLOUD_API_URL.value_from(settings)
    cloud_ui_url = NEBULA_CLOUD_UI_URL.value_from(settings)
...
def default_cloud_ui_url(settings, value):
    ...
    # Otherwise, infer a value from the API URL
    ui_url = api_url = NEBULA_CLOUD_API_URL.value_from(settings)

    if api_url.startswith("https://api.nebula.cloud"):
        ui_url = ui_url.replace(
            "https://api.nebula.cloud", "https://app.nebula.cloud", 1
        )
...
"""Nebula's home directory. Defaults to `~/.nebula`. This
directory may be created automatically when required.
"""

NEBULA_EXTRA_ENTRYPOINTS = Setting(
    str,
    default="",
)
"""
Modules for Nebula to import when Nebula is imported.
...
If accessed when `NEBULA_TEST_MODE` is not set, `None` is returned.
"""

NEBULA_API_TLS_INSECURE_SKIP_VERIFY = Setting(
    bool,
    default=False,
)
...
NEBULA_API_SSL_CERT_FILE = Setting(
    str,
...
NEBULA_API_URL = Setting(
    str,
    default=None,
)
"""
If provided, the URL of a hosted Nebula API. Defaults to `None`.

When using Nebula Cloud, this will include an account and workspace.
"""

NEBULA_SILENCE_API_URL_MISCONFIGURATION = Setting(
    bool,
    default=False,
)
"""If `True`, disable the warning when a user accidentally misconfigure its `NEBULA_API_URL`
Sometimes when a user manually set `NEBULA_API_URL` to a custom url,reverse-proxy for example,
we would like to silence this warning so we will set it to `FALSE`.
"""

NEBULA_API_KEY = Setting(
    str,
    default=None,
    is_secret=True,
)
"""API key used to authenticate with a the Nebula API. Defaults to `None`."""

NEBULA_API_ENABLE_HTTP2 = Setting(bool, default=True)
"""
...
NEBULA_CLIENT_MAX_RETRIES = Setting(int, default=5)
"""
...
See `NEBULA_CLIENT_RETRY_EXTRA_CODES` for details on which HTTP status codes are
retried.
"""

NEBULA_CLIENT_RETRY_JITTER_FACTOR = Setting(float, default=0.2)
"""
...
NEBULA_CLIENT_RETRY_EXTRA_CODES = Setting(
...
NEBULA_CLIENT_CSRF_SUPPORT_ENABLED = Setting(bool, default=True)
"""
Determines if CSRF token handling is active in the Nebula client for API
requests.
...
NEBULA_CLOUD_API_URL = Setting(
    str,
    default="https://api.nebula.cloud/api",
    value_callback=check_for_deprecated_cloud_url,
)
"""API URL for Nebula Cloud. Used for authentication."""


NEBULA_CLOUD_URL = Setting(
    str,
    default=None,
    deprecated=True,
    deprecated_start_date="Dec 2022",
    deprecated_help="Use `NEBULA_CLOUD_API_URL` instead.",
)
"""
DEPRECATED: Use `NEBULA_CLOUD_API_URL` instead.
"""

NEBULA_UI_URL = Setting(
    Optional[str],
    default=None,
    value_callback=default_ui_url,
)
"""
The URL for the UI. By default, this is inferred from the NEBULA_API_URL.

When using Nebula Cloud, this will include the account and workspace.
When using an ephemeral server, this will be `None`.
"""


NEBULA_CLOUD_UI_URL = Setting(
    str,
    default=None,
    value_callback=default_cloud_ui_url,
)
"""
The URL for the Cloud UI. By default, this is inferred from the NEBULA_CLOUD_API_URL.

Note: NEBULA_UI_URL will be workspace specific and will be usable in the open source too.
      In contrast, this value is only valid for Cloud and will not include the workspace.
"""

NEBULA_API_REQUEST_TIMEOUT = Setting(
    float,
    default=60.0,
)
"""The default timeout for requests to the API"""

NEBULA_EXPERIMENTAL_WARN = Setting(bool, default=True)
"""
If enabled, warn on usage of experimental features.
"""

NEBULA_PROFILES_PATH = Setting(
    Path,
    default=Path("${NEBULA_HOME}") / "profiles.toml",
    value_callback=template_with_settings(NEBULA_HOME),
)
"""The path to a profiles configuration files."""

NEBULA_RESULTS_DEFAULT_SERIALIZER = Setting(
    str,
    default="pickle",
)
...
NEBULA_RESULTS_PERSIST_BY_DEFAULT = Setting(
    bool,
    default=False,
)
"""
...
NEBULA_TASKS_REFRESH_CACHE = Setting(
    bool,
    default=False,
)
"""
...
NEBULA_TASK_DEFAULT_RETRIES = Setting(int, default=0)
"""
...
NEBULA_FLOW_DEFAULT_RETRIES = Setting(int, default=0)
"""
...
NEBULA_FLOW_DEFAULT_RETRY_DELAY_SECONDS = Setting(Union[int, float], default=0)
"""
This value sets the retry delay seconds for all flows.
This value does not overwrite individually set retry delay seconds
"""

NEBULA_TASK_DEFAULT_RETRY_DELAY_SECONDS = Setting(
...
NEBULA_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS = Setting(int, default=30)
"""
The number of seconds to wait before retrying when a task run
cannot secure a concurrency slot from the server.
"""

NEBULA_LOCAL_STORAGE_PATH = Setting(
    Path,
    default=Path("${NEBULA_HOME}") / "storage",
    value_callback=template_with_settings(NEBULA_HOME),
)
"""The path to a block storage directory to store things in."""

NEBULA_MEMO_STORE_PATH = Setting(
    Path,
    default=Path("${NEBULA_HOME}") / "memo_store.toml",
    value_callback=template_with_settings(NEBULA_HOME),
)
"""The path to the memo store file."""

NEBULA_MEMOIZE_BLOCK_AUTO_REGISTRATION = Setting(
    bool,
    default=True,
)
"""
...
NEBULA_LOGGING_LEVEL = Setting(
    str,
    default="INFO",
    value_callback=debug_mode_log_level,
)
"""
The default logging level for Nebula loggers. Defaults to
"INFO" during normal operation. Is forced to "DEBUG" during debug mode.
"""


NEBULA_LOGGING_INTERNAL_LEVEL = Setting(
    str,
    default="ERROR",
    value_callback=debug_mode_log_level,
)
"""
The default logging level for Nebula's internal machinery loggers. Defaults to
"ERROR" during normal operation. Is forced to "DEBUG" during debug mode.
"""

NEBULA_LOGGING_SERVER_LEVEL = Setting(
    str,
    default="WARNING",
)
"""The default logging level for the Nebula API server."""

NEBULA_LOGGING_SETTINGS_PATH = Setting(
    Path,
    default=Path("${NEBULA_HOME}") / "logging.yml",
    value_callback=template_with_settings(NEBULA_HOME),
)
"""
The path to a custom YAML logging configuration file. If
no file is found, the default `logging.yml` is used.
Defaults to a logging.yml in the Nebula home directory.
"""

NEBULA_LOGGING_EXTRA_LOGGERS = Setting(
    str,
    default="",
    value_callback=get_extra_loggers,
)
"""
Additional loggers to attach to Nebula logging at runtime.
Values should be comma separated. The handlers attached to the 'nebula' logger
will be added to these loggers. Additionally, if the level is not set, it will
be set to the same level as the 'nebula' logger.
"""

NEBULA_LOGGING_LOG_PRINTS = Setting(
    bool,
    default=False,
)
"""
If set, `print` statements in flows and tasks will be redirected to the Nebula logger
for the given run. This setting can be overridden by individual tasks and flows.
"""

NEBULA_LOGGING_TO_API_ENABLED = Setting(
    bool,
    default=True,
)
"""
...
NEBULA_LOGGING_TO_API_BATCH_INTERVAL = Setting(float, default=2.0)
"""The number of seconds between batched writes of logs to the API."""

NEBULA_LOGGING_TO_API_BATCH_SIZE = Setting(
    int,
    default=4_000_000,
)
"""The maximum size in bytes for a batch of logs."""

NEBULA_LOGGING_TO_API_MAX_LOG_SIZE = Setting(
    int,
    default=1_000_000,
)
"""The maximum size in bytes for a single log."""

NEBULA_LOGGING_TO_API_WHEN_MISSING_FLOW = Setting(
...
NEBULA_SQLALCHEMY_POOL_SIZE = Setting(
    int,
    default=None,
)
"""
Controls connection pool size when using a PostgreSQL database with the Nebula API. If not set, the default SQLAlchemy pool size will be used.
"""

NEBULA_SQLALCHEMY_MAX_OVERFLOW = Setting(
    int,
    default=None,
)
"""
Controls maximum overflow of the connection pool when using a PostgreSQL database with the Nebula API. If not set, the default SQLAlchemy maximum overflow value will be used.
"""

NEBULA_LOGGING_COLORS = Setting(
    bool,
    default=True,
)
"""Whether to style console logs with color."""

NEBULA_LOGGING_MARKUP = Setting(
    bool,
    default=False,
)
"""
...
NEBULA_TASK_INTROSPECTION_WARN_THRESHOLD = Setting(
    float,
    default=10.0,
)
"""
...
and each element needs to be inspected. See `nebula.utilities.annotations.quote`
for more details.
Defaults to `10.0`.
Set to `0` to disable logging the warning.
"""

NEBULA_AGENT_QUERY_INTERVAL = Setting(
    float,
    default=15,
)
"""
...
NEBULA_AGENT_PREFETCH_SECONDS = Setting(
    int,
    default=15,
)
"""
...
NEBULA_ASYNC_FETCH_STATE_RESULT = Setting(bool, default=False)
"""
Determines whether `State.result()` fetches results automatically or not.
In Nebula 2.6.0, the `State.result()` method was updated to be async
...
NEBULA_API_BLOCKS_REGISTER_ON_START = Setting(
    bool,
    default=True,
)
"""
...
NEBULA_API_DATABASE_PASSWORD = Setting(
    str,
    default=None,
    is_secret=True,
)
"""
Password to template into the `NEBULA_API_DATABASE_CONNECTION_URL`.
...
NEBULA_API_DATABASE_CONNECTION_URL = Setting(
    str,
    default=None,
...
format. Nebula currently supports SQLite and Postgres. Note that all
Nebula database engines must use an async driver - for SQLite, use
...
Defaults to a sqlite database stored in the Nebula home directory.

If you need to provide password via a different environment variable, you use
the `NEBULA_API_DATABASE_PASSWORD` setting. For example:

```
NEBULA_API_DATABASE_PASSWORD='mypassword'
NEBULA_API_DATABASE_CONNECTION_URL='postgresql+asyncpg://postgres:${NEBULA_API_DATABASE_PASSWORD}@localhost/nebula'
```
"""

NEBULA_API_DATABASE_ECHO = Setting(
    bool,
    default=False,
)
"""If `True`, SQLAlchemy will log all SQL issued to the database. Defaults to `False`."""

NEBULA_API_DATABASE_MIGRATE_ON_START = Setting(
    bool,
    default=True,
)
"""If `True`, the database will be upgraded on application creation. If `False`, the database will need to be upgraded manually."""

NEBULA_API_DATABASE_TIMEOUT = Setting(
    Optional[float],
    default=10.0,
)
"""
...
NEBULA_API_DATABASE_CONNECTION_TIMEOUT = Setting(
    Optional[float],
    default=5,
)
...
NEBULA_API_SERVICES_SCHEDULER_LOOP_SECONDS = Setting(
    float,
    default=60,
)
...
NEBULA_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE = Setting(
    int,
    default=100,
)
...
NEBULA_API_SERVICES_SCHEDULER_MAX_RUNS = Setting(
    int,
    default=100,
)
...
NEBULA_API_SERVICES_SCHEDULER_MIN_RUNS = Setting(
    int,
    default=3,
)
...
NEBULA_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME = Setting(
    timedelta,
...
NEBULA_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME = Setting(
    timedelta,
...
NEBULA_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE = Setting(
    int,
    default=500,
)
...
NEBULA_API_SERVICES_LATE_RUNS_LOOP_SECONDS = Setting(
    float,
    default=5,
)
...
NEBULA_API_SERVICES_LATE_RUNS_AFTER_SECONDS = Setting(
    timedelta,
...
NEBULA_API_SERVICES_PAUSE_EXPIRATIONS_LOOP_SECONDS = Setting(
    float,
    default=5,
)
...
NEBULA_API_SERVICES_CANCELLATION_CLEANUP_LOOP_SECONDS = Setting(
    float,
    default=20,
)
...
NEBULA_API_DEFAULT_LIMIT = Setting(
    int,
    default=200,
)
...
NEBULA_SERVER_API_HOST = Setting(
    str,
    default="127.0.0.1",
)
"""The API's host address (defaults to `127.0.0.1`)."""

NEBULA_SERVER_API_PORT = Setting(
    int,
    default=4200,
)
"""The API's port address (defaults to `4200`)."""

NEBULA_SERVER_API_KEEPALIVE_TIMEOUT = Setting(
    int,
    default=5,
)
"""
...
Note this setting only applies when calling `nebula server start`; if hosting the
API with another tool you will need to configure this there instead.
"""

NEBULA_SERVER_CSRF_PROTECTION_ENABLED = Setting(bool, default=False)
"""
Controls the activation of CSRF protection for the Nebula server API.
...
CSRF token management. See NEBULA_CLIENT_CSRF_SUPPORT_ENABLED for more.
"""

NEBULA_SERVER_CSRF_TOKEN_EXPIRATION = Setting(timedelta, default=timedelta(hours=1))
"""
...
NEBULA_UI_ENABLED = Setting(
    bool,
    default=True,
)
"""Whether or not to serve the Nebula UI."""

NEBULA_UI_API_URL = Setting(
    str,
    default=None,
    value_callback=default_ui_api_url,
)
"""The connection url for communication from the UI to the API.
Defaults to `NEBULA_API_URL` if set. Otherwise, the default URL is generated from
`NEBULA_SERVER_API_HOST` and `NEBULA_SERVER_API_PORT`. If providing a custom value,
the aforementioned settings may be templated into the given string.
"""

NEBULA_SERVER_ANALYTICS_ENABLED = Setting(
    bool,
    default=True,
)
"""
When enabled, Nebula sends anonymous data (e.g. count of flow runs, package version)
on server startup to help us improve our product.
"""

NEBULA_API_SERVICES_SCHEDULER_ENABLED = Setting(
    bool,
    default=True,
)
...
NEBULA_API_SERVICES_LATE_RUNS_ENABLED = Setting(
    bool,
    default=True,
)
...
NEBULA_API_SERVICES_FLOW_RUN_NOTIFICATIONS_ENABLED = Setting(
    bool,
    default=True,
)
...
NEBULA_API_SERVICES_PAUSE_EXPIRATIONS_ENABLED = Setting(
    bool,
    default=True,
)
...
NEBULA_API_TASK_CACHE_KEY_MAX_LENGTH = Setting(int, default=2000)
"""
...
NEBULA_API_SERVICES_CANCELLATION_CLEANUP_ENABLED = Setting(
    bool,
    default=True,
)
...
NEBULA_API_MAX_FLOW_RUN_GRAPH_NODES = Setting(int, default=10000)
"""
The maximum size of a flow run graph on the v2 API
"""

NEBULA_API_MAX_FLOW_RUN_GRAPH_ARTIFACTS = Setting(int, default=10000)
"""
The maximum number of artifacts to show on a flow run graph on the v2 API
"""

NEBULA_EXPERIMENTAL_ENABLE_ARTIFACTS_ON_FLOW_RUN_GRAPH = Setting(bool, default=True)
"""
Whether or not to enable artifacts on the flow run graph.
"""

NEBULA_EXPERIMENTAL_ENABLE_STATES_ON_FLOW_RUN_GRAPH = Setting(bool, default=True)
"""
Whether or not to enable flow run states on the flow run graph.
"""

NEBULA_EXPERIMENTAL_ENABLE_EVENTS_CLIENT = Setting(bool, default=True)
"""
Whether or not to enable experimental Nebula work pools.
"""

NEBULA_EXPERIMENTAL_WARN_EVENTS_CLIENT = Setting(bool, default=False)
"""
Whether or not to warn when experimental Nebula work pools are used.
"""

NEBULA_EXPERIMENTAL_ENABLE_WORK_POOLS = Setting(bool, default=True)
"""
Whether or not to enable experimental Nebula work pools.
"""

NEBULA_EXPERIMENTAL_WARN_WORK_POOLS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Nebula work pools are used.
"""

NEBULA_EXPERIMENTAL_ENABLE_WORKERS = Setting(bool, default=True)
"""
Whether or not to enable experimental Nebula workers.
"""

NEBULA_EXPERIMENTAL_WARN_WORKERS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Nebula workers are used.
"""

NEBULA_EXPERIMENTAL_WARN_VISUALIZE = Setting(bool, default=False)
"""
Whether or not to warn when experimental Nebula visualize is used.
"""

NEBULA_EXPERIMENTAL_ENABLE_ENHANCED_CANCELLATION = Setting(bool, default=True)
"""
Whether or not to enable experimental enhanced flow run cancellation.
"""

NEBULA_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION = Setting(bool, default=False)
"""
Whether or not to warn when experimental enhanced flow run cancellation is used.
"""

NEBULA_EXPERIMENTAL_ENABLE_DEPLOYMENT_STATUS = Setting(bool, default=True)
"""
Whether or not to enable deployment status in the UI
"""

NEBULA_EXPERIMENTAL_WARN_DEPLOYMENT_STATUS = Setting(bool, default=False)
"""
Whether or not to warn when deployment status is used.
"""

NEBULA_EXPERIMENTAL_FLOW_RUN_INPUT = Setting(bool, default=False)
"""
Whether or not to enable flow run input.
"""

NEBULA_EXPERIMENTAL_WARN_FLOW_RUN_INPUT = Setting(bool, default=True)
"""
Whether or not to enable flow run input.
"""

NEBULA_RUNNER_PROCESS_LIMIT = Setting(int, default=5)
"""
Maximum number of processes a runner will execute in parallel.
"""

NEBULA_RUNNER_POLL_FREQUENCY = Setting(int, default=10)
"""
Number of seconds a runner should wait between queries for scheduled work.
"""

NEBULA_RUNNER_SERVER_MISSED_POLLS_TOLERANCE = Setting(int, default=2)
"""
Number of missed polls before a runner is considered unhealthy by its webserver.
"""

NEBULA_RUNNER_SERVER_HOST = Setting(str, default="localhost")
"""
The host address the runner's webserver should bind to.
"""

NEBULA_RUNNER_SERVER_PORT = Setting(int, default=8080)
"""
The port the runner's webserver should bind to.
"""

NEBULA_RUNNER_SERVER_LOG_LEVEL = Setting(str, default="error")
"""
The log level of the runner's webserver.
"""

NEBULA_RUNNER_SERVER_ENABLE = Setting(bool, default=False)
"""
Whether or not to enable the runner's webserver.
"""

NEBULA_WORKER_HEARTBEAT_SECONDS = Setting(float, default=30)
"""
Number of seconds a worker should wait between sending a heartbeat.
"""

NEBULA_WORKER_QUERY_SECONDS = Setting(float, default=10)
"""
Number of seconds a worker should wait between queries for scheduled flow runs.
"""

NEBULA_WORKER_PREFETCH_SECONDS = Setting(float, default=10)
"""
...
NEBULA_WORKER_WEBSERVER_HOST = Setting(
    str,
    default="0.0.0.0",
)
"""
...
NEBULA_WORKER_WEBSERVER_PORT = Setting(
    int,
    default=8080,
)
"""
...
NEBULA_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK = Setting(
    str,
    default="local-file-system/nebula-task-scheduling",
)
"""The `block-type/block-document` slug of a block to use as the default storage
for autonomous tasks."""

NEBULA_TASK_SCHEDULING_DELETE_FAILED_SUBMISSIONS = Setting(
    bool,
    default=True,
)
"""
...
NEBULA_TASK_SCHEDULING_MAX_SCHEDULED_QUEUE_SIZE = Setting(
    int,
    default=1000,
)
"""
...
NEBULA_TASK_SCHEDULING_MAX_RETRY_QUEUE_SIZE = Setting(
    int,
    default=100,
)
"""
...
NEBULA_TASK_SCHEDULING_PENDING_TASK_TIMEOUT = Setting(
    timedelta,
...
NEBULA_EXPERIMENTAL_ENABLE_FLOW_RUN_INFRA_OVERRIDES = Setting(bool, default=False)
"""
Whether or not to enable infrastructure overrides made on flow runs.
"""

NEBULA_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES = Setting(bool, default=True)
"""
Whether or not to warn infrastructure when experimental flow runs overrides are used.
"""

NEBULA_EXPERIMENTAL_ENABLE_EXTRA_RUNNER_ENDPOINTS = Setting(bool, default=False)
"""
Whether or not to enable experimental worker webserver endpoints.
"""

NEBULA_EXPERIMENTAL_ENABLE_ARTIFACTS = Setting(bool, default=True)
"""
Whether or not to enable experimental Nebula artifacts.
"""

NEBULA_EXPERIMENTAL_WARN_ARTIFACTS = Setting(bool, default=False)
"""
Whether or not to warn when experimental Nebula artifacts are used.
"""

NEBULA_EXPERIMENTAL_ENABLE_WORKSPACE_DASHBOARD = Setting(bool, default=True)
"""
Whether or not to enable the experimental workspace dashboard.
"""

NEBULA_EXPERIMENTAL_WARN_WORKSPACE_DASHBOARD = Setting(bool, default=False)
"""
Whether or not to warn when the experimental workspace dashboard is enabled.
"""

NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING = Setting(bool, default=False)
"""
Whether or not to enable experimental task scheduling.
"""

NEBULA_EXPERIMENTAL_ENABLE_WORK_QUEUE_STATUS = Setting(bool, default=True)
"""
Whether or not to enable experimental work queue status in-place of work queue health.
"""

NEBULA_EXPERIMENTAL_ENABLE_PYDANTIC_V2_INTERNALS = Setting(bool, default=False)
"""
...
NEBULA_DEFAULT_RESULT_STORAGE_BLOCK = Setting(
    str,
    default=None,
)
"""The `block-type/block-document` slug of a block to use as the default result storage."""

NEBULA_DEFAULT_WORK_POOL_NAME = Setting(str, default=None)
"""
The default work pool to deploy to.
"""

NEBULA_DEFAULT_DOCKER_BUILD_NAMESPACE = Setting(
    str,
    default=None,
)
"""
...
NEBULA_UI_SERVE_BASE = Setting(
    str,
    default="/",
)
"""
The base URL path to serve the Nebula UI from.

Defaults to the root path.
"""

NEBULA_UI_STATIC_DIRECTORY = Setting(
    str,
    default=None,
)
"""
...
class Settings(SettingsFieldsMixin):
    """
    Contains validated Nebula settings.
    ...
    Settings().NEBULA_PROFILES_PATH  # PosixPath('${NEBULA_HOME}/profiles.toml')
    ```
    """
    ...
    @validator(NEBULA_LOGGING_LEVEL.name, NEBULA_LOGGING_SERVER_LEVEL.name)
    ...
    def post_root_validators(cls, values):
        ...
        if not values["NEBULA_SILENCE_API_URL_MISCONFIGURATION"]:
...
def get_current_settings() -> Settings:
    ...
    """
    from nebula.context import SettingsContext
...
def temporary_settings(
    ...
    Examples:
        >>> from nebula.settings import NEBULA_API_URL
        >>>
        >>> with temporary_settings(updates={NEBULA_API_URL: "foo"}):
        >>>    assert NEBULA_API_URL.value() == "foo"
        >>>
        >>>    with temporary_settings(set_defaults={NEBULA_API_URL: "bar"}):
        >>>         assert NEBULA_API_URL.value() == "foo"
        >>>
        >>>    with temporary_settings(restore_defaults={NEBULA_API_URL}):
        >>>         assert NEBULA_API_URL.value() is None
        >>>
        >>>         with temporary_settings(set_defaults={NEBULA_API_URL: "bar"})
        >>>             assert NEBULA_API_URL.value() == "bar"
        >>> assert NEBULA_API_URL.value() is None
    """
    import nebula.context

    context = nebula.context.get_settings_context()
    ...
    with nebula.context.SettingsContext(profile=context.profile, settings=new_settings):
        yield new_settings
...
def load_profiles() -> ProfilesCollection:
    ...
    user_profiles_path = NEBULA_PROFILES_PATH.value()
...
def save_profiles(profiles: ProfilesCollection) -> None:
    ...
    """
    profiles_path = NEBULA_PROFILES_PATH.value()
...
def update_current_profile(settings: Dict[Union[str, Setting], Any]) -> Profile:
    ...
    current_profile = nebula.context.get_settings_context().profile
