...
def get_nebula_image_name(
    nebula_version: str = None, python_version: str = None, flavor: str = None
) -> str:
    """
    Get the Nebula image name matching the current Nebula and Python versions.

    Args:
        nebula_version: An optional override for the Nebula version.
    ...
    """
    parsed_version = (nebula_version or nebula.__version__).split("+")
    is_prod_build = len(parsed_version) == 1
    nebula_version = (
        parsed_version[0]
        if is_prod_build
        else "sha-" + nebula.__version_info__["full-revisionid"][:7]
    )

    python_version = python_version or python_version_minor()

    tag = slugify(
        f"{nebula_version}-python{python_version}" + (f"-{flavor}" if flavor else ""),
        lowercase=False,
        max_length=128,
    ...
    image = "nebula" if is_prod_build else "nebula-dev"
    return f"kozmoai/{image}:{tag}"
...
# Labels to apply to all images built with Nebula
IMAGE_LABELS = {
...
def parse_image_tag(name: str) -> Tuple[str, Optional[str]]:
    ...
    - If a tag exists, this function parses and returns the image registry and tag,
      ...
      - Example 1: 'kozmoai/nebula:latest' -> ('kozmoai/nebula', 'latest')
    ...
    if len(name_parts) == 1:
        ...
        # 1. Separates index (hostname.io or kozmoai) from path:tag (folder/subfolder:latest or nebula:latest)
...
def generate_default_dockerfile(context: Optional[Path] = None):
    ...
    if not context:
        context = Path.cwd()
    lines = []
    base_image = get_nebula_image_name()
    lines.append(f"FROM {base_image}")
    dir_name = context.name

    if (context / "requirements.txt").exists():
        lines.append(f"COPY requirements.txt /opt/nebula/{dir_name}/requirements.txt")
        lines.append(
            f"RUN python -m pip install -r /opt/nebula/{dir_name}/requirements.txt"
        )

    lines.append(f"COPY . /opt/nebula/{dir_name}/")
    lines.append(f"WORKDIR /opt/nebula/{dir_name}/")
