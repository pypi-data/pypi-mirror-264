...
- **The flow is called by the user or an existing flow run is executed in a new process.**
    ...
    See `Flow.__call__` and `nebula.engine.__main__` (`python -m nebula.engine`)
...
- **The task run is orchestrated through states, calling the user's function as necessary.**
    ...
    user thread as we do for flows. See [#9855](https://github.com/kozmoai/nebula/pull/9855).
"""
...
from nebula._internal.compatibility.deprecated import deprecated_parameter
from nebula._internal.compatibility.experimental import experimental_parameter
from nebula._internal.concurrency.api import create_call, from_async, from_sync
from nebula._internal.concurrency.calls import get_current_call
from nebula._internal.concurrency.cancellation import CancelledError, get_deadline
from nebula._internal.concurrency.threads import wait_for_global_loop_exit
from nebula.client.orchestration import NebulaClient, get_client
from nebula.client.schemas import FlowRun, OrchestrationResult, TaskRun
from nebula.client.schemas.filters import FlowRunFilter
from nebula.client.schemas.objects import (
    StateDetails,
    StateType,
    TaskRunInput,
    TaskRunResult,
)
from nebula.client.schemas.responses import SetStateStatus
from nebula.client.schemas.sorting import FlowRunSort
from nebula.client.utilities import inject_client
from nebula.context import (
    FlowRunContext,
    NebulaObjectRegistry,
    TagsContext,
    TaskRunContext,
)
from nebula.deployments import load_flow_from_flow_run
from nebula.events import Event, emit_event
...
from nebula.flows import Flow, load_flow_from_entrypoint
from nebula.futures import NebulaFuture, call_repr, resolve_futures_to_states
from nebula.input import keyset_from_paused_state
from nebula.input.run_input import run_input_subclass_from_type
from nebula.logging.configuration import setup_logging
from nebula.logging.handlers import APILogHandler
...
from nebula.results import BaseResult, ResultFactory, UnknownResult
from nebula.settings import (
    NEBULA_DEBUG_MODE,
    NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING,
    NEBULA_LOGGING_LOG_PRINTS,
    NEBULA_TASK_INTROSPECTION_WARN_THRESHOLD,
    NEBULA_TASKS_REFRESH_CACHE,
    NEBULA_UI_URL,
)
...
from nebula.utilities.annotations import allow_failure, quote, unmapped
from nebula.utilities.asyncutils import (
    gather,
    is_async_fn,
    sync_compatible,
)
from nebula.utilities.callables import (
    collapse_variadic_parameters,
    explode_variadic_parameter,
    get_parameter_defaults,
    parameters_to_args_kwargs,
)
from nebula.utilities.collections import StopVisiting, isiterable, visit_collection
from nebula.utilities.pydantic import PartialModel
from nebula.utilities.text import truncated_to
...
def enter_flow_run_engine_from_flow_call(
    ...
    wait_for: Optional[Iterable[NebulaFuture]],
    ...
    registry = NebulaObjectRegistry.get()
...
async def retrieve_flow_then_begin_flow_run(
    ...
    entrypoint = os.environ.get("NEBULA__FLOW_ENTRYPOINT")
...
async def begin_flow_run(
    ...
    display_state = repr(terminal_state) if NEBULA_DEBUG_MODE else str(terminal_state)
...
async def orchestrate_flow_run(
    ...
    while state.is_running():
        ...
        # block storage and send the resulting data document to the Nebula API instead.
        # This prevents the pickled return value of flow runs
        # from being sent to the Nebula API and stored in the Nebula database.
        ...
        if state.type != terminal_state.type and NEBULA_DEBUG_MODE:
            logger.debug(
                (
...
def enter_task_run_engine(
    ...
    mapped: bool,
) -> Union[NebulaFuture, Awaitable[NebulaFuture], TaskRun]:
    ...
    if not flow_run_context:
        if (
            not NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value()
            ...
            raise RuntimeError(
                ...
                " `nebula config set NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING=true`"
                " and use `your_task.submit()` instead of `your_task()`."
                " Mapping autonomous tasks is not yet supported."
            )
        from nebula.task_engine import submit_autonomous_task_run_to_engine
...
) -> List[Union[NebulaFuture, Awaitable[NebulaFuture], TaskRun]]:
...
async def collect_task_run_inputs(expr: Any, max_depth: int = -1) -> Set[TaskRunInput]:
    ...
    def add_futures_and_states_to_inputs(obj):
        if isinstance(obj, NebulaFuture):
...
async def begin_task_run(
    ...
    log_prints: bool,
    settings: nebula.context.SettingsContext,
):
    """
    ...
    remote worker:
    --> Remote worker imports Nebula (may not occur)
    ...
    """
    maybe_flow_run_context = nebula.context.FlowRunContext.get()
    ...
    async with AsyncExitStack() as stack:
        ...
        if nebula.context.SettingsContext.get() != settings:
...
async def orchestrate_task_run(
    ...
    threshold = NEBULA_TASK_INTROSPECTION_WARN_THRESHOLD.value()
    if threshold and introspection_time > threshold:
        logger.warning(
            f"Task parameter introspection took {introspection_time} seconds "
            f", exceeding `NEBULA_TASK_INTROSPECTION_WARN_THRESHOLD` of {threshold}. "
            "Try wrapping large task parameters with "
            "`nebula.utilities.annotations.quote` for increased performance, "
            "e.g. `my_task(quote(param))`. To disable this message set "
            "`NEBULA_TASK_INTROSPECTION_WARN_THRESHOLD=0`."
        )
    ...
    # Setting on task level overrules the Nebula setting (env var)
    refresh_cache = (
        task.refresh_cache
        if task.refresh_cache is not None
        else NEBULA_TASKS_REFRESH_CACHE.value()
    )
    ...
    # If debugging, use the more complete `repr` than the usual `str` description
    display_state = repr(state) if NEBULA_DEBUG_MODE else str(state)
...
async def wait_for_task_runs_and_report_crashes(
    task_run_futures: Iterable[NebulaFuture], client: NebulaClient
) -> Literal[True]:
    crash_exceptions = []
...
@asynccontextmanager
async def report_flow_run_crashes(flow_run: FlowRun, client: NebulaClient, flow: Flow):
    """
...
@asynccontextmanager
async def report_task_run_crashes(task_run: TaskRun, client: NebulaClient):
    """
...
async def resolve_inputs(
    ...
    """
    Resolve any `Quote`, `NebulaFuture`, or `State` types nested in parameters into
    data.
    ...
    def collect_futures_and_states(expr, context):
        ...
        if isinstance(expr, NebulaFuture):
            futures.add(expr)
        if is_state(expr):
            states.add(expr)
...
async def propose_state(
    ...
    """
    Propose a new state for a flow run or task run, invoking Nebula orchestration logic.

    If the proposed state is accepted, the provided `state` will be augmented with
     details and returned.

    If the proposed state is rejected, a new state returned by the Nebula API will be
    returned.

    If the proposed state results in a WAIT instruction from the Nebula API, the
    function will sleep and attempt to propose the state again.

    If the proposed state results in an ABORT instruction from the Nebula API, an
    ...
    Returns:
        a [State model][nebula.client.schemas.objects.State] representation of the
            flow or task run state

    Raises:
        ValueError: if neither task_run_id or flow_run_id is provided
        nebula.exceptions.Abort: if an ABORT instruction is received from
            the Nebula API
    """
    ...
    if response.status == SetStateStatus.ACCEPT:
        ...
        raise nebula.exceptions.Abort(response.details.reason)
...
def link_state_to_result(state: State, result: Any) -> None:
    ...
    We do not set an attribute, e.g. `__nebula_state__`, on the result because:
...
def should_log_prints(flow_or_task: Union[Flow, Task]) -> bool:
    ...
    if flow_or_task.log_prints is None:
        if flow_run_context:
            ...
            return NEBULA_LOGGING_LOG_PRINTS.value()
...
async def _run_flow_hooks(flow: Flow, flow_run: FlowRun, state: State) -> None:
    ...
    enable_cancellation_and_crashed_hooks = (
        os.environ.get("NEBULA__ENABLE_CANCELLATION_AND_CRASHED_HOOKS", "true").lower()
        == "true"
    )
...
async def check_api_reachable(client: NebulaClient, fail_message: str):
...
def emit_task_run_state_change_event(
    ...
    return emit_event(
        ...
        event=f"nebula.task-run.{validated_state.name}",
        payload={
            "intended": {
        ...
        resource={
            "nebula.resource.id": f"nebula.task-run.{task_run.id}",
            "nebula.resource.name": task_run.name,
            "nebula.state-message": truncated_to(
                state_message_truncation_length, validated_state.message
            ),
            "nebula.state-name": validated_state.name or "",
            "nebula.state-timestamp": (
                validated_state.timestamp.isoformat()
                if validated_state and validated_state.timestamp
                else ""
            ),
            "nebula.state-type": str(validated_state.type.value),
        },
        follows=follows,
    )
...
if __name__ == "__main__":
    try:
        flow_run_id = UUID(
            sys.argv[1] if len(sys.argv) > 1 else os.environ.get("NEBULA__FLOW_RUN_ID")
        )
    except Exception:
        engine_logger.error(
