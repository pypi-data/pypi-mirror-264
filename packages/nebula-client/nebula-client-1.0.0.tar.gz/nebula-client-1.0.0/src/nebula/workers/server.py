...
import uvicorn
from nebula._vendor.fastapi import APIRouter, FastAPI, status
from nebula._vendor.fastapi.responses import JSONResponse

from nebula.settings import (
    NEBULA_WORKER_WEBSERVER_HOST,
    NEBULA_WORKER_WEBSERVER_PORT,
)
from nebula.workers.base import BaseWorker
from nebula.workers.process import ProcessWorker
...
def start_healthcheck_server(
    ...
    uvicorn.run(
        webserver,
        host=NEBULA_WORKER_WEBSERVER_HOST.value(),
        port=NEBULA_WORKER_WEBSERVER_PORT.value(),
        log_level=log_level,
    )
