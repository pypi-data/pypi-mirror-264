...
from nebula.client.schemas.objects import BlockDocument
from nebula.infrastructure.base import Infrastructure
from nebula.utilities.collections import get_from_dict
from nebula.workers.base import BaseWorker, BaseWorkerResult
...
from nebula.events.related import object_as_related_resource, tags_as_related_resources
from nebula.events.schemas import RelatedResource
from nebula.utilities.templating import (
    apply_values,
)

if TYPE_CHECKING:
    from nebula.client.schemas.objects import Flow, FlowRun
...
class BlockWorkerJobConfiguration(BaseModel):
    ...
    def is_using_a_runner(self):
        return (
            ...
            and "nebula flow-run execute" in shlex.join(self.block.command)
        )
    ...
    async def from_template_and_values(
        cls, base_job_template: dict, values: dict, client: "NebulaClient" = None
    ):
...
class BlockWorker(BaseWorker):
    ...
    async def run(
        ...
        # logic for applying infra overrides taken from src/nebula/agent.py
