...
from nebula.infrastructure import Infrastructure, InfrastructureResult, Process
from nebula.logging import get_logger
from nebula.settings import NEBULA_AGENT_PREFETCH_SECONDS
from nebula.states import Crashed, Pending, StateType, exception_to_failed_state


@deprecated_class(
    start_date="Mar 2024",
    help="Use a worker instead. Refer to the upgrade guide for more information: https://docs.nebula.io/latest/guides/upgrade-guide-agents-to-workers/.",
)
class NebulaAgent:
    def __init__(
        self,
    ...
    async def get_and_submit_flow_runs(self) -> List[FlowRun]:
        ...
        if not self.started:
            raise RuntimeError(
                "Agent is not started. Use `async with NebulaAgent()...`"
            )

        self.logger.debug("Checking for scheduled flow runs...")

        before = pendulum.now("utc").add(
            seconds=self.prefetch_seconds or NEBULA_AGENT_PREFETCH_SECONDS.value()
        )
