"""
Module containing the base workflow class and decorator - for most use cases, using the [`@flow` decorator][nebula.flows.flow] is preferred.
"""
...
from nebula.deployments.runner import DeploymentImage, EntrypointType, deploy
from nebula.filesystems import ReadableDeploymentStorage
from nebula.runner.storage import (
    BlockStorageAdapter,
    RunnerStorage,
...
from nebula._internal.schemas.validators import raise_on_name_with_banned_characters
from nebula.client.schemas.objects import Flow as FlowSchema
from nebula.client.schemas.objects import FlowRun, MinimalDeploymentSchedule
from nebula.client.schemas.schedules import SCHEDULE_TYPES
from nebula.context import NebulaObjectRegistry, registry_from_script
...
from nebula.results import ResultSerializer, ResultStorage
...
from nebula.task_runners import BaseTaskRunner, ConcurrentTaskRunner
from nebula.utilities.annotations import NotSet
from nebula.utilities.asyncutils import is_async_fn, sync_compatible
...
from nebula.utilities.importtools import import_object
from nebula.utilities.visualization import (
...
if TYPE_CHECKING:
    from nebula.deployments.runner import FlexibleScheduleList, RunnerDeployment


@NebulaObjectRegistry.register_instances
class Flow(Generic[P, R]):
    """
    A Nebula workflow definition.

    !!! note
        We recommend using the [`@flow` decorator][nebula.flows.flow] for most use-cases.

    Wraps a function with an entrypoint to the Nebula engine. To preserve the input
    ...
    Args:
        ...
        persist_result: An optional toggle indicating whether the result of this flow
            ...
            that Nebula should choose whether the result should be persisted depending on
        ...
        result_serializer: An optional serializer to use to serialize the result of this
            ...
            in this flow. If not provided, the value of `NEBULA_RESULTS_DEFAULT_SERIALIZER`
    ...
    def __init__(
        ...
        self.retries = (
            retries if retries is not None else NEBULA_FLOW_DEFAULT_RETRIES.value()
        )

        self.retry_delay_seconds = (
            retry_delay_seconds
            if retry_delay_seconds is not None
            else NEBULA_FLOW_DEFAULT_RETRY_DELAY_SECONDS.value()
        )
        ...
        if module in ("__main__", "__nebula_loader__"):
    ...
    async def from_source(
        ...
        Examples:
            ...
            ```python
            from nebula import flow
            from nebula.runner.storage import GitRepository
    ...
    async def deploy(
        ...
        and create a deployment via the Nebula API that will run the flow on the given schedule.
        ...
        if print_next_steps:
            ...
            console.print(
                f"\n\t$ nebula deployment run '{self.name}/{name}'\n",
                style="blue",
            )
            if NEBULA_UI_URL:
                console.print(
                    "\nYou can also run your flow via the Nebula UI:"
                    f" [blue]{NEBULA_UI_URL.value()}/deployments/deployment/{deployment_ids[0]}[/]\n"
                )
        ...
        **kwargs: "P.kwargs",
        ...
        Args:
            *args: Arguments to run the flow with.
            return_state: Return a Nebula State containing the result of the
                flow run.
        ...
        Returns:
            ...
            If `return_state` is True, returns the result of the flow run
                wrapped in a Nebula State which provides error handling.
        ...
        Examples:
            ...
            >>>     my_flow("foo")
        """
        from nebula.engine import enter_flow_run_engine_from_flow_call
...
def flow(
    ...
    *,
    ...
    """
    Decorator to designate a function as a Nebula workflow.
    ...
    Args:
        ...
        log_prints: If set, `print` statements in the flow will be redirected to the
            Nebula logger for the flow run. Defaults to `None`, which indicates that
            the value from the parent flow should be used. If this is a parent flow,
            the default is pulled from the `NEBULA_LOGGING_LOG_PRINTS` setting.
