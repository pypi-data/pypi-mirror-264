...
from nebula.client.schemas import (
...
from nebula.events.instrument import (
    ResourceTuple,
...
from nebula.utilities.collections import listrepr, remove_nested_keys
from nebula.utilities.dispatch import lookup_type, register_base_type
...
class Block(BaseModel, ABC):
    ...
    `_block_type_id` are reserved by Nebula as Block metadata fields, but
    ...
    def _to_block_document(
        ...
        # reported from https://github.com/kozmoai/nebula-dbt/issues/54
    ...
    def _event_kind(self) -> str:
        return f"nebula.block.{self.get_block_type_slug()}"
    ...
    def _event_method_called_resources(self) -> Optional[ResourceTuple]:
        ...
        return (
            {
                "nebula.resource.id": (
                    f"nebula.block-document.{self._block_document_id}"
                ),
                "nebula.resource.name": self._block_document_name,
            },
            [
                {
                    "nebula.resource.id": (
                        f"nebula.block-type.{self.get_block_type_slug()}"
                    ),
                    "nebula.resource.role": "block-type",
                }
            ],
        )
    ...
    async def _get_block_document(
        ...
        except nebula.exceptions.ObjectNotFound as e:
            raise ValueError(
    ...
    @sync_compatible
    @inject_client
    async def register_type_and_schema(cls, client: "NebulaClient" = None):
        """
        Makes block available for configuration with current Nebula API.
        ...
        Args:
            client: Optional client to use for registering type and schema with the
                Nebula API. A new client will be created and used if one is not
                provided.
        """
        if cls.__name__ == "Block":
        ...
        except nebula.exceptions.ObjectNotFound:
    ...
    async def _save(
        ...
        except nebula.exceptions.ObjectAlreadyExists as err:
            if overwrite:
    ...
    def get_block_placeholder(self) -> str:
        ...
        Returns:
            str: The block placeholder for the current block in the format
                `nebula.blocks.{block_type_name}.{block_document_name}`
        ...
        return f"nebula.blocks.{self.get_block_type_slug()}.{block_document_name}"
