...
try:
    ...
    pass
from nebula._vendor.starlette.status import WS_1008_POLICY_VIOLATION
...
from nebula.events import Event
from nebula.logging import get_logger
from nebula.settings import NEBULA_API_KEY, NEBULA_API_URL
...
class EventsClient(abc.ABC):
    """The abstract interface for all Nebula Events clients"""
...
class NullEventsClient(EventsClient):
    """A Nebula Events client implementation that does nothing"""
...
class AssertingEventsClient(EventsClient):
    """A Nebula Events client that records all events sent to it for inspection during
    tests."""
...
def _get_api_url_and_key(
    ...
    api_url = api_url or NEBULA_API_URL.value()
    api_key = api_key or NEBULA_API_KEY.value()

    if not api_url or not api_key:
        raise ValueError(
            "api_url and api_key must be provided or set in the Nebula configuration"
        )

    return api_url, api_key


class NebulaCloudEventsClient(EventsClient):
    """A Nebula Events client that streams Events to a Nebula Cloud Workspace"""
    ...
    def __init__(
        ...
        """
        Args:
            api_url: The base URL for a Nebula Cloud workspace
    ...
    async def __aenter__(self) -> "NebulaCloudEventsClient":
...
class NebulaCloudEventSubscriber:
    """
    Subscribes to a Nebula Cloud event stream, yielding events as they occur.

    Example:

        from nebula.events.clients import NebulaCloudEventSubscriber
        from nebula.events.filters import EventFilter, EventNameFilter

        filter = EventFilter(event=EventNameFilter(prefix=["nebula.flow-run."]))

        async with NebulaCloudEventSubscriber(api_url, api_key, filter) as subscriber:
    ...
    async def __aenter__(self) -> "NebulaCloudEventSubscriber":
        # Don't handle any errors in the initial connection, because these are most
        # likely a permission or configuration issue that should propagate
        await self._reconnect()
        return self
    ...
    async def _reconnect(self) -> None:
        ...
        from nebula.events.filters import EventOccurredFilter
    ...
    def __aiter__(self) -> "NebulaCloudEventSubscriber":
        return self
