...
def add_to_database(filepath: str, key_column: str,keyvalue: str, data: list,table_name: str = "data") -> bool:
    """
    Add data to a SQLite database.

    Parameters:
        - filepath (str): The full path including the filename of the SQLite database file.
        - key_column (str): The name of the column serving as the primary key in the database table.
    ...
    try:
        ...
        new_keys = [entry[key_column] for entry in data if entry[key_column] not in existing_keys]
        
        if not new_keys:
            print("All key values already exist in the database.")
            conn.close()
            return False
        ...
        for entry in data:
            if entry[key_column] in new_keys:
