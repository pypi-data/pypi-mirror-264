...
def add_to_database(filepath: str, data: dict, table_name: str = "data") -> bool:
    """
    Add data to a SQLite database.

    Parameters:
        - filepath (str): The full path including the filename of the SQLite database file.
        - data (dict): A dictionary containing the data to be added to the database. 
    ...
    Example: 
        - add_to_database("/path/to/database/example.db", {"id": 1, "name": "John", "age": 30})
    """
    try:
        # Connect to SQLite database
        conn = sqlite3.connect(filepath)
        ...
        if not primary_key_columns:
            print(f"No primary key columns found for table '{table_name}'.")
            conn.close()
            return False
        
        # Check if key value already exists
        key_conditions = ' AND '.join([f"{col} = ?" for col in primary_key_columns])
        cursor.execute(f"SELECT * FROM {table_name} WHERE {key_conditions}", tuple(data[col] for col in primary_key_columns))
        existing_data = cursor.fetchone()
        if existing_data:
            print(f"The data with the same primary key already exists in the database.")
            conn.close()
            return False
...
def load_data_by_key(filepath: str, keyvalue: str, key_column: str, table_name: str = "data") -> dict:
    """
