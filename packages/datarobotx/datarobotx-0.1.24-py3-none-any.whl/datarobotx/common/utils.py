...
class FilesSender:
    ...
    def __init__(self, payload: Union[PayloadType, List[PayloadType]]) -> None:
        ...
        self.disable_chunked_uploads = context.disable_chunked_uploads

    def reader(
        self, file_name: str
    ) -> Union[AsyncGenerator[bytes, str], bytes, io.BufferedReader, io.BytesIO]:
        # https://datarobot.atlassian.net/browse/RAPTOR-10125
        if self.disable_chunked_uploads:
            self.pbar.close()  # progress bars do not work with non-chunked uploads
            return self.payload_dict[file_name]
        else:
            return self.areader(file_name)

    async def areader(self, file_name: str) -> AsyncGenerator[bytes, str]:
