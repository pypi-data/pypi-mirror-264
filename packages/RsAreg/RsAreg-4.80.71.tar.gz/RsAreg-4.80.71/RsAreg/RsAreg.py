...
class RsAreg:
	...
	_driver_options = "SupportedInstrModels = AREG, SupportedIdnPatterns = AREG, SimulationIdnString = 'Rohde&Schwarz,AREG100A,100001,4.80.71.0019'"
	...
	def __init__(self, resource_name: str, id_query: bool = True, reset: bool = False, options: str = None, direct_session: object = None):
		...
		self._core.driver_version = '4.80.71.0019'
	...
	def assert_minimum_version(min_version: str) -> None:
		...
		curr_version_list = '4.80.71.0019'.split('.')
		...
		for i in range(count):
			...
			if curr > minimum:
				break
			if curr < minimum:
				raise RsInstrException(f"Assertion for minimum RsAreg version failed. Current version: '4.80.71.0019', minimum required version: '{min_version}'")
	...
	def _sync_to_custom_properties(self, cloned: 'RsAreg') -> None:
