...
class RowCls:
	...
	def set(self, frequency: int, channel=repcap.Channel.Default, index=repcap.Index.Default) -> None:
		...
		Snippet: driver.source.areGenerator.frontend.fe.antenna.custom.flist.row.set(frequency = 1, channel = repcap.Channel.Default, index = repcap.Index.Default) \n
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:ANTenna:CUSTom:FLISt:ROW{index_cmd_val} {param}')

	def get(self, channel=repcap.Channel.Default, index=repcap.Index.Default) -> int:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:FE<CH>:ANTenna:CUSTom:FLISt:ROW<DI> \n
		Snippet: value: int = driver.source.areGenerator.frontend.fe.antenna.custom.flist.row.get(channel = repcap.Channel.Default, index = repcap.Index.Default) \n
		Sets the frequency value in the selected row of the list. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Fe')
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Row')
		...
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:AREGenerator:FRONtend:FE{channel_cmd_val}:ANTenna:CUSTom:FLISt:ROW{index_cmd_val}?')
