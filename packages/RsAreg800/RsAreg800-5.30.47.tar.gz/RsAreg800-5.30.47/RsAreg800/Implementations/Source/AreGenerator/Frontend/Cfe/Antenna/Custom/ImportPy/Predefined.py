...
class PredefinedCls:
	...
	def set(self, import_filename: str, channel=repcap.Channel.Default) -> None:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:CFE<CH>:ANTenna:CUSTom:IMPort:PREDefined \n
		Snippet: driver.source.areGenerator.frontend.cfe.antenna.custom.importPy.predefined.set(import_filename = 'abc', channel = repcap.Channel.Default) \n
		Imports a predefined file for standard antennas, stored on the R&S AREG800A. \n
			:param import_filename: string
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:ANTenna:CUSTom:IMPort:PREDefined {param}')
