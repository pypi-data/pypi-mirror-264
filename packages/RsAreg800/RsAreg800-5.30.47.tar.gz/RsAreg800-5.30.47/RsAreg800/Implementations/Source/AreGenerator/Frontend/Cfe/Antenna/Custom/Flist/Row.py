...
class RowCls:
	...
	def set(self, frequency: int, channel=repcap.Channel.Default, index=repcap.Index.Default) -> None:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:CFE<CH>:ANTenna:CUSTom:FLISt:ROW<DI> \n
		Snippet: driver.source.areGenerator.frontend.cfe.antenna.custom.flist.row.set(frequency = 1, channel = repcap.Channel.Default, index = repcap.Index.Default) \n
		Sets the frequency value in the selected row of the list. \n
			:param frequency: integer Range: 500E6 to 1E12
		...
		self._core.io.write(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:ANTenna:CUSTom:FLISt:ROW{index_cmd_val} {param}')

	def get(self, channel=repcap.Channel.Default, index=repcap.Index.Default) -> int:
		"""SCPI: [SOURce<HW>]:AREGenerator:FRONtend:CFE<CH>:ANTenna:CUSTom:FLISt:ROW<DI> \n
		Snippet: value: int = driver.source.areGenerator.frontend.cfe.antenna.custom.flist.row.get(channel = repcap.Channel.Default, index = repcap.Index.Default) \n
		Sets the frequency value in the selected row of the list. \n
			:param channel: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Cfe')
			:param index: optional repeated capability selector. Default value: Nr1 (settable in the interface 'Row')
			:return: frequency: integer Range: 500E6 to 1E12"""
		channel_cmd_val = self._cmd_group.get_repcap_cmd_value(channel, repcap.Channel)
		index_cmd_val = self._cmd_group.get_repcap_cmd_value(index, repcap.Index)
		response = self._core.io.query_str(f'SOURce<HwInstance>:AREGenerator:FRONtend:CFE{channel_cmd_val}:ANTenna:CUSTom:FLISt:ROW{index_cmd_val}?')
