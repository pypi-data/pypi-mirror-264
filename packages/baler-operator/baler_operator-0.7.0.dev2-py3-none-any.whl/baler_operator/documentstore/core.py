...
def comission_haystack_datasource(owner, spec, name, namespace, meta, **kwargs):
    ...
    underlying resources with the owner label (pipelines.baler.com/owned-by: <owner>)
    :param spec: The haystack pipeline spec
    ...
    if (
        annotations.get("auto-provision-datastore.pipelines.baler.com/enabled", "false")
        == "false"
    ):
        logger.info(
...
def decomission_haystack_datasource(owner, spec, name, namespace, meta, **kwargs):
    ...
    if (
        annotations.get("preserve-datastore.pipelines.baler.com/enabled", "false")
        == "true"
    ):
        logger.info(
