...
def decomission_haystack_pipeline(owner, spec, name, namespace, meta, **kwargs):
    ...
    to prove ownership (pipelines.baler.com/owned-by: <owner>)
...
def comission_haystack_pipeline(owner, spec, name, namespace, meta, **kwargs):
    ...
    compute_type = annotations.get("compute.pipelines.baler.com/type", "cpu")
    # TODO: compute.pipeline.baler.com annotations should be validated strictly
    # so for example typos
    compute_cpu = annotations.get("compute.pipelines.baler.com/request-cpu", "500m")
    compute_memory = annotations.get(
        "compute.pipelines.baler.com/request-memory", "1Gi"
    )
    ...
    compute_gpu = annotations.get("compute.pipelines.baler.com/request-gpu", "0")
    # Get the node-selector annotation and transform it to a dictionary
    # so it can be embedded in the deployment
    node_selector = annotations.get("compute.pipelines.baler.com/node-selector", "")
    ...
    tolerations = annotations.get("compute.pipelines.baler.com/tolerations", "")
    logger.info(f"🔍 Tolerations: {tolerations}")
    tolerations_list = tolerations_parse_annotation_string_to_list(tolerations)
    logger.info(f"🔍 Tolerations list: {tolerations_list}")
    ...
    if compute_gpu != "0" and compute_type == "cpu":
        msg = f"""❌ Haystack pipeline {name} requests 0+ GPU, but the compute type is set/defaults to CPU.
                                 Please set the compute type to GPU explicitly in the annotations: compute.pipelines.baler.com/type: gpu
                                 """
        raise errors.UserFailure(msg)
