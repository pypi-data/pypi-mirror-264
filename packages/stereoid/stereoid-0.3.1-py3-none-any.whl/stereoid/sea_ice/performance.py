...
class ATIPerf(object):
    ...
    def sigma_phase(self, inc_0_in, sigma_0_in, az=None):
        ...
        # FIX-ME include RASR
        coh = coh_snr * self.sys_coh
        stdv = np.sqrt((1. - coh ** 2.) / (2. * nlk * coh ** 2.))
        return stdv

    def sigma_dop(self, inc_0, sigma_0, az=None, tau=None):
        if tau is None:
            tau = self.tau
        sigma_fd = self.sigma_phase(inc_0, sigma_0, az=az) / np.pi / 2 / tau
        return sigma_fd

    def sigma_vdop(self, inc_0, sigma_0, az=None, tau=None):
        if tau is None:
            tau = self.tau
        wl = cnst.c / self.f0
        sigma_v = wl / 2 * self.sigma_dop(inc_0, sigma_0, az=az, tau=tau)
        return sigma_v
...
class DCAPerf(object):
    ...
    def sigma_dop(self, inc_0_in, sigma_0_in, az=None, degree=True):
        ...
        swths = self.sar_modes.inc2swath(inc_0 + np.zeros_like(snr), degree=degree)
