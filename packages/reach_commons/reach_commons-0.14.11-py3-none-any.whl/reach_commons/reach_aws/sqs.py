...
class SQSClient(BaseSQSClient):
    ...
    def client(self):
        ...
        if self.s3_bucket_name:
            ...
            client.large_payload_support = self.s3_bucket_name
            client.always_through_s3 = True
            # self.client.message_size_threshold = default is 262144
            # self.client.always_through_s3 = True/False conforme necessário
            return client
...
class OutscraperSQSNotifier(SQSClient):
    ...
    def __init__(self, region_name=DEFAULT_REGION, logger=get_reach_logger()):
        super().__init__(
            ...
            logger=logger,
            topic_name=f"{os.environ.get('ENV', 'Staging')}-{self.TOPIC_NAME}",
            s3_bucket_name=f"{os.environ.get('ENV', 'Staging')}-sqs-google-reviews".lower(),
        )
