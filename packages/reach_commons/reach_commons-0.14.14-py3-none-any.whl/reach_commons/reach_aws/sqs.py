...
class OutscraperSQSNotifier(SQSClient):
    ...
    def __init__(self, region_name=DEFAULT_REGION, logger=get_reach_logger()):
        ...
        self.s3_bucket_name = (
            f"{os.environ.get('ENV', 'Staging')}-sqs-google-reviews".lower()
        )
    ...
    def notify_for_patch_google_reviews(
        ...
        run_data_consolidation=False,
        delay_seconds=1,
    ):
        timestamp = datetime.now()
        time_prefix = timestamp.strftime("%Y/%m/%d/%H/%M/%S")
        message_data = {
            "business_id": business_id,
            "reviews": reviews,
            "send_notification_email": send_notification_email,
            "last_chunk": run_data_consolidation,
        }

        message_attributes = {"service_name": self.SERVICE_NAME}
        s3_pointer = self.send_to_s3(message_data=message_data, time_prefix=time_prefix)

        return self.publish(
            message_data=s3_pointer,
            message_attributes=message_attributes,
            delay_seconds=delay_seconds,
        )

    def send_to_s3(self, time_prefix, message_data):
        message_body_str = json.dumps(message_data)
        encoded_message = message_body_str.encode()

        s3_key = f"{time_prefix}/{uuid.uuid4()}.json"
        self.s3_client.put_object(
            Bucket=self.s3_bucket_name, Key=s3_key, Body=encoded_message
        )
        return {"s3_bucket_name": self.s3_bucket_name, "s3_key": s3_key}
