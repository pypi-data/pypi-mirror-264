...
class NumpyResultsData(AbstractResultsData):
    ...
    def update(
        ...
        if ode_sol is not None:
            ...
            n_minor_steps = int(ode_sol.n_steps)

            # Reconstruct the signal at all ODE solver steps
            signals = self.scan_eval_sources(context, ys, ts, n_minor_steps)

            # Trim to the number of accepted steps.
            n_acc_steps = ode_sol.stats["num_accepted_steps"]
            ts = ts[:n_acc_steps]
            for key in self.source_dict:
                signals[key] = signals[key][:n_acc_steps]
