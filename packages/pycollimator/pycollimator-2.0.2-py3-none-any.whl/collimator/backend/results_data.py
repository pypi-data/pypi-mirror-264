...
class AbstractResultsData(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def update(self, context: ContextBase, ode_sol: ODESolution) -> AbstractResultsData:
    ...
    def scan_eval_sources(
        ...
        """Scan over all ODE solver steps and collect signals."""
        carry = (context, ys, ts)
        _, signals = self._scan(
            self._eval_scan_fun,
            carry,
            np.arange(n_steps),
        )
