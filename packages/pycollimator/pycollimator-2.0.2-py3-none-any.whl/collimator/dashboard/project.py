...
from collimator.framework import Diagram, InstanceParameter
...
class InitScriptVariables:
    ...
    def get_variable(self, name: str) -> InstanceParameter:
        var = self._variables.get(name)
        if var is None:
            raise KeyError(f"Variable '{name}' not found")
        return InstanceParameter(name=name, string_value=name, evaluated_value=var)
...
class Project:
    ...
    def get_model(
        ...
        Args:
            ...
            parameters (dict[str, model_json.Parameter], optional): A dictionary of parameters for the model. Defaults to None.
    ...
    def create_submodel_instance(
        ...
        **kwargs,
        ...
        if name not in self._submodels:
            raise KeyError(f"Submodel '{name}' not found. Available submodels: {list(self._submodels.keys())}")
        ref_id = self._submodels.get(name)
        if ref_id is None:
            raise ValueError(f"Submodel '{name}' not found")
        return ReferenceSubdiagram.create_diagram(
            ref_id, instance_name, instance_parameters=instance_parameters,
            **kwargs,
        )
        ...
        if submodel is None:
            for submodel_name, ref_id in self._submodels.items():
                if ref_id == submodel_uuid:
                    submodel = model_api.get_reference_submodel_by_name(project_uuid, submodel_name)
                    break
    ...
    def _save_submodel(self, diagram: Diagram):
        ...
        self._save_single_submodel(model_json, submodel_uuid=diagram.ref_id)
    ...
    def save_model(self, diagram: Diagram, configuration: model_json.Configuration = None) -> str:
        """
    ...
    def save_submodel(
        ...
        Args:
            ...
            name (str): The name of the submodel.
            parameter_definitions (list[model_json.ParameterDefinition], optional): A list of parameter definitions for the submodel. Defaults to None.
        ...
        ref_id = ReferenceSubdiagram.register(constructor, parameter_definitions, ref_id=ref_id)
...
def get_project_by_uuid(project_uuid: str) -> Project:
    ...
    for model_summary in submodels_response["submodels"]:
        ...
        if submodel is None:
            logger.warning(
                "Could not find submodel %s (%s)", model_summary["name"], model_summary["uuid"]
            )
            continue
    ...
    return Project(
        summary=project_summary, models=models, files=files,
        submodels=submodels, init_scripts=init_scripts_vars,
    )
