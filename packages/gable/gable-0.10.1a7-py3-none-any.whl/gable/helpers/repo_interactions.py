...
def get_ci_git_origins() -> list[str | None]:
...
def get_origin_url(local_repo_origin: str) -> str:
    ...
    logger.trace(
        f"\tCI environment variable URLs: {[origin for origin in get_ci_git_origins() if origin is not None]}"
    )
    # CI Environment variables take precedence, and come in priority order
    chosen_origin = next(
        origin
        for origin in get_ci_git_origins() + [local_repo_origin]
