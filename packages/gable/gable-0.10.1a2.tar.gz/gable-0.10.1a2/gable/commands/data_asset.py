...
def register_data_asset(
    ...
    registered_assets = response.get("registered", [])
    # Should always be there, but just in case...
    success = success and response.get("success", True)
    ...
    # Only raise an exception if we got a full error response or no registered. If we got back an
    # any registered assets, then we'll just log the individual errors, but still return a 0
    # exit code. We may want to revisit this later, but for now it means we won't break CI/CD
    # pipelines that are using this command while we work out the bugs...
    if not success and len(registered_assets) == 0:
        raise click.ClickException(
            f"{EMOJI.RED_X.value} Registration failed: {str(response)}"
        )

    # Warn if we didn't register all assets
    if not success:
        warning_message = (
            f"{EMOJI.YELLOW_WARNING.value} Discovery failed for some data assets"
            if dry_run
            else f"{EMOJI.YELLOW_WARNING.value} Registration failed for some data assets"
        )
        logger.warning(warning_message)
        logger.warning(f"\t{message}")
    success_message = (
        f"{EMOJI.GREEN_CHECK.value} Discovery successful:\n{registered_output}"
        if dry_run
        else f"{EMOJI.GREEN_CHECK.value} Registration successful:\n{registered_output}"
    )
    logger.info(success_message)
    # If there was a problem this was already logged above
    if success:
        logger.info(message)
