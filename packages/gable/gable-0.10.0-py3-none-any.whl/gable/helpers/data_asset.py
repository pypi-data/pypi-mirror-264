...
def gather_pyspark_asset_data(
    ...
    try:
        ...
        log_msg: Dict[str, str] = json.loads(sca_results)
        event_schemas: Dict[str, Dict[str, Any]] = json.loads(log_msg["message"])

    except json.JSONDecodeError as exc:
        logger.opt(exception=exc).info(f"Error analyzing PySpark job: {sca_results}")
        raise click.ClickException(
            f"Error analyzing the PySpark job: {sca_results}: {exc}"
        ) from exc
    project_repo = get_git_repo_info(project_root)
    git_ssh_repo = f"{strip_git_prefix(project_repo['gitSSHRepo'])}"
