"""
This module contains a comprehensive `prompt` dictionary that serves as a repository of prompts for guiding the AI agent's interactions across various operational scenarios, including execution, planning, and information retrieval tasks. These prompts are meticulously crafted to instruct the AI in performing its duties, ranging from code generation and amendment to task decomposition and planning, as well as error analysis and tool usage.
...
prompt = {
    'execute_prompt' : {
        ...
        1. Code Structure: Begin with the necessary import statement: from friday.tool_repository.basic_tools.base_action import BaseAction. Then, define the class using the class name which is the same as the task name provided by the user.
        ...
        1. The class must start with from friday.tool_repository.basic_tools.base_action import BaseAction.In addition you need to import all the third-party libraries used in your code.
        ...
        from friday.tool_repository.manager.tool_request_util import ToolRequestUtil
    ...
    'planning_prompt' : {
        ...
        2. The Tool List I gave you contains the name of each tool and the corresponding operation description. These tools are all atomic code task. You can refer to these atomic operations to decompose the code task.
