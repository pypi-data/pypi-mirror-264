...
class nequip_lr_lambda(object):
    ...
    def plot(self, max_epoch, label=None, show=True):
        epochs, lrs = self.get_lrs(max_epoch)
    ...
    def plot_ax(self, ax, max_epoch, label=None):
        epochs, lrs = self.get_lrs(max_epoch)
        data = ax.plot(epochs, lrs, label=label)
        ax.set_xlabel("epoch")
        ax.set_ylabel("learning rate scale factor (0 to 1)")
        ax.set_ylim(0, 1.2)
        return data
