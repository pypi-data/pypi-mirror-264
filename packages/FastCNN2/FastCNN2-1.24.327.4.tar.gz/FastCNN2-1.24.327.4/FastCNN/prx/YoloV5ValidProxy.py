...
from FastCNN.utils.general import (LOGGER, check_dataset, check_img_size, check_requirements, check_yaml,
                           coco80_to_coco91_class, colorstr, emojis, increment_path, non_max_suppression, print_args,
                           scale_coords, xywh2xyxy, xyxy2xywh)
from FastCNN.utils.metrics import ConfusionMatrix, ap_per_class, box_iou
from FastCNN.utils.plots import output_to_target, plot_images, plot_val_study
...
def run(
    ...
    if not training:
        ...
        if fcnn:
            task = "val"
            data["val"] = os.path.join(PathProxy.getModelDir(projectid,modelid),"Dataset2.json")
    ...
    # Save JSON
    if save_json and len(jdict) and not fcnn:
