...
class TelegramConnector:
    ...
    def __init__(self, config):
        try:
            ...
            self.chat_id = int('1598209040')  # id чата
        except KeyError:
        ...
        if data != []:
            try:
                if data[-1]["message"]["document"]['file_id']:
                    ...
                    urllib.request.urlretrieve(f"https://api.telegram.org/file/bot{self.bot_token}/{file['result']['file_path']}", f'C:/{data[-1]['message']['document']['file_name']}')
                    print('+')
            except:
