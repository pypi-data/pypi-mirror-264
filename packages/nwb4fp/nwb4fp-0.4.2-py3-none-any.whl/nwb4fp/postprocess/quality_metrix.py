...
from nwb4fp.postprocess.Get_positions import load_positions
...
def test_clusterInfo(path, temp_folder,save_path_test,vedio_search_directory):
    ...
    print(file[1])
    try:
        arr_with_new_col = load_positions(path,vedio_search_directory,raw_path,UD)
        new_row = pd.DataFrame({'dlc': "file exist"})
    except FileNotFoundError:
        new_row = pd.DataFrame({'dlc': "file not found"})
