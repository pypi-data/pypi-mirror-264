"""Module to parse Haskell code using the tree-sitter library."""
import warnings
import tree_sitter
...
class TreesitterHaskell(BaseTreeParser):
    """Class to parse Haskell code using the tree-sitter library."""
        ...
        if node.type == self.method_declaration_identifier:
            ...
            if (
                ...
                if (
                    ...
                    prev_node: tree_sitter.Node = node.prev_named_sibling
                    if (
    ...
    def _query_method_name(self, node: tree_sitter.Node) -> str | None:
        if node.type in ["signature", self.method_declaration_identifier]:
            for child in node.children:
...
TreeParserRegistry.register_treesitter(Language.HASKELL, TreesitterHaskell)
