...
def main() -> int:
    ...
    if not early_args.no_config_file:
        file = OptionLoader.resolve_effective_config_file(early_args.config_file)

        if file:
            try:
                option_loader.update_from_yaml_file(file)
            except (ValueError, AttributeError) as e:
                error_message = f"Error loading config from {file}: {e}"
    ...
    # Unless the --no-config-file option was specified, we write the configuration file **if it does not exist**.
    if not early_args.no_config_file and not ("subcommand" in args and args.subcommand == ConfigSubCommand.key):
        option_loader.write_to_yaml_file(overwrite=False)
    ...
    if "subcommand" in args and args.subcommand == ConfigSubCommand.key:
        sub_commands[ConfigSubCommand.key].handle_command(args)
        return 0
    ...
    if "subcommand" in args and args.subcommand in sub_commands:
        ...
        sub_commands[args.subcommand].handle_command(final_args)
    else:
