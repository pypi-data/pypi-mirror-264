...
from tlcconfig.options import get_default_config_dir
...
class ConfigSubCommand(SubCommand):
    ...
    def create_subparser(
        ...
        config_parser.add_argument(
            "--set-api-key",
            help="Set the API key in the configuration.",
        )
    ...
    def handle_command(self, args: argparse.Namespace) -> int:
        ...
        if "set_api_key" in args and args.set_api_key:
            if args.set_api_key != "":
                config_file_location = get_default_config_dir()
                api_key_file = config_file_location + os.path.sep + "api_key.txt"
                with open(api_key_file, "w") as f:
                    f.write(args.set_api_key)
                print(f"API key set to {args.set_api_key} in: {api_key_file}")
            return 0
