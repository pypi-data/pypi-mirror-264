...
def _get_user_name() -> str:
    from tlc.core.saas.api_key import ApiKey
...
class DynamicInformation(Widget):
    ...
    dashboard_url = reactive("")
    object_service_url = reactive("")
    user_name = reactive("")
    ...
    def __init__(self) -> None:
        ...
        self.set_reactive(
            ...
            self.format_log_file(LoggerConfigurator.instance().get_current_log_file_path()),
        )
        self.set_reactive(DynamicInformation.log_level_name, self.format_log_level(_get_current_log_level_name()))
        self.set_reactive(DynamicInformation.config_file_path, self.format_config_file_path(_get_config_file_path()))
    ...
    def compose(self) -> ComposeResult:
        metadata = VerticalScroll(
            HorizontalScroll(
                Container(
                    Label(id="user_name"),
                    Label(id="dashboard_url"),
                    Label(id="object_service_url"),
                    Label(id="status_message"),
                ),
                Container(
                    Label(id="tenant_name"),
                    Label(self.config_file_path, id="config_file_path"),
                    Label(self.log_level_name, id="log_level_name"),
                    Label(self.log_file_path, id="log_file_path"),
                ),
            ),
            id="dynamic_information",
        )
        # metadata.border_title = "Dynamic Information"
        yield metadata

    def watch_user_name(self, value: str) -> None:
        """Called when user_name changes."""
        self.query_one("#user_name", Label).update(self.format_user_name(self.user_name))

    def watch_tenant_name(self, value: str) -> None:
        """Called when tenant_name changes."""
        self.query_one("#tenant_name", Label).update(self.format_tenant_name(self.tenant_name))

    def watch_config_file_path(self, value: str) -> None:
        """Called when config_file_path changes."""
        self.query_one("#config_file_path", Label).update(self.format_config_file_path(self.config_file_path))

    def watch_log_level_name(self, value: str) -> None:
        """Called when log_level_name changes."""
        self.query_one("#log_level_name", Label).update(f"Log Level: {self.log_level_name}")

    def watch_log_file_path(self, value: str) -> None:
        """Called when log_file_path changes."""
        self.query_one("#log_file_path", Label).update(self.format_log_file(value))

    def watch_dashboard_url(self, value: str) -> None:
        self.query_one("#dashboard_url", Label).update(
            f"Dashboard URL: [link={self.dashboard_url}]{self.dashboard_url}[/link]"
        )

    def watch_status_message(self, value: str) -> None:
        self.query_one("#status_message", Label).update(f"Status: {self.status_message}")

    def watch_object_service_url(self, value: str) -> None:
        formatted_urls = " - ".join([f"[link={url}]{url}[/link]" for url in self.object_service_url])

        self.query_one("#object_service_url", Label).update(f"Object Service URL: {formatted_urls}")
...
class ObjectServiceTUI(App):
    ...
    STATIC_INFO = """
# Dashboard URL: [https://dashboard.dev.3lc.ai](https://dashboard.dev.3lc.ai)

"""

    @staticmethod
    def get_rich_table() -> Table:
        table = Table(title="3LC Object Service")
        table.add_row(f"PID: {os.getpid()}", f"IP: {socket.gethostbyname(socket.gethostname())}")
        table.add_row(f"Platform: {platform.platform()}", "")
        table.add_row(f"3LC Version: {_PACKAGE_VERSION}", f"3LC Git revision: {_PACKAGE_GIT_REVISION}")

        return table

    @staticmethod
    def get_static_info() -> str:
        s = ObjectServiceTUI.STATIC_INFO
        return s
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main_container"):
            # Dynamic Information
            self.dynamic_information = Container(DynamicInformation(), id="dynamic_information_container")
    ...
    def on_object_service_tui_tlcimport_failed_message(self, message: TLCImportFailedMessage) -> None:
        ...
        self.query_one(DynamicInformation).status_message = "Failed"
        pass
    ...
    def on_object_service_tui_object_service_started_message(self, message: ObjectServiceStartedMessage) -> None:
        ...
        self.query_one(DynamicInformation).status_message = "Running"
    ...
    def on_ready(self) -> None:
        ...
        self.query_one(DynamicInformation).status_message = "Starting..."
    ...
    def after_object_service_startup_handler(self, starlite: Starlite) -> None:
        ...
        # We query the starlite.state dict which has been set by the various startup-handlers.
        self.query_one(DynamicInformation).object_service_url = starlite.state[
            StarliteStateConstants.OBJECT_SERVICE_RUNNING_URLS
        ]
        self.query_one(
            DynamicInformation
        ).dashboard_url = tlccli.subcommands.object_service.get_effective_dashboard_url(starlite.state)
