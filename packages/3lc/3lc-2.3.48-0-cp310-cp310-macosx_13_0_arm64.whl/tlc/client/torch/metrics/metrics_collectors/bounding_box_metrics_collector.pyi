...
from tlc.core.builtins.constants.number_roles import NUMBER_ROLE_BB_CENTER_X as NUMBER_ROLE_BB_CENTER_X, NUMBER_ROLE_BB_CENTER_Y as NUMBER_ROLE_BB_CENTER_Y, NUMBER_ROLE_BB_MIN_X as NUMBER_ROLE_BB_MIN_X, NUMBER_ROLE_BB_MIN_Y as NUMBER_ROLE_BB_MIN_Y, NUMBER_ROLE_BB_SIZE_X as NUMBER_ROLE_BB_SIZE_X, NUMBER_ROLE_BB_SIZE_Y as NUMBER_ROLE_BB_SIZE_Y
...
class YOLOAnnotation(TypedDict):
    """A single ground truth annotation in the YOLO format.

    Corresponds to the official YOLO annotation format:
...
class YOLOGroundTruth(TypedDict):
    """A single ground truth annotation in the YOLO format.
...
class YOLOPrediction(TypedDict):
    """A single prediction in the YOLO results format."""
    annotations: list[YOLOAnnotation]
...
class BoundingBoxMetricsCollector(MetricsCollector):
    ...
    preprocess_fn: Incomplete
    compute_derived_metrics: Incomplete
    iou_threshold: Incomplete
    derived_metrics_mode: Incomplete
    def __init__(self, model: torch.nn.Module, classes: list[str], label_mapping: dict[int, int], iou_threshold: float = 0.5, compute_derived_metrics: bool = False, derived_metrics_mode: str = 'relaxed', extra_metrics_fn: Callable[[list[COCOGroundTruth], list[COCOPrediction], dict[str, list[Any]] | None], None] | None = None, preprocess_fn: Callable[[SampleData, SampleData], tuple[list[COCOGroundTruth], list[COCOPrediction]]] | None = None, compute_aggregates: bool = True) -> None: ...
    def compute_metrics(self, batch: SampleData, predictions: SampleData | None = None, _: dict[int, torch.Tensor] | None = None) -> dict[str, MetricData]:
        """Compute metrics for bounding box predictions.
    ...
    @staticmethod
    def preprocess(batch: SampleData, predictions: SampleData) -> tuple[list[COCOGroundTruth], list[COCOPrediction]]:
        """Default preprocessor for the raw batch and predictions.

        This preprocessor recognizes and transforms detectron2 samples/predictions, otherwise it assumes that the
        samples/predictions are in the COCO format.
...
def compute_iou(bb1: list[float], bb2: list[float]) -> float:
    """Calculates intersection over union for 2 bounding boxes in XYXY_ABS format"""

class YOLOBoundingBoxMetricsCollector(BoundingBoxMetricsCollector):
    @property
    def column_schemas(self) -> dict[str, Schema]:
        """Returns the column schemas for the metrics computed by this collector."""

def convert_yolo_to_coco_annotation(annotation: YOLOAnnotation, width: int | None, height: int | None) -> COCOAnnotation: ...
def yolo_prediction_to_coco(prediction: YOLOPrediction, width: int | None, height: int | None) -> COCOPrediction: ...
def yolo_ground_truth_to_coco(ground_truth: YOLOGroundTruth) -> COCOGroundTruth: ...
