        ...
        def after_object_service_startup_handler(starlite: starlite.Starlite) -> None:
            if starlite.state[StarliteStateConstants.OBJECT_SERVICE_RUNNING_URLS]:
                for url in starlite.state[StarliteStateConstants.OBJECT_SERVICE_RUNNING_URLS]:
                    print(f"Object Service URL: {url}")

            print(f"Dashboard URL: {get_effective_dashboard_url(starlite.state)}")
...
def get_effective_dashboard_url(state: starlite.State) -> str:
    ...
    from tlc.core.saas.runtime_config import DASHBOARD_SERVICE_URL
    from tlc.service.object_service import StarliteStateConstants

    query_parameter = ""
    if state.get(StarliteStateConstants.NGROK_OBJECT_SERVICE_URL):
        query_parameter = urlencode({"object_service": state.get(StarliteStateConstants.NGROK_OBJECT_SERVICE_URL)})

    if state.get(StarliteStateConstants.NGROK_DASHBOARD_URL):
        dashboard_url = state.get(StarliteStateConstants.NGROK_DASHBOARD_URL)
    elif state.get(StarliteStateConstants.DASHBOARD_URL):
        dashboard_url = state.get(StarliteStateConstants.DASHBOARD_URL)
    else:
