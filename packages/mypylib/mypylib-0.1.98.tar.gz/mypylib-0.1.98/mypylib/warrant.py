...
def warrant_bible_convert_to_c_need():

    files = os.listdir(f'{expanduser("~")}/warrant')
    files.sort(reverse=True)
    print(f'讀取權證達人寶典: {files[0]}')
    dict_warrant_to_info, dict_stock_to_warrant = read_warrant_bible(f'{expanduser("~")}/warrant/{files[0]}')
    ...
    with open(f'{expanduser("~")}/warrant/{files[0].split(".")[0]}.txt', "w+") as fp:
