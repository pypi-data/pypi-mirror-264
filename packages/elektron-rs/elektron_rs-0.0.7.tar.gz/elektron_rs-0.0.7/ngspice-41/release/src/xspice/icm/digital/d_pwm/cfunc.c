...
static void cm_d_pwm_callback(Mif_Private_t *mif_private,
...
void cm_d_pwm(Mif_Private_t *mif_private)
{
    ...
    dc_size = mif_private->param[1]->size;
    frequency = mif_private->param[2]->element[0].rvalue;
...
//                cm_message_send(d_pwm_positive_dc_error);
            ...
            *phase = *phase_old + frequency * (mif_private->circuit.time - mif_private->circuit.t[1]);
            ...
            if((*t1 <= mif_private->circuit.time) && (mif_private->circuit.time <= *t3)) { /* output high */
                ...
                *t3 = mif_private->circuit.t[1] + (1 - dphase)/frequency;
            ...
            if((*t3 <= mif_private->circuit.time) && (mif_private->circuit.time <= *t1)) { /* output low */
                ...
                *t1 = mif_private->circuit.t[1] + ( (1.0 - dc) - dphase)/frequency;
