...
class SaxiIcoDataset_fs(Dataset):
    ...
    def getitem_per_hemisphere(self, hemisphere, idx):
        ...
        if self.normalize:
            ...
            channel_mins = vertex_features.min(dim=0).values
            channel_maxs = vertex_features.max(dim=0).values

            # Normalize vertex features to 0/1 range per channel
            vertex_features = (vertex_features - channel_mins) / (channel_maxs - channel_mins)
...
class SaxiIcoDataModule_fs(pl.LightningDataModule):
    def __init__(self,batch_size,data_train,data_val,data_test,train_transform=None,val_and_test_transform=None, num_workers=6,name_class='fsqc_qc',freesurfer_path=None,normalize=False):
        super().__init__()
        self.batch_size = batch_size 
        self.data_train = data_train
        self.data_val = data_val
    ...
    def setup(self,stage=None):
        # Assign train/val datasets for use in dataloaders
        self.train_dataset = SaxiIcoDataset_fs(self.df_train,self.train_transform,name_class = self.name_class,freesurfer_path = self.freesurfer_path,normalize=self.normalize)
