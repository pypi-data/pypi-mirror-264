...
def SaxiIcoClassification_fs_train(args, checkpoint_callback, mount_point, train, val, test, early_stop_callback):
    ...
    #Creation of Dataset
    brain_data = SaxiIcoDataModule_fs(args.batch_size,train,val,test,train_transform=train_transform,val_and_test_transform=val_and_test_transform,num_workers=args.num_workers,name_class=args.class_column,freesurfer_path=args.fs_path,normalize=True)
    df_train = pd.read_csv(train)
    unique_classes = np.sort(np.unique(df_train[args.class_column]))
