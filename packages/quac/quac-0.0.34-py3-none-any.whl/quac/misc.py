...
def putinrow(cls: DuckTypeProtocol):
    '''
    Returns
    -------
    types: tuple[type, ...]
    ...
    if getattr(cls, __AREINROW__, False): 
    ...
    for t in curtypes:
        ...
        if isduck(t):
            inrow = getattr(t, __AREINROW__, False)
            if not inrow:
                # attempt to put `t.__ducktype__` in row.
                try:
                    types, inrow, duckq = putinrow(t)
