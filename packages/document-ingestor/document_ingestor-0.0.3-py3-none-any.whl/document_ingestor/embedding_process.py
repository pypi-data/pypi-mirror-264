...
def generate_embedding(data, openai_api_key="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"):
    ...
    for idx, item in enumerate(data):
        ...
        for text in texts:
            ...
            if token_count > 50000:
                ...
                mxm_toen=0
                try:
                    response = client_openai.embeddings.create(input= text_contents, model=model).data
                except:
                    time.sleep(120)
                    try:
                        response = client_openai.embeddings.create(input= text_contents, model=model).data
                    except:
                        print("OpenAI API request error! Please check your openAI credits or try after sometime.")
                        return None
                for idx, response in enumerate(response):
                    vectors.append({"id":str(i), "values":response.embedding, "metadata":text_vectors[idx][1]})
                    i += 1
                text_vectors = []
                token_count = 0
                # sleep for 1 min
                time.sleep(60)
    try:
        response = client_openai.embeddings.create(input= [row[0] for row in text_vectors], model=model).data
    except:
        time.sleep(120)
        try:
            response = client_openai.embeddings.create(input= [row[0] for row in text_vectors], model=model).data
