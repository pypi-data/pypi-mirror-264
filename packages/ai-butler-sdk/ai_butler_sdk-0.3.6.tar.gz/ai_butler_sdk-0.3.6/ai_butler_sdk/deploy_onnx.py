...
class DeployOnnxInfer:
    ...
    def docker_run(self, port: int) -> str:
        ...
        container_name = f"{self.docker_image_name.split(':')[0].split('/')[-1]}-{port}"
        container_id = client.containers.run(
            image=self.docker_image_name,
            detach=True,  # 是否以后台模式运行
            volumes={f"/srv/app/{self.onnx_weight_path}": {"bind": f"/srv/app/{settings.DEPLOY_ONNX_WEIGHT_DIR}", "mode": "rw"}},
