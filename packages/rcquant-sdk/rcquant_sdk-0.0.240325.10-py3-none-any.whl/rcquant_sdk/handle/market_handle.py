...
class MarketHandle():
    ...
    def fin_save_basetick_list(self, instrument_id, df, compress='xz') -> Tuple[bool, str]:
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]
        ...
        start = datetime.datetime.now().timestamp()
        b, m, params = self.__create_fin_persists_save_param_data(instrument_id, "BTick", df, compress)
        if b == False:
            return [b, m]
        end = datetime.datetime.now().timestamp()
        print('fin_save_ohlc_list compress %s' % (end - start))
    ...
    def __unpack_fin_persist_read_param_data_to_df(self, reqrsp: ReqRsp, columns):
        ...
        for b in tempbytes:
            if len(b) <= 0:
                continue
            olist = msgpack.unpackb(b)
            df = pd.DataFrame(olist, columns=columns)
            dfrtn = pd.concat([dfrtn, df], ignore_index=True, copy=False)
        return dfrtn
    ...
    def __unpack_decompress_buffers(self, req_rsp: ReqRsp):
        ...
        for r in rsp_list:
            ...
            for df in rspparams.DataFileds:
                if df.Mark == 'zip':
                    tempbytes .append(zlib.decompress(df.Buffer))
                elif df.Mark == 'xz':
                    tempbytes .append(lzma.decompress(df.Buffer))
        return tempbytes

    def __create_fin_persists_save_param_data(self, instrument_id: str, range: str, df, compress: str = 'xz'):
        if not isinstance(df, pd.DataFrame):
            return [False, "df 数据类型格式不是 DataFrame", {}]
        ...
        for day, day_list in groups:
            ...
            filed.Day = day
            filed.Mark = compress
            tuple_list = [tuple(row) for row in day_list.values.tolist()]
            if compress == 'zip':
                filed.Buffer = zlib.compress(msgpack.packb(tuple_list))
            else:
                filed.Buffer = lzma.compress(msgpack.packb(tuple_list))
