...
@LastEditTime: 2024-03-25 13:34:35
...
class ShakeShopBaseModel():
    ...
    def init_token(self, code: str = '') -> bool:
        ...
        :last_editors: HuangJianYi
        """
        request_redis_key = f"init_token:{str(self.app_key)}_{self.test_mode}"
    ...
    def _refresh_token(self) -> None:
        ...
        :last_editors: HuangJianYi
        """
        request_redis_key = f"refresh_token:{str(self.app_key)}_{self.test_mode}"
        if SevenHelper.is_continue_request(request_redis_key, 2000) == True:
            return False
        path = '/token/refresh'
    ...
    def product_search_list(self, page=1, size=100, name="", product_id=[], status=0, product_type=-1, custom_params={}, field="product_id,name,img,status,product_type,out_product_id,outer_product_id", is_log=False):
        """
