        ...
        try:
            ...
            while self.max_loops == "auto" or loop_count < self.max_loops:
                ...
                while attempt < self.retry_attempts and not success:
                    try:
                        response_args = (task_prompt, *args) if img is None else (task_prompt, img, *args)
                        ...
                        print(f"Attempt {attempt+1}: Error generating response: {e}")
                        attempt += 1

                if not success:
                    print("Failed to generate a valid response after retry attempts.")
                    break  # Exit the loop if all retry attempts fail

                # Check stopping conditions
                if self.stopping_token and self.stopping_token in response:
                    break
                elif self.stopping_condition and self._check_stopping_condition(response):
                    break
                elif self.stopping_func and self.stopping_func(response):
                    break
