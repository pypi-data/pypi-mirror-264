...
from nebula_gcp.workers.cloud_run import CloudRunWorker
...
from nebula_gcp.cloud_run import CloudRunJob, CloudRunJobResult, Execution, Job
...
def mock_client(monkeypatch, mock_credentials):
    ...
    monkeypatch.setattr(
        "nebula_gcp.cloud_run.CloudRunJob._get_client",
        get_mock_client,
    )
...
class TestCloudRunJobContainerSettings:
    def test_captures_nebula_env(self, cloud_run_job):
        base_setting = {}
...
def test_get_client_uses_correct_endpoint(monkeypatch, mock_credentials, cloud_run_job):
    ...
    mock = Mock()
    monkeypatch.setattr("nebula_gcp.cloud_run.discovery.build", mock)
