...
from nebula.server.schemas.actions import DeploymentCreate

from nebula_gcp.credentials import GcpCredentials
from nebula_gcp.utilities import slugify_name
from nebula_gcp.workers.cloud_run import (
    CloudRunWorker,
...
class TestCloudRunWorkerJobConfiguration:
    ...
    def test_populate_image_if_not_present(self, cloud_run_worker_job_config):
        ...
        # defaults to nebula image
        assert "image" in container
        assert container["image"].startswith("docker.io/kozmoai/nebula:")
    ...
    def test_populate_or_format_command_doesnt_exist(self, cloud_run_worker_job_config):
        ...
        assert container["command"] == ["nebula", "flow-run", "execute"]
...
def mock_client(monkeypatch, mock_credentials):
    ...
    monkeypatch.setattr(
        "nebula_gcp.workers.cloud_run.CloudRunWorker._get_client",
        get_mock_client,
    )
...
class TestCloudRunWorker:
    ...
    async def test_behavior_called_when_job_get_fails(
        ...
        async with CloudRunWorker("my-work-pool") as cloud_run_worker:
            ...
            monkeypatch.setattr("nebula_gcp.cloud_run.Job.get", raise_exception)
    ...
    async def test_behavior_called_when_execution_get_fails(
        ...
        async with CloudRunWorker("my-work-pool") as cloud_run_worker:
            ...
            monkeypatch.setattr("nebula_gcp.cloud_run.Execution.get", raise_exception)
