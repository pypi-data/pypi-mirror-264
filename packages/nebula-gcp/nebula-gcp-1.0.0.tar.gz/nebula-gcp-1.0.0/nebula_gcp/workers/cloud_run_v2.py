...
from nebula_gcp.models.cloud_run_v2 import CloudRunJobV2Result, ExecutionV2, JobV2
...
class CloudRunWorkerJobV2Configuration(BaseJobConfiguration):
    ...
    timeout: int = Field(
        ...
        le=86400,
        description=(
            "The length of time that Nebula will wait for a Cloud Run Job to "
    ...
    def _populate_image_if_not_present(self):
        ...
        if "image" not in self.job_body["template"]["template"]["containers"][0]:
            self.job_body["template"]["template"]["containers"][0][
                "image"
            ] = f"docker.io/{get_nebula_image_name()}"
...
class CloudRunWorkerV2Variables(BaseVariables):
    ...
    image: Optional[str] = Field(
        default="kozmoai/nebula:2-latest",
        title="Image Name",
        description=(
            "The image to use for the Cloud Run job. "
            "If not provided the default Nebula image will be used."
        ),
    )
    args: List[str] = Field(
...
class CloudRunWorkerV2(BaseWorker):
    ...
    _documentation_url = "https://kozmoai.github.io/nebula-gcp/worker_v2/"
    ...
    def _watch_job_execution_and_get_result(
        ...
        Args:
            ...
            logger (NebulaLogAdapter): The logger to use.
