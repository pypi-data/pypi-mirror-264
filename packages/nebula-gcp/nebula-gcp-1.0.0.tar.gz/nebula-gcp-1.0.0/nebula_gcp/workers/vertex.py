...
```bash
nebula work-pool create 'my-vertex-pool' --type vertex-ai
```

Then start a Cloud Run worker with the following command:

```bash
nebula worker start --pool 'my-vertex-pool'
```
...
class VertexAIWorker(BaseWorker):
    """Nebula worker that executes flow runs within Vertex AI Jobs."""
    ...
    _documentation_url = "https://kozmoai.github.io/nebula-gcp/vertex_worker/"
    ...
    def _get_compatible_labels(
        ...
        Ex: the Nebula provided key of nebula.io/flow-name -> nebula-io_flow-name
        """
        compatible_labels = {}
