...
```bash
nebula work-pool create 'my-cloud-run-pool' --type cloud-run
```

Then start a Cloud Run worker with the following command:

```bash
nebula worker start --pool 'my-cloud-run-pool'
```

## Configuration
Read more about configuring work pools
[here](https://docs.nebula.io/latest/concepts/work-pools/#work-pool-overview).
...
from nebula.exceptions import InfrastructureNotFound
from nebula.logging.loggers import NebulaLogAdapter
...
from nebula_gcp.cloud_run import Execution, Job
from nebula_gcp.credentials import GcpCredentials
from nebula_gcp.utilities import slugify_name
...
class CloudRunWorkerJobConfiguration(BaseJobConfiguration):
    ...
    Attributes:
        ...
        timeout: The length of time that Nebula will wait for a Cloud Run Job.
    ...
    timeout: Optional[int] = Field(
        ...
        title="Job Timeout",
        description=(
            "The length of time that Nebula will wait for a Cloud Run Job to complete "
    ...
    def _populate_image_if_not_present(self):
        """Adds the latest nebula image to the job if one is not already provided."""
        try:
            if (
                "image"
                ...
                self.job_body["spec"]["template"]["spec"]["template"]["spec"][
                    "containers"
                ][0]["image"] = f"docker.io/{get_nebula_image_name()}"
        except KeyError:
...
class CloudRunWorkerVariables(BaseVariables):
    ...
    timeout: Optional[int] = Field(
        ...
        title="Job Timeout",
        description=(
            "The length of time that Nebula will wait for Cloud Run Job state changes."
        ),
    )
...
class CloudRunWorker(BaseWorker):
    """Nebula worker that executes flow runs within Cloud Run Jobs."""
    ...
    _documentation_url = "https://kozmoai.github.io/nebula-gcp/cloud_run_worker/"
