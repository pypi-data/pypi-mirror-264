...
class lo2Lang:
    def __init__(self, code_file: str = '', log_path: str = '', 
        ...
        self.pattern_sentence_count_2 = re.compile(
            r"^\[\s*(\d+)\s*,\s*(\d+)\s*\]$"
        )  # 匹配 [x1, x2]
    ...
    def code_lexer(self, lo2_code, tokens={}, pattern_name="", child_oracle_flag=False):
        ...
        for k, v in lo2_code.items():
            ...
            elif k == "oracle" or child_oracle_flag == True:
                
                # 对于子oracle 序列第一项是一个str 标识该子oracle的名称
                if child_oracle_flag == True:
            ...
            # a module
                ...
                #    self.code_lexer(v, tokens, f"{pattern_name}.{k}" if pattern_name else k)
                #elif isinstance(v, dict) and "oracle" in v:
                #    self.code_lexer(v, tokens, f"{pattern_name}.{k}" if pattern_name else k)
    ...
    def __update_state(self, node: dict[str, str], idx: int):
        """
        更新指定模块的上下文状态
        ...
        if not is_child_oracle_flag: # 如果当前匹配项不是子oracle 则直接更新
