...
class ApplianceLoader():
    def __init__(self, defintions_directory: Path, data_directory: Path):
        ...
        self.agent_loader = AgentLoader(defintions_directory, data_directory)
    ...
    def load(self, appliance_name: str) -> dict:
        ...
        shortcode = definition.get('shortcode', appliance_name)
        name = definition.get('name', f'{inflection.titleize(appliance_name)}')
        purpose = definition.get('purpose', "Generated by appliance loader.")
        tools = definition.get('tools', [])

        agents = [self.agent_loader.load_agent(shortcode) for shortcode in definition.get('agents', [])]
        type = KomodoApplianceType(definition.get('type', 'enterprise'))
        features = [KomodoFeatures(feature) for feature in definition.get('features', [])]
        users = [KomodoUser(**user) for user in definition.get('users', [])]
