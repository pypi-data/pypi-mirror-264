...
class PolygonSearch(KomodoTool):
    ...
    purpose = "Leverages Polygon.io to conduct detailed searches across various financial data sources. It supports a wide range of parameters as documented in Polygon.io's guidelines. Downloads data from Polygon.io using a provided HTTPS query string. The function automatically includes an API key, managed by the platform. The response from Polygon.io is returned in a JSON format. In case of error, you must retry by fixing the query string. You must convert any unix timestamps returned to ISO 8601 format for display."
    ...
    definition = {
        ...
        "function": {
            ...
            "notes": {
                "type": "string",
                "description": "Always use the MOST recent data when preparing your response to user, "
                               "and for options always know the most recent strike price to keep reporting relevant."
            }
        }
    }
