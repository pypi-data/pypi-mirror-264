...
class CoordinatorAgent(KomodoAgent):
    ...
    def __init__(self, appliance: KomodoApp, run_agent_as_tool, run_workflow_as_tool):
        ...
        appliance_data = appliance.config.locations().appliance_data(appliance.shortcode)
        cache_path = appliance.config.locations().cache_path()
        self.add_tool(DirectoryReader(appliance_data))
        self.add_tool(FileReader(appliance_data, cache_path))
