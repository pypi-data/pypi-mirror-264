...
export function RoleStore(props) {
  ...
  const conversations = async (flag) => {
    try {
      const agent = await ApiGet(API_Path.agentConversationsGetUrl(reactSelect?.shortcode));
      if (agent?.status === 200) {
  ...
  const getUserFiles = async (shortcode) => {
    ...
    try {
      const files = await ApiGet(API_Path.collectionsGetCollectionUrl(shortcode));
