...
class RemoteException(Exception):
    ...
    def __init__(self, remote_type: str, remote_msg: str):
        super().__init__(remote_type, remote_msg)
    ...
    async def set_exception(self, exception: Exception):
        ...
        if channel:
            await channel._write_failure(
                self._msg_id, [type(exception).__name__, str(exception)]
            )
...
class Channel:
    ...
    def __init__(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        asyncio.open_connection or given to an asycnio.start_server
        callback.
        """
        self._reader = reader
        self._writer = writer
