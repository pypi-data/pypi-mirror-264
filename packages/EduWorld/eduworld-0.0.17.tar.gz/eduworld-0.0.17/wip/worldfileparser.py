...
class WorldFileParser:
    """Class for loading world files in AlgoWorld World format (.aww extension)
    Return
    Usage:
        wf = WorldFileParser()
        board = wf.load("file_name_with_or_without_extension")
    ...
    def _find_world_path(self, world_name):
        ...
        package_path = Path(__file__).absolute().parent.parent / "worlds"
