        ...
        while not self.stop_event.is_set():
            try:
                if is_full_day_closed():
                    self.logger.info(f"{self.session_id}: full day closed")
                    break

                elif self.is_before_opening():
                    self.logger.info(f"{self.session_id}: market is not open yet")
                    await asyncio.sleep(1)
                    continue

                elif self.is_safe_to_close():
                    self.logger.info(f"{self.session_id}: market is already closed")
                    break
