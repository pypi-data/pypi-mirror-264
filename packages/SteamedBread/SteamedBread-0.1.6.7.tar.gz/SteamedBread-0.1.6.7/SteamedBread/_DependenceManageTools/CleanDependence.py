...
# 不能直接卸载, 需要通过卸载SteamedBread 间接清理
BLACKLIST = [
    "allure-pytest",
    "colorama",
    "ddddocr",
...
def autoremove(names, yes=False):
    ...
    flag = any([_ in BLACKLIST for _ in names])
    if flag and POWER_PACKAGE not in names:
        print(f"""
欲卸载的库及依赖: {names}
预制的黑名单列表: {BLACKLIST}
建议直接卸载依赖: {POWER_PACKAGE}
⚠️⚠️⚠️⚠️⚠️⚠️⚠️⚠️: 卸载{POWER_PACKAGE}后, 将不能继续使用 SteamedBread-Uninstall 命令工具了
        """)
        return

    print(f"你要卸载的依赖及其子依赖: {names}")
