...
class Resultatoversigt:
    ...
    def __post_init__(self):
        logger.info("Starter aggregering")
    ...
    def __save_dataframe(self) -> None:
        agg_file_name = "Aggregeret_datasæt.csv"
        self.data_agg.to_csv(Path(self.output_dir, agg_file_name), encoding="utf-8-sig", sep=";", decimal=",", index=False)
        logger.info(f"Det aggregerede datasæt er gemt i {self.output_dir} med navnet: {agg_file_name}")
        
    def __aggregering(self):
        self.data_filtered = self.__filter_data()
        self.data_agg = self.__agg_hierarchy()
        if self.config.replies_calc:
            self.data_agg = self.__calculate_replies()
        self.data_agg = self.__aggregating_vars()
        if self.config.include_themes:
            self.__calculate_themes()
        self.__sort_columns()
        if self.config.remove_minreplies:
            self.data_agg = self.data_agg[self.data_agg["Svar"]>=self.config.min_replies]
        self.data_agg = self.data_agg.sort_values(by=self.config.ptypeTs, na_position="first")
        self.data_agg = self.data_agg.rename(self.config.rename_vars, axis="columns")
    ...
    def __sort_columns(self) -> None:
        ...
        self.data_agg = self.data_agg.reindex(columns=columnsSortList)
        
    def __calculate_themes(self) -> None:
        for theme, var in self.config.themes.items():
            self.data_agg[theme] = self.data_agg[var].mean(axis=1)
    ...
    def __calculate_replies(self) -> pd.DataFrame:
        ...
        return pd.merge(self.data_agg, repliesDF, how="left", left_on="ptypeId", right_on="ptypeId")
    ...
    def __aggregating_vars(self) -> pd.DataFrame:
        ...
        return pd.merge(self.data_agg, aggDataFinal, how="left", left_on="ptypeId", right_on="ptypeId")
