...
def inter_gene_annotater(basket):
    ...
    genes.sort(key=lambda x: x[0]) #sort by start position of the gene
...
def essentials(chunk,variables):
    ...
    for key in chunk:
        ...
        if sum(insertions) == 0: #gene has no insertions
            ...
            domain_motivs = np.sum(chunk[key].GC_content,axis=0)*2 #number of each motif in the gene (sum is gene lenght)
