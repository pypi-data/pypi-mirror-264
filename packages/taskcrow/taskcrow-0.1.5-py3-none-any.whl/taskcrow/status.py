...
class Status(Enum):
  READY = (auto(), 'ready')
  PENDING = (auto(), 'pending')  # 작업 시작전
  IN_PROCESSING = (auto(), 'in_processing')
  COMPLETE = (auto(), 'complete')
  # 완료되었으나, 특이사항 발생
  COMPLETE_BUT_EMPTY = (auto(), 'complete_but_empty')
  RE_DO_COMPLETE = (auto(), 're_do_complete')
  # Error status
  ERROR = (auto(), 'error')
...
if __name__ == '__main__':
  ...
  print(Status.READY) # 멤버를 문자열로 변환
  print(Status.get_status_by_code(2))  # 코드로부터 상태 검색
  print(Status.list_all_statuses())  # 모든 상태 목록 출력
