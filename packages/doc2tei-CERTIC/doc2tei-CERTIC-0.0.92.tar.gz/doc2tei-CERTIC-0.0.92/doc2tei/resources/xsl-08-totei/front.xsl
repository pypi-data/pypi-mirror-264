...
<xsl:template match="*:p[starts-with(@text:style-name,'TEI_archeo')][not(preceding-sibling::*:p[starts-with(@text:style-name,'TEI_archeo')])]">
  <div type="archeo">
    <p rend="{concat('archeo_',substring-after(@text:style-name,'TEI_archeo_'))}"><xsl:apply-templates/></p>
    <xsl:for-each select="following-sibling::*:p[starts-with(@text:style-name,'TEI_archeo')]">
      <p rend="{concat('archeo_',substring-after(@text:style-name,'TEI_archeo_'))}"><xsl:apply-templates select="node()"/></p>
    </xsl:for-each>
  </div>
</xsl:template>
