...
class MicroserviceTestCase(HEAAioHTTPTestCase):
    ...
    def expected_one_id(self) -> str | None:
        """
        Gets the ID of the expected GET data.

        :return: an id string or None if not found.
        :raises TypeError: if _expected_one is None.
        """
        first_expected_one_data = self._expected_one[0]['collection']['items'][0]['data']
        return next((e.get('value') for e in first_expected_one_data if e['name'] == 'id'), None)

    def body_put_id(self) -> str | None:
        """
        ...
        return next((e.get('value') for e in self._body_put['template']['data'] if e['name'] == 'id'), None)
