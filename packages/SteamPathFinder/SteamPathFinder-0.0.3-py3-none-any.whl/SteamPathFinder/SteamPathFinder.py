...
def get_vdf_file_path(steam_path):
    ...
    if not os.path.exists(vdf_file_path):
        raise Exception("ERROR: libraryfolders.vdf file does not exist.")
        return None
...
def get_steam_path():
    try:
        ...
        raise Exception("ERROR: Steam is not installed.")
        return None
    
    if not os.path.exists(steam_path):
        raise Exception("ERROR: The Steam path does not exist or is not accessible.")
        return None
...
def get_app_path(steam_path, app_id):
    ...
    try:
        ...
        raise Exception(f"ERROR: Unable to read the libraryfolders.vdf file. {e}")
        return None
        
    try:
        parsed = vdf.loads(data)
    except Exception as e:
        raise Exception(f"ERROR: Error parsing the VDF file. {e}")
        return None
    ...
    if app_path is None:
        raise Exception(f"ERROR: Could not find the installation path for app {app_id}.")
...
def get_game_path(steam_path, app_id, game_name):
    ...
    if not os.path.exists(game_path):
        raise Exception(f"ERROR: The game path does not exist. Check if the game name '{game_name}' is correct and the game is properly installed.")
        return None
