...
from .models import Target, Action, Emitter, Pulse, Instance, Machine, Alert, TargetStatus, InstanceStatus
from .props import ActionProps, EmitterProps, TargetProps, InstanceProps
...
class RshipExecClient:
    ...
    async def connect(self):
        self.websocket = await asyncio.wait_for(websockets.connect(f"ws://{self.rship_host}:{self.rship_port}"), timeout=5)
        print(f"Connected to Rship server at ws://{self.rship_host}:{self.rship_port}")
        if self.on_connect:
            self.on_connect()
    ...
    async def set_data(self, item: Any):
        event = MEvent(item=item, item_type=type(item).__name__, change_type=MEventType.SET,
    ...
    async def save_action(self, action: Action):
        self.actions[action.id] = action
        await self.set_data(action)

    async def save_emitter(self, emitter: Emitter):
        self.emitters[emitter.id] = emitter
        await self.set_data(emitter)
