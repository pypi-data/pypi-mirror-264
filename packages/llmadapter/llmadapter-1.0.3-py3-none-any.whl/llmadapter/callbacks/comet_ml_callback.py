...
import llmadapter
from llmadapter.callbacks.base import BaseCallbackHandler
from llmadapter.callbacks.utils import (
...
from llmadapter.schema import AgentAction, AgentFinish, Generation, LLMResult

LANGCHAIN_MODEL_NAME = "llmadapter-model"
...
class CometCallbackHandler(BaseMetadataCallbackHandler, BaseCallbackHandler):
    ...
    def __init__(
        ...
        self.experiment.log_other("Created from", "llmadapter")
        if tags:
        ...
        warning = (
            ...
            "based on updates to `llmadapter`. Please report any issues to "
    ...
    def flush_tracker(
        ...
        task_type: Optional[str] = "inference",
        workspace: Optional[str] = None,
        project_name: Optional[str] = "comet-llmadapter-demo",
        tags: Optional[Sequence] = None,
        name: Optional[str] = None,
        visualizations: Optional[List[str]] = None,
        complexity_metrics: bool = False,
        ...
        Args:
            ...
            langchain_asset: The llmadapter asset to save.
    ...
    def _log_session(self, langchain_asset: Optional[Any] = None) -> None:
        try:
            ...
            self.experiment.log_table("llmadapter-llm-session.csv", llm_session_df)
        except Exception:
            self.comet_ml.LOGGER.warning(
        ...
        try:
            metadata = {"langchain_version": str(llmadapter.__version__)}
            # Log the llmadapter low-level records as a JSON file directly
            self.experiment.log_asset_data(
                self.action_records, "llmadapter-action_records.json", metadata=metadata
            )
        except Exception:
            self.comet_ml.LOGGER.warning(
                "Failed to log session data to Comet",
        ...
        for idx, (prompt, output) in enumerate(zip(prompts, outputs)):
            ...
            for visualization in self.visualizations:
                try:
                    ...
                    self.experiment.log_asset_data(
                        html,
                        name=f"llmadapter-viz-{visualization}-{idx}.html",
