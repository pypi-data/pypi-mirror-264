...
def get_object_classes():
    ...
    from openai_old.object_classes import OBJECT_CLASSES
...
def convert_to_openai_object(
    ...
    if isinstance(resp, openai_old.openai_response.OpenAIResponse):
    ...
    if plain_old_data:
        ...
        resp, openai_old.openai_object.OpenAIObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, str):
            klass = get_object_classes().get(
                klass_name, openai_old.openai_object.OpenAIObject
            )
        else:
            klass = openai_old.openai_object.OpenAIObject
...
def default_api_key() -> str:
    if openai_old.api_key_path:
        with open(openai_old.api_key_path, "rt") as k:
            api_key = k.read().strip()
            if not api_key.startswith("sk-"):
                raise ValueError(f"Malformed API key in {openai_old.api_key_path}.")
            return api_key
    elif openai_old.api_key is not None:
        return openai_old.api_key
    else:
        raise openai_old.error.AuthenticationError(
