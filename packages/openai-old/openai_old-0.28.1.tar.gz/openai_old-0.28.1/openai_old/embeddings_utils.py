...
import openai_old
from openai_old.datalib.numpy_helper import numpy as np
from openai_old.datalib.pandas_helper import pandas as pd
...
def get_embedding(text: str, engine="text-similarity-davinci-001", **kwargs) -> List[float]:
    ...
    return openai_old.Embedding.create(input=[text], engine=engine, **kwargs)["data"][0]["embedding"]
...
async def aget_embedding(
    ...
    return (await openai_old.Embedding.acreate(input=[text], engine=engine, **kwargs))["data"][0][
        "embedding"
    ]
...
def get_embeddings(
    ...
    data = openai_old.Embedding.create(input=list_of_text, engine=engine, **kwargs).data
...
async def aget_embeddings(
    ...
    data = (await openai_old.Embedding.acreate(input=list_of_text, engine=engine, **kwargs)).data
    return [d["embedding"] for d in data]
