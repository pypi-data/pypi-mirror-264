...
import openai_old
from openai_old import error, util, version
from openai_old.openai_response import OpenAIResponse
from openai_old.util import ApiType
...
def _make_session() -> requests.Session:
    if openai_old.requestssession:
        if isinstance(openai_old.requestssession, requests.Session):
            return openai_old.requestssession
        return openai_old.requestssession()
    if not openai_old.verify_ssl_certs:
        warnings.warn("verify_ssl_certs is ignored; openai always verifies.")
    s = requests.Session()
    proxies = _requests_proxies_arg(openai_old.proxy)
    if proxies:
        s.proxies = proxies
    s.mount(
        "https://",
...
class APIRequestor:
    def __init__(
        ...
        self.api_base = api_base or openai_old.api_base
        self.api_key = key or util.default_api_key()
        self.api_type = (
            ApiType.from_str(api_type)
            if api_type
            else ApiType.from_str(openai_old.api_type)
        )
        self.api_version = api_version or openai_old.api_version
        self.organization = organization or openai_old.organization
    ...
    def request_headers(
        ...
        if openai_old.app_info:
            user_agent += " " + self.format_app_info(openai_old.app_info)
        ...
        if openai_old.app_info:
            ua["application"] = openai_old.app_info
    ...
    async def arequest_raw(
        ...
        *,
        ...
        request_kwargs = {
            ...
            "headers": headers,
            "data": data,
            "proxy": _aiohttp_proxies_arg(openai_old.proxy),
            "timeout": timeout,
        }
        try:
...
class AioHTTPSession(AsyncContextManager):
    ...
    async def __aenter__(self):
        self._session = openai_old.aiosession.get()
