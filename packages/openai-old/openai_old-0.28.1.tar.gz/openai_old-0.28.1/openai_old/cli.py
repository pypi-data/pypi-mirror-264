...
import openai_old
from openai_old.upload_progress import BufferReader
from openai_old.validators import (
...
class Engine:
    @classmethod
    def get(cls, args):
        engine = openai_old.Engine.retrieve(id=args.id)
        display(engine)

    @classmethod
    def update(cls, args):
        engine = openai_old.Engine.modify(args.id, replicas=args.replicas)
        display(engine)
    ...
    def generate(cls, args):
        ...
        resp = openai_old.Engine(id=args.id).generate(
    ...
    @classmethod
    def list(cls, args):
        engines = openai_old.Engine.list()
        display(engines)
...
class Completion:
    ...
    def create(cls, args):
        ...
        resp = openai_old.Completion.create(
            engine=args.engine,
            model=args.model,
            n=args.n,
            max_tokens=args.max_tokens,
...
class Deployment:
    @classmethod
    def get(cls, args):
        resp = openai_old.Deployment.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = openai_old.Deployment.delete(args.id)
        print(model)

    @classmethod
    def list(cls, args):
        models = openai_old.Deployment.list()
        print(models)

    @classmethod
    def create(cls, args):
        models = openai_old.Deployment.create(
...
class Model:
    @classmethod
    def get(cls, args):
        resp = openai_old.Model.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        model = openai_old.Model.delete(args.id)
        print(model)
...
class File:
    ...
    @classmethod
    def get(cls, args):
        resp = openai_old.File.retrieve(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        file = openai_old.File.delete(args.id)
        print(file)

    @classmethod
    def list(cls, args):
        file = openai_old.File.list()
        print(file)
...
class Image:
    ...
    def create_variation(cls, args):
        with open(args.image, "rb") as file_reader:
            buffer_reader = BufferReader(file_reader.read(), desc="Upload progress")
        resp = openai_old.Image.create_variation(
            image=buffer_reader,
            size=args.size,
            n=args.num_images,
            response_format=args.response_format,
    ...
    def create_edit(cls, args):
        ...
        resp = openai_old.Image.create_edit(
            image=image_reader,
            mask=mask_reader,
            prompt=args.prompt,
            size=args.size,
...
class Audio:
    ...
    def transcribe(cls, args):
        ...
        resp = openai_old.Audio.transcribe_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
    ...
    def translate(cls, args):
        with open(args.file, "rb") as r:
            file_reader = BufferReader(r.read(), desc="Upload progress")
        resp = openai_old.Audio.translate_raw(
            # Required
            model=args.model,
            file=file_reader,
            filename=args.file,
...
class FineTune:
    ...
    def _maybe_upload_file(
        ...
        if check_if_file_exists:
            bytes = len(content)
            matching_files = openai_old.File.find_matching_files(
    ...
    def _get_or_upload(cls, file, check_if_file_exists=True):
        try:
            ...
            openai_old.File.retrieve(file)
            return file
        except openai_old.error.InvalidRequestError:
            pass
    ...
    def create(cls, args):
        ...
        resp = openai_old.FineTune.create(**create_args)
    ...
    @classmethod
    def get(cls, args):
        resp = openai_old.FineTune.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = openai_old.FineTune.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise openai_old.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = openai_old.FineTune.retrieve(id=args.id)["result_files"][0]
        resp = openai_old.File.download(id=result_file["id"])
        print(resp.decode("utf-8"))

    @classmethod
    def events(cls, args):
        if args.stream:
            raise openai_old.error.OpenAIError(
                message=(
        ...
        resp = openai_old.FineTune.list_events(id=args.id)  # type: ignore
        print(resp)
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            status = openai_old.FineTune.retrieve(job_id).status
            sys.stdout.write(
        ...
        events = openai_old.FineTune.stream_events(job_id)
        ...
        resp = openai_old.FineTune.retrieve(id=job_id)
        status = resp["status"]
        if status == "succeeded":
    ...
    @classmethod
    def cancel(cls, args):
        resp = openai_old.FineTune.cancel(id=args.id)
        print(resp)

    @classmethod
    def delete(cls, args):
        resp = openai_old.FineTune.delete(sid=args.id)
        print(resp)
...
class FineTuningJob:
    ...
    def list(cls, args):
        has_ft_jobs = False
        for fine_tune_job in openai_old.FineTuningJob.auto_paging_iter():
            has_ft_jobs = True
            print(fine_tune_job)
        if not has_ft_jobs:
    ...
    def _maybe_upload_file(
        ...
        sys.stdout.flush()
        status = openai_old.File.wait_for_processing(resp["id"])
        if status != "processed":
            raise openai_old.error.OpenAIError(
    ...
    def create(cls, args):
        ...
        resp = openai_old.FineTuningJob.create(**create_args)
        print(resp)
        return

    @classmethod
    def get(cls, args):
        resp = openai_old.FineTuningJob.retrieve(id=args.id)
        print(resp)

    @classmethod
    def results(cls, args):
        fine_tune = openai_old.FineTuningJob.retrieve(id=args.id)
        if "result_files" not in fine_tune or len(fine_tune["result_files"]) == 0:
            raise openai_old.error.InvalidRequestError(
                f"No results file available for fine-tune {args.id}", "id"
            )
        result_file = openai_old.FineTuningJob.retrieve(id=args.id)["result_files"][0]
        resp = openai_old.File.download(id=result_file)
        print(resp.decode("utf-8"))
    ...
    def events(cls, args):
        ...
        while has_more:
            resp = openai_old.FineTuningJob.list_events(id=args.id, after=after)  # type: ignore
    ...
    @classmethod
    def cancel(cls, args):
        resp = openai_old.FineTuningJob.cancel(id=args.id)
        print(resp)
...
class WandbLogger:
    @classmethod
    def sync(cls, args):
        import openai_old.wandb_logger

        resp = openai_old.wandb_logger.WandbLogger.sync(
            id=args.id,
