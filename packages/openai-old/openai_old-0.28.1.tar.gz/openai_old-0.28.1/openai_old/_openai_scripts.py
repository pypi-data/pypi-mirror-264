...
import openai_old
from openai_old import version
from openai_old.cli import api_register, display_error, tools_register, wandb_register
...
def main():
    ...
    openai_old.debug = True
    if args.api_key is not None:
        openai_old.api_key = args.api_key
    if args.api_base is not None:
        openai_old.api_base = args.api_base
    if args.organization is not None:
        openai_old.organization = args.organization
    if args.proxy is not None:
        openai_old.proxy = {}
        for proxy in args.proxy:
            if proxy.startswith('https'):
                openai_old.proxy['https'] = proxy
            elif proxy.startswith('http'):
                openai_old.proxy['http'] = proxy

    try:
        args.func(args)
    except openai_old.error.OpenAIError as e:
        display_error(e)
        return 1
