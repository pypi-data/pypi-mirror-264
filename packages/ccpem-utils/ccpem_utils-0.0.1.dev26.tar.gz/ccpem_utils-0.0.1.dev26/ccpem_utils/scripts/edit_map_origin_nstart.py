...
def main():
    ...
    if args.use_record == "origin":
        if not args.refmap and not all(o for o in ref_origin):
            raise ValueError(
                "Input either a reference map or origin/nstart coordinates to set"
            )
    ...
    # use nstart
        ...
        if not args.refmap and not all(n for n in ref_nstart):
            raise ValueError(
                "Input either a reference map or nstart coordinates to set"
            )
    ...
    # both
        ...
        if not args.refmap and (
            not all(o for o in ref_origin) or not all(n for n in ref_nstart)
        ):
            raise ValueError(
                "Input either a reference map or both origin and nstart coordinates "
                "to set"
            )
        # same origin? just create a symlink
