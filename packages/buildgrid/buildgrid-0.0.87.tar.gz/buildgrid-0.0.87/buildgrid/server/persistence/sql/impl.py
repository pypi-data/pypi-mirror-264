...
class SQLDataStore:
    ...
    def synchronize_bot_lease(
        ...
        with self._sql.session(exceptions_to_not_raise_on=[Exception]) as session:
            ...
            if not bots:
                raise InvalidArgumentError(f"Bot does not exist while validating leases: bot_id=[{bot_id}]")
            try:
                ...
                if len(bots) > 1:
                    raise BotSessionMismatchError(
                        f"Bot id [{bot_id}] is registered to more than one bot session. "
                        f"names=[{', '.join(bot.name for bot in bots)}]"
                    )
                if bots[0].name != bot_name:
                    raise BotSessionMismatchError(
                        f"Mismatch between client supplied client_bot_id=[{bot_id}] with client_bot_name=[{bot_name}] "
                        f"and buildgrid record of for BotSession with name=[{bots[0].name}]."
                    )
                bot = bots[0]
                ...
                if bot.lease_id is None:
                    ...
                    raise BotSessionClosedError("Bot session lease id does not match the database.")
                ...
                if not job:
                    raise BotSessionClosedError("Bot session lease id points to non-existent job.")
                ...
                if not job.leases:
                    raise BotSessionClosedError("Leases were changed while job was being locked.")

                db_lease = job.leases[0]

                # Another case where something bad has happened. Stop working now to try and recover.
                if session_lease and job.name != session_lease.id:
                    raise BotSessionClosedError("Bot session lease id does not match the database.")
                ...
                if not session_lease:
                    if db_lease.state != LeaseState.PENDING.value:
                        raise BotSessionClosedError("Session has no lease and database entry not in pending state.")
                    ...
                    if self.logstream_channel and self.logstream_instance is not None:
                        try:
                            ...
                            LOGGER.warning("Failed to create log stream", exc_info=e)
                ...
                if session_lease.state == LeaseState.COMPLETED.value and db_lease.state in (
                    ...
                    if (
                        ...
                        and (job.n_tries or 0) < self.MAX_N_TRIES
                    ):
                        self._retry_job_lease(session, job, db_lease)
                    else:
                ...
                if session_lease.state == LeaseState.CANCELLED.value:
                    raise BotSessionClosedError("Illegal attempt from session to set state as cancelled.")
                ...
                raise BotSessionClosedError(
                    "Unsupported lease state transition. "
                    f"worker_lease_state={session_lease.state} database_lease_state={db_lease.state}"
                )
