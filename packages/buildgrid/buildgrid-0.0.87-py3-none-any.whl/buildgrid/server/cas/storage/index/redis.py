...
class RedisIndex(IndexABC):
    ...
    def bulk_delete(self, digests: List[Digest]) -> List[str]:
        # Note that this only deletes from the index itself, not the
        # underlying storage even though this is part of the StorageABC.
        # However this behavior is what the bgd cleanup process (the only
        # caller of this method) expects of an index.
        ...
        while n_bytes > 0:
            ...
            for key, ttl in zip(keys, ttls):
                ...
                if digest and (blob_time <= threshold_time) and (digest.hash not in seen):
                    ...
                    keys_to_mark.append(digest)

            if keys_to_mark:
                LOGGER.debug(f"marking {len(keys_to_mark)} keys")
                deleteable.extend(keys_to_mark)
                seen.update([digest.hash for digest in keys_to_mark])
                if not dry_run:

                    def mark_keys(r: "redis.Redis[bytes]") -> None:
                        # We "mark" keys for deletion by actually just
                        # deleting the key. But since we are only "marking"
                        # the key to be deleted we do not adjust the
                        # total_size here. After "marking" bgd cleanup will
                        # delete from the underlying storage and then call
                        # our bulk_delete which is where the total_size is
                        # finally adjusted.
                        pipe = r.pipeline(transaction=False)
                        for digest in keys_to_mark:
                            pipe.delete(self._construct_key(digest))
                        pipe.execute()

                    self._redis.execute_rw(mark_keys)

            if cursor == 0:  # scan finished
                LOGGER.debug("cursor exhausted")
                break

        return [], list(deleteable)
