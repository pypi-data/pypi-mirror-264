...
def controller(request, asset_client: AssetClient):
    ...
    if use_cache == "action-cache":
        ...
        data_store.set_instance_name("sql")
    else:
        data_store = SQLDataStore(
            sql_provider,
            storage,
...
def controller_max_execution_timeout(request):
    ...
    with controller.execution_instance:
        yield controller
...
def test_uncacheable_action(controller, context):
    scheduler = controller.execution_instance.scheduler
...
def test_action_deduplication(controller, context):
...
def test_max_execution_timeout(controller_max_execution_timeout, context, sleep_time, n_operations):
