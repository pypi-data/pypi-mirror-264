...
from buildgrid.server.metrics_utils import DurationMetric, generator_method_duration_metric, publish_counter_metric
...
class SQLIndex(IndexABC):
    ...
    def get_blob(self, digest: Digest) -> Optional[IO[bytes]]:
        ...
        with self._sql.scoped_session() as session:
            if entry := session.query(IndexEntry).filter(IndexEntry.digest_hash == digest.hash).first():
                ...
                self._bulk_delete([digest], session)
    ...
    def _bulk_read_blobs_with_fallback(self, digests: List[Digest]) -> Dict[str, bytes]:
        ...
        with self._sql.scoped_session() as session:
            ...
            if digests_expected_not_in_storage:
                self._bulk_delete(digests_expected_not_in_storage, session)
            session.commit()
    ...
    def _windowed_lru_digests(
        self, q: "Query[Any]", column: Column[Any], renew_whereclauses: bool = True
        ...
        with self._sql.scoped_session(exceptions_to_not_rollback_on=[StaleDataError]) as session:
            ...
            if include_marked:
                ...
                if monitoring_bus.is_enabled:
                    ...
                    publish_counter_metric(CLEANUP_INDEX_PREMARKED_BLOBS_METRIC_NAME, len(storage_digests), metadata)
            ...
            if collected_bytes < n_bytes:
                ...
                window = self._windowed_lru_digests(unmarked_query, IndexEntry.accessed_timestamp, renew_windows)
            ...
            return inline_digests, storage_digests

    def bulk_delete(self, digests: List[Digest]) -> List[str]:
        with self._sql.scoped_session(exceptions_to_not_rollback_on=[StaleDataError]) as session:
            return self._bulk_delete(digests, session)

    def _bulk_delete(self, digests: List[Digest], session: Session) -> List[str]:
