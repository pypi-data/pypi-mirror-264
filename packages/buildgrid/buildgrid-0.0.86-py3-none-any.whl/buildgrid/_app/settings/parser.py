...
class Remote(YamlFactory):
    ...
    def __new__(
        ...
        return RemoteStorage(url, instance_name, options_tuple, credentials, retries, max_backoff, request_timeout)
    ...
    def __new__(
        ...
        **kwargs,
        ...
        if bot_session_keepalive_timeout <= 0:
            click.echo(
                click.style(
                    f"ERROR: bot_session_keepalive_timeout must be greater than zero: {bot_session_keepalive_timeout}",
                    fg="red",
                    bold=True,
                ),
                err=True,
