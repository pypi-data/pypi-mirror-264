...
from nebula.task_server import TaskServer, serve
...
def mock_task_server_start(monkeypatch):
    monkeypatch.setattr(
        "nebula.task_server.TaskServer.start", mock_start := AsyncMock()
    )
    return mock_start
...
def mock_create_subscription(monkeypatch):
    monkeypatch.setattr(
        "nebula.task_server.TaskServer._subscribe_to_task_scheduling",
...
class TestServe:
    async def test_serve_raises_if_task_scheduling_not_enabled(self, foo_task):
        with temporary_settings({NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING: False}):
            with pytest.raises(
                RuntimeError,
                match="set NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING to True",
            ):
...
async def test_task_server_can_execute_a_single_async_single_task_run(
    ...
    updated_task_run = await nebula_client.read_task_run(task_run.id)
...
class TestTaskServerTaskRunRetries:
    async def test_task_run_via_task_server_respects_retry_policy(self, nebula_client):
        count = 0
    ...
    async def test_task_run_via_task_server_respects_retry_condition_fn(
        self, should_retry, nebula_client
    ):
        count = 0
...
class TestTaskServerTaskResults:
    ...
    async def test_task_run_via_task_server_respects_persist_result(
        self, persist_result, nebula_client
    ):
    ...
    async def test_task_run_via_task_server_respects_result_storage_key(
        self, storage_key, nebula_client
    ):
    ...
    async def test_task_run_via_task_server_respects_caching(
        self, async_foo_task, nebula_client, caplog
    ):
        count = 0
        ...
        new_updated_task_run = await nebula_client.read_task_run(task_run.id)
...
class TestTaskServerTaskTags:
    async def test_task_run_via_task_server_respects_tags(
        self, async_foo_task, nebula_client
    ):
        @task(tags=["foo", "bar"])
...
class TestTaskServerTaskStateHooks:
    async def test_task_run_via_task_server_runs_on_completion_hook(
        self, async_foo_task, nebula_client, capsys
    ):
