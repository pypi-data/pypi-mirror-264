...
from nebula.serializers import (
    CompressedSerializer,
    JSONSerializer,
    PickleSerializer,
    Serializer,
    nebula_json_object_decoder,
    nebula_json_object_encoder,
)
...
class TestJSONSerializer:
    ...
    def test_uses_alternative_json_library(self, monkeypatch):
        ...
        serializer.dumps("test")
        serializer.loads(b"test")
        dumps_mock.assert_called_once_with("test", default=nebula_json_object_encoder)
        loads_mock.assert_called_once_with(
            "test", object_hook=nebula_json_object_decoder
        )

    def test_allows_custom_encoder(self, monkeypatch):
        fake_object_encoder = MagicMock(return_value="foobar!")
        nebula_object_encoder = MagicMock()

        monkeypatch.setattr(
            "nebula.fake_object_encoder", fake_object_encoder, raising=False
        )
        monkeypatch.setattr(
            "nebula.serializers.nebula_json_object_encoder",
            nebula_object_encoder,
        )

        serializer = JSONSerializer(object_encoder="nebula.fake_object_encoder")

        # Encoder hooks are only called for unsupported objects
        obj = uuid.uuid4()
        result = serializer.dumps(obj)
        assert result == b'"foobar!"'
        nebula_object_encoder.assert_not_called()
    ...
    def test_allows_custom_decoder(self, monkeypatch):
        ...
        nebula_object_decoder = MagicMock()

        monkeypatch.setattr(
            "nebula.fake_object_decoder", fake_object_decoder, raising=False
        )

        monkeypatch.setattr(
            "nebula.serializers.nebula_json_object_decoder",
            nebula_object_decoder,
        )

        serializer = JSONSerializer(object_decoder="nebula.fake_object_decoder")
        ...
        nebula_object_decoder.assert_not_called()
    ...
    def test_allows_custom_kwargs(self, monkeypatch):
        ...
        dumps_mock.assert_called_once_with(
            "test", default=nebula_json_object_encoder, foo="bar"
        )
        loads_mock.assert_called_once_with(
            "test", object_hook=nebula_json_object_decoder, bar="foo"
        )
