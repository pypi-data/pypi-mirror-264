...
from nebula.exceptions import CancelledRun, CrashedRun, FailedRun
from nebula.results import (
    LiteralResult,
    PersistedResult,
    ResultFactory,
    UnpersistedResult,
...
class TestReturnValueToState:
    @pytest.fixture
    async def factory(self, nebula_client):
        return await ResultFactory.default_factory(client=nebula_client)
    ...
    async def test_returns_single_state_with_data_to_persist(self, nebula_client):
        factory = await ResultFactory.default_factory(
            client=nebula_client, persist_result=True
        )
    ...
    async def test_non_nebula_types_return_completed_state(self, factory):
