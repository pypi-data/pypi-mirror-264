...
from nebula.filesystems import (
    GCS,
    S3,
    Azure,
    GitHub,
...
from nebula.testing.utilities import AsyncMock, MagicMock
from nebula.utilities.filesystem import tmpchdir

TEST_PROJECTS_DIR = nebula.__development_base_path__ / "tests" / "test-projects"
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "nebula.filesystems.S3 has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `S3Bucket` block from nebula-aws instead.",
        ),
        (
            GCS,
            {"bucket_path": "bucket/path"},
            "nebula.filesystems.GCS has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `GcsBucket` block from nebula-gcp instead.",
        ),
        (
            Azure,
            {"bucket_path": "bucket/path"},
            "nebula.filesystems.Azure has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `AzureBlobStorageContainer` block from nebula-azure instead.",
        ),
        (
            GitHub,
            {"repository": "https://github.com/org/repo.git"},
            "nebula.filesystems.GitHub has been deprecated."
            " It will not be available after Sep 2024."
            " Use the `GitHubRepository` block from nebula-github instead.",
        ),
    ],
)
...
class TestRemoteFileSystem:
    ...
    async def test_put_directory_flat(self):
        ...
        await fs.put_directory(
            ...
            ignore_file=os.path.join(
                TEST_PROJECTS_DIR, "flat-project", ".nebulaignore"
            ),
        )
    ...
    async def test_put_directory_tree(self):
        ...
        await fs.put_directory(
            ...
            ignore_file=os.path.join(
                TEST_PROJECTS_DIR, "tree-project", ".nebulaignore"
            ),
        )
    ...
    async def test_put_directory_put_file_count(self):
        ignore_file = os.path.join(TEST_PROJECTS_DIR, "tree-project", ".nebulaignore")
        ...
        included_files = nebula.utilities.filesystem.filter_files(
            os.path.join(TEST_PROJECTS_DIR, "tree-project"),
            ignore_patterns,
            include_dirs=False,
        )
...
class TestGitHub:
    ...
    async def test_repository_default(self, monkeypatch):
        ...
        monkeypatch.setattr(nebula.filesystems, "run_process", mock)
        g = GitHub(repository="nebula")
        await g.get_directory()

        assert mock.await_count == 1
        expected_cmd = ["git", "clone", "nebula"]
    ...
    async def test_reference_default(self, monkeypatch):
        ...
        mock = AsyncMock(return_value=p())
        monkeypatch.setattr(nebula.filesystems, "run_process", mock)
        g = GitHub(repository="nebula", reference="2.0.0")
        await g.get_directory()

        assert mock.await_count == 1
        expected_cmd = ["git", "clone", "nebula", "-b", "2.0.0", "--depth", "1"]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_token_added_correctly_from_credential(self, monkeypatch):
        ...
        monkeypatch.setattr(nebula.filesystems, "run_process", mock)
        credential = "XYZ"
        repo = "https://github.com/kozmoai/nebula.git"
        g = GitHub(
            repository=repo,
        ...
        expected_cmd = [
            "git",
            "clone",
            f"https://{credential}@github.com/kozmoai/nebula.git",
            "--depth",
            "1",
        ]
        assert mock.await_args[0][0][: len(expected_cmd)] == expected_cmd
    ...
    async def test_ssh_fails_with_credential(self, monkeypatch):
        ...
        with pytest.raises(InvalidRepositoryURLError, match=error_msg):
            GitHub(
                repository="git@github.com:kozmoai/nebula.git",
                access_token=credential,
            )
...
class TestAzure:
    def test_init(self, monkeypatch):
        ...
        monkeypatch.setattr("nebula.filesystems.RemoteFileSystem", remote_storage_mock)
        Azure(
