...
from nebula.context import FlowRunContext, NebulaObjectRegistry, TaskRunContext
...
from nebula.infrastructure import Infrastructure, Process
from nebula.logging.loggers import flow_run_logger, get_logger
...
from nebula.utilities.filesystem import relative_path_to_current_platform, tmpchdir
...
def load_deployments_from_yaml(
    ...
    with NebulaObjectRegistry(capture_failures=True) as registry:
        for node in nodes:
            with tmpchdir(path):
...
class Deployment(BaseModel):
    ...
    A Nebula Deployment definition, used for specifying and building deployments.
    ...
    Examples:
        ...
        >>> from nebula.deployments import Deployment
        >>>
        ...
        ...     tags=["aws"],
        ...     storage=storage,
        ...     infra_overrides=dict("env.NEBULA_LOGGING_LEVEL"="DEBUG"),
        >>> )
        >>> deployment.apply()
    ...
    async def to_yaml(self, path: Path) -> None:
        ...
        with open(path, "w") as f:
            # write header
            f.write(
                "###\n### A complete description of a Nebula Deployment for flow"
                f" {self.flow_name!r}\n###\n"
            )
    ...
    async def upload_to_storage(
        self, storage_block: str = None, ignore_file: str = ".nebulaignore"
    ) -> Optional[int]:
        """
        ...
        Args:
            ...
            ignore_file: an optional path to a `.nebulaignore` file that specifies
                filename patterns to ignore when uploading to remote storage; if not
                provided, looks for `.nebulaignore` in the current working directory
        """
        file_count = None
        if storage_block:
        ...
        **kwargs,
        ...
        if not deployment.storage and not is_docker_based and not deployment.path:
            ...
            if not deployment.path:
                deployment.path = "/opt/nebula/flows"
