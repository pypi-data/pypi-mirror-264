...
from nebula.client.schemas import sorting
from nebula.client.utilities import inject_client
from nebula.filesystems import ReadableFileSystem
from nebula.results import PersistedResult
from nebula.serializers import Serializer
from nebula.server.database.dependencies import temporary_database_interface
...
def nebula_test_harness():
    ...
    with TemporaryDirectory() as temp_dir:
        with ExitStack() as stack:
            ...
            DB_PATH = "sqlite+aiosqlite:///" + str(Path(temp_dir) / "nebula-test.db")
            stack.enter_context(
                nebula.settings.temporary_settings(
                    # Clear the NEBULA_API_URL
                    restore_defaults={nebula.settings.NEBULA_API_URL},
                    # Use a temporary directory for the database
                    updates={
                        nebula.settings.NEBULA_API_DATABASE_CONNECTION_URL: DB_PATH,
                        nebula.settings.NEBULA_API_URL: None,
                    },
                )
            )
            yield
...
async def get_most_recent_flow_run(client: NebulaClient = None):
    if client is None:
        client = get_client()
...
async def assert_uses_result_storage(
    state: State, storage: Union[str, ReadableFileSystem], client: "NebulaClient"
):
    assert isinstance(state.data, PersistedResult)
    assert_blocks_equal(
