...
from nebula._internal.compatibility.experimental import (
...
from nebula.settings import (
    NEBULA_EXPERIMENTAL_WARN,
    NEBULA_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES,
)
...
from nebula._vendor.starlette import status

import nebula
import nebula.exceptions
import nebula.settings
import nebula.states
...
from nebula.client.schemas.actions import (
    ArtifactCreate,
    BlockDocumentCreate,
    BlockDocumentUpdate,
    BlockSchemaCreate,
...
from nebula.client.schemas.filters import (
...
from nebula.client.schemas.responses import (
    DeploymentResponse,
    FlowRunResponse,
    WorkerFlowRunResponse,
)
from nebula.client.schemas.schedules import SCHEDULE_TYPES
from nebula.client.schemas.sorting import (
...
from nebula.deprecated.data_documents import DataDocument
from nebula.events.schemas import Automation, ExistingAutomation
from nebula.logging import get_logger
from nebula.settings import (
    NEBULA_API_DATABASE_CONNECTION_URL,
    NEBULA_API_ENABLE_HTTP2,
    NEBULA_API_KEY,
    NEBULA_API_REQUEST_TIMEOUT,
    NEBULA_API_SSL_CERT_FILE,
    NEBULA_API_TLS_INSECURE_SKIP_VERIFY,
    NEBULA_API_URL,
    NEBULA_CLIENT_CSRF_SUPPORT_ENABLED,
    NEBULA_CLOUD_API_URL,
    NEBULA_UNIT_TEST_MODE,
)
from nebula.utilities.collections import AutoEnum

if TYPE_CHECKING:
    from nebula.flows import Flow as FlowObject
    from nebula.tasks import Task as TaskObject

from nebula.client.base import ASGIApp, NebulaHttpxClient, app_lifespan_context
...
def get_client(httpx_settings: Optional[dict] = None) -> "NebulaClient":
    """
    Retrieve a HTTP client for communicating with the Nebula REST API.
    ...
    ```
    """
    ctx = nebula.context.get_settings_context()
    api = NEBULA_API_URL.value()

    if not api:
        # create an ephemeral API if none was provided
        from nebula.server.api.server import create_app
...
class NebulaClient:
    """
    An asynchronous client for interacting with the [Nebula REST API](/api-ref/rest-api/).
    ...
    Examples:
        ...
        Say hello to a Nebula REST API
    ...
    def __init__(
        ...
        *,
        ...
        if NEBULA_API_TLS_INSECURE_SKIP_VERIFY:
            httpx_settings.setdefault("verify", False)
        else:
            cert_file = NEBULA_API_SSL_CERT_FILE.value()
            if not cert_file:
        ...
        httpx_settings["headers"].setdefault("X-NEBULA-API-VERSION", api_version)
        if api_key:
        ...
        if isinstance(api, str):
            ...
            httpx_settings.setdefault(
                ...
                httpx.Limits(
                    ...
                    # The Nebula Cloud LB will keep connections alive for 30s.
            ...
            httpx_settings.setdefault("http2", NEBULA_API_ENABLE_HTTP2.value())

            self.server_type = (
                ServerType.CLOUD
                if api.startswith(NEBULA_CLOUD_API_URL.value())
        ...
        # Connect to an in-process application
            ...
            # refs:
            # - https://github.com/kozmoai/nebula/pull/9637
            ...
            httpx_settings.setdefault("base_url", "http://ephemeral-nebula/api")
        ...
        httpx_settings.setdefault(
            "timeout",
            httpx.Timeout(
                connect=NEBULA_API_REQUEST_TIMEOUT.value(),
                read=NEBULA_API_REQUEST_TIMEOUT.value(),
                write=NEBULA_API_REQUEST_TIMEOUT.value(),
                pool=NEBULA_API_REQUEST_TIMEOUT.value(),
            ),
        )

        if not NEBULA_UNIT_TEST_MODE:
            httpx_settings.setdefault("follow_redirects", True)

        enable_csrf_support = (
            self.server_type != ServerType.CLOUD
            and NEBULA_CLIENT_CSRF_SUPPORT_ENABLED.value()
        )

        self._client = NebulaHttpxClient(
    ...
    async def create_flow(self, flow: "FlowObject") -> UUID:
        """
        Create a flow in the Nebula API.

        Args:
            flow: a [Flow][nebula.flows.Flow] object
    ...
    async def read_flow(self, flow_id: UUID) -> Flow:
        """
        Query the Nebula API for a flow by id.

        Args:
            flow_id: the flow ID of interest

        Returns:
            a [Flow model][nebula.client.schemas.objects.Flow] representation of the flow
        """
    ...
    async def read_flows(
        ...
        *,
        ...
        """
        Query the Nebula API for flows. Only flows matching all criteria will
        be returned.
    ...
    async def read_flow_by_name(
        ...
        """
        Query the Nebula API for a flow by name.
    ...
    async def create_flow_run_from_deployment(
        ...
        *,
        ...
        context: dict = None,
        state: nebula.states.State = None,
        name: str = None,
        ...
        Raises:
            httpx.RequestError: if the Nebula API does not successfully create a run for any reason
        ...
        if job_variables is not None and experiment_enabled("flow_run_infra_overrides"):
            if (
                NEBULA_EXPERIMENTAL_WARN
                and NEBULA_EXPERIMENTAL_WARN_FLOW_RUN_INFRA_OVERRIDES
            ):
                warnings.warn(
                    EXPERIMENTAL_WARNING.format(
                        feature="Flow run job variables",
                        group="flow_run_infra_overrides",
                        help="To use this feature, update your workers to Nebula 2.16.4 or later. ",
                    ),
                    ExperimentalFeature,
                    stacklevel=3,
                )
        ...
        context = context or {}
        state = state or nebula.states.Scheduled()
        tags = tags or []
    ...
    async def delete_flow_run(
        ...
        Raises:
            nebula.exceptions.ObjectNotFound: If request returns 404
        ...
        try:
            ...
            if e.response.status_code == status.HTTP_404_NOT_FOUND:
                raise nebula.exceptions.ObjectNotFound(http_exc=e) from e
            else:
                raise
    ...
    async def create_concurrency_limit(
        ...
        """
        Create a tag concurrency limit in the Nebula API. These limits govern concurrently
        running tasks.
    ...
    async def create_work_queue(
        ...
        Raises:
            nebula.exceptions.ObjectAlreadyExists: If request returns 409
            httpx.RequestError: If request fails
        ...
        try:
            ...
            if e.response.status_code == status.HTTP_409_CONFLICT:
                raise nebula.exceptions.ObjectAlreadyExists(http_exc=e) from e
    ...
    async def read_work_queue_by_name(
        ...
        Raises:
            nebula.exceptions.ObjectNotFound: if no work queue is found
    ...
    async def match_work_queues(
        ...
        """
        Query the Nebula API for work queues with names with a specific prefix.
    ...
    async def create_block_type(self, block_type: BlockTypeCreate) -> BlockType:
        """
        Create a block type in the Nebula API.
        """
        try:
            response = await self._client.post(
                "/block_types/",
    ...
    async def create_block_schema(self, block_schema: BlockSchemaCreate) -> BlockSchema:
        """
        Create a block schema in the Nebula API.
        """
        try:
            response = await self._client.post(
                "/block_schemas/",
    ...
    async def create_block_document(
        ...
        """
        Create a block document in the Nebula API. This data is used to configure a
        corresponding Block.
    ...
    async def update_block_document(
        ...
        """
        Update a block document in the Nebula API.
        """
        try:
    ...
    async def delete_block_type(self, block_type_id: UUID):
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                raise nebula.exceptions.ProtectedBlockError(
                    "Protected block types cannot be deleted."
                ) from e
            else:
                raise
    ...
    async def read_deployment(
        ...
        """
        Query the Nebula API for a deployment by id.

        Args:
            deployment_id: the deployment ID of interest

        Returns:
            a [Deployment model][nebula.client.schemas.objects.Deployment] representation of the deployment
        """
        try:
    ...
    async def read_deployment_by_name(
        ...
        """
        Query the Nebula API for a deployment by name.
    ...
    async def read_deployments(
        ...
        *,
        ...
        """
        Query the Nebula API for deployments. Only deployments matching all
    ...
    async def read_deployment_schedules(
        ...
        """
        Query the Nebula API for a deployment's schedules.
    ...
    async def read_flow_run(self, flow_run_id: UUID) -> FlowRun:
        """
        Query the Nebula API for a flow run by id.
    ...
    async def read_flow_runs(
        ...
        *,
        ...
        """
        Query the Nebula API for flow runs. Only flow runs matching all criteria will
        be returned.
    ...
    async def set_flow_run_state(
        ...
        Args:
            ...
            force: if True, disregard orchestration logic when setting the state,
                forcing the Nebula API to accept the state
    ...
    async def read_flow_run_states(
        ...
        return pydantic.parse_obj_as(List[nebula.states.State], response.json())
    ...
    async def read_task_run(self, task_run_id: UUID) -> TaskRun:
        """
        Query the Nebula API for a task run by id.
    ...
    async def read_task_runs(
        ...
        *,
        ...
        """
        Query the Nebula API for task runs. Only task runs matching all criteria will
        be returned.
    ...
    async def read_flow_run_notification_policies(
        ...
        """
        Query the Nebula API for flow run notification policies. Only policies matching all criteria will
        be returned.
    ...
    async def read_artifacts(
        ...
        *,
        ...
        """
        Query the Nebula API for artifacts. Only artifacts matching all criteria will
        be returned.
        Args:
    ...
    async def read_worker_metadata(self) -> Dict[str, Any]:
        """Reads worker metadata stored in Nebula collection registry."""
    ...
    async def create_automation(self, automation: Automation) -> UUID:
        """Creates an automation in Nebula Cloud."""
        if self.server_type != ServerType.CLOUD:
            raise RuntimeError("Automations are only supported for Nebula Cloud.")
    ...
    async def __aenter__(self):
        ...
        if self._ephemeral_app:
            self.logger.debug(
                ...
                f"{NEBULA_API_DATABASE_CONNECTION_URL.value()}"
            )
        else:
    ...
    def __enter__(self):
        raise RuntimeError(
            "The `NebulaClient` must be entered with an async context. Use 'async "
            "with NebulaClient(...)' not 'with NebulaClient(...)'"
        )
