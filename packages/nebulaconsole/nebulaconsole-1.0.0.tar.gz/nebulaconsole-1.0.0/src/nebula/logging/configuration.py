...
# This path will be used if `NEBULA_LOGGING_SETTINGS_PATH` is null
...
def load_logging_config(path: Path) -> dict:
    ...
    for key_tup, val in flat_config.items():
        env_val = os.environ.get(
            ...
            to_envvar("NEBULA_LOGGING_" + "_".join(key_tup)).upper()
        )
        if env_val:
            val = env_val
...
def setup_logging(incremental: Optional[bool] = None) -> dict:
    ...
    config = load_logging_config(
        (
            NEBULA_LOGGING_SETTINGS_PATH.value()
            if NEBULA_LOGGING_SETTINGS_PATH.value().exists()
    ...
    # Copy configuration of the 'nebula.extra' logger to the extra loggers
    extra_config = logging.getLogger("nebula.extra")

    for logger_name in NEBULA_LOGGING_EXTRA_LOGGERS.value():
