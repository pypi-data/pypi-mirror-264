...
from nebula.exceptions import MissingResult
...
from nebula.settings import (
    NEBULA_DEFAULT_RESULT_STORAGE_BLOCK,
    NEBULA_LOCAL_STORAGE_PATH,
    NEBULA_RESULTS_DEFAULT_SERIALIZER,
    NEBULA_RESULTS_PERSIST_BY_DEFAULT,
...
from nebula.utilities.pydantic import add_type_dispatch
...
async def get_default_result_storage() -> ResultStorage:
    ...
    """
    return (
        await Block.load(NEBULA_DEFAULT_RESULT_STORAGE_BLOCK.value())
        if NEBULA_DEFAULT_RESULT_STORAGE_BLOCK.value() is not None
        else LocalFileSystem(basepath=NEBULA_LOCAL_STORAGE_PATH.value())
    )
...
async def get_or_create_default_task_scheduling_storage() -> ResultStorage:
    ...
    default_storage_name, storage_path = cache_key = (
        NEBULA_TASK_SCHEDULING_DEFAULT_STORAGE_BLOCK.value(),
        NEBULA_LOCAL_STORAGE_PATH.value(),
    )
...
def get_default_result_serializer() -> ResultSerializer:
    ...
    Generate a default file system for result storage.
    """
    return NEBULA_RESULTS_DEFAULT_SERIALIZER.value()
...
def get_default_persist_setting() -> bool:
    ...
    """
    return NEBULA_RESULTS_PERSIST_BY_DEFAULT.value()
...
def _format_user_supplied_storage_key(key):
    ...
    runtime_vars = {key: getattr(nebula.runtime, key) for key in dir(nebula.runtime)}
    return key.format(**runtime_vars, parameters=nebula.runtime.task_run.parameters)
...
class ResultFactory(pydantic.BaseModel):
    ...
    @classmethod
    @inject_client
    async def default_factory(cls, client: "NebulaClient" = None, **kwargs):
        """
    ...
    @inject_client
    async def from_flow(
        cls: Type[Self], flow: "Flow", client: "NebulaClient" = None
    ) -> Self:
        """
    ...
    @inject_client
    async def from_task(
        cls: Type[Self], task: "Task", client: "NebulaClient" = None
    ) -> Self:
        """
...
class LiteralResult(BaseResult):
    ...
    These values are stored inline and JSON serialized when sent to the Nebula API.
    ...
    async def create(
        ...
        description = f"Result with value `{obj}` persisted to Nebula."
...
class PersistedResult(BaseResult):
    ...
    @sync_compatible
    @inject_client
    async def get(self, client: "NebulaClient") -> R:
        """
    ...
    @inject_client
    async def _read_blob(self, client: "NebulaClient") -> "PersistedResultBlob":
        assert (
            self.storage_block_id is not None
        ), "Unexpected storage block ID. Was it persisted?"
...
class PersistedResultBlob(pydantic.BaseModel):
    ...
    serializer: Serializer
    data: bytes
    nebula_version: str = pydantic.Field(default=nebula.__version__)
...
class UnknownResult(BaseResult):
    ...
    async def create(
        ...
        description = "Unknown result persisted to Nebula."
        return cls(value=obj, artifact_type="result", artifact_description=description)
