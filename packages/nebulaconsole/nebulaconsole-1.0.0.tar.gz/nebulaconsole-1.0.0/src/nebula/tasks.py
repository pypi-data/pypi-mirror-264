"""
Module containing the base workflow task class and decorator - for most use cases, using the [`@task` decorator][nebula.tasks.task] is preferred.
"""
...
from nebula.context import FlowRunContext, NebulaObjectRegistry
...
from nebula.utilities.asyncutils import Async, Sync
...
if TYPE_CHECKING:
    from nebula.context import TaskRunContext
...
class Task(Generic[P, R]):
    """
    A Nebula task definition.

    !!! note
        We recommend using [the `@task` decorator][nebula.tasks.task] for most use-cases.

    Wraps a function with an entrypoint to the Nebula engine. Calling this class within a flow function
    ...
    Args:
        ...
        tags: An optional set of tags to be associated with runs of this task. These
            tags are combined with any tags defined by a `nebula.tags` context at
            task runtime.
        ...
        log_prints: If set, `print` statements in the task will be redirected to the
            Nebula logger for the task run. Defaults to `None`, which indicates
    ...
    def __init__(
        ...
        self.retries = (
            retries if retries is not None else NEBULA_TASK_DEFAULT_RETRIES.value()
        )
        if retry_delay_seconds is None:
            retry_delay_seconds = NEBULA_TASK_DEFAULT_RETRY_DELAY_SECONDS.value()
    ...
    def __call__(
        ...
        **kwargs: P.kwargs,
        ...
        the result is wrapped in a Nebula State which provides error handling.
        """
        from nebula.engine import enter_task_run_engine
        ...
        if (
            NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING.value()
            and not FlowRunContext.get()
        ):
            from nebula import get_client
    ...
    def submit(
        ...
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> Awaitable[NebulaFuture[T, Async]]:
        ...
    ...
    def submit(
        ...
        *args: Any,
        ...
        wait_for: Optional[Iterable[NebulaFuture]] = None,
        **kwargs: Any,
    ) -> Union[NebulaFuture, Awaitable[NebulaFuture], TaskRun, Awaitable[TaskRun]]:
        """
        ...
        Returns:
            ...
            If `return_state` is True a future wrapped in a Nebula State allowing asynchronous access to
                the state of the task
        ...
        from nebula.engine import create_autonomous_task_run, enter_task_run_engine
    ...
    def map(
        ...
        *args: P.args,
        **kwargs: P.kwargs,
    ) -> Awaitable[List[NebulaFuture[T, Async]]]:
        ...
        ...
        **kwargs: Any,
        ...
        Args:
            *args: Iterable and static arguments to run the tasks with
            return_state: Return a list of Nebula States that wrap the results
                of each task run.
        ...
        from nebula.engine import begin_task_map, enter_task_run_engine
    ...
    def serve(self, task_runner: Optional[BaseTaskRunner] = None) -> "Task":
        ...
        establish a websocket connection with the Nebula server and listen for
        ...
        if not NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING:
            raise ValueError(
        ...
        from nebula.task_server import serve
...
def task(
    ...
    *,
    ...
    """
    Decorator to designate a function as a task in a Nebula workflow.
    ...
    Examples:
        ...
        >>> from nebula.tasks import task_input_hash
