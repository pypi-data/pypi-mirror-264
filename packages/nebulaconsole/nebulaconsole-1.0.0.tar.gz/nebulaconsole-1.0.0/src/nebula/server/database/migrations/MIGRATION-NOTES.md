...
Adds a table for storing key / value configuration options for Nebula REST API in the database.
...
Upgrading to an existing database to use migrations requires the use of `nebula database alembic stamp` before a reset will drop existing tables.
