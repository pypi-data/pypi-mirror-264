...
from nebula.server.utilities.database import (
    JSON,
    UUID,
    GenerateUUID,
    Pydantic,
...
from nebula.server.utilities.encryption import decrypt_fernet, encrypt_fernet
...
class BaseORMConfiguration(ABC):
    """
    Abstract base class used to inject database-specific ORM configuration into Nebula.

    Modifications to core Nebula REST API data structures can have unintended consequences.
    Use with caution.
    ...
    def _create_orm_models(
        ...
        """
        Defines the ORM models used in Nebula REST API and binds them to the `self`. This method
...
class AsyncPostgresORMConfiguration(BaseORMConfiguration):
    ...
    def versions_dir(self) -> Path:
        """Directory containing migrations"""
        return (
            Path(nebula.server.database.__file__).parent
            / "migrations"
            / "versions"
            / "postgresql"
        )
