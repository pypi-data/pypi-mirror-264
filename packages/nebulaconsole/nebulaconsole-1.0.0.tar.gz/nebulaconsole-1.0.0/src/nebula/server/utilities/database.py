"""
Utilities for interacting with Nebula REST API database and ORM layer.

Nebula supports both SQLite and Postgres. Many of these utilities
allow the Nebula REST API to seamlessly switch between the two.
"""
...
def get_dialect(
    ...
    Primary use case is figuring out whether the Nebula REST API is communicating with
    SQLite or Postgres.
    ...
    Example:
        ...
        dialect = get_dialect(NEBULA_API_DATABASE_CONNECTION_URL.value())
        if dialect == "sqlite":
            print("Using SQLite!")
        else:
