...
class NebulaBaseModel(BaseModel):
    """A base pydantic.BaseModel for all Nebula schemas and pydantic models.

    As the basis for most Nebula schemas, this base model usually ignores extra
    ...
    Nebula client loading data from a server running a possibly-newer version
    of Nebula will be able to process those new fields gracefully. However,
    when NEBULA_TEST_MODE is on, extra fields are forbidden in order to catch
    ...
    class Config:
        ...
        if os.getenv("NEBULA_TEST_MODE", "0").lower() in ["1", "true"]:
            extra = "forbid"
        else:
            extra = "ignore"
        ...
        json_encoders = {
            ...
            # for Secret dict in nebula.blocks.fields
    ...
    def _reset_fields(self) -> Set[str]:
        """A set of field names that are reset when the NebulaBaseModel is copied.
        These fields are also disregarded for equality comparisons.
        """
        return set()

    def __eq__(self, other: Any) -> bool:
        """Equaltiy operator that ignores the resettable fields of the NebulaBaseModel.

        NOTE: this equality operator will only be applied if the NebulaBaseModel is
        the left-hand operand. This is a limitation of Python.
        """
        copy_dict = self.dict(exclude=self._reset_fields())
        if isinstance(other, NebulaBaseModel):
...
class IDBaseModel(NebulaBaseModel):
    """
    A NebulaBaseModel with an auto-generated UUID ID value.
...
class ORMBaseModel(IDBaseModel):
    """
    A NebulaBaseModel with an auto-generated UUID ID value and created /
