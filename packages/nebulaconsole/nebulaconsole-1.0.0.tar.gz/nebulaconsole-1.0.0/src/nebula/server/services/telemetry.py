...
from nebula.server.models import configuration
from nebula.server.schemas.core import Configuration
...
class Telemetry(LoopService):
    """
    This service sends anonymous data (e.g. count of flow runs) to Nebula to help us
    improve. It can be toggled off with the NEBULA_SERVER_ANALYTICS_ENABLED setting.
    """
    ...
    def __init__(self, loop_seconds: int = None, **kwargs):
        ...
        self.telemetry_environment = os.environ.get(
            "NEBULA_API_TELEMETRY_ENVIRONMENT", "production"
        )

    @inject_db
    async def _fetch_or_set_telemetry_session(self, db: NebulaDBInterface):
        """
    ...
    async def run_once(self):
        ...
        heartbeat = {
            ...
            "payload": {
                ...
                "nebula_version": nebula.__version__,
        ...
        try:
            async with httpx.AsyncClient() as client:
                result = await client.post(
                    "https://sens-o-matic.nebula.io/",
                    json=heartbeat,
                    headers={"x-nebula-event": "nebula_server"},
                )
            ...
            self.logger.error(
                ...
                exc_info=NEBULA_DEBUG_MODE.value(),
            )
