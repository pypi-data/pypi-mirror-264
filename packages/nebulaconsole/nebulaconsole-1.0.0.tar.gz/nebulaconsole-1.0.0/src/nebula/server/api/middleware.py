...
from nebula._vendor.fastapi import status
...
class CsrfMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if (
            settings.NEBULA_SERVER_CSRF_PROTECTION_ENABLED.value()
            and request_needs_csrf_protection
        ):
            incoming_token = request.headers.get("Nebula-Csrf-Token")
            incoming_client = request.headers.get("Nebula-Csrf-Client")
