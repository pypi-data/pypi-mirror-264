...
from nebula.server.task_queue import MultiQueue, TaskQueue
from nebula.server.utilities import subscriptions
...
router = NebulaRouter(prefix="/task_runs", tags=["Task Runs"])
...
async def scheduled_task_subscription(websocket: WebSocket):
    websocket = await subscriptions.accept_nebula_socket(websocket)
    if not websocket:
        return
