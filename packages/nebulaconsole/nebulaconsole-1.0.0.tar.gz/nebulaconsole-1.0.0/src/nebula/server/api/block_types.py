...
from nebula._vendor.fastapi import Body, Depends, HTTPException, Path, Query, status

from nebula.blocks.core import _should_update_block_type
from nebula.server import models, schemas
from nebula.server.api import dependencies
...
router = NebulaRouter(prefix="/block_types", tags=["Block types"])
...
async def create_block_type(
    ...
    """
    # API-created blocks cannot start with the word "Nebula"
    # as it is reserved for system use
    if block_type.name.lower().startswith("nebula"):
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            detail="Block type names beginning with 'Nebula' are reserved.",
        )
    try:
