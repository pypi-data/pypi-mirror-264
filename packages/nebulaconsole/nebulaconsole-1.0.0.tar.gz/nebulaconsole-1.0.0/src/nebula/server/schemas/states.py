...
class CountByState(NebulaBaseModel):
...
class State(StateBaseModel, Generic[R]):
    ...
    def from_orm_without_result(
        cls,
        orm_state: Union[
            "nebula.server.database.orm_models.ORMFlowRunState",
            "nebula.server.database.orm_models.ORMTaskRunState",
        ],
    ...
    def result(self, raise_on_failure: bool = True, fetch: Optional[bool] = None):
        ...
        warnings.warn(
            (
                ...
                " `nebula.server.schemas.states.State` and will be removed in a future"
                " release. When result retrieval is needed, use `nebula.states.State`."
            ),
            DeprecationWarning,
            stacklevel=2,
        )
    ...
    def to_state_create(self):
        ...
        from nebula.client.schemas import State

        warnings.warn(
            (
                "Use of `nebula.server.schemas.states.State` from the client is"
                " deprecated and support will be removed in a future release. Use"
                " `nebula.states.State` instead."
            ),
            DeprecationWarning,
            stacklevel=2,
        )
