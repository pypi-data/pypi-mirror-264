...
Intended for internal use by the Nebula REST API.
"""
...
from nebula.server.api.workers import WorkerLookups
...
from nebula.server.utilities.database import json_contains
...
async def read_deployment(
    session: sa.orm.Session, deployment_id: UUID, db: NebulaDBInterface
):
...
async def read_deployment_by_name(
    session: sa.orm.Session, name: str, flow_name: str, db: NebulaDBInterface
):
...
async def schedule_runs(
    ...
    """
    if min_runs is None:
        min_runs = NEBULA_API_SERVICES_SCHEDULER_MIN_RUNS.value()
    if max_runs is None:
        max_runs = NEBULA_API_SERVICES_SCHEDULER_MAX_RUNS.value()
    ...
    if min_time is None:
        min_time = NEBULA_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME.value()
...
async def _insert_scheduled_flow_runs(
    session: sa.orm.Session, runs: List[Dict], db: NebulaDBInterface
) -> List[UUID]:
    """
...
async def check_work_queues_for_deployment(
    db: NebulaDBInterface, session: sa.orm.Session, deployment_id: UUID
...
async def _update_deployment_last_polled(
    db: NebulaDBInterface, session: sa.orm.Session, deployment_ids: List[UUID]
) -> None:
    """
...
async def delete_schedules_for_deployment(
    db: NebulaDBInterface, session: AsyncSession, deployment_id: UUID
) -> bool:
    """
