...
async def create_flow(
    session: sa.orm.Session, flow: schemas.core.Flow, db: NebulaDBInterface
):
    """
    Creates a new flow.
...
@inject_db
async def read_flow(session: sa.orm.Session, flow_id: UUID, db: NebulaDBInterface):
    """
    Reads a flow by id.
...
@inject_db
async def read_flow_by_name(session: sa.orm.Session, name: str, db: NebulaDBInterface):
    """
...
async def delete_flow(
    session: sa.orm.Session, flow_id: UUID, db: NebulaDBInterface
) -> bool:
    """
    Delete a flow by id.
