...
from nebula.blocks.core import Block, InvalidBlockRegistration
...
blocks_app = NebulaTyper(name="block", help="Commands for working with blocks.")
blocktypes_app = NebulaTyper(
...
async def register(
    ...
    Examples:
        ...
        $ nebula block register -m nebula_aws.credentials
        \b
        Register block types in a .py file:
        $ nebula block register -f my_blocks.py
    """
    ...
    msg = (
        ...
        "go to the Blocks page in the Nebula UI.\n"
    )
...
async def block_ls():
    ...
    table = Table(
        title="Blocks", caption="List Block Types using `nebula block type ls`"
    )
...
async def block_create(
    block_type_slug: str = typer.Argument(
        ...,
        help="A block type slug. View available types with: nebula block type ls",
        show_default=False,
    ),
):
    """
    Generate a link to the Nebula UI to create a block.
    """
    async with get_client() as client:
        try:
        ...
        if not NEBULA_UI_URL:
            exit_with_error(
                "Nebula must be configured to use a hosted Nebula server or "
                "Nebula Cloud to display the Nebula UI"
            )

        block_link = f"{NEBULA_UI_URL.value()}/blocks/catalog/{block_type.slug}/create"
        app.console.print(
...
async def list_types():
    ...
    for blocktype in sorted(block_types, key=lambda x: x.name):
        table.add_row(
            ...
            f"nebula block create {blocktype.slug}",
        )
