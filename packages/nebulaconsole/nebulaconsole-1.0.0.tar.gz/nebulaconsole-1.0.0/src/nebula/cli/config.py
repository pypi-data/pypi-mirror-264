...
help_message = """
    Commands for interacting with Nebula settings.
"""

config_app = NebulaTyper(name="config", help=help_message)
...
def set_(settings: List[str]):
    ...
    try:
        new_profile = nebula.settings.update_current_profile(parsed_settings)
    ...
    for setting, value in parsed_settings.items():
        ...
        if nebula.settings.SETTING_VARIABLES[setting].deprecated:
            app.console.print(
                f"[yellow]{nebula.settings.SETTING_VARIABLES[setting].deprecated_message}."
            )
...
def validate():
    ...
    profile = profiles[nebula.context.get_settings_context().profile.name]
...
def unset(settings: List[str]):
    ...
    for setting in settings:
        ...
        parsed.add(nebula.settings.SETTING_VARIABLES[setting])
...
def view(
    ...
    default_settings = nebula.settings.get_default_settings()
    env_settings = nebula.settings.get_settings_from_env()
    ...
    app.console.print(f"NEBULA_PROFILE={context.profile.name!r}")
