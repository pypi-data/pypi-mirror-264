...
from nebula.cli.cloud import cloud_app, confirm_logged_in
...
webhook_app = NebulaTyper(
    name="webhook", help="Commands for interacting with Nebula Cloud Webhooks"
)
...
async def ls():
    ...
    async with get_cloud_client(host=NEBULA_API_URL.value()) as client:
...
async def create(
    ...
    if not template:
        exit_with_error(
            ...
            ' "your.event.name", "resource": { "nebula.resource.id":'
            ' "your.resource.id" } }'
            " \nhttps://docs.nebula.io/latest/cloud/webhooks/#webhook-templates"
        )
