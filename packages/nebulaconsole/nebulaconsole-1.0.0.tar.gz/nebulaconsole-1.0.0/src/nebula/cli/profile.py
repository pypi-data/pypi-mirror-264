...
from nebula.cli.cloud import CloudUnauthorizedError, get_cloud_client
from nebula.cli.root import app
from nebula.client.orchestration import ServerType, get_client
from nebula.context import use_profile
from nebula.utilities.collections import AutoEnum

profile_app = NebulaTyper(
    name="profile", help="Commands for interacting with your Nebula profiles."
)
...
def create(
    ...
    if from_name:
        ...
        profiles.add_profile(nebula.settings.Profile(name=name, settings={}))

    nebula.settings.save_profiles(profiles)
    ...
    app.console.print(
        textwrap.dedent(
            ...
            Use created profile for future, subsequent commands:
                nebula profile use {name!r}

            Use created profile temporarily for a single command:
                nebula -p {name!r} config view
            """
        )
    )
...
async def use(name: str):
    ...
    status_messages = {
        ConnectionStatus.CLOUD_CONNECTED: (
            exit_with_success,
            f"Connected to Nebula Cloud using profile {name!r}",
        ),
        ConnectionStatus.CLOUD_ERROR: (
            exit_with_error,
            f"Error connecting to Nebula Cloud using profile {name!r}",
        ),
        ConnectionStatus.CLOUD_UNAUTHORIZED: (
            exit_with_error,
            f"Error authenticating with Nebula Cloud using profile {name!r}",
        ),
        ConnectionStatus.ORION_CONNECTED: (
            exit_with_success,
            f"Connected to Nebula server using profile {name!r}",
        ),
        ConnectionStatus.ORION_ERROR: (
            exit_with_error,
            f"Error connecting to Nebula server using profile {name!r}",
        ),
        ConnectionStatus.EPHEMERAL: (
            exit_with_success,
            (
                f"No Nebula server specified using profile {name!r} - the API will run"
                " in ephemeral mode."
            ),
        ),
        ConnectionStatus.INVALID_API: (
            exit_with_error,
            "Error connecting to Nebula API URL",
        ),
    }
...
def rename(name: str, new_name: str):
    ...
    nebula.context.get_settings_context().profile
    if profiles.active_name == name:
        profiles.set_active(new_name)
    if os.environ.get("NEBULA_PROFILE") == name:
        app.console.print(
            f"You have set your current profile to {name!r} with the "
            "NEBULA_PROFILE environment variable. You must update this variable to "
...
async def check_orion_connection():
    ...
    try:
        ...
        if exc.response.status_code == status.HTTP_404_NOT_FOUND:
            # if the route does not exist, attempt to connect as a hosted Nebula
            # instance
            try:
                # inform the user if Nebula API endpoints exist, but there are
                # connection issues
                ...
                if connect_error is not None:
                    ...
                    # if the client is using an ephemeral Nebula app, inform the user
        ...
        # if no Nebula API URL has been set, httpx will throw a TypeError
        try:
            # try to connect with the client anyway, it will likely use an
            # ephemeral Nebula instance
            client = get_client(httpx_settings=httpx_settings)
            async with client:
                connect_error = await client.api_healthcheck()
            if connect_error is not None:
