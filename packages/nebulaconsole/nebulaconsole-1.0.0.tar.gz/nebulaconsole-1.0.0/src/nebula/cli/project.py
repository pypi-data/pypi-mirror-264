...
from nebula.deployments import find_nebula_directory, initialize_project
from nebula.deployments import register_flow as register
from nebula.deployments.steps.core import run_steps
from nebula.exceptions import ObjectNotFound

# Deprecated compatibility
project_app = NebulaTyper(
    name="project",
    help="Deprecated. Use `nebula init` instead.",
    deprecated=True,
    deprecated_name="nebula project",
    deprecated_start_date="Jun 2023",
    deprecated_help="Use `nebula` instead.",
)
app.add_typer(project_app, aliases=["projects"])

recipe_app = NebulaTyper(
    name="recipe",
    help="Deprecated. Use `nebula init` instead.",
    deprecated=True,
    deprecated_name="nebula project recipe",
    deprecated_start_date="Jun 2023",
    deprecated_help="Use `nebula init` instead.",
)
...
async def ls():
    ...
    recipe_paths = nebula.__module_path__ / "deployments" / "recipes"
    recipes = {}

    for recipe in recipe_paths.iterdir():
        if recipe.is_dir() and (recipe / "nebula.yaml").exists():
            with open(recipe / "nebula.yaml") as f:
    ...
    table = Table(
        ...
        caption=(
            "Run `nebula project init --recipe <recipe>` to initialize a project with"
            " a recipe."
        ),
        caption_style="red",
    )
...
async def init(
    ...
    if not recipe and is_interactive():
        ...
        for r in recipe_paths.iterdir():
            if r.is_dir() and (r / "nebula.yaml").exists():
                with open(r / "nebula.yaml") as f:
    ...
    if recipe and (recipe_paths / recipe / "nebula.yaml").exists():
        with open(recipe_paths / recipe / "nebula.yaml") as f:
    ...
    try:
        ...
        if "Unknown recipe" in str(exc):
            exit_with_error(
                f"Unknown recipe {recipe!r} provided - run [yellow]`nebula init"
...
async def clone(
    ...
    app.console.print(
        generate_deprecation_message(
            "The `nebula project clone` command",
            start_date="Jun 2023",
        )
    )
...
async def register_flow(
    ...
    app.console.print(
        (
            ...
            f" {(find_nebula_directory()/'flows.json').resolve()!s}"
        ),
        style="green",
    )
