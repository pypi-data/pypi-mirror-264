...
from nebula.input.actions import (
...
if TYPE_CHECKING:
    from nebula.client.schemas.objects import FlowRunInput
    from nebula.states import State

if HAS_PYDANTIC_V2:
    from nebula._internal.pydantic.v2_schema import create_v2_schema
...
class RunInput(pydantic.BaseModel):
    ...
    async def respond(
        ...
        flow_run_id = None
        if self.metadata.sender and self.metadata.sender.startswith("nebula.flow-run"):
