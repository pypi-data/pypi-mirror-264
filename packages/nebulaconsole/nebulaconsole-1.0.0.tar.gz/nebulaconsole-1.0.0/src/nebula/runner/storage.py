...
from nebula.filesystems import ReadableDeploymentStorage, WritableDeploymentStorage
from nebula.logging.loggers import get_logger
from nebula.utilities.collections import visit_collection
...
class GitRepository:
    ...
    def to_pull_step(self) -> Dict:
        ...
        if isinstance(self._credentials, Block):
            pull_step["nebula.deployments.steps.git_clone"][
                "credentials"
            ...
            if isinstance(self._credentials.get("access_token"), Secret):
                pull_step["nebula.deployments.steps.git_clone"]["credentials"] = {
                    **self._credentials,
                    "access_token": (
                        "{{"
...
class RemoteStorage:
    ...
    Examples:
        ...
        ```python
        from nebula.runner.storage import RemoteStorage
    ...
    def to_pull_step(self) -> dict:
        ...
        if required_package:
            step["nebula.deployments.steps.pull_from_remote_storage"][
                "requires"
            ] = required_package
        return step
...
class BlockStorageAdapter:
    ...
    def to_pull_step(self) -> dict:
        ...
        if hasattr(self._block, "get_pull_step"):
            ...
            return {
                "nebula.deployments.steps.pull_with_block": {
