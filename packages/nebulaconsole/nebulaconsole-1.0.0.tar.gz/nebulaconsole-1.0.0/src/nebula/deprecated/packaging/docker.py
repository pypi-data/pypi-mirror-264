...
from nebula.deprecated.packaging.base import PackageManifest, Packager
from nebula.deprecated.packaging.serializers import SourceSerializer
from nebula.flows import Flow, load_flow_from_script
from nebula.software import CondaEnvironment, PythonEnvironment
...
class DockerPackager(Packager):
    ...
    def set_default_base_image(cls, values):
        if not values.get("base_image") and not values.get("dockerfile"):
            values["base_image"] = get_nebula_image_name(
                flavor=(
                    "conda"
