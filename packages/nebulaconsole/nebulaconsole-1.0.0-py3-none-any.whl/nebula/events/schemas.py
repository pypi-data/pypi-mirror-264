...
from nebula.events.actions import ActionTypes, RunDeployment
from nebula.utilities.collections import AutoEnum

# These are defined by Nebula Cloud
...
class ResourceSpecification(NebulaBaseModel):
    __root__: Dict[str, Union[str, List[str]]]


class Labelled(NebulaBaseModel):
...
class Resource(Labelled):
    ...
    def requires_resource_id(cls, values: Dict[str, Any]):
        ...
        if "nebula.resource.id" not in labels:
            raise ValueError("Resources must include the nebula.resource.id label")
        if not labels["nebula.resource.id"]:
            raise ValueError("The nebula.resource.id label must be non-empty")

        return values

    @property
    def id(self) -> str:
        return self["nebula.resource.id"]
...
class RelatedResource(Resource):
    ...
    def requires_resource_role(cls, values: Dict[str, Any]):
        ...
        if "nebula.resource.role" not in labels:
            raise ValueError(
                "Related Resources must include the nebula.resource.role label"
            )
        if not labels["nebula.resource.role"]:
            raise ValueError("The nebula.resource.role label must be non-empty")

        return values

    @property
    def role(self) -> str:
        return self["nebula.resource.role"]


class Event(NebulaBaseModel):
...
class Trigger(NebulaBaseModel, abc.ABC):
    """
...
class EventTrigger(ResourceTrigger):
    ...
    after: Set[str] = Field(
        ...
        description=(
            ...
            "trailing wildcards, like `nebula.flow-run.*`"
        ),
    )
    ...
    for_each: Set[str] = Field(
        ...
        description=(
            ...
            'example, `"for_each": ["related:flow:nebula.resource.id"]` would '
...
class NebulaMetric(Enum):
    lateness = "lateness"
    duration = "duration"
    successes = "successes"


class MetricTriggerQuery(NebulaBaseModel):
...
class Automation(NebulaBaseModel):
...
class AutomationCreateFromTrigger(NebulaBaseModel):
...
class DeploymentTrigger(AutomationCreateFromTrigger):
    ...
    def owner_resource(self) -> Optional[str]:
        return f"nebula.deployment.{self._deployment_id}"
