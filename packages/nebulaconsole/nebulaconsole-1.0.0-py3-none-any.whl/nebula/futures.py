...
from nebula._internal.concurrency.event_loop import run_coroutine_in_loop_from_async
...
from nebula.utilities.asyncutils import A, Async, Sync, sync
from nebula.utilities.collections import StopVisiting, visit_collection
...
class NebulaFuture(Generic[R, A]):
    """
    ...
    Examples:
        ...
        >>> @flow
        >>> def my_flow():
        >>>     future = my_task.submit()  # NebulaFuture[str, Sync] includes result type
    ...
    @overload
    def wait(
        self: "NebulaFuture[R, Async]", timeout: None = None
    ) -> Awaitable[State[R]]:
        ...

    @overload
    def wait(self: "NebulaFuture[R, Sync]", timeout: None = None) -> State[R]:
        ...

    @overload
    def wait(
        self: "NebulaFuture[R, Async]", timeout: float
    ) -> Awaitable[Optional[State[R]]]:
        ...

    @overload
    def wait(self: "NebulaFuture[R, Sync]", timeout: float) -> Optional[State[R]]:
        ...
    ...
    @overload
    def get_state(
        self: "NebulaFuture[R, Async]", client: NebulaClient = None
    ) -> Awaitable[State[R]]:
        ...

    @overload
    def get_state(
        self: "NebulaFuture[R, Sync]", client: NebulaClient = None
    ) -> State[R]:
        ...

    def get_state(self, client: NebulaClient = None):
        """
    ...
    @inject_client
    async def _get_state(self, client: NebulaClient = None) -> State[R]:
    ...
    def __repr__(self) -> str:
        return f"NebulaFuture({self.name!r})"

    def __bool__(self) -> bool:
        warnings.warn(
            (
                "A 'NebulaFuture' from a task call was cast to a boolean; "
...
async def resolve_futures_to_data(
    expr: Union[NebulaFuture[R, Any], Any],
    raise_on_failure: bool = True,
) -> Union[R, Any]:
    """
    Given a Python built-in collection, recursively find `NebulaFutures` and build a
