...
from nebula.client.base import NebulaHttpxClient
...
def get_cloud_client(
    ...
    if infer_cloud_url is False:
        host = host or NEBULA_CLOUD_API_URL.value()
    else:
        configured_url = nebula.settings.NEBULA_API_URL.value()
...
class CloudUnauthorizedError(NebulaException):
    """
...
class CloudClient:
    def __init__(
        ...
        if not NEBULA_UNIT_TEST_MODE.value():
            httpx_settings.setdefault("follow_redirects", True)
        self._client = NebulaHttpxClient(**httpx_settings, enable_csrf_support=False)
