...
from nebula.events.clients import AssertingEventsClient
from nebula.events.filters import EventFilter
from nebula.events.worker import EventsWorker
from nebula.settings import NEBULA_API_URL, get_current_settings, temporary_settings
from nebula.testing.utilities import AsyncMock
from nebula.utilities.processutils import open_process


@pytest.fixture(autouse=True)
def add_nebula_loggers_to_caplog(caplog):
    import logging
...
async def hosted_api_server(unused_tcp_port_factory):
    """
    Runs an instance of the Nebula API server in a subprocess instead of the using the
    ...
    async with open_process(
        ...
        async with httpx.AsyncClient() as client:
            ...
            if not response:
                raise RuntimeError(
                    "Timed out while attempting to connect to hosted test Nebula API."
                )
...
def use_hosted_api_server(hosted_api_server):
    """
    Sets `NEBULA_API_URL` to the test session's hosted API endpoint.
    """
    with temporary_settings({NEBULA_API_URL: hosted_api_server}):
...
def mock_emit_events_to_cloud(monkeypatch):
    monkeypatch.setattr("nebula.events.utilities.emit_events_to_cloud", lambda: True)
