...
from nebula.server.services.loop_service import LoopService, run_multiple_services
from nebula.settings import (
    NEBULA_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE,
    NEBULA_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE,
    NEBULA_API_SERVICES_SCHEDULER_LOOP_SECONDS,
    NEBULA_API_SERVICES_SCHEDULER_MAX_RUNS,
    NEBULA_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME,
    NEBULA_API_SERVICES_SCHEDULER_MIN_RUNS,
    NEBULA_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME,
)
from nebula.utilities.collections import batched_iterable
...
class Scheduler(LoopService):
    ...
    def __init__(self, loop_seconds: float = None, **kwargs):
        super().__init__(
            loop_seconds=(
                ...
                or NEBULA_API_SERVICES_SCHEDULER_LOOP_SECONDS.value()
            ),
            **kwargs,
        )
        self.deployment_batch_size: int = (
            NEBULA_API_SERVICES_SCHEDULER_DEPLOYMENT_BATCH_SIZE.value()
        )
        self.max_runs: int = NEBULA_API_SERVICES_SCHEDULER_MAX_RUNS.value()
        self.min_runs: int = NEBULA_API_SERVICES_SCHEDULER_MIN_RUNS.value()
        self.max_scheduled_time: datetime.timedelta = (
            NEBULA_API_SERVICES_SCHEDULER_MAX_SCHEDULED_TIME.value()
        )
        self.min_scheduled_time: datetime.timedelta = (
            NEBULA_API_SERVICES_SCHEDULER_MIN_SCHEDULED_TIME.value()
        )
        self.insert_batch_size = (
            NEBULA_API_SERVICES_SCHEDULER_INSERT_BATCH_SIZE.value()
        )

    @inject_db
    async def run_once(self, db: NebulaDBInterface):
        """
    ...
    @inject_db
    def _get_select_deployments_to_schedule_query(self, db: NebulaDBInterface):
        """
