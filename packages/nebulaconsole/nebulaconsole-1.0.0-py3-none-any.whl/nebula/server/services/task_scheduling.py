...
from nebula.server.schemas import filters, states
from nebula.server.services.loop_service import LoopService
from nebula.settings import NEBULA_TASK_SCHEDULING_PENDING_TASK_TIMEOUT
...
class TaskSchedulingTimeouts(LoopService):
    ...
    def __init__(self, loop_seconds: float = None, **kwargs):
        ...
        super().__init__(
            ...
            or NEBULA_TASK_SCHEDULING_PENDING_TASK_TIMEOUT.value().total_seconds(),
            **kwargs,
        )
    ...
    async def reschedule_pending_runs(self, session: AsyncSession):
        ...
        older_than = (
            pendulum.now("UTC") - NEBULA_TASK_SCHEDULING_PENDING_TASK_TIMEOUT.value()
        )
