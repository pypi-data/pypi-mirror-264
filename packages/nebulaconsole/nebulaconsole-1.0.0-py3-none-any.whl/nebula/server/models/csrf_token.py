...
from nebula.server.database.dependencies import db_injector
from nebula.server.database.interface import NebulaDBInterface
from nebula.server.schemas import core
...
async def create_or_update_csrf_token(
    ...
    expiration = (
        ...
        + settings.NEBULA_SERVER_CSRF_TOKEN_EXPIRATION.value()
    )
...
@db_injector
async def delete_expired_tokens(db: NebulaDBInterface, session: AsyncSession) -> int:
