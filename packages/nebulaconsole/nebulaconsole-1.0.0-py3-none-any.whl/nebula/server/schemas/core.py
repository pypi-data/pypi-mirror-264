"""
Full schemas of Nebula REST API objects.
"""
...
import nebula.server.database
from nebula.server.schemas import schedules, states
from nebula.server.utilities.schemas.bases import (
    NebulaBaseModel,
    ORMBaseModel,
)
...
from nebula.settings import (
    NEBULA_API_TASK_CACHE_KEY_MAX_LENGTH,
)
from nebula.utilities.collections import dict_to_flatdict, flatdict_to_dict, listrepr
from nebula.utilities.names import generate_slug, obfuscate, obfuscate_string
...
class TaskRun(ORMBaseModel):
    ...
    def validate_cache_key_length(cls, cache_key):
        if cache_key and len(cache_key) > NEBULA_API_TASK_CACHE_KEY_MAX_LENGTH.value():
            raise ValueError(
                "Cache key exceeded maximum allowed length of"
                f" {NEBULA_API_TASK_CACHE_KEY_MAX_LENGTH.value()} characters."
            )
        return cache_key
...
class BlockDocument(ORMBaseModel):
    ...
    async def from_orm_model(
        cls,
        session,
        orm_block_document: "nebula.server.database.orm_models.ORMBlockDocument",
