...
Nebula enforces on a state transition.
"""
...
from nebula.results import UnknownResult
...
from nebula.server.models import concurrency_limits
from nebula.server.orchestration.policies import BaseOrchestrationPolicy
from nebula.server.orchestration.rules import (
...
from nebula.server.schemas import core, filters, states
from nebula.server.schemas.states import StateType
from nebula.server.task_queue import TaskQueue
from nebula.settings import (
    NEBULA_EXPERIMENTAL_ENABLE_TASK_SCHEDULING,
    NEBULA_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS,
)
from nebula.utilities.math import clamped_poisson_interval
...
class SecureTaskConcurrencySlots(BaseOrchestrationRule):
    ...
    specified by the "NEBULA_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS" setting
        ...
        for tag, cl in run_limits.items():
            ...
            if limit == 0:
                ...
                await self.delay_transition(
                    NEBULA_TASK_RUN_TAG_CONCURRENCY_SLOT_WAIT_SECONDS.value(),
...
class WaitForScheduledTime(BaseOrchestrationRule):
    ...
    used by the Nebula REST API instance. This rule will identify transitions from scheduled
