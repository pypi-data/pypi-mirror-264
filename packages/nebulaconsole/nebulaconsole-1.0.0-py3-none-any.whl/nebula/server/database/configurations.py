...
from nebula.settings import (
    NEBULA_API_DATABASE_CONNECTION_TIMEOUT,
    NEBULA_API_DATABASE_ECHO,
    NEBULA_API_DATABASE_TIMEOUT,
    NEBULA_SQLALCHEMY_MAX_OVERFLOW,
    NEBULA_SQLALCHEMY_POOL_SIZE,
    NEBULA_UNIT_TEST_MODE,
)
from nebula.utilities.asyncutils import add_event_loop_shutdown_callback
...
class BaseDatabaseConfiguration(ABC):
    """
    Abstract base class used to inject database connection configuration into Nebula.

    This configuration is responsible for defining how Nebula REST API creates and manages
    ...
    def __init__(
        ...
        self.echo = echo or NEBULA_API_DATABASE_ECHO.value()
        self.timeout = timeout or NEBULA_API_DATABASE_TIMEOUT.value()
        self.connection_timeout = (
            connection_timeout or NEBULA_API_DATABASE_CONNECTION_TIMEOUT.value()
        )
        self.sqlalchemy_pool_size = (
            sqlalchemy_pool_size or NEBULA_SQLALCHEMY_POOL_SIZE.value()
        )
        self.sqlalchemy_max_overflow = (
            sqlalchemy_max_overflow or NEBULA_SQLALCHEMY_MAX_OVERFLOW.value()
        )
...
class AioSqliteConfiguration(BaseDatabaseConfiguration):
    ...
    async def engine(self) -> AsyncEngine:
        ...
        if sqlite3.sqlite_version_info < self.MIN_SQLITE_VERSION:
            ...
            raise RuntimeError(
                f"Nebula requires sqlite >= {required} but we found version "
        ...
        if cache_key not in ENGINES:
            ...
            # see https://github.com/kozmoai/nebula/pull/6702
    ...
    def setup_sqlite(self, conn, record):
        ...
        if NEBULA_UNIT_TEST_MODE.value() is True:
