...
from nebula._vendor.fastapi import HTTPException, status

from nebula.server.utilities.server import NebulaRouter

router = NebulaRouter(prefix="/collections", tags=["Collections"])

GLOBAL_COLLECTIONS_VIEW_CACHE: TTLCache = TTLCache(maxsize=200, ttl=60 * 10)

REGISTRY_VIEWS = (
    "https://raw.githubusercontent.com/kozmoai/nebula-collection-registry/main/views"
)
KNOWN_VIEWS = {
...
async def read_view_content(view: str) -> Dict[str, Any]:
    """Reads the content of a view from the nebula-collection-registry."""
    try:
