"""
Routes for admin-level interactions with the Nebula REST API.
"""
from nebula._vendor.fastapi import Body, Depends, Response, status
...
router = NebulaRouter(prefix="/admin", tags=["Admin"])


@router.get("/settings")
async def read_settings() -> nebula.settings.Settings:
    """
    Get the current Nebula REST API settings.

    Secret setting values will be obfuscated.
    """
    return nebula.settings.get_current_settings().with_obfuscated_secrets()


@router.get("/version")
async def read_version() -> str:
    """Returns the Nebula version number"""
    return nebula.__version__
