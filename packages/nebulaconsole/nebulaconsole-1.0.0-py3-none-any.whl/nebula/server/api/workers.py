...
async def create_work_pool(
    ...
    if work_pool.name.lower().startswith("nebula"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Work pools starting with 'Nebula' are reserved for internal use.",
        )
...
async def update_work_pool(
    ...
    if work_pool_name.lower().startswith("nebula") and (
        set(update_values).difference({"is_paused", "concurrency_limit"})
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
                "Work pools starting with 'Nebula' are reserved for internal use "
...
async def delete_work_pool(
    ...
    if work_pool_name.lower().startswith("nebula"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=(
                "Work pools starting with 'Nebula' are reserved for internal use and"
                " can not be deleted."
            ),
        )
