...
from nebula.client.schemas.actions import WorkPoolCreate
from nebula.exceptions import ObjectAlreadyExists
...
import nebula
from nebula.blocks.core import Block, BlockNotSavedError
from nebula.logging import get_logger
from nebula.settings import (
    NEBULA_EXPERIMENTAL_WARN,
    NEBULA_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION,
    NEBULA_UI_URL,
    get_current_settings,
)
from nebula.utilities.asyncutils import sync_compatible

MIN_COMPAT_NEBULA_VERSION = "2.0b12"


if TYPE_CHECKING:
    from nebula.client.schemas.objects import Deployment, Flow, FlowRun
...
class Infrastructure(Block, abc.ABC):
    ...
    async def publish_as_work_pool(self, work_pool_name: Optional[str] = None):
        ...
        try:
            async with nebula.get_client() as client:
        ...
        if NEBULA_UI_URL:
            console.print(
                ...
                f" {NEBULA_UI_URL.value()}/work-pools/work-pool/{work_pool.name}"
            )
        ...
        console.print(f"\t[blue]nebula worker start --pool {work_pool.name}[/]\n")
    ...
    @property
    def logger(self):
        return get_logger(f"nebula.infrastructure.{self.type}")

    @property
    def is_using_a_runner(self):
        return self.command is not None and "nebula flow-run execute" in shlex.join(
            self.command
        )
    ...
    def _base_flow_run_command() -> List[str]:
        ...
        if experiment_enabled("enhanced_cancellation"):
            if (
                NEBULA_EXPERIMENTAL_WARN
                and NEBULA_EXPERIMENTAL_WARN_ENHANCED_CANCELLATION
            ):
                warnings.warn(
            ...
            return ["nebula", "flow-run", "execute"]

        return ["python", "-m", "nebula.engine"]
    ...
    def _base_flow_run_environment(flow_run: "FlowRun") -> Dict[str, str]:
        ...
        """
        environment = {}
        environment["NEBULA__FLOW_RUN_ID"] = str(flow_run.id)
        return environment
    ...
    def _base_deployment_labels(deployment: "Deployment") -> Dict[str, str]:
        ...
        if deployment.updated is not None:
            labels["nebula.io/deployment-updated"] = deployment.updated.in_timezone(
                "utc"
            ).to_iso8601_string()
        return labels
