...
from nebula.client.orchestration import NebulaClient, ServerType, get_client
from nebula.client.schemas.filters import FlowFilter
from nebula.client.schemas.objects import DeploymentSchedule
...
from nebula.deployments import Deployment, load_deployments_from_yaml
...
from nebula.states import Scheduled
...
from nebula.utilities.filesystem import create_default_ignore_file
...
async def get_deployment(client: NebulaClient, name, deployment_id):
    if name is None and deployment_id is not None:
        try:
            deployment = await client.read_deployment(deployment_id)
        except NebulaHTTPStatusError:
...
async def check_work_pool_exists(
    work_pool_name: Optional[str], client: NebulaClient = None
):
    if work_pool_name is not None:
        try:
            ...
            app.console.print(
                f"$ nebula work-pool create {work_pool_name!r}\n", style="blue"
            )
...
async def _print_deployment_work_pool_instructions(
    work_pool_name: str, client: NebulaClient = None
):
    ...
    command = f"$ nebula agent start -p {work_pool_name!r}"
    if work_pool.type != "nebula-agent":
        if experiment_enabled("workers"):
            ...
            command = f"$ nebula worker start -p {work_pool_name!r}"
        else:
            blurb = (
            ...
            command = (
                "$ nebula config set"
                " NEBULA_EXPERIMENTAL_ENABLE_WORKERS=True\n$ nebula"
...
async def inspect(name: str):
    ...
    Example:
        \b
        $ nebula deployment inspect "hello-world/my-deployment"
        {
            ...
            'infrastructure': {
                ...
                'labels': {},
                'name': None,
                'command': ['python', '-m', 'nebula.engine'],
                'stream_output': True
            }
        }
        ...
        if client.server_type == ServerType.CLOUD:
            deployment_json["automations"] = [
                ...
                for a in await client.read_resource_related_automations(
                    f"nebula.deployment.{deployment.id}"
                )
            ]
...
@deployment_app.command(
    ...
    deprecated_help="Use 'nebula deployment schedule create' instead.",
...
async def _set_schedule(
    ...
    This command is deprecated. Use 'nebula deployment schedule create' instead.
    """
    assert_deployment_name_format(name)
        ...
        async with get_client() as client:
            ...
            if len(deployment.schedules) > 1:
                return exit_with_error(
                    ...
                    "Use `nebula deployment schedules create` instead."
                )
...
@deployment_app.command(
    ...
    deprecated=True,
    deprecated_start_date="Mar 2024",
    deprecated_help="Use 'nebula deployment schedule pause' instead.",
...
async def _pause_schedule(
    ...
    This command is deprecated. Use `nebula deployment schedule pause` instead.
    """
    assert_deployment_name_format(name)
    ...
    async with get_client() as client:
        ...
        if len(deployment.schedules) > 1:
            return exit_with_error(
                f"Deployment {name!r} has multiple schedules. Use `nebula deployment"
...
@deployment_app.command(
    ...
    deprecated=True,
    deprecated_start_date="Mar 2024",
    deprecated_help="Use 'nebula deployment schedule resume' instead.",
...
async def _resume_schedule(
    ...
    This command is deprecated. Use `nebula deployment schedule resume` instead.
    """
    ...
    async with get_client() as client:
        ...
        if len(deployment.schedules) > 1:
            return exit_with_error(
                f"Deployment {name!r} has multiple schedules. "
                "Use `nebula deployment schedule pause <deployment_name> <schedule_id>"
            )
...
async def run(
    ...
    if NEBULA_UI_URL:
        run_url = f"{NEBULA_UI_URL.value()}/flow-runs/flow-run/{flow_run.id}"
    else:
...
def _load_deployments(path: Path, quietly=False) -> NebulaObjectRegistry:
    """
...
@deployment_app.command(
    ...
    deprecated_help="Use 'nebula deploy' to deploy flows via YAML instead.",
)
async def apply(
    ...
    async with get_client() as client:
        for path in paths:
            ...
            if client.server_type != ServerType.CLOUD and deployment.triggers:
                app.console.print(
                    (
                        ...
                        f"Nebula Cloud. Triggers defined in {path!r} will be "
                        "ignored."
                    ),
                    style="red",
                )
            ...
            if deployment.work_pool_name is not None:
                ...
                app.console.print(
                    f"$ nebula agent start -q {deployment.work_queue_name!r}",
                    style="blue",
                )
            else:
                app.console.print(
...
async def delete(
    ...
    Examples:
        \b
        $ nebula deployment delete test_flow/test_deployment
        $ nebula deployment delete --id dfd3e220-a130-4149-9af6-8d487e02fea6
    """
    async with get_client() as client:
        if name is None and deployment_id is not None:
            try:
...
async def build(
    ...
    if create_default_ignore_file(path="."):
        app.console.print(
            (
                "Default '.nebulaignore' file written to"
                f" {(Path('.') / '.nebulaignore').absolute()}"
            ),
            style="green",
        )
