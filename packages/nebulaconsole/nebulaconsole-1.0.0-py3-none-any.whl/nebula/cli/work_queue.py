...
from nebula.client.schemas.filters import WorkPoolFilter, WorkPoolFilterId
from nebula.client.schemas.objects import DEFAULT_AGENT_WORK_POOL_NAME
from nebula.exceptions import ObjectAlreadyExists, ObjectNotFound

work_app = NebulaTyper(
    ...
    if tags:
        ...
        output_msg = dedent(
            ...
            Start an agent to pick up flow runs from the work queue:
                nebula agent start -q '{result.name}'

            Inspect the work queue:
                nebula work-queue inspect '{result.name}'
            """
        )
    else:
        if not pool:
        ...
        output_msg = dedent(
            ...
            Created work queue with properties:
                ...
                concurrency limit - {limit}
            Start an agent to pick up flow runs from the work queue:
                nebula agent start -q '{result.name} -p {pool}'
