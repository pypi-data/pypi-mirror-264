...
from nebula.cli._prompts import confirm
...
from nebula.plugins import load_nebula_collections
from nebula.settings import (
    NEBULA_WORKER_HEARTBEAT_SECONDS,
    NEBULA_WORKER_PREFETCH_SECONDS,
    NEBULA_WORKER_QUERY_SECONDS,
)
from nebula.utilities.dispatch import lookup_type
...
from nebula.workers.server import start_healthcheck_server

worker_app = NebulaTyper(
...
async def start(
    ...
    install_policy: InstallPolicy = typer.Option(
        ...
        "--install-policy",
        help="Install policy to use workers from Nebula integration packages.",
    ...
    async with worker_cls(
        ...
        heartbeat_interval_seconds=NEBULA_WORKER_HEARTBEAT_SECONDS.value(),
        ...
        async with anyio.create_task_group() as tg:
            ...
            tg.start_soon(
                partial(
                    ...
                    interval=NEBULA_WORKER_QUERY_SECONDS.value(),
                    run_once=run_once,
            ...
            tg.start_soon(
                partial(
                    ...
                    interval=NEBULA_WORKER_QUERY_SECONDS.value() * 2,
                    run_once=run_once,
                    printer=app.console.print,
                    jitter_range=0.3,
                    backoff=4,
            ...
            if with_healthcheck:
                ...
                server_thread = threading.Thread(
                    ...
                    target=partial(
                        ...
                        worker=worker,
                        query_interval_seconds=NEBULA_WORKER_QUERY_SECONDS.value(),
                    ),
                    daemon=True,
                )
                server_thread.start()
    ...
    try:
        ...
        if work_pool.is_push_pool or work_pool.is_managed_pool:
            exit_with_error(
                ...
                "See https://docs.nebula.io/latest/guides/deployment/push-work-pools/ "
                "for more details."
            )
...
async def _get_worker_class(
    ...
    if worker_type == "nebula-agent":
        exit_with_error(
            "'nebula-agent' typed work pools work with Nebula Agents instead of"
            " Workers. Please use the 'nebula agent start' to start a Nebula Agent."
        )
    ...
    if worker_cls is None:
        ...
        if package:
            ...
            # Confirm with the user for installation in an interactive session
                ...
                message = (
                    "Could not find the Nebula integration library for the"
