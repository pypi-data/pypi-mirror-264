"""
Command line interface for interacting with Nebula Cloud
"""
import signal
import traceback
import urllib.parse
...
import typer
import uvicorn
from nebula._vendor.fastapi import FastAPI
...
from nebula.client.cloud import CloudUnauthorizedError, get_cloud_client
from nebula.client.schemas import Workspace
from nebula.context import get_settings_context
...
from nebula.utilities.compat import raise_signal

# Set up the `nebula cloud` and `nebula cloud workspaces` CLI applications
cloud_app = NebulaTyper(name="cloud", help="Commands for interacting with Nebula Cloud")
workspace_app = NebulaTyper(
    name="workspace", help="Commands for interacting with Nebula Cloud Workspaces"
)
...
async def login_with_browser() -> str:
    ...
    async with anyio.create_task_group() as tg:
        ...
        ui_login_url = NEBULA_CLOUD_UI_URL.value() + f"/auth/client?callback={callback}"
...
async def login(
    key: Optional[str] = typer.Option(
        None, "--key", "-k", help="API Key to authenticate with Nebula"
    ),
    ...
    """
    Log in to Nebula Cloud.
    Creates a new profile configured to use the specified NEBULA_API_KEY.
    ...
    env_var_api_key = NEBULA_API_KEY.value()

    if env_var_api_key and key and env_var_api_key != key:
        exit_with_error(
            "Cannot log in with a key when a different NEBULA_API_KEY is present as an"
    ...
    if env_var_api_key and env_var_api_key == key:
        ...
        if not is_valid_key:
            ...
            exit_with_error(f"Unable to authenticate with Nebula Cloud. {help_message}")

    already_logged_in_profiles = []
    for name, profile in profiles.items():
        profile_key = profile.settings.get(NEBULA_API_KEY)
        if (
        ...
        try:
            ...
            if key.startswith("pcu"):
                help_message = (
                    ...
                    " (https://cloud.nebula.io). Make sure that you generate API key"
                    " using Cloud 2 (https://app.nebula.cloud)"
                )
            ...
            exit_with_error(f"Error connecting to Nebula Cloud: {exc!r}")
        ...
        if prompt_switch_workspace:
            ...
            if current_workspace:
                ...
                exit_with_error(
                    ...
                    f" {NEBULA_CLOUD_UI_URL.value()} and try again."
                )

    update_current_profile(
        {
            NEBULA_API_KEY: key,
            NEBULA_API_URL: workspace.api_url(),
        }
    )

    exit_with_success(
        f"Authenticated with Nebula Cloud! Using workspace {workspace.handle!r}."
    )
...
async def logout():
    ...
    Reset NEBULA_API_KEY and NEBULA_API_URL to default.
    """
    current_profile = nebula.context.get_settings_context().profile
    if current_profile is None:
        exit_with_error("There is no current profile set.")

    if current_profile.settings.get(NEBULA_API_KEY) is None:
        exit_with_error("Current profile is not logged into Nebula Cloud.")
    ...
    exit_with_success("Logged out from Nebula Cloud.")


@cloud_app.command()
async def open():
    """
    Open the Nebula Cloud UI in the browser.
    """
    confirm_logged_in()
    ...
    if current_profile is None:
        exit_with_error(
            "There is no current profile set - set one with `nebula profile create"
            " <name>` and `nebula profile use <name>`."
        )

    current_workspace = get_current_workspace(
        await nebula.get_cloud_client().read_workspaces()
    )
    if current_workspace is None:
        exit_with_error(
            "There is no current workspace set - set one with `nebula cloud workspace"
...
async def set(
    ...
    profile = update_current_profile({NEBULA_API_URL: workspace.api_url()})
