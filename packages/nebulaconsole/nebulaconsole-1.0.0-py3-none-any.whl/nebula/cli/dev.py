"""
Command line interface for working with Nebula Server
"""
import json
import os
import platform
...
from nebula.cli.agent import start as start_agent
...
from nebula.utilities.dockerutils import get_nebula_image_name, python_version_minor
from nebula.utilities.filesystem import tmpchdir
from nebula.utilities.processutils import run_process
...
def exit_with_error_if_not_editable_install():
    if (
        nebula.__module_path__.parent == "site-packages"
        or not (nebula.__development_base_path__ / "setup.py").exists()
    ):
        exit_with_error(
            "Development commands require an editable Nebula installation. "
            "Development commands require content outside of the 'nebula' module  "
            "which is not available when installed into your site-packages. "
            f"Detected module path: {nebula.__module_path__}."
        )
...
def agent_process_entrypoint(**kwargs):
    ...
    All kwargs are forwarded to  `nebula.cli.agent.start`.
    """
    import inspect
    ...
    for name, param in start_agent_signature.parameters.items():
        if name not in kwargs:
            ...
            # Some defaults are Nebula `SettingsOption.value` methods
...
def build_docs(
    ...
    if not schema_path:
        schema_path = (
            nebula.__development_base_path__ / "docs" / "api-ref" / "schema.json"
        ).absolute()
...
The built UI will be located at {nebula.__development_base_path__ / "ui"}
...
def build_ui(
    ...
    exit_with_error_if_not_editable_install()
    with tmpchdir(nebula.__development_base_path__ / "ui"):
        if not no_install:
    ...
    with tmpchdir(nebula.__development_base_path__):
        if os.path.exists(nebula.__ui_static_path__):
            app.console.print("Removing existing build files...")
            shutil.rmtree(nebula.__ui_static_path__)

        app.console.print("Copying build into src...")
        shutil.copytree("ui/dist", nebula.__ui_static_path__)
...
async def api(
    ...
    server_env["NEBULA_API_SERVICES_RUN_IN_APP"] = str(services)
    server_env["NEBULA_API_SERVICES_UI"] = "False"
    server_env["NEBULA_UI_API_URL"] = f"http://{host}:{port}/api"
    ...
    async with anyio.create_task_group() as tg:
        if not exclude_api:
            tg.start_soon(
                partial(
                    api,
                    host=NEBULA_SERVER_API_HOST.value(),
                    port=NEBULA_SERVER_API_PORT.value(),
                )
            )
        if not exclude_ui:
            tg.start_soon(ui)
        if not exclude_agent:
            ...
            if not exclude_api:
                host = f"http://{NEBULA_SERVER_API_HOST.value()}:{NEBULA_SERVER_API_PORT.value()}/api"  # noqa
            else:
                host = NEBULA_API_URL.value()
...
def build_image(
    ...
    tag = get_nebula_image_name(python_version=python_version, flavor=flavor)
    ...
    command = [
        "docker",
        "build",
        str(nebula.__development_base_path__),
        "--tag",
        tag,
        "--platform",
        f"linux/{arch}",
    ...
    if flavor:
        command += ["--build-arg", f"BASE_IMAGE=nebula-{flavor}"]
...
@dev_app.command()
def container(bg: bool = False, name="nebula-dev", api: bool = True, tag: str = None):
    """
    ...
    blocking_cmd = "nebula dev api" if api else "sleep infinity"
    tag = tag or get_nebula_image_name()
    ...
    container: Container = client.containers.create(
        ...
        command=[
            ...
            "-c",
            (  # noqa
                "pip install -e /opt/nebula/repo\\[dev\\] && touch /READY &&"
                f" {blocking_cmd}"
            ),
        ],
        name=name,
        auto_remove=True,
        working_dir="/opt/nebula/repo",
        volumes=[f"{nebula.__development_base_path__}:/opt/nebula/repo"],
        shm_size="4G",
    )
...
def kubernetes_manifest():
    ...
    Example:
        $ nebula dev kubernetes-manifest | kubectl apply -f -
    """
    exit_with_error_if_not_editable_install()

    template = Template(
        (
            nebula.__module_path__ / "cli" / "templates" / "kubernetes-dev.yaml"
        ).read_text()
    )
    manifest = template.substitute(
        {
            "nebula_root_directory": nebula.__development_base_path__,
            "image_name": get_nebula_image_name(),
        }
    )
    print(manifest)
