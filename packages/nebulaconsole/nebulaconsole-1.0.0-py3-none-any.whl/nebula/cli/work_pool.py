...
from nebula.cli._prompts import prompt_select_from_table
...
from nebula.client.schemas.actions import WorkPoolCreate, WorkPoolUpdate
from nebula.exceptions import ObjectAlreadyExists, ObjectNotFound
from nebula.infrastructure.provisioners import (
    _provisioners,
    get_infrastructure_provisioner_for_work_pool_type,
)
from nebula.settings import update_current_profile
from nebula.workers.utilities import (
    get_available_work_pool_types,
    get_default_base_job_template_for_infrastructure_type,
)

work_pool_app = NebulaTyper(
...
def set_work_pool_as_default(name: str):
    profile = update_current_profile({"NEBULA_DEFAULT_WORK_POOL_NAME": name})
    app.console.print(
    ...
    app.console.print(
        (
            "To change your default work pool, run:\n\n\t[blue]nebula config set"
            " NEBULA_DEFAULT_WORK_POOL_NAME=<work-pool-name>[/]\n"
        ),
    )
...
async def create(
    ...
    base_job_template: typer.FileText = typer.Option(
        ...
        help=(
            ...
            " unspecified, Nebula will use the default base job template for the given"
            " worker type."
        ),
    ),
    ...
    Examples:
        ...
        Create a Kubernetes work pool in a paused state:
            \b
            $ nebula work-pool create "my-pool" --type kubernetes --paused
        \b
        Create a Docker work pool with a custom base job template:
            \b
            $ nebula work-pool create "my-pool" --type docker --base-job-template ./base-job-template.json
    ...
    async with get_client() as client:
        ...
        try:
            ...
            if (
                ...
                app.console.print(
                    f"\t[blue]nebula worker start --pool {work_pool.name}[/]\n"
                )
            if set_as_default:
...
async def inspect(
    ...
    \b
    Examples:
        $ nebula work-pool inspect "my-pool"
...
async def pause(
    ...
    \b
    Examples:
        $ nebula work-pool pause "my-pool"
...
async def resume(
    ...
    \b
    Examples:
        $ nebula work-pool resume "my-pool"
...
async def update(
    ...
    \b
    Examples:
        $ nebula work-pool update "my-pool"
...
async def provision_infrastructure(
    ...
    \b
    Examples:
        $ nebula work-pool provision-infrastructure "my-pool"

        $ nebula work-pool provision-infra "my-pool"
...
async def delete(
    ...
    \b
    Examples:
        $ nebula work-pool delete "my-pool"
...
async def set_concurrency_limit(
    ...
    \b
    Examples:
        $ nebula work-pool set-concurrency-limit "my-pool" 10
...
async def clear_concurrency_limit(
    ...
    \b
    Examples:
        $ nebula work-pool clear-concurrency-limit "my-pool"
...
async def get_default_base_job_template(
    ...
    \b
    Examples:
        $ nebula work-pool get-default-base-job-template --type kubernetes
    """
...
async def preview(
    ...
    \b
    Examples:
        $ nebula work-pool preview "my-pool" --hours 24
