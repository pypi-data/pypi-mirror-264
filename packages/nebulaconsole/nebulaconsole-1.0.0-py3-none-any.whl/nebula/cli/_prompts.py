...
from nebula.client.collections import get_collections_metadata_client
from nebula.client.orchestration import NebulaClient
from nebula.client.schemas.actions import BlockDocumentCreate, WorkPoolCreate
...
from nebula.exceptions import ObjectAlreadyExists, ObjectNotFound
from nebula.flows import load_flow_from_entrypoint
from nebula.infrastructure.container import DockerRegistry
from nebula.settings import NEBULA_UI_URL
from nebula.utilities.processutils import get_sys_executable, run_process
...
async def prompt_build_custom_docker_image(
    ...
    return {"nebula_docker.deployments.steps.build_docker_image": build_step}
...
async def prompt_push_custom_docker_image(
    ...
    build_docker_image_step["nebula_docker.deployments.steps.build_docker_image"][
        "image_name"
    ] = full_image_name
    ...
    if confirm("Is this a private registry?", console=console):
        ...
        if confirm(
            "Would you like use nebula-docker to manage Docker registry credentials?",
            console=console,
            default=False,
        ):
            try:
                import nebula_docker
            except ImportError:
                console.print("Installing nebula-docker...")
                await run_process(
                    [get_sys_executable(), "-m", "pip", "install", "nebula-docker"],
                    stream_output=True,
                )
                import nebula_docker

            credentials_block = nebula_docker.DockerRegistryCredentials
            push_step[
                "credentials"
            ] = "{{ nebula_docker.docker-registry-credentials.docker_registry_creds_name }}"
        else:
            credentials_block = DockerRegistry
            push_step[
                "credentials"
            ] = "{{ nebula.docker-registry.docker_registry_creds_name }}"
    ...
    return {
        "nebula_docker.deployments.steps.push_docker_image": push_step
...
async def prompt_create_work_pool(
    ...
    if not confirm(
        ...
        raise ValueError(
            ...
            " name via the '--pool' flag or in your nebula.yaml file."
        )
...
async def prompt_select_remote_flow_storage(
    console: Console, client: NebulaClient = None
) -> Optional[str]:
...
async def prompt_select_blob_storage_credentials(
    console: Console, storage_provider: str, client: NebulaClient = None
) -> str:
    """
    ...
    if existing_credentials_blocks:
        ...
        if selected_credentials_block and (
            ...
            return f"{{{{ nebula.blocks.{creds_block_type_slug}.{selected_block} }}}}"
    ...
    if NEBULA_UI_URL:
        console.print(
            ...
            f"\n[blue]{NEBULA_UI_URL.value()}/blocks/block/{new_block_document.id}[/]\n"
        )
    return f"{{{{ nebula.blocks.{creds_block_type_slug}.{new_block_document.name} }}}}"
