...
from .connector import fastchat_llm_judge, lm_evaluation_harness
...
class Evaluator:
    ...
    def get_args(self):
        ...
        parser.add_argument(
            ...
            help="model name, e.g. HF model or OpenAI API models",
        )
    ...
    def run(self, model=None, benchmark=None, **kwargs):
        ...
        # mt_bench (with FastChat)
        if args.mt_bench:
            start_time = time.time()
            #############################################
            cmd = fastchat_llm_judge(
                model_path=args.ckpt_path,
                # model_id=args.ckpt_path.split('/')[-1],
