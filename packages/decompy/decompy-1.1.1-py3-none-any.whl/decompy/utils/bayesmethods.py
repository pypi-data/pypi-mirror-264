...
def rmf_matrix(M: np.array):
    ...
    if M.shape[1] == 1:
        ...
        svd_MU, svd_Ms, svd_MVT = np.linalg.svd(
        ...
        while not cmet:
            ...
            for j in range(1, R):
                ...
                if svd_Ms[j] > 0:
                    ...
                    lbr = np.log(iv(xn, 0.5 * (m - j - 2))) - np.log(
                        iv(xd, 0.5 * (m - j - 2))
                    )  # besselI with expon.scaled = True returns e^(-x)I_v(x)
                    if np.isnan(lbr):
                        lbr = 0.5 * (np.log(xd) - np.log(xn))
                    lr += (
                        lbr + (xn - xd) + 0.5 * (m - j - 2) * (np.log(xd) - np.log(xn))
                    )
...
def ln2moment(mu, s2, lmax):
    ...
    for i in range(2, 2 * lmax + 1):
        lmom[i] = lmom[i - 1] + np.log(
            (i - 1) * s2 * np.exp(lmom[i - 2] - lmom[i - 1]) + mu
        )
        if i % 2 == 0:
...
def rXL(mu, sigma, ell, nu=1):
    ...
    ell
        The parameter "ell" represents the shape parameter of the XL random variable. It determines the shape
    of the distribution and affects the tail behavior. A higher value of "ell" results in heavier tails.
    nu, optional
    ...
    theta = 0.5 * mu * (1 + np.sqrt(1 + 8 * ell * sigma**2 / mu**2))
    tau = 1 / np.sqrt(1 / sigma**2 + 2 * ell / theta**2)
    a = -2 * theta - mu
    b = theta**2 + 2 * mu * theta + nu * tau**2 + sigma**2 * (-nu * 2 * ell - 1)
    c = (-mu * theta**2) + (nu + 4 * ell + 1) * sigma**2 * theta - mu * nu * tau**2
    d = -2 * ell * sigma**2 * theta**2 - 2 * ell * nu * sigma**2 * tau**2

    z4 = roots_quartic(a, b, c, d)
    xc = np.real(z4[np.isreal(z4)])
    ln2m = ln2moment(mu, sigma, ell)[ell]
    lM = np.max(
        ldxl(xc, mu, sigma, ell, ln2m)
        - (-np.log(tau) + np.log(t.pdf((xc - theta) / tau, nu)))
    )
    sample = True
    while sample:
        x = theta + np.random.standard_t(nu) * tau
        lrratio = (
            ldxl(x, mu, sigma, ell, ln2m)
            - (-np.log(tau) + np.log(t.pdf((x - theta) / tau, nu)))
            - lM
        )
