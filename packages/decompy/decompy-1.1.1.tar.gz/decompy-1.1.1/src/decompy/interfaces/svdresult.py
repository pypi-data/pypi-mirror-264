...
class SVDResult:
    ...
    def __init__(self, U: np.ndarray, D: np.ndarray, V: np.ndarray, **kwargs) -> None:
        ...
        assert (
            D.ndim == 1 and U.ndim == 2 and V.ndim == 2
        ), "Mismatched shape in outputted singular values"
        assert (
            D.shape[0] == U.shape[1] and D.shape[0] == V.shape[1]
        ), "Mismatched shape in outputted singular values and vectors"
        self.U = U
        self.D = D
        self.V = V
        self.metrics = kwargs
