...
from nebula_kubernetes import KubernetesWorker
from nebula_kubernetes.utilities import _slugify_label_value, _slugify_name
from nebula_kubernetes.worker import KubernetesWorkerJobConfiguration
...
def mock_core_client(monkeypatch, mock_cluster_config):
    ...
    monkeypatch.setattr(
        "nebula_kubernetes.worker.KubernetesWorker._get_core_client",
        get_core_client,
    )
    return mock
...
def mock_batch_client(monkeypatch, mock_cluster_config, mock_job):
    ...
    monkeypatch.setattr(
        "nebula_kubernetes.worker.KubernetesWorker._get_batch_client",
        get_batch_client,
    )
    return mock
...
def enable_store_api_key_in_secret(monkeypatch):
    monkeypatch.setenv("NEBULA_KUBERNETES_WORKER_STORE_NEBULA_API_IN_SECRET", "true")
...
from_template_and_values_cases = [
    (
        ...
        lambda flow_run, deployment, flow: KubernetesWorkerJobConfiguration(
            command="nebula flow-run execute",
            env={
                **get_current_settings().to_environment_variables(exclude_unset=True),
                "NEBULA__FLOW_RUN_ID": str(flow_run.id),
            },
            labels={
                "nebula.io/flow-run-id": str(flow_run.id),
                "nebula.io/flow-run-name": flow_run.name,
                "nebula.io/version": _slugify_label_value(nebula.__version__),
                "nebula.io/deployment-id": str(deployment.id),
                "nebula.io/deployment-name": deployment.name,
                "nebula.io/flow-id": str(flow.id),
                "nebula.io/flow-name": flow.name,
            },
            name=flow_run.name,
            namespace="default",
            job_manifest={
        ...
        # default base template with custom env
        {
            ...
            "variables": {
                ...
                "properties": {
                    ...
                    "image": {
                        "title": "Image",
                        "description": "The image reference of a container image to use for created jobs. If not set, the latest Nebula image will be used.",
    ...
    (
        ...
        lambda flow_run, deployment, flow: KubernetesWorkerJobConfiguration(
            ...
            labels={
                "nebula.io/flow-run-id": str(flow_run.id),
                "nebula.io/flow-run-name": flow_run.name,
                "nebula.io/version": nebula.__version__,
                "nebula.io/deployment-id": str(deployment.id),
                "nebula.io/deployment-name": deployment.name,
                "nebula.io/flow-id": str(flow.id),
                "nebula.io/flow-name": flow.name,
...
class TestKubernetesWorkerJobConfiguration:
    ...
    async def test_user_can_supply_a_sidecar_container_and_volume(self, flow_run):
        ...
        # the nebula-job container is still populated
        assert pod["containers"][0]["name"] == "nebula-job"
        assert pod["containers"][0]["args"] == ["nebula", "flow-run", "execute"]
...
class TestKubernetesWorker:
    ...
    async def test_cluster_uid_uses_env_var_if_set(
        ...
        async with KubernetesWorker(work_pool_name="test") as k8s_worker:
            monkeypatch.setenv("NEBULA_KUBERNETES_CLUSTER_UID", "test-uid")
            fake_status = MagicMock(spec=anyio.abc.TaskStatus)
    ...
    async def test_can_store_api_key_in_secret(
        ...
        with temporary_settings(updates={NEBULA_API_KEY: "fake"}):
            async with KubernetesWorker(work_pool_name="test") as k8s_worker:
                ...
                assert {
                    ...
                    "valueFrom": {
                        "secretKeyRef": {
                            "name": f"nebula-{_slugify_name(k8s_worker.name)}-api-key",
                            "key": "value",
                        }
                    },
                } in env
    ...
    async def test_create_job_failure(
        ...
        response.data = {
            ...
            "metadata": {},
            "status": "Failure",
            "message": 'jobs.batch is forbidden: User "system:serviceaccount:helm-test:nebula-worker-dev" cannot create resource "jobs" in API group "batch" in the namespace "nebula"',
            "reason": "Forbidden",
        ...
        async with KubernetesWorker(work_pool_name="test") as k8s_worker:
            with pytest.raises(
                ...
                match=re.escape(
                    ...
                    '"system:serviceaccount:helm-test:nebula-worker-dev" cannot '
    ...
    async def test_allows_unsetting_environment_variables(
        ...
        async with KubernetesWorker(work_pool_name="test") as k8s_worker:
            ...
            assert "NEBULA_TEST_MODE" not in env_names
