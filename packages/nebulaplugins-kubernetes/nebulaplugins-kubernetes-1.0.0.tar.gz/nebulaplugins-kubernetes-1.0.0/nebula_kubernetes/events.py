...
class KubernetesEventsReplicator:
    """Replicates Kubernetes pod events to Nebula events."""
    ...
    def _pod_as_resource(self, pod: "V1Pod") -> Dict[str, str]:
        ...
        return {
            "nebula.resource.id": f"nebula.kubernetes.pod.{pod.metadata.uid}",
            "nebula.resource.name": pod.metadata.name,
            "kubernetes.namespace": pod.metadata.namespace,
        }

    def _replicate_pod_events(self):
        """Replicate Kubernetes pod events as Nebula Events."""
        seen_phases = set()
        last_event = None
    ...
    def _emit_pod_event(
        ...
        """Emit a Nebula event for a Kubernetes pod event."""
        ...
        return emit_event(
            event=f"nebula.kubernetes.pod.{pod_phase.lower()}",
            resource=resource,
