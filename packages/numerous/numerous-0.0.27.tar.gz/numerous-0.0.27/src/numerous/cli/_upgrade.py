...
def _check_for_updates_internal() -> None:
    ...
    if current_version and version.parse(state.latest_version) > version.parse(
        ...
        message = f"Heads up - A newer version of Numerous is available {current_version} → {state.latest_version}"
        print(blue(message))  # noqa: T201
        print(f"Please upgrade by running {green('pip install --upgrade numerous')} for the best experience.")  # noqa: T201
        print()  # noqa: T201
