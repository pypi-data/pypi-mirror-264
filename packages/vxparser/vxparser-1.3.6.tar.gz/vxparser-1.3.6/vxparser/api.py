...
async def root(response: Response, response_class=HTMLResponse):
    #data = '<html><head><title>Some HTML in here</title><link href="style.css" rel="stylesheet"></head><body>'
    #listdir = os.listdir(listpath)
    #for l in listdir:
        #data += '<a href="'+l+'">'+l+'</a><br>'
    return HTMLResponse(content=html.root, status_code=200)
