...
async def approximate_packages(
    ...
    conda_pkgs = defaultdict(list)
    pip_pkgs = defaultdict(list)
    for pkg in packages:
        if pkg["source"] == "conda":
            conda_pkgs[pkg["conda_name"]].append(pkg)
        else:
            pip_pkgs[pkg["name"]].append(pkg)
    ...
    for package_result in results:
        # Figure out if we have a package that matches the result
        pkgs = conda_pkgs.get(package_result["conda_name"]) or pip_pkgs.get(package_result["name"]) or []
        if len(pkgs) == 1:
            pkg = pkgs[0]
        elif not pkgs:
            pkg = None
            ...
            pkg = next(
                (
                    p
                    for p in pkgs
                    if p["conda_name"] == package_result["conda_name"] and p["name"] == package_result["name"]
                ),
                None,
            )
            if not pkg:
        ...
        if (
            package_result["name"] == "python"
            and package_result["conda_name"] == "python"
            and not user_conda_installed_python
            and coiled_selected_python
        ):
            ...
            if package_result["error"]:
                ...
                if not channel:
                    raise ValueError("Expected a channel from the backend")
                finalized_packages.append({
                    "name": "python",
                    "source": "conda",
                    "channel": channel,
        ...
        # Handle packages that were added by approximate-packages directly
        elif pkg is None:
            finalized_packages.append({
            ...
            if channel and channel == package_result["note"]:
                package_result["note"] = None
            finalized_packages.append({
                "name": pkg["name"],
                "source": pkg["source"],
                # Need to use package_result channel_url for custom PyPI repos
